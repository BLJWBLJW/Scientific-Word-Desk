...
%TCIDATA{LastRevised=Friday, February 04, 2011 11:49:33}
...
The $N$-Representability Problem is to find the \emph{necessary and sufficient
conditions} that a positive trace class operator, that acts on two electron
space, must satisfy in order to be the contraction of a $N$-electron state
density operator and thus be a Second Order Reduced Density Operator (SORDO).
It was first precisely formulated in quantum chemistry \cite{Coleman63} by
Coleman as a result of Coulsons \emph{challenge }\cite{Coleman2000}, which was
inspired by work initiated by Husimi \cite{Husimi}. The significance of the
problem derives from the realization that the properties of molecular systems,
at a fixed geometry, are fully described by Hamiltonians that describe the
kinetic energy of the individual electrons, interactions of the electrons with
external and internal electric and magnetic fields and \emph{only }the
simultaneous pair wise interactions of the electrons with each other. As
simultaneous three, four and higher body interactions are not significant the
$N$-electron state density operator contains redundant information and the
energy of molecular systems can be fully expressed in terms of the SORDO of
the system, which depends on fewer variables. Hence the determination of the
ground state energy can be formulated as a minimization of a linear functional
over operators that act in two electron space that are \emph{constrained} to
be SORDO's, instead of a higher dimensional minimization over $N$-electron
...
\cite{Mazziotti}. This work has, however, resulted in the characterization of
numerous necessary conditions (see for instance \cite{NecCon}), the innovative
contracted Schr\"{o}dinger equation method \cite{ContSch}, the relation to
complexity theory \cite{NPhard} and various variants of the problem i.e.
pure/ensemble, particle conserving/ particle nonconserving and finite/infinite dimensional.

In a paper \cite{Weiner83} the $N$-Representability Problem and the less
restrictive Representability Problem was shown to be one of finding conditions
for the existence of extensions of a linear functional from a subspace to a
whole vector space, placing it firmly in the area of mathematical analysis and
allowing one to utilize theorems proved in this topic over the last hundred
years. The classical Hahn-Banach theorem \cite{Encyclo} on extensions,
...
functionals defined on ordered vector spaces by Krein \cite{Krein} and to
self-adjoint subspaces of $c^{\ast}$-algebras that \emph{might or might not
}contain the identity by Segal \cite{Segal}. In order for the extension to be
a state \cite{Encyclo} i.e. a positive normalized linear functional on the
whole algebra its value on the identity must be one i.e.
\begin{equation}
...
\end{equation}
It can easily be seen that if $X\geq0$ then (Appendix \ref{order})
\begin{equation}
\left\Vert X\right\Vert \leq1\Leftrightarrow I\geq X\geq0
\end{equation}
and thus
\begin{equation}
=\left\{  X\in W\left\vert X\in W_{+};\left\Vert X\right\Vert \leq1\right.
\right\}  ,
\end{equation}
so $W_{<}$ is in the positive part of the unit ball of the algebra and $W_{>}$
belongs to the annulus
\begin{equation}
W_{>}=\left\{  X\in W\left\vert X\geq I;\left\Vert X\right\Vert \geq1\right.
\right\}  .
\end{equation}
...
has shown that the extreme points of the positive part of the unit ball in a
$w^{\ast}$-algebra are projectors and in particular belong to the unit sphere
\begin{equation}
...
In the context of the representability problem for electrons the relevant
$w^{\ast}$-algebra is the Fock algebra and the subspace is the one formed by
one and two electron operators, while the $N$-Representability Problem can be
expressed by the addition of further constraints. The lower limits in eq.
$\left(  \ref{NecSuff}\right)  $ correspond to the well known \cite{PQG}
\emph{necessary} P, Q and G conditions, however the upper limits in eq.
...
them as \emph{generalized }P, Q and G conditions.
...
to states of the algebra. This shows that the representability problem can be
...
that can be expressed as linear combinations of one and two particle operators
...
\subsection{Fermi Fock Space \label{Fermi Fock Space}}

We consider the Fermi Fock space, $\mathcal{H}_{F},$ based on the one electron
Hilbert space $\mathcal{H}^{1},$ defined by the direct sum
\begin{equation}
...
^{N}},
\end{equation}
where $P_{\mathcal{H}^{N}}$ is the orthogonal projector onto $\mathcal{H}%
^{N}.$

\subsection{Fermi Fock Algebra \label{Fermi Fock Alg}}

The fermion Fock algebra is the space of bounded operators $\left(
\text{Appendix \ref{Operators}}\right)  $ acting in $\mathcal{H}_{\mathcal{F}%
}$ i.e.
\begin{equation}
...
1\leq j\leq r\right\}  ,$ which are defined by their action on $\left\vert
\phi\right\rangle $ by
\begin{equation}
...
j\leq r\right\}  $ is a complete orthonormal basis of one-particle space
...
\end{equation}
of a particle conserving sub algebra $\mathcal{F}_{e}$ and a particle non
conserving subspace of operators $\mathcal{F}_{0}$. The sub algebra
$\mathcal{F}_{e}$ can be further decomposed as a direct sum of sub spaces
\begin{equation}
\mathcal{F}_{e}=%
...
\end{equation}
or as a direct sum of sub algebras
\begin{equation}
\mathcal{F}_{e}=%
%TCIMACRO{\dbigoplus \limits_{0\leq N\leq r}}%
%BeginExpansion
...
operators acting in $\mathcal{H}^{N}.$ The sub space $\mathcal{F}_{0}$ can be
...
\mathcal{B}_{1}\left(  \mathcal{H}_{\mathcal{F}}\right)  .
\end{equation}
One can show (Appendix SecQuant) that $\Gamma$ is \emph{actually the second
quantization map}
\begin{equation}
...
\end{equation}
which can be expressed as exterior multiplication by the identity operator in
$\mathcal{F}$
\begin{equation}
\Gamma\left(  \left\vert \varphi_{j_{1}}\cdots\varphi_{j_{M}}\right\rangle
\left\langle \varphi_{k_{1}}\cdots\varphi_{k_{N}}\right\vert \right)
=\left\vert \varphi_{j_{1}}\cdots\varphi_{j_{M}}\right\rangle \left\langle
\varphi_{k_{1}}\cdots\varphi_{k_{N}}\right\vert \wedge I_{\mathcal{H}%
_{\mathcal{F}}}.
\end{equation}
...
The Fermi Fock algebra is a concrete realization of a $w^{\ast}$-algebra
...
The important property of $w^{\ast}$-algebras vis-\`{a}-vis $c^{\ast}%
$-algebras is that they are the closure of their idempotents, which in the
case of $\mathcal{F}$ are the orthogonal projectors onto subspaces of
$\mathcal{H}_{\mathcal{F}}.$

\subsection{States \label{States}}

A state on a $w^{\ast}$-algebra is a bounded positive linear function, $f$, on
the algebra such that $f\left(  I\right)  =1$, where $I$ is the identity
element of the algebra, it is called a normal state if it can be identified
with an element of the predual of the algebra otherwise it is an abnormal
state. In this paper only normal states belonging to the predual
$\mathcal{B}_{1}\left(  \mathcal{H}_{\mathcal{F}}\right)  $ of $\mathcal{B}%
\left(  \mathcal{H}_{\mathcal{F}}\right)  $ are considered.

In the Fermi Fock algebra normal states are in one to one correspondence with
normalized density operators i.e.
\begin{equation}
...
A \emph{partial} state is a postive linear functional defined on a self
...
X\in\mathcal{W},I\geq X\geq0\right\}  \label{BallCond}%
\end{equation}
and%
\begin{equation}
...
X\in\mathcal{W},X\geq I\right\}  . \label{AnnCon}%
\end{equation}
...
$ is the subspace formed by one and two particle conserving operators
\begin{equation}
\Lambda_{2}=%
...
\end{equation}
as reduced states correspond to SORDO's through%
\begin{equation}
...
P  &  \geq0\\
P^{2}  &  =P\\
\left\Vert P\right\Vert _{\mathcal{B}\left(  \mathcal{H}_{\mathcal{F}}\right)
}  &  =\sup_{\left\vert \Psi\right\rangle \in\mathcal{H}_{\mathcal{F}}%
}\left\{  \frac{\left\langle \left.  \Psi\right\vert P\Psi\right\rangle
}{\left\langle \left.  \Psi\right\vert \Psi\right\rangle }\right\}  =1.
\end{align}
We first characterize the projectors in $\mathcal{B}\left(  \mathcal{H}%
_{\mathcal{F}}\right)  _{e1},$ then $\mathcal{B}\left(  \mathcal{H}%
_{\mathcal{F}}\right)  _{e2}$ and finally $\Lambda_{2}.$
...
}\right)  _{e1}$ can be expressed as (Appendix \ref{Operators} )
\end{subequations}
\begin{equation}
...
\mathcal{H}_{\mathcal{F}}\right)  _{e1}$ are of the form
\begin{equation}
...
\subsection{Projectors in $\mathcal{B}\left(  \mathcal{H}_{\mathcal{F}%
}\right)  _{e2}$ \label{Proj2}}

Every positive operator in $\mathcal{B}\left(  \mathcal{H}_{\mathcal{F}%
}\right)  _{e2}$ can be expressed as (Appendix \ref{Operators} )
\begin{equation}
...
order that the two particle operator $\lambda_{2}$ be a projection operator it
must be of the form
\begin{equation}
...
{2}$ unitary matrix i.e.
\begin{equation}
\mathbf{u}_{2}^{\dagger}\mathbf{u}_{2}=\mathbf{u}_{2}\mathbf{u}_{2}^{\dagger
...
_{\mathcal{F}}\right)  _{e2}$ but by using the canonical anticommutation
relationships eq. $\left(  \ref{CAR}\right)  $ one observe that
\begin{equation}
...
\end{equation}
is also a projection operator belonging to $\Lambda_{2}.$

\subsubsection{Projections from Products of One Particle Operators}

By considering the positivity, idempotency and normalization relationships for
products $\lambda_{1}\lambda_{1}^{\prime}$ of one particle operators of the
...
}a-a^{\dagger}b^{\dagger}ba\in\Lambda_{2}.
\end{equation}
...
The Necessary conditions P, Q and G conditions for representability of a
$\binom{r}{2}\times\binom{r}{2}$ hermitian matrices were quickly discovered
\cite{PQG} and are bases independent i.e. invariant under the unitary group
$U\left(  \mathcal{H}^{1}\right)  $ of one electron space. In the notation of
this paper they are:
...
\mathbb{D}_{2}\geq0.
\end{equation}
The basic variables of the ground state energy optimization problem are the
complex matrix elements $\left\{  \left.  D_{2j_{1}j_{2}k_{1}k_{2}}\right\vert
1\leq j_{1}<j_{2}\leq r,1\leq k_{1}<k_{2}\leq r\right\}  .$
...
\begin{equation}
Q_{j_{1}j_{2}k_{1}k_{2}}=\frac{1}{2\left(  N-1\right)  }\sum_{1\leq l\leq
...
as
\begin{equation}
G_{j_{1}j_{2}k_{1}k_{2}}=\frac{1}{2\left(  N-1\right)  }\sum_{1\leq l\leq
r}\left\{  D_{2j_{2}lk_{1}l}\delta_{j_{1}k_{2}}+D_{2j_{1}lk_{2}l}\delta
_{j_{2}k_{1}}\right\}  -D_{2j_{1}j_{2}k_{1}k_{2}}.
\end{equation}


The generalized P, Q and G conditions are \emph{Necessary and Sufficient
}conditions for the existence of positive extensions of bounded linear
functionals
\begin{equation}
f:\Lambda_{2}\rightarrow\mathbb{C}%
\end{equation}
to a states of $\mathcal{B}\left(  \mathcal{H}_{\mathcal{F}}\right)  ,$ that
are based on the theorems of Krien \cite{Krein}, Segal \cite{Segal} and
Kadison \cite{Kadison}.
...
and $b^{\dagger}$ are the creators of those states. As
\begin{equation}
...
\section{Particle and Pair Number Conditions \label{PartPair}}

In order to obtain $N$-representable extension one must constrain the
...
$\mathcal{B}\left(  \mathcal{H}_{\mathcal{F}}\right)  _{e1}$ must be related
to its values on $\mathcal{B}\left(  \mathcal{H}_{\mathcal{F}}\right)  _{e2}$
by
\begin{equation}
f\left(  a_{j}^{\dagger}a_{k}\right)  =\left(  N-1\right)  f\left(
a_{j}^{\dagger}\mathcal{N}_{1}a_{k}\right)  =\left(  N-1\right)  \sum_{1\leq
l\leq r}f\left(  a_{j}^{\dagger}a_{l}^{\dagger}a_{l}a_{k}\right)  ,
\label{Def1D}%
\end{equation}
thus if the functional is only defined on $\mathcal{B}\left(  \mathcal{H}%
_{\mathcal{F}}\right)  _{e2}$ one can define allowable extensions to
$\mathcal{B}\left(  \mathcal{H}_{\mathcal{F}}\right)  _{e1}$ by eq. $\left(
\ref{Def1D}\right)  .$

In terms of matrix elements the definition eq. $\left(  \ref{Def1D}\right)  $
becomes
\begin{equation}
D_{1jk}=\left(  N-1\right)  \sum_{1\leq l\leq r}D_{2jlkl}%
\end{equation}
and one can see that if the value of the functional on the pair operator is
set by eq. $\left(  \ref{Numpairs}\right)  $ then the particle number
...
\begin{equation}
\sum_{1\leq j<k\leq r}D_{2jkjk}=\binom{N}{2}%
\end{equation}


\item Generalized P-Condition%
\begin{equation}
1\geq D_{2abab}\geq0\,\forall a,b
\end{equation}


\item Generalized Q-Condition%
\begin{equation}
1\geq\left(  N-1\right)  \sum_{1\leq l\leq r}D_{2alal}-D_{2abab}\geq0\,\forall
a,b
\end{equation}
...
\begin{equation}
1\geq\left(  N-1\right)  \sum_{1\leq l\leq r}\left\{  D_{2alal}+D_{2blbl}%
\right\}  -D_{2abab}\geq0\,\forall a,b.
\end{equation}
...
\section{Appendix A \label{order}}

\section{Appendix B \label{Operators}}
...
denoted $\mathcal{B}_{0}\left(  \mathcal{H}_{\mathcal{F}}\right)  $ and is not closed.

\subsection{Bounded Operators}

The operator norm (also called the uniform norm) is defined as
\begin{equation}
\left\Vert X\right\Vert =\sup_{\left\vert \Phi\right\rangle \in\mathcal{H}%
_{F}}\left\{  \left(  \frac{\left\langle \left.  \Phi\right\vert X^{\dagger
}X\Phi\right\rangle }{\left\langle \left.  \Phi\right\vert \Phi\right\rangle
}\right)  ^{\frac{1}{2}}\right\}  =\sup_{\left\vert \Phi\right\rangle
\in\mathcal{H}_{F}}\left\{  \frac{\left\Vert X\Phi\right\Vert _{\mathcal{H}%
_{F}}}{\left\Vert \Phi\right\Vert _{\mathcal{H}_{F}}}\right\}  ,
\end{equation}
...
\left\Vert X\right\Vert <\infty\right.  \right\}  .
\end{equation}
If one restricts attention to self adjoint operators $X=X^{\dagger}$ then it
can be shown that
\begin{equation}
\left\Vert X\right\Vert =\sup_{\left\vert \Phi\right\rangle \in\mathcal{H}%
_{F}}\left\{  \left\vert \frac{\left\langle \left.  \Phi\right\vert
X\Phi\right\rangle }{\left\langle \left.  \Phi\right\vert \Phi\right\rangle
}\right\vert \right\}
\end{equation}
...
\begin{equation}
\left\Vert X\right\Vert =\sup_{\left\vert \Phi\right\rangle \in\mathcal{H}%
_{F}}\left\{  \frac{\left\langle \left.  \Phi\right\vert X\Phi\right\rangle
...
\Phi_{j}\right\rangle \left\langle \Psi_{j}\right\vert \right\Vert =0.
\end{equation}
...
closure with respect to the $\left\Vert {}\right\Vert $ norm produces
$\mathcal{C}\left(  \mathcal{H}_{\mathcal{F}}\right)  .$
...
closure with respect to $\left\Vert {}\right\Vert $ norm produces
$\mathcal{C}\left(  \mathcal{H}_{\mathcal{F}}\right)  .$
...
operator operator is $\mathcal{B}\left(  \mathcal{H}_{\mathcal{F}}\right)  ,$
where these limits can be defined as
\begin{equation}
X=\text{\textrm{s-}}\lim_{m\rightarrow\infty}X_{m}\Longleftrightarrow
\lim_{m\rightarrow\infty}\left\Vert \left(  X-X_{m}\right)  \Phi\right\Vert
=0\quad\forall\left\vert \Phi\right\rangle \in\mathcal{H}_{\mathcal{F}},
\end{equation}
in contrast to the uniform operator topology limits that are given by%
\begin{equation}
X=\lim_{m\rightarrow\infty}X_{m}\Longleftrightarrow\lim_{m\rightarrow\infty
}\left\Vert \left(  X-X_{m}\right)  \right\Vert =0=\lim_{m\rightarrow\infty
}\sup_{\left\vert \Phi\right\rangle \in\mathcal{H}_{F}}\left\{  \left(
\frac{\left\langle \left.  \Phi\right\vert \left(  X-X_{m}\right)  ^{\dagger
}\left(  X-X_{m}\right)  ^{\dagger}\Phi\right\rangle }{\left\langle \left.
\Phi\right\vert \Phi\right\rangle }\right)  ^{\frac{1}{2}}\right\}  .
\end{equation}
...
\section{Appendix C \label{Projection}}
...
\end{equation}
the condition that it is a projector i.e. idempotent is that%
\begin{equation}
...
\end{equation}
The idempotency condition eq. $\left(  \ref{idem}\right)  $ also implies that
\begin{equation}
\lambda\geq0.
\end{equation}
...
\end{align}
The condition eq. $\left(  \ref{idem1}\right)  $ becomes
\end{subequations}
\begin{equation}
...
\begin{equation}
\lambda_{1}=\sum_{\sigma\in\left\{  1,\cdots,r\right\}  }b_{\sigma}^{\dagger
}b_{\sigma}%
\end{equation}
then
\begin{equation}
...
and%
\begin{equation}
\lambda=\lambda_{2}=\sum_{\substack{1\leq j_{1}<j_{2}\leq r\\1\leq k_{1}%
<k_{2}\leq r}}\lambda_{2j_{1}j_{2}k_{1}k_{2}}a_{j_{1}}^{\dagger}a_{j_{2}%
...
g_{l}^{\dagger} &  =\sum_{1\leq j_{1}<j_{2}\leq r}u_{lj_{1}j_{2}}a_{j_{1}%
}^{\dagger}a_{j_{2}}^{\dagger}=\frac{1}{2}\sum_{1\leq j_{1},j_{2}\leq
r}u_{lj_{1}j_{2}}a_{j_{1}}^{\dagger}a_{j_{2}}^{\dagger};\,u_{lj_{1}j_{2}%
}=-u_{lj_{2}j_{1}}\\
...
=\delta_{l_{1}l_{2}}\\
\mathbb{U}_{2}^{\dagger}\mathbb{U}_{2} &  =\mathbb{U}_{2}\mathbb{U}%
_{2}^{\dagger}=\mathbb{I}_{\binom{r}{2}}\equiv\operatorname*{Tr}\left\{
\mathbb{U}_{1l_{1}}^{\dagger}\mathbb{U}_{1l_{2}}\right\}  =\delta_{l_{1}l_{2}%
},1\leq l_{1},l_{2}\leq r\\
\mathbb{U}_{1l_{2}} &  =-\mathbb{U}_{1l_{2}}^{t}%
\end{align}
The condition eq. $\left(  \ref{beta}\right)  $ comes from noting that the
projection of $\lambda_{2}$ on $\mathcal{H}^{2}$ is given by
\end{subequations}
\begin{equation}
...
\end{equation}
$\lambda_{2}$ is the second quantization of that projection%
\begin{equation}
...
and
\begin{equation}
1=\left\Vert \lambda_{2}\right\Vert \geq\left\Vert P_{\mathcal{H}^{2}}%
\lambda_{2}P_{\mathcal{H}^{2}}\right\Vert =\sigma\left(  P_{\mathcal{H}^{2}%
}\lambda_{2}P_{\mathcal{H}^{2}}\right)  ,
\end{equation}
where $\sigma\left(  P_{\mathcal{H}^{2}}\lambda_{2}P_{\mathcal{H}^{2}}\right)
$ is the spectral radius of $P_{\mathcal{H}^{2}}\lambda_{2}P_{\mathcal{H}^{2}%
}$.

The idempotency condition eq. $\left(  \ref{idem2}\right)  $ in this case is
\begin{equation}
...
\end{equation}
In order to compare the LHS\ to the RHS of eq. $\left(  \ref{lam2}\right)  $
one brings the RHS of  eq. $\left(  \ref{lam2}\right)  $ to normal form
producing
\begin{equation}
...
\end{equation}
which can be developed by noting
\begin{equation}
\left[  g_{l_{1}}^{{}},g_{l_{2}}^{\dagger}\right]  =\frac{1}{4}\sum
_{\substack{1\leq j_{1},j_{2}\leq r\\1\leq k_{1},k_{2}\leq r}}\bar{u}%
_{l_{1}j_{1}j_{2}l_{1}}u_{l_{2}k_{1}k_{2}}\left[  a_{j_{2}}a_{j_{1}},a_{k_{1}%
}^{\dagger}a_{k_{2}}^{\dagger}\right]  =-\frac{1}{4}\sum_{\substack{1\leq
j_{1},j_{2}\leq r\\1\leq k_{1},k_{2}\leq r}}\bar{u}_{l_{1}j_{1}j_{2}}%
u_{l_{2}k_{1}k_{2}}\left[  a_{k_{1}}^{\dagger}a_{k_{2}}^{\dagger},a_{j_{2}%
}a_{j_{1}}\right]
\end{equation}%
\begin{align}
\left[  g_{l_{1}}^{{}},g_{l_{2}}^{\dagger}\right]   &  =-\sum_{\substack{1\leq
j_{1}<j_{2}\leq r\\1\leq k_{1}<k_{2}\leq r}}\left\{  \bar{u}_{l_{1}j_{1}j_{2}%
}u_{l_{2}j_{2}j_{1}}-\bar{u}_{l_{1}j_{1}j_{2}}u_{l_{2}j_{1}j_{2}}+\bar
{u}_{l_{1}j_{1}j_{2}}u_{l_{2}j_{1}k_{2}}a_{k_{2}}^{\dagger}a_{j_{2}}-\bar
{u}_{l_{1}j_{1}j_{2}}u_{l_{2}k_{1}j_{1}}a_{k_{1}}^{\dagger}a_{j_{2}}\right.
\\
&  \left.  -\bar{u}_{l_{1}j_{1}j_{2}}u_{l_{2}j_{2}k_{2}}a_{k_{2}}^{\dagger
}a_{j_{1}}+\bar{u}_{l_{1}j_{1}j_{2}}u_{l_{2}k_{1}j_{2}}a_{k_{1}}^{\dagger
}a_{j_{1}}\right\}
\end{align}%
\begin{align}
\left[  g_{l_{1}}^{{}},g_{l_{2}}^{\dagger}\right]   &  =\frac{1}{4}%
\sum_{\substack{1\leq j_{1},j_{2}\leq r\\1\leq k_{1},k_{2}\leq r}}\bar
{u}_{l_{1}j_{1}j_{2}l_{1}}u_{l_{2}k_{1}k_{2}}\left[  a_{j_{2}}a_{j_{1}%
},a_{k_{1}}^{\dagger}a_{k_{2}}^{\dagger}\right]  \\
&  =-\frac{1}{4}\sum_{\substack{1\leq j_{1},j_{2}\leq r\\1\leq k_{1},k_{2}\leq
r}}\bar{u}_{l_{1}j_{1}j_{2}}u_{l_{2}k_{1}k_{2}}\left[  a_{k_{1}}^{\dagger
}a_{k_{2}}^{\dagger},a_{j_{2}}a_{j_{1}}\right]  \\
&  =-\frac{1}{4}\sum_{\substack{1\leq j_{1},j_{2}\leq r\\1\leq k_{1},k_{2}\leq
r}}\bar{u}_{l_{1}j_{1}j_{2}}u_{l_{2}k_{1}k_{2}}\left\{  I_{\mathcal{H}%
_{\mathcal{F}}}\delta_{j_{2}k_{1}}\delta_{j_{1}k_{2}}-I_{\mathcal{H}%
_{\mathcal{F}}}\delta_{j_{2}k_{2}}\delta_{j_{1}k_{1}}\right.  \\
&  \left.  a_{k_{2}}^{\dagger}a_{j_{2}}\delta_{j_{1}k_{1}}-a_{k_{1}}^{\dagger
}a_{j_{2}}\delta_{j_{1}k_{2}}-a_{k_{2}}^{\dagger}a_{j_{1}}\delta_{j_{2}k_{1}%
}+a_{k_{1}}^{\dagger}a_{j_{1}}\delta_{j_{2}k_{2}}\right\}  \\
&  =-\frac{1}{4}\sum_{\substack{1\leq j_{1},j_{2}\leq r\\1\leq k_{1},k_{2}\leq
r}}\left\{  -\bar{u}_{l_{1}j_{1}j_{2}}u_{l_{2}j_{1}j_{2}}-\bar{u}_{l_{1}%
j_{1}j_{2}}u_{l_{2}j_{1}j_{2}}+\bar{u}_{l_{1}j_{2}j_{1}}u_{l_{2}k_{2}j_{1}%
}a_{k_{2}}^{\dagger}a_{j_{2}}+\bar{u}_{l_{1}j_{2}j_{1}}u_{l_{2}k_{1}j_{1}%
}a_{k_{1}}^{\dagger}a_{j_{2}}\right.  \\
&  \left.  +\,\bar{u}_{l_{1}j_{1}j_{2}}u_{l_{2}k_{2}j_{2}}a_{k_{2}}^{\dagger
}a_{j_{1}}+\bar{u}_{l_{1}j_{1}j_{2}}u_{l_{2}k_{1}j_{2}}a_{k_{1}}^{\dagger
}a_{j_{1}}\right\}
\end{align}%
\begin{align}
&  =-\frac{1}{4}\sum_{\substack{1\leq j_{1},j_{2}\leq r\\1\leq k_{1},k_{2}\leq
r}}\left\{  -\delta_{l_{1}l_{2}}-\delta_{l_{1}l_{2}}+\left(  \mathbb{U}%
_{1l_{1}}^{\dagger}\mathbb{U}_{1l_{2}}\right)  _{j_{2}k_{2}}a_{k_{2}}%
^{\dagger}a_{j_{2}}+\left(  \mathbb{U}_{1l_{1}}^{\dagger}\mathbb{U}_{1l_{2}%
}\right)  _{j_{2}k_{1}}a_{k_{1}}^{\dagger}a_{j_{2}}\right.  \\
&  \left.  +\left(  \mathbb{U}_{1l_{1}}^{\dagger}\mathbb{U}_{1l_{2}}\right)
_{j_{1}k_{2}}\,a_{k_{2}}^{\dagger}a_{j_{1}}+\left(  \mathbb{U}_{1l_{1}%
}^{\dagger}\mathbb{U}_{1l_{2}}\right)  _{j_{1}k_{1}}a_{k_{1}}^{\dagger
}a_{j_{1}}\right\}
\end{align}%
\begin{equation}
=\frac{1}{2}\delta_{l_{1}l_{2}}-\sum_{1\leq j_{1},k_{1}\leq r}\left(
\mathbb{U}_{1l_{1}}^{\dagger}\mathbb{U}_{1l_{2}}\right)  _{j_{1}k_{1}}%
a_{k_{1}}^{\dagger}a_{j_{1}}%
\end{equation}
...
}+a_{j_{2}+s}^{\dagger}a_{j_{2}+s}\right)  a_{j_{4}}a_{j_{4+s}}  &
=0\nonumber\\
a_{j_{1}}^{\dagger}a_{j_{1+s}}^{\dagger}a_{j_{3}}^{\dagger}a_{j_{3+s}%
}^{\dagger}a_{j_{2+s}}a_{j_{2}}a_{j_{4}}a_{j_{4+s}}  &  =0
\end{align}
and the only solution is $s=1.$ Consider $s=2$ then e.g.and the only solution
is $s=1.$ Consider $s=2$ then e.g.%
\begin{align}
...
\end{equation}
This operator is a projector as
\begin{align}
\left(  a_{j_{2}}a_{j_{1}}a_{j_{1}}^{\dagger}a_{j_{2}}^{\dagger}\right)  ^{2}
&  =a_{j_{2}}a_{j_{1}}a_{j_{1}}^{\dagger}a_{j_{2}}^{\dagger}a_{j_{2}}a_{j_{1}%
}a_{j_{1}}^{\dagger}a_{j_{2}}^{\dagger}=a_{j_{2}}a_{j_{1}}a_{j_{1}}^{\dagger
}\left(  1-a_{j_{2}}a_{j_{2}}^{\dagger}\right)  a_{j_{1}}a_{j_{1}}^{\dagger
}a_{j_{2}}^{\dagger}=\nonumber\\
&  =a_{j_{2}}a_{j_{1}}a_{j_{1}}^{\dagger}a_{j_{1}}a_{j_{1}}^{\dagger}a_{j_{2}%
}^{\dagger}-a_{j_{2}}a_{j_{1}}a_{j_{1}}^{\dagger}a_{j_{2}}a_{j_{2}}^{\dagger
}a_{j_{1}}a_{j_{1}}^{\dagger}a_{j_{2}}^{\dagger}\nonumber\\
&  =a_{j_{2}}a_{j_{1}}\left(  1-a_{j_{1}}a_{j_{1}}^{\dagger}\right)  a_{j_{1}%
}^{\dagger}a_{j_{2}}^{\dagger}+a_{j_{2}}a_{j_{1}}a_{j_{2}}a_{j_{1}}a_{j_{1}%
}^{\dagger}a_{j_{2}}^{\dagger}a_{j_{1}}^{\dagger}a_{j_{2}}^{\dagger
}\nonumber\\
&  =a_{j_{2}}a_{j_{1}}a_{j_{1}}^{\dagger}a_{j_{2}}^{\dagger}-a_{j_{2}}%
a_{j_{1}}a_{j_{1}}a_{j_{1}}^{\dagger}a_{j_{1}}^{\dagger}a_{j_{2}}^{\dagger
}\nonumber\\
...
\end{align}
\emph{One should note however that}
\begin{equation}
a_{j_{2}}a_{j_{1}}a_{j_{1}}^{\dagger}a_{j_{2}}^{\dagger}\notin\Lambda
...
but
\begin{equation}
I_{\mathcal{H}_{\mathcal{F}}}-a_{j_{2}}a_{j_{1}}a_{j_{1}}^{\dagger}a_{j_{2}%
}^{\dagger}%
\end{equation}
does, as
\begin{equation}
I_{\mathcal{H}_{\mathcal{F}}}-a_{j_{2}}a_{j_{1}}a_{j_{1}}^{\dagger}a_{j_{2}%
}^{\dagger}=a_{j_{1}}^{\dagger}a_{j_{1}}+a_{j_{2}}^{\dagger}a_{j_{2}},
\end{equation}
which is a projector as%
\begin{align}
\left(  I_{\mathcal{H}_{\mathcal{F}}}-a_{j_{2}}a_{j_{1}}a_{j_{1}}^{\dagger
}a_{j_{2}}^{\dagger}\right)  ^{2}  &  =I_{\mathcal{H}_{\mathcal{F}}}%
-2a_{j_{2}}a_{j_{1}}a_{j_{1}}^{\dagger}a_{j_{2}}^{\dagger}+\left(  a_{j_{2}%
}a_{j_{1}}a_{j_{1}}^{\dagger}a_{j_{2}}^{\dagger}\right)  ^{2}\nonumber\\
&  =I_{\mathcal{H}_{\mathcal{F}}}-a_{j_{2}}a_{j_{1}}a_{j_{1}}^{\dagger
}a_{j_{2}}^{\dagger}.
\end{align}
...
=a_{j_{2}}^{\dagger}\left(  I_{\mathcal{H}_{\mathcal{F}}}-a_{j_{1}}^{\dagger
}a_{j_{1}}\right)  a_{j_{2}}=a_{j_{2}}^{\dagger}a_{j_{2}}-a_{j_{2}}^{\dagger
}a_{j_{1}}^{\dagger}a_{j_{1}}a_{j_{2}}.
\end{equation}
The operator $a_{j_{2}}^{\dagger}a_{j_{1}}a_{j_{1}}^{\dagger}a_{j_{2}%
}=n_{j_{1}}n_{j_{2}}$ is a projector as
\begin{equation}
n_{j_{1}}n_{j_{2}}n_{j_{1}}n_{j_{2}}=n_{j_{1}}n_{j_{1}}n_{j_{2}}n_{j_{2}%
}=n_{j_{1}}n_{j_{2}}%
\end{equation}
and
\begin{equation}
...
\section{Appendix D \label{Commutation Relationship}}

The commutator%
\begin{equation}
\left[  a_{k_{1}}^{\dagger}a_{k_{2}}^{\dagger},a_{j_{4}}a_{j_{3}}\right]
\end{equation}
is evaluated by bringing $a_{j_{4}}a_{j_{3}}a_{j_{1}}^{\dagger}a_{j_{2}%
}^{\dagger}$ to normal form as
\begin{align}
&  a_{j_{4}}a_{j_{3}}a_{k_{1}}^{\dagger}a_{k_{2}}^{\dagger}=\nonumber\\
I_{\mathcal{H}_{\mathcal{F}}} &  \delta_{j_{4}k_{2}}\delta_{j_{3}k_{1}%
}-I_{\mathcal{H}_{\mathcal{F}}}\delta_{j_{4}k_{1}}\delta_{j_{3}k_{2}}%
-a_{k_{2}}^{\dagger}a_{j_{4}}\delta_{j_{3}k_{1}}+a_{k_{1}}^{\dagger}a_{j_{4}%
}\delta_{j_{3}k_{2}}+a_{k_{2}}^{\dagger}a_{j_{3}}\delta_{j_{4}k_{1}}-a_{k_{1}%
}^{\dagger}a_{j_{3}}\delta_{j_{4}k_{2}}+a_{k_{1}}^{\dagger}a_{k_{2}}^{\dagger
}a_{j_{4}}a_{j_{3}},
\end{align}
and thus
\begin{equation}
...
}^{\dagger}a_{j_{3}}\delta_{j_{4}k_{2}}.
\end{equation}


\section{Appendix E \label{SecQuant}}

\section{Appendix F \label{Number}}

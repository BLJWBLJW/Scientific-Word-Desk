...
%TCIDATA{LastRevised=Tuesday, January 24, 2006 13:44:41}
...
The Levy-Leib construction implicitly determines a universal energy functional
of charge densities associated with pure states, as noted by many authors this
construction can be extended to mixed states. $\lceil$ Not every density has a
bound ground state associated with it, but there is always a minimum to the
constrained optimization$\rfloor$. We express the Levy-Leib construction in
three ways,
...
in the cone of positive operat$_{{}}$ors of $\mathcal{B}_{1}\left(
\mathcal{H}^{N}\right)  $. $\lceil$If $D^{N}\in\mathcal{B}_{1}\left(
\mathcal{H}^{N}\right)  $ then
\begin{equation}
...
\rightarrow L_{1}\left(  \mathbb{X}\right)
\end{equation}
is surjective. Its coordinates are
\begin{equation}
...
$L_{1}\left(  \mathbb{X}\right)  $ then $D\Xi\left(  a\right)  $ is
...
b\right)  \mathbf{.}%
\end{equation}
The converse is also true. $\lceil$A set $\left\{  \left\vert y\right\}
\right\}  $ is an overcomplete basis if any element $g$ can be uniquely
expanded as
\begin{equation}
...
localized in a region of space $\mathbf{\Delta}$ so that
\begin{equation}
\widehat{\Phi\left(  \mathbf{x}\right)  ^{\dagger}\Phi\left(  \mathbf{x}%
\right)  }\left\vert \overset{}{a}\right)  =0\,\forall\mathbf{x\in\Delta}^{c}%
\end{equation}
...
\mathbf{\Delta}\right)  .\mathbf{\rfloor}$
...
types, i.e. thay have some density in the region of space where the potential
is supported.
...
where
\begin{align}
\mathbb{X}  &  \mathbb{=}\bigcup\limits_{1\leq j\leq r}\mathbb{\Delta}_{j}\\
...
_{N}\right\rangle .
\end{equation}
The following theorem is very important: Let
\begin{equation}
\Phi_{j}^{\dagger}\Phi_{j}\left\vert \psi_{1}\cdots\psi_{N}\right\rangle
\neq0;\,1\leq j\leq r^{\prime}%
\end{equation}
then
\begin{align}
\Omega\left(  \lambda\right)  \left\vert \psi_{1}\cdots\psi_{N}\right\rangle
...
&  \Leftrightarrow\left\{  \Omega\left(  \lambda\right)  \left\vert \psi
_{j}\right\rangle =0;\,1\leq j\leq N\right\}  \Leftrightarrow\lambda=0.
\end{align}

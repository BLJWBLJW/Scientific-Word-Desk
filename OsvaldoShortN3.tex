...
\documentclass[12pt,aps,amsfonts,amssymb,tightenlines]{revtex4}%
\usepackage{amsfonts}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{graphicx}%
...
%TCIDATA{LastRevised=Saturday, February 07, 2004 11:40:10}
...
\begin{document}
\title{Antisymmetrized Geminal Power Coherent States}
\author{Brian Weiner}
...
particle unitary group is presented in terms of coset generators of this
group. For even number of electrons these states form non linear manifolds of
Antisymmetrized Geminal Powers, that are parameterized by complex coordinates.
Different classes of manifolds of Antisymmetrized Geminal Power states are
associated with distinct coset decompositions and produce different
restrictions of the time dependent Schr\"{o}dinger equation.
...
Hamiltonian. An example of this is the effect of radiation on a molecular
subsystem, which can be monitored by changes in the molecules density and
reduced density matrices. If the radiation is weak the behavior of the
interaction is well described by the linear response of the molecular system
to the radiation, i.e. is linearly proportional to the radiation strength,
while if the radiation is intense the molecular system responds in a nonlinear
way. The interaction of the electromagnetic field with molecules can be
approximated in a standard semi classical fashion by averaging over the
photonic degrees of freedom to produce effective one electron observables.
This leads to a reduced Hamiltonian for the molecular subsystem that includes
perturbing effective time dependent one body potentials which describe the
interaction with the radiation field. In the case of weak intensity radiation,
where the linear response regime is sufficient to describe the changes in the
molecular system, one needs to only consider responses that are first order in
these observables.
...
subgroup, on the reference state. In the study of electronic systems it is
...
manifolds, $\mathcal{M},$ of the $N$-electron Hilbert space, $\mathcal{H}%
^{N},$ generated by cosets formed by one-particle operators.
...
considerations will be developed in future manuscripts.
...
study in detail the forms of AGP\ coherent states. We also show how the
coherent state construction applied to the unitary group of $N$-electron space
can reproduce the familiar configurational interaction expansion. We conclude
with a summary and discussion in section \ref{discussion}.
...
realizations of many classical Lie groups, Lie algebras and universal
enveloping algebras \cite{Encyclo}. In particular the generators
\begin{equation}
...
r\right)  $ in $\mathcal{B}\left(  \mathcal{H}_{F}\right)  $.

The representation of $so\left(  2r+1,\mathbb{R}\right)  $ is irreducible,
while the representations of $so\left(  2r,\mathbb{R}\right)  $ and $u\left(
...
$2s$-dimensional Hilbert space, the subgroups, $U\left(  N\right)  \times
...
s,\mathbb{Q}\right)  $ of a $s$-dimensional quaternionic Hilbert space. We
signify the CS's that they induce as $G/K$-CS's, where for example $G=U\left(
2s\right)  $ and the subgroup $K\in\left\{  SO\left(  2s,\mathbb{R}\right)
,U\left(  N\right)  \times U\left(  2s-N\right)  ,USp\left(  2s\right)
\right\}  $.
...
$ respectively, (see \cite{Gilmore_book} for the definitions of these groups).
...
\begin{subequations}
\begin{align}
&  U\left(  2s\right)  \left/  U\left(  N\right)  \times U\left(  2s-N\right)
\overset{}{}\right.  ;\quad0\leq N\leq2s\label{c1}\\
...
smaller Manifolds. This technical feature, although important in the
classification of Riemannian Manifolds, is too restrictive when considering
manifolds of CS's, where we also need to examine decompositions generated by
subgroups of these maximal compact groups. We thus enlarge the designation
...
\times U\left(  2s-N\right)  \right\vert 0\leq N\leq2s\right\}  ,USp\left(
2s\right)  \right\}  ,
\end{equation}
...
representation of $SO\left(  2s,\mathbb{R}\right)  $ and we conjecture that
there are \emph{no }subgroups of $SO\left(  2s,\mathbb{R}\right)  ,$ which
\emph{are not} subgroups of $U\left(  N\right)  \times U\left(  2s-N\right)  $
or $USp\left(  2s\right)  ,$ that produce one-dimensional representations on
$\mathcal{H}^{N}$. Thus there are \emph{no }CS manifolds in $\mathcal{H}^{N}$
based on subgroups of $SO\left(  2s,\mathbb{R}\right)  $ that cannot be
described in terms of $U\left(  N\right)  \times U\left(  2s-N\right)  $ or
$USp\left(  2s\right)  $ CS's.

The only subgroup from the set $\left\{  \left.  U\left(  N\right)  \times
U\left(  2s-N\right)  \right\vert 0\leq N\leq2s\right\}  $ that produces a
one-dimensional representation in $\mathcal{H}^{N}$ is $U\left(  N\right)
\times U\left(  2s-N\right)  .$ These $U\left(  2s\right)  \left/  U\left(
N\right)  \times U\left(  2s-N\right)  \overset{}{}\right.  $-coherent states
...
$U\left(  N\right)  \times U\left(  2s-N\right)  $ is actually a subgroup of
$USp\left(  2s\right)  $ and we display in section $\left(  \ref{cohstate}%
\right)  $ that $U\left(  2s\right)  /\left[  U\left(  N\right)  \times
U\left(  2s-N\right)  \right]  $-CS's can in fact be described in terms of the
class of $U\left(  2s\right)  /USp\left(  2s\right)  $-CS's.
...
$U\left(  2s\right)  $ can be expressed in terms of the cosets appearing in
eq. $\left(  \ref{USPCS}\right)  ,$ which have the form
\begin{equation}
...
s\right\}  $ of canonical coefficients \cite{Weiner&ortiz04} of the geminal
that determines the AGP\ state according to:

\begin{description}
\item[$\bullet$] $\sigma=$ number of non zero, non equal $\left\{
c_{j}\right\}  $'s

\item[$\bullet$] $\omega_{j}=$ number of equal coefficients in the $j^{th}$ set
...
We now consider a few particular examples. If all the coefficients are
different and non zero, so that $\xi=0,\omega=0,\rho=0,$ the isotropy subgroup
is
\begin{equation}
...
case the action of coset on the reference AGP generates the manifold
containing all AGP\ states.

If $\xi\neq0$ the reference AGP\ is produced by a degenerate geminal and the
action of the associated coset only produces a submanifold of AGP\ states, all
of which correspond to geminals that have the same null space $\left(
\text{see Appendix \ref{agp}}\right)  $ as the reference AGP.

If $\rho\neq0$ then the geminal has some extreme components and again the
coset only generates a sub manifold of AGP's from the reference.

If $\sigma=0$, $\xi=0$ and $\rho=s$ then the reference state is an extreme
AGP\ (see Appendix \ref{agp}) and the action of the coset on the reference
produces the manifold of \emph{all }extreme AGP states.
...
maximal compact subgroups $U\left(  2M\right)  \times U\left(  2s-2M\right)  $
and $USp\left(  2s\right)  ,$ while others are reducible and correspond to non
maximal compact subgroups $USp\left(  2\omega_{1}\right)  \times\cdots\times
...
physical properties e.g. $U\left(  2M\right)  \times U\left(  2s-2M\right)  $
...
2\omega_{1}\right)  \times\cdots\times USp\left(  2\omega_{\rho}\right)
\times SU\left(  2\right)  ^{\sigma}\times SU\left(  2\xi\right)  $ invariant
...
for a group $G$ if there is an isotropy subgroup $K,$ that is contained in
$G,$ that has $1$-dimensional representation on $\mathcal{H}^{N}$ defined by
\begin{equation}
...
$2M$-electron AGP\ states, $\left\vert g^{N}\right\rangle $ invariant up to an
overall phase factor and thus satisfy eq. $\left(  \ref{subgpcon}\right)  $.
AGP states are given \cite{Weiner&ortiz04} by%
\begin{equation}
\left\vert g^{N}\right\rangle =\sum_{1\leq j_{1}<j_{2}<\cdots<j_{N}\leq
s}c_{j_{1}}\cdots c_{j_{N}}\left\vert \eta_{j_{1}}\eta_{j_{1}+s}\cdots
...
$\left\{  \left.  c_{j}\right\vert 1\leq j\leq s\right\}  $ are positive real
numbers called the canonical coefficients of a two electron geminal
$\left\vert g\right\rangle $ that has the canonical expansion
\begin{equation}
\left\vert g\right\rangle =\sum_{1\leq j\leq s}c_{j}\left\vert \eta_{j}%
\eta_{j+s}\right\rangle . \label{gemcan}%
\end{equation}
The state $\left\vert g^{N}\right\rangle $ can also be expressed in terms of
the coset $SO\left(  2r,\mathbb{R}\right)  /SU\left(  r\right)  $ of the group
$SO\left(  2r,\mathbb{R}\right)  $ acting on the zero particle vacuum state
$\left\vert \phi\right\rangle $ of $\mathcal{H}_{F}$ \cite{Deumens87} as
\begin{equation}
\left\vert g^{N}\right\rangle =P_{N}\exp\left\{  \sum_{1\leq j\leq s}%
c_{j}b_{j}^{\dagger}b_{j+s}^{\dagger}\right\}  \left\vert \phi\right\rangle ,
\end{equation}
where $P_{N}$ is the orthogonal projector onto $\mathcal{H}^{N}$ and $\left\{
\left.  b_{j}^{\dagger}\right\vert 1\leq j\leq s\right\}  $ are the creators
for the one particle states $\left\{  \left.  \left\vert \eta_{j}\right\rangle
\right\vert 1\leq j\leq r\right\}  $ i.e.
\begin{equation}
b_{j}^{\dagger}\left\vert \phi\right\rangle =\left\vert \eta_{j}\right\rangle
.
\end{equation}
This form has been explored in some detail in \cite{Weiner94}. However we will
not pursue this form of the AGP\ state further in this article.
...
$\left\vert g^{N}\right\rangle $, state produces a manifold, $\mathcal{M}%
\left(  \sigma,\omega_{1},\cdots,\omega_{\rho},\xi\right)  $ of AGP states
given by
\begin{equation}
\mathcal{M}\left(  \sigma,\omega_{1},\cdots,\omega_{\rho},\xi\right)
=\left\{  \left[  U\left(  2s\right)  \left/  K\overset{}{}\right.  \right]
\times K\right\}  \left\vert g^{N}\right\rangle , \label{su2act}%
\end{equation}
where
\begin{equation}
...
\end{equation}
and the geminal is of type $\left(  \sigma,\omega_{1},\cdots,\omega_{\rho}%
,\xi\right)  $. Each element of the manifold can be expressed as
\begin{equation}
\left\vert g\left(  c\right)  ^{N}\right\rangle =ck_{\omega_{1}}\cdots
k_{\omega_{\rho}}k_{\sigma}k_{\xi}\left\vert g^{N}\right\rangle =c\left\vert
g^{N}\right\rangle ,
\end{equation}
...
}\in SU\left(  2\right)  ^{\sigma},k_{\xi}\in SU\left(  2\xi\right)  ,c\in
U\left(  2s\right)  /K.
\end{equation}
...
The isotropy subgroup, $K$, for an AGP reference state that has non equal and
nonzero canonical coefficients $\left\{  \left.  c_{j}\right\vert 1\leq j\leq
s\right\}  ,$ i.e. $\sigma=s$ and $\left\vert g\right\rangle $ belongs the
class $\left\{  s,0,0\right\}  ,$ is the product group $SU\left(  2\right)
...
different to the real compact isotropy group). Restricting attention to the
...
$\left\vert g^{N}\right\rangle $ i.e.
\begin{equation}
b\left\vert g^{N}\right\rangle =0\quad\forall b\in N_{-1}\oplus\mathcal{K},
\end{equation}
while the subalgebras $\left\{  N_{+1},N_{0}\right\}  $ have the property
\begin{equation}
b\left\vert g^{N}\right\rangle \neq0\quad\forall b\in N_{+1}\oplus N_{0}.
\end{equation}
...
\end{equation}
where $\mathbf{\tilde{z}}\equiv\left(  \mathbf{z}_{+},\mathbf{\lambda,z}%
_{-}\right)  .$ The action of $U\left(  2s\right)  $ on the AGP state is
\begin{align}
u\left(  \mathbf{\tilde{z},\eta}\right)  \left\vert g^{N}\right\rangle  &
=\tilde{c}\left(  \mathbf{\tilde{z}}\right)  k\left(  \mathbf{\eta}\right)
\left\vert g^{N}\right\rangle \nonumber\\
&  =c_{+1}\left(  \mathbf{z}_{+}\right)  c_{0}\left(  \mathbf{\lambda}\right)
c_{-1}\left(  \mathbf{z}_{-}\right)  k\left(  \mathbf{\eta}\right)  \left\vert
g^{N}\right\rangle =c_{+1}\left(  \mathbf{z}_{+}\right)  c_{0}\left(
\mathbf{\lambda}\right)  \left\vert g^{N}\right\rangle ,
\end{align}
...
g^{N}\right\rangle \equiv c\left(  \mathbf{z}\right)  \left\vert
g^{N}\right\rangle
\end{equation}
...
&  \quad\quad\quad\quad\quad\quad\quad\quad\left.  -s\leq j<k\leq s,\left\vert
j\right\vert \neq\left\vert k\right\vert ;\,\sigma\left(  j\right)
=\operatorname{sign}\left(  j\right)  \right\} \label{l1}\\
...
i\left(  a_{j}^{\dagger}a_{-j}-a_{-j}^{\dagger}a_{j}\right)  ;1\leq j\leq
s\right\} \label{l2}\\
...
&  \quad\quad\quad\quad\quad\quad\quad\quad\left.  -s\leq j<k\leq s,\left\vert
j\right\vert \neq\left\vert k\right\vert ;\,\sigma\left(  j\right)
=\operatorname{sign}\left(  j\right)  \right\} \label{l3}\\
\mathcal{K}  &  =\mathbb{R-}\operatorname{linspan}\left\{  i\left(
a_{j}^{\dagger}a_{-j}+a_{-j}^{\dagger}a_{j}\right)  ,i\left(  a_{j}^{\dagger
}a_{j}-a_{-j}^{\dagger}a_{-j}\right)  ,\right. \nonumber\\
&  \quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\left.  \left(
a_{j}^{\dagger}a_{-j}-a_{-j}^{\dagger}a_{j}\right)  \right\}  ;\quad1\leq
j\leq s, \label{l4}%
\end{align}
where we have translated the indexing of the Canonical General Spin Orbitals
(CGSO's) by
\end{subequations}
\begin{equation}
...
\right\} \nonumber\\
&  \equiv\exp\sum\limits_{_{\substack{-s\leq j<k\leq s\\\left\vert
...
\end{align}%
\begin{equation}
c_{0}\left(  \mathbf{\lambda}\right)  \equiv\exp\left\{  \sum
\limits_{_{\substack{1\leq j<\leq s\\1\leq k\leq4}}}\lambda_{jk}%
n_{jk}\right\}  , \label{co3}%
\end{equation}
and
\begin{equation}
h\left(  \mathbf{\eta}\right)  \equiv\exp\left\{  \sum
\limits_{_{\substack{1\leq j<\leq s\\1\leq k\leq3}}}\eta_{jk}u_{jk}\right\}  ,
\label{co4}%
\end{equation}
where $\left\{  z_{+jk},z_{-jk},\lambda_{jk}\right\}  \in\mathbb{C},\left\{
\eta_{jk}\right\}  \in\mathbb{R}$ , and $\left\{  n_{jk},u_{jk}\right\}  $ are
given by eqs. $\left(  \ref{l2}\right)  $ and $\left(  \ref{l4}\right)  $ respectively.
...
$c_{+1}\left(  \mathbf{z}_{+}\right)  c_{0}\left(  \mathbf{\lambda}\right)  $.
Some of the dependencies between the variables $\left\{  \mathbf{z}%
_{+}\right\}  $ and $\left\{  \mathbf{\lambda}\right\}  $ can be avoided if
one considers unnormalized coherent states by modifying the coset
representative $c_{0}\left(  \mathbf{\lambda}\right)  $ and explicitly
considering the norm of the state in various expectation value expressions.

\subsection{$USp\left(  \omega_{j}\right)  $ factors}

The changes from the $SU\left(  2\right)  ^{s}$ case can be illustrated by an
$\sigma=s-2,\rho=1,\omega_{1}=2$ example (in this case $\left\vert
g\right\rangle $ belongs to the class $\left\{  s-2,2,0\right\}  $), which
clearly leads to the general picture for $\rho>1$. Here we have $s-2$
eigenvalues, $\left\{  \left.  c_{j}^{2}\right\vert 3\leq j\leq s\right\}  $
of $gg^{\dag}$ that are doubly degenerate and one fourfold degenerate
eigenvalue, $c_{1}^{2}$, of $gg^{\dag}$, which is associated with two doubly
degenerate eigenvalues, $\left\{  c_{1},-c_{1}\right\}  $ of $g$ affiliated
with CGSO's which can be linear combinations of $\left\{  \left\vert \eta
_{1}\right\rangle ,\left\vert \eta_{2}\right\rangle \right\}  $ and $\left\{
\left\vert \eta_{-1}\right\rangle ,\left\vert \eta_{-2}\right\rangle \right\}
$ respectively \cite{Weiner&ortiz04}. In this case unitary transformations,
...
j=\pm1,k=\pm1\right\}  $ of CGSO's deleted.

The factors $u_{+1}\left(  \mathbf{z}_{\omega+}\right)  \in\exp U_{+11}%
,u_{-1}\left(  \mathbf{z}_{\omega-}\right)  \in\exp U_{-11},u_{\omega0}\left(
\mathbf{\lambda}_{\omega}\right)  \in\exp U_{01},k_{\omega}\left(
\mathbf{\eta}_{\omega}\right)  \in USp\left(  4\right)  $, where $U_{+1}%
,U_{0},U_{-1}$ and $usp\left(  4\right)  $ are expanded in the following bases%
\begin{equation}
U_{+11}=\mathbb{C-}\operatorname{linspan}\left\{  a_{k}^{\dagger}a_{j}%
+\sigma\left(  j\right)  \sigma\left(  k\right)  a_{-j}^{\dagger}a_{-k};\quad
j=1,-1;k=2,-2\right\}  \label{u1}%
\end{equation}%
\begin{align}
...
\quad\quad\quad\quad\quad\quad\left.  j=1,-1;k=2,-2\overset{}{}\right\}
\label{u2}%
\end{align}%
\begin{equation}
U_{-11}=\mathbb{C-}\operatorname{linspan}\left\{  a_{k}^{\dagger}a_{j}%
-\sigma\left(  j\right)  \sigma\left(  k\right)  a_{-j}^{\dagger}a_{-k};\quad
j=1,-1;k=2,-2\right\}  \label{u3}%
\end{equation}%
\begin{align}
&  usp\left(  2\omega_{1}\right)  =usp\left(  4\right) \nonumber\\
&  \qquad=\mathbb{R-}\operatorname{linspan}\left\{  i\left(  a_{j}^{\dagger
}a_{j}-a_{-j}^{\dagger}a_{-j}\right)  ,i\left(  a_{j}^{\dagger}a_{-j}%
+a_{-j}^{\dagger}a_{j}\right)  ,\left(  a_{j}^{\dagger}a_{-j}-a_{-j}^{\dagger
}a_{j}\right)  ,\right. \nonumber\\
&  \qquad\qquad\qquad\qquad\left(  a_{k}^{\dagger}a_{j}-\sigma\left(
j\right)  \sigma\left(  k\right)  a_{-j}^{\dagger}a_{-k}-a_{j}^{\dagger}%
a_{k}+\sigma\left(  j\right)  \sigma\left(  k\right)  a_{-k}^{\dagger}%
a_{-j}\right)  ,\nonumber\\
&  \qquad\qquad\qquad i\left(  a_{k}^{\dagger}a_{j}-\sigma\left(  j\right)
\sigma\left(  k\right)  a_{-j}^{\dagger}a_{-k}+a_{j}^{\dagger}a_{k}%
-\sigma\left(  j\right)  \sigma\left(  k\right)  a_{-k}^{\dagger}%
a_{-j}\right)  ;\nonumber\\
&  \qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\left.
j=1,-1;k=2,-2\overset{}{}\right\}  , \label{u4}%
\end{align}
...
\mathbf{\lambda}_{\sigma}\right)  \left\vert g^{N}\right\rangle ,
\end{equation}
...
\rho\right\}  $ annihilate $\left\vert g^{N}\right\rangle $ i.e.
\begin{equation}
b\left\vert g^{N}\right\rangle =0\quad\forall b\in\bigoplus\limits_{1\leq
j\leq\rho}U_{-1j}\oplus\bigoplus\limits_{1\leq j\leq\rho}usp\left(
2\omega_{j}\right)  ,
\end{equation}
while the subalgebras $\left\{  \left.  U_{+1j},U_{0j}\right\vert 1\leq
j\leq\rho\right\}  $ have the property
\begin{equation}
b\left\vert g^{N}\right\rangle \neq0\quad\forall b\in\bigoplus\limits_{1\leq
j\leq\rho}U_{+1j}\oplus\bigoplus\limits_{1\leq j\leq\rho}U_{0j}.
\end{equation}
...
$-coherent state manifold, $\mathcal{M}\left(  s-\omega,\omega_{1}%
,\cdots,\omega_{\rho},\xi\right)  $, is then given by
\begin{equation}
\left\vert \Phi\left(  \mathbf{z}\right)  \right\rangle =c_{+1}\left(
...
$\sigma=s-1,\rho=0,\xi=1$ example, ($\left\vert g\right\rangle $ belongs the
class $\left\{  s-1,0,1\right\}  $), which also clearly leads to the general
...
\begin{align}
&  su\left(  2\right)  =\nonumber\\
&  \mathbb{R-}\operatorname{linspan}\left\{  i\left(  a_{1}^{\dagger}%
a_{-1}+a_{-1}^{\dagger}a_{1}\right)  ,i\left(  a_{1}^{\dagger}a_{1}%
-a_{-1}^{\dagger}a_{-1}\right)  ,\left(  a_{1}^{\dagger}a_{-1}-a_{-1}%
^{\dagger}a_{1}\right)  \right\}
\end{align}
...
$\left\vert g^{N}\right\rangle $ i.e.
\begin{equation}
b\left\vert g^{N}\right\rangle =0\quad\forall b\in su\left(  2\xi\right)  .
\end{equation}
...
class $\left\{  M,0,s-M\right\}  $ cannot in fact exist, while the class
$\left\{  0,M,s-M\right\}  $ does and such reference AGP's are IPS's and the
$U\left(  2s\right)  /\left[  USp\left(  2M\right)  \times SU\left(
2s-2M\right)  \right]  $-coherent state manifold is in fact the Thouless
$U\left(  2s\right)  /\left[  U\left(  2M\right)  \times U\left(
2s-2M\right)  \right]  $-coherent state manifold.

\subsection{$U\left(  \mathcal{H}^{N}\right)  /U\left(  1\right)  $-Coherent
States}

It is illuminating and instructive to consider the CS formulation of the
manifold, $\mathcal{M}_{N},$ of all $N$-particle states based on the unitary
group $U\left(  \mathcal{H}^{N}\right)  \equiv U\left(  \left(
%TCIMACRO{\QATOP{r}{N}}%
%BeginExpansion
\genfrac{}{}{0pt}{}{r}{N}%
%EndExpansion
\right)  \right)  $ of $\mathcal{H}^{N}.$ In this case the configuration space
formed by the coherent sates represents all of the states $\left\vert
\Psi\right\rangle \in\mathcal{H}^{N}$ via a parameterization $\left\{
\left\vert \Phi\left(  \mathbf{z}\right)  \right\rangle \right\}  $ and the
\emph{exact }quantum evolution equations can be expressed in terms of
classical equations of motion, that are equivalent to the full TDSE, for paths
belonging to $\mathcal{M}_{N}$. The isotropy subgroup is
\begin{align}
K  &  =\left\{  \left.  g\right\vert g\in G,g\left\vert \Phi_{1}\right\rangle
=e^{i\theta\left(  g\right)  }\left\vert \Phi_{1}\right\rangle \right\}
\nonumber\\
&  =\left\{  \exp\left(  i\lambda\left(  g\right)  \left\vert \Phi
_{1}\right\rangle \left\langle \Phi_{1}\right\vert \right)  \exp\left(
...
\begin{equation}
U\left(  \mathcal{H}^{N}\right)  /K=\left\{  \exp\left(  \sum_{i>1}\eta
_{i}\left\vert \Phi_{i}\right\rangle \left\langle \Phi_{1}\right\vert \right)
\exp\left(  \eta_{1}\left\vert \Phi_{1}\right\rangle \left\langle \Phi
_{1}\right\vert \right)  \exp\left(  -\sum_{i>1}\bar{\eta}_{i}\left\vert
\Phi_{i}\right\rangle \left\langle \Phi_{1}\right\vert \right)  \right\}  ,
\end{equation}
...
\section{Appendix: AGP States \label{agp}}

The $N=2M$ electron AGP\ states, $\left\vert g^{N}\right\rangle $ can be
expanded as
\begin{equation}
\left\vert g^{N}\right\rangle =\sum_{1\leq j_{1}<j_{2}<\cdots<j_{N}\leq
s}c_{j_{1}}\cdots c_{j_{N}}\left\vert \eta_{j_{1}}\eta_{j_{1}+s}\cdots
\eta_{j_{N}}\eta_{j_{N}+s}\right\rangle ,
\end{equation}
where $\left\{  \left.  \left\vert \eta_{j}\right\rangle \right\vert 1\leq
j\leq r=2s\right\}  $ are Canonical General Spin Orbitals (CGSO) which form a
complete orthonormal basis for $\mathcal{H}^{1},$ and $\left\{  \left.
c_{j}\right\vert 1\leq j\leq s\right\}  $ are positive real numbers called the
canonical coefficients of the two electron geminal $\left\vert g\right\rangle
$ that has the canonical expansion
\begin{equation}
\left\vert g\right\rangle =\sum_{1\leq j\leq s}c_{j}\left\vert \eta_{j}%
\eta_{j+s}\right\rangle . \label{gemexp}%
\end{equation}


The canonical coefficients are the square roots of the occupation numbers
$\left\{  \left.  n_{j}=n_{j+s}\right\vert 1\leq j\leq s\right\}  $ of the
First Order Reduced Density Operator (FORDO)\ $D^{1}\left(  g\right)  $ of the
two electron state $\left\vert g\right\rangle $. The number, $\nu$, of non
zero $\left\{  \left.  n_{j}\right\vert 1\leq j\leq s\right\}  $ is called the
rank of the geminal $\left\vert g\right\rangle $, the dimension of the null
space of $\left\vert g\right\rangle .$is equal to $2\left(  s-\nu\right)  $.
If all the occupation are equal (hence nonzero) the geminal is called extreme
and the state $\left\vert g^{N}\right\rangle $ an extreme AGP\ state.

The CGSO's $\left\{  \left.  \left\vert \eta_{j}\right\rangle \right\vert
1\leq j\leq r\right\}  $ diagonalize both the FORDO of the geminal state
$\left\vert g\right\rangle $ and the AGP\ state $\left\vert g^{N}\right\rangle
$ and are thus Natural General Spin Orbitals (NGSO's) of these operators,
\emph{however }NGSO's of these operators \emph{are not }necessarily CGSO's! In
fact if one multiplies $\left\{  \left.  \left\vert \eta_{j}\right\rangle
\right\vert 1\leq j\leq r\right\}  $ by arbitrary, but at least some non
equal, phase factors a \emph{different }geminal $\left\vert g\right\rangle $
is produced. It is interesting to observe that different geminals produced in
such a manner have the same FORDO.

...
%TCIDATA{LastRevised=Saturday, July 14, 2007 10:22:54}
...
use the symbol $\mathbf{\psi}$ and when we refer to NGSOs we will use the
...
,\mathbf{\psi}\right)  $ (first introduced by Hinze \cite{Hinze}), which is
\emph{self--adjoint only} at stationary points $\mathbf{\psi}_{s}$ of the
orbital variation. Further if $\mathbf{n}_{s}$ is a stationary point of the
occupation number variation, then the diagonal elements of the matrix of
$F\left(  \mathbf{n}_{s},\mathbf{\psi}_{s}\right)  $ in the CGSO basis
$\left\{  \left\vert \left\vert \psi_{j}\right\rangle \right\vert 1\leq
j\leq2s\right\}  $ are ionization energies associated with these orbitals
...
ionization energies associated with the orbitals $\left\{  \left\vert \chi
...
_{s}\right)  ^{N}\right\rangle ,$ will \emph{not }be in a canonical form in
terms of the basis $\left\{  \left.  \left\vert \chi_{j}\right\rangle
...
the orbitals and a commutative nonunitary group of positive transformations,
...
equivalence classes $\left[  D^{1}\right]  $ and $\mathbf{\theta}$ produces
...
correspond to each other in a bijective manner \cite{Yukalov00} and can be
...
\mathbf{0}\right)  \right)  \right]  $ also can be expressed in canonical form
as
\begin{equation}
...
\right)  $, given by \cite{Yukalov00}\cite{Coleman97}
\begin{align}
...
expanded in terms of $\mathbf{\theta}$--dependent NGSOs $\left\{  \left.
...
\mathbf{R}_{1},\cdots,\mathbf{R}_{N_{nuc}}\right)  $ in terms of the CGSOs
...
variables even though $\mathbf{\psi}$ determines $\mathbf{\psi}^{\ast}$.) The
one--electron matrix elements $\left\{  \left\langle \psi_{j}\left\vert
h_{1}\psi_{k}\right.  \right\rangle \right\}  $ wrt the CGSOs are
\begin{equation}
...
\mathbf{r}_{1},\xi_{1}\right)  \psi_{k}^{\ast}\left(  \mathbf{r}_{2},\xi
_{2}\right)  \frac{1}{\left\Vert \mathbf{r}_{1}-\mathbf{r}_{2}\right\Vert
}\psi_{l}\left(  \mathbf{r}_{1},\xi_{1}\right)  \psi_{m}\left(  \mathbf{r}%
_{2},\xi_{2}\right)  \right. \nonumber\\
&  -\left.  \psi_{j}^{\ast}\left(  \mathbf{r}_{1},\xi_{1}\right)  \psi
_{k}^{\ast}\left(  \mathbf{r}_{2},\xi_{2}\right)  \frac{1}{\left\Vert
\mathbf{r}_{1}-\mathbf{r}_{2}\right\Vert }\psi_{l}\left(  \mathbf{r}_{2}%
,\xi_{2}\right)  \psi_{m}\left(  \mathbf{r}_{1},\xi_{1}\right)  \right\}
...
NGSOs for the class $\left[  D^{1}\left(  g^{N}\left(  \mathbf{0}\right)
\right)  \right]  $. As $\left(  \mathbf{n},\mathbf{\varphi,\theta}\right)  $
uniquely defines a $2N$-electron AGP-FORDO, the expression in eq.\ $\left(
...
where
\begin{equation}
e^{i\mathbf{\theta}}\left\vert \mathbf{\varphi}^{\prime}\right\rangle =\left(
...
\right\vert \mathbf{\alpha}=\mathbf{\alpha}^{\dagger},\operatorname{Tr}%
\mathbf{\alpha}=0\right\}  , \label{orb}%
\end{equation}
...
}\right)  p\left(  \mathbf{\theta}\right)  u\left(  \mathbf{\alpha}\right)
}\left\vert g_{0}^{N}\right\rangle \right\vert \quad0\leq\theta_{j}%
<2\pi;\,\eta_{j}\in\mathbb{R};\right. \nonumber\\
&  \qquad\qquad\qquad\qquad\qquad\left.  \mathbf{\alpha}=\mathbf{\alpha
}^{\dagger},\operatorname{Tr}\mathbf{\alpha}=0,1\leq j\neq k\pm s\leq
...
where
\begin{equation}
\mathcal{H}\left(  \mathbf{\eta},\mathbf{\alpha},\mathbf{\theta}\right)
...
\mathbf{n,\psi}\right)  $ must be self--adjoint when based on an optimized
state. The details of the derivations are contained in Appendices
\ref{Deriv_Fock}, \ref{VarGroupPar} and \ref{StatCon}, where expressions based
on general $2N$-electron states are obtained.
...
\varepsilon_{kj}\left(  \left\langle \left.  \psi_{j}\right\vert \psi
...
a generalized Fock operator $F\left(  \mathbf{n,\psi}\right)  $. The
...
stationary point of the orbital variation, $F\left(  \mathbf{n,\psi}\right)  $
becomes self--adjoint. Hence
\begin{equation}
...
by $\left(  \mathbf{\eta,\alpha,\theta}\right)  $. Setting
\begin{equation}
...
\begin{equation}
U_{orb}=\left\{  u\left(  \mathbf{\beta}\right)  =\exp\sum_{1\leq\left\vert
j\right\vert <\left\vert k\right\vert \leq s}i\left\{  \beta_{jk}%
q_{jk}^{\dagger}+\beta_{jk}^{\ast}q_{jk}\right\}  \right\}  ,
\end{equation}
where
\begin{equation}
...
^{2}\left(  \mathbf{n}\right)  $ the SORDM of the $2N$-electron AGP\ state
...
\begin{equation}
F\left(  \mathbb{D}^{2}\mathbf{,\psi}\right)  =\sum_{1\leq m,n\leq2s}\left\{
\left(  \mathbb{H}_{1}\left(  \mathbf{\psi}\right)  \mathbb{D}^{1}\right)
_{mn}+\frac{1}{2}\left(  L_{2}^{1}\left(  \mathbb{V}_{2}\left(  \mathbf{\psi
}\right)  \mathbb{D}^{2}\right)  \right)  _{mn}\right\}  \left\vert \psi
...
\begin{equation}
F\left(  \mathbf{n},\mathbf{\varphi}\right)  _{qp}=\frac{\left\langle \left.
...
\begin{equation}
F\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  _{pp}=\frac{\left\langle
\left.  \Psi\right\vert \left\{  a_{p}^{\dagger}a_{p}H-a_{p}^{\dagger}%
Ha_{p}\right\}  \Psi\right\rangle }{\left\langle \left.  \Psi\right\vert
\Psi\right\rangle }=\mathcal{E}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)
\Omega_{p}-\frac{\left\langle \left.  a_{p}\Psi\right\vert Ha_{p}%
\Psi\right\rangle }{\left\langle \left.  \Psi\right\vert \Psi\right\rangle },
\end{equation}
...
\begin{equation}
F\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  _{pp}=\Omega_{p}\left\{
\mathcal{E}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  -\frac{\left\langle
...
\mathcal{E}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  -\mathcal{E}%
_{2N-1}\left(  \mathbb{D}^{2},\mathbf{\varphi,}\varphi_{p}\right)  \right\}  ,
\end{equation}
where $\mathcal{E}_{2N-1}\left(  \mathbb{D}^{2},\mathbf{\varphi,}\varphi
...
$=0$ then $F\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  _{pp}=0$ as the
GSO $\left\vert \varphi_{p}\right\rangle $ is unoccupied. In Appendix
\ref{Fock&Energy} we show that the total energy can be expressed in terms of
the Fock operator $F\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  \ $as
\begin{equation}
...
\end{equation}
where $\mathcal{E}_{1}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  $ is the
expectation value of the one--electron part of the Hamiltonian. Hence the
energy $\mathcal{E}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  $ can be
expressed as
\begin{equation}
\mathcal{E}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  =\frac{1}{2}%
\sum_{1\leq p\leq2s}\left\{  \Omega_{p}I\left(  \varphi_{p}\right)
+\mathcal{E}_{1}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  \right\}  ,
\label{TEexpec}%
\end{equation}
...
,\mathbf{\varphi}\right)  -\mathcal{E}_{2N-1}\left(  \mathbb{D}^{2}%
,\mathbf{\varphi},\varphi_{p}\right)  .
\end{equation}
...
$\left\{  F\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  _{qp}\right\}  $
can be diagonalized by a unitary transformation $u$ of the GSO basis $\left\{
\left\vert \varphi_{j}\right\rangle \right\}  $ to produce
\begin{equation}
\tilde{F}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  _{qp}=\frac
...
\end{equation}
The eigenvalues of the matrix $F\left(  \mathbb{D}^{2},\mathbf{\varphi
}\right)  $ (the diagonal elements of the matrix $\tilde{F}\left(
\mathbb{D}^{2},\mathbf{\varphi}\right)  $ ) are
\begin{align}
\varepsilon_{p}  &  =\tilde{F}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)
...
&  =\mathcal{N}_{p}\left\{  \mathcal{E}\left(  \mathbb{D}^{2},\mathbf{\varphi
}\right)  -\mathcal{E}_{2N-1}\left(  \mathbb{D}^{2},\mathbf{\varphi,}%
\varsigma_{p}\right)  \right\}  ,
\end{align}
...
$=\left\vert \Psi\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  \right\rangle
...
^{2},\mathbf{\varsigma}\right)  \right\rangle ,
\end{equation}
...
\begin{equation}
\mathbb{D}^{2}\left(  \mathbf{\varphi}\right)  =\mathbb{D}^{2}%
\end{equation}
and
\begin{equation}
\mathbb{D}^{2}\left(  \mathbf{\varsigma}\right)  =\mathbb{\tilde{D}}^{2},
\end{equation}
which leads to
\begin{equation}
\tilde{F}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  \equiv F\left(
\mathbb{D}^{2}\left(  \mathbf{\varsigma}\right)  ,\mathbf{\varsigma}\right)
\end{equation}
and
\begin{equation}
F\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  \equiv F\left(
\mathbb{D}^{2}\left(  \mathbf{\varphi}\right)  ,\mathbf{\varphi}\right)  .
\end{equation}
One should note that the FORDO $D^{1}\left(  \Psi\right)  \equiv$
$D^{1}\left(  \mathbb{D}^{2}\left(  \mathbf{\varphi}\right)  \right)  \equiv
D^{1}\left(  \mathbb{D}^{2}\left(  \mathbf{\varsigma}\right)  \right)  $ is
still not in general diagonal in the basis $\left\{  \left\vert \varsigma
_{p}\right\rangle \right\}  $ i.e.\ these GSO's are not NGSOs of $\left\vert
\Psi\right\rangle $.

One can, however, also express the generalized Fock operator $F\left(
\mathbb{D}^{2},\mathbf{\varphi}\right)  $ in terms of the NGSOs $\left\{
\left\vert \upsilon_{j}\right\rangle \right\}  $ producing the generalized
Fock matrix $\mathbb{F}\left(  \mathbb{D}^{2}\left(  \mathbf{\upsilon}\right)
,\mathbf{\upsilon}\right)  ,$ that is not in general diagonal, but has
diagonal elements given by
\begin{equation}
F\left(  \mathbb{D}^{2}\left(  \mathbf{\upsilon}\right)  ,\mathbf{\upsilon
}\right)  _{pp}=N_{p}\left\{  \mathcal{E}\left(  \mathbb{D}^{2}\left(
\mathbf{\upsilon}\right)  ,\mathbf{\upsilon}\right)  ???-\mathcal{E}%
_{2N-1}\left(  \mathbb{D}^{2}\left(  \mathbf{\upsilon}\right)
,\mathbf{\upsilon,}\upsilon_{p}\right)  \right\}  ,
\end{equation}
...
$\equiv\mathcal{E}_{1}\left(  \mathbb{D}^{2}\left(  \mathbf{\upsilon}\right)
,\mathbf{\upsilon}\right)  $ becomes%
\begin{equation}
\mathcal{E}_{1}\left(  \mathbb{D}^{2}\left(  \mathbf{\upsilon}\right)
,\mathbf{\upsilon}\right)  =\sum_{1\leq p\leq2s}N_{p}\left\langle \left.
...
}\right)  \iota_{p}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  ,
\end{equation}
where we have introduced another set of new quantities $\iota_{p}\left(
\mathbb{D}^{2},\mathbf{\varphi}\right)  $defined as
\begin{equation}
\iota_{p}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  =\mathcal{E}%
_{p}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  -\left\langle \left.
\varphi_{p}\right\vert h_{1}\varphi_{p}\right\rangle
\end{equation}
and $N_{p}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  $are the occupation
...
\mathbf{n},\mathbf{\psi}\right)  $ can be expressed as
\begin{equation}
\mathcal{E}\left(  \mathbf{n},\mathbf{\psi}\right)  =\frac{1}{2}\sum_{1\leq
p\leq2s}\left\{  N_{p}I\left(  \psi_{p}\right)  +N_{p}\left\langle \left.
\psi_{p}\right\vert h_{1}\psi_{p}\right\rangle \right\}  \equiv\frac{1}{2}%
%TCIMACRO{\tsum \limits_{1\leq p\leq2s}}%
%BeginExpansion
{\textstyle\sum\limits_{1\leq p\leq2s}}
%EndExpansion
...
\sum_{1\leq q\leq2s}\left\{  h_{1mq}\left(  \mathbf{\varphi}\right)
+J_{mq}\left(  \mathbf{\varphi}\right)  -K_{mq}\left(  \mathbf{\varphi
...
\mathbf{\varphi}\right)  $ with matrix elements
\begin{equation}
f\left(  \mathbf{\varphi}\right)  _{mq}=h_{1mq}\left(  \mathbf{\varphi
...
\begin{equation}
F\left(  \mathbf{\varphi}\right)  =F\left(  \mathbf{\varphi}\right)
^{\dagger}%
\end{equation}
implies that
\begin{equation}
\left[  \mathfrak{f}\left(  \mathbf{\varphi}\right)  ,D^{1}\right]  =0.
\label{HF}%
\end{equation}
...
representation of $\mathfrak{f}\left(  \mathbf{\varphi}\right)  $ wrt the
basis $\left\{  \left\vert \varphi_{j}\right\rangle \right\}  $ is given by
\begin{equation}
\mathfrak{f}\left(  \mathbf{\varphi}\right)  \left\vert \mathbf{\varphi
}\right\rangle =\left\vert \mathbf{\varphi}\right\rangle \left(
\begin{array}
[c]{cc}%
\mathbb{F}_{o}\left(  \mathbf{\varphi}\right)  & \mathbb{O}\\
\mathbb{O} & \mathbb{F}_{u}\left(  \mathbf{\varphi}\right)
\end{array}
\right)  .
\end{equation}
...
\begin{equation}
\mathfrak{f}\left(  \mathbf{\varphi}\right)  \left\vert \psi_{j}\right\rangle
=\left\vert \psi_{j}\right\rangle \varepsilon_{j};\quad1\leq j\leq2s
\end{equation}
...
group parameters $\mathbf{\alpha}$ that are constrained to form a hermitian
matrix. This condition can be easily enforced by expressing the $r\times r$
matrix $\mathbf{\alpha}$ in terms of $r^{2}$ unconstrained real parameters. We
show that this formulation of the energy is equivalent to one in terms of a
SORDM\ $\mathbb{D}^{2}$ and GSOs that are constrained to be orthonormalized.
...
\mathbf{\alpha}\right)  $, of orthonormal GSOs by considering a fixed
reference state $D_{0}^{2N}$ and one--electron unitary transformations
$U\left(  \mathbf{\alpha}\right)  $ by setting%
\begin{equation}
...
\end{align}
Each new nonzero value of $\mathbf{\alpha}$ determines a new state
...
\begin{equation}
\left(  \left.  \mathbf{r}_{1},\xi_{1},\mathbf{r}_{2},\xi_{2}\right\vert
\varphi_{j}\varphi_{k}\right)  =\varphi_{j}\left(  \mathbf{r}_{1},\xi
_{1}\right)  \varphi_{k}\left(  \mathbf{r}_{2},\xi_{2}\right)  ,
\end{equation}
where
\begin{equation}
\varphi_{j}\left(  \mathbf{r}_{1},\xi_{1}\right)  =\left\langle \left.
\mathbf{r}_{1},\xi_{1}\right\vert \varphi_{j}\right\rangle .
\end{equation}
We will consider the energy in terms of the SORDM\ $\mathbb{D}^{2}$ and GSOs
$\mathbf{\varphi}$ given by
\begin{equation}
...
},\delta\mathbf{\varphi}\right\}  $ in the GSOs is given by the Taylors series
as
\begin{align}
...
^{1}\mathcal{E}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  ^{t}%
...
\end{equation}
where $\mathbb{H}_{2}\left(  \mathbf{\varphi}\right)  $ is the
...
$\mathbf{\alpha}$ evaluated around $\mathbf{\alpha}$=$\mathbf{0}$ is
\begin{align}
...
^{2},\mathbf{0}\right)  }{\partial\alpha_{p}q}\right\}  $ can also be
...
\partial_{\mathbf{\phi}^{\ast}}^{1}\mathcal{E}\left(  \mathbf{n}%
,\mathbf{\varphi},\mathbf{\theta}\right)   &  =\frac{1}{S_{N}\left(
\mathbf{n}\right)  }\left\{  \partial_{\mathbf{\phi}^{\ast}}^{1}%
\mathcal{H}\left(  \mathbf{n},\mathbf{\varphi},\mathbf{\theta}\right)
-\mathcal{E}\left(  \mathbf{n},\mathbf{\varphi},\mathbf{\theta}\right)
\partial_{\mathbf{\phi}^{\ast}}^{1}S_{N}\left(  \mathbf{n}\right)  \right\}
=\left[  \partial_{\mathbf{\phi}}^{1}\mathcal{E}\left(  \mathbf{n}%
...
\mathbf{\varphi}\right\}  $ maintain the orthonormality of the NGSOs i.e.\
\end{subequations}
\begin{equation}
...
\partial_{\mathbf{\phi}^{\ast}}^{1}\mathcal{E}\left(  \mathbf{n}%
,\mathbf{\varphi},\mathbf{\theta}\right)   &  =\frac{1}{S_{N}\left(
\mathbf{n}\right)  }\partial_{\mathbf{\phi}^{\ast}}^{1}\mathcal{H}\left(
\mathbf{n},\mathbf{\varphi},\mathbf{\theta}\right) \\
\partial_{\mathbf{\theta}}^{1}\mathcal{E}\left(  \mathbf{n},\mathbf{\varphi
},\mathbf{\theta}\right)   &  =\frac{1}{S_{N}\left(  \mathbf{n}\right)
...
},\mathbf{\alpha},\mathbf{\theta}\right\}  $ and the energy functional
\end{subequations}
\begin{equation}
\mathcal{E}\left(  \mathbf{\eta},\mathbf{\alpha},\mathbf{\theta}\right)
=\frac{\mathcal{H}\left(  \mathbf{\eta},\mathbf{\alpha},\mathbf{\theta
...
g\left(  \mathbf{\eta},\mathbf{\alpha},\mathbf{\theta}\right)  ^{N}\right\vert
Hg\left(  \mathbf{\eta},\mathbf{\alpha},\mathbf{\theta}\right)  ^{N}%
\right\rangle }{S_{N}\left(  \mathbf{\eta}\right)  }%
\end{equation}
and considers the derivatives
\begin{subequations}
\label{GFderiv}%
\begin{align}
\partial_{\mathbf{\eta}}^{1}\mathcal{E}\left(  \mathbf{\eta},\mathbf{\alpha
},\mathbf{\theta}\right)   &  =\frac{1}{S_{N}\left(  \mathbf{\eta}\right)
}\left\{  \partial_{\mathbf{\eta}}^{1}\mathcal{H}\left(  \mathbf{\eta
},\mathbf{\alpha},\mathbf{\theta}\right)  -\mathcal{E}\left(  \mathbf{\eta
},\mathbf{\alpha},\mathbf{\theta}\right)  \partial_{\mathbf{\eta}}^{1}%
S_{N}\left(  \mathbf{\eta}\right)  \right\} \\
\partial_{\mathbf{\alpha}}^{1}\mathcal{E}\left(  \mathbf{\eta},\mathbf{\alpha
},\mathbf{\theta}\right)   &  =\frac{1}{S_{N}\left(  \mathbf{\eta}\right)
}\partial_{\mathbf{\alpha}}^{1}\mathcal{H}\left(  \mathbf{\eta},\mathbf{\alpha
},\mathbf{\theta}\right) \\
\partial_{\mathbf{\theta}}^{1}\mathcal{E}\left(  \mathbf{\eta},\mathbf{\alpha
},\mathbf{\theta}\right)   &  =\frac{1}{S_{N}\left(  \mathbf{\eta}\right)
}\partial_{\mathbf{\theta}}^{1}\mathcal{H}\left(  \mathbf{\eta},\mathbf{\alpha
},\mathbf{\theta}\right)  .
\end{align}
The generators of the GSO and the phase variations produce
\end{subequations}
\begin{equation}
\partial_{\mathbf{\alpha}_{jk}}^{1}\mathcal{E}\left(  \mathbf{\eta
},\mathbf{\alpha},\mathbf{\theta}\right)  =\frac{i}{S_{N}\left(  \mathbf{\eta
}\right)  }\left\langle \left.  g\left(  \mathbf{\eta},\mathbf{\alpha
},\mathbf{\theta}\right)  ^{N}\right\vert \left[  H,a_{j}^{\dagger}%
a_{k}\right]  g\left(  \mathbf{\eta},\mathbf{\alpha},\mathbf{\theta}\right)
^{N}\right\rangle ;\quad1\leq j\neq k\pm s\leq2s \label{Dlamda}%
\end{equation}
and
\begin{equation}
...
},\mathbf{\theta}\right)  ^{N}\right\vert \left[  H,\left(  a_{j}^{\dagger
}a_{j}+a_{j+s}^{\dagger}a_{j+s}\right)  \right]  g\left(  \mathbf{\eta
},\mathbf{\alpha},\mathbf{\theta}\right)  ^{N}\right\rangle ;\quad1\leq j\leq
s, \label{Dtheta}%
\end{equation}
...
,\mathbf{\alpha},\mathbf{\theta}\right)   &  =\frac{1}{S_{N}\left(
\mathbf{\eta}\right)  }\left\{  \left\langle \left.  g\left(  \mathbf{\eta
},\mathbf{\alpha},\mathbf{\theta}\right)  ^{N}\right\vert \left[  H,\left(
a_{j}^{\dagger}a_{j}+a_{j+s}^{\dagger}a_{j+s}\right)  \right]  _{+}g\left(
\mathbf{\eta},\mathbf{\alpha},\mathbf{\theta}\right)  ^{N}\right\rangle
\right. \nonumber\\
&  -\left.  2\mathcal{E}\left(  \mathbf{\eta},\mathbf{\alpha},\mathbf{\theta
}\right)  \left\langle \left.  g\left(  \mathbf{\eta},\mathbf{\alpha
},\mathbf{\theta}\right)  ^{N}\right\vert \left(  a_{j}^{\dagger}a_{j}%
+a_{j+s}^{\dagger}a_{j+s}\right)  g\left(  \mathbf{\eta},\mathbf{\alpha
...
\mathbf{\eta},\mathbf{\alpha},\mathbf{\theta}\right)  $
\begin{equation}
\partial_{\mathbf{\theta}_{j}}^{1}\mathcal{E}\left(  \mathbf{\eta
},\mathbf{\alpha},\mathbf{\theta}\right)  =\frac{i}{S_{N}\left(  \mathbf{\eta
}\right)  }\left\langle \left.  g\left(  \mathbf{\eta},\mathbf{\alpha
...
},\mathbf{\alpha},\mathbf{\theta}\right)  ^{N}\right\rangle =0;\quad1\leq
j\leq s, \label{s1}%
\end{equation}
and%
\begin{equation}
...
\begin{align}
&  \left\langle \left.  g\left(  \mathbf{\eta},\mathbf{\alpha},\mathbf{\theta
}\right)  ^{N}\right\vert \left[  H,\left(  a_{j}^{\dagger}a_{j}%
-a_{j+s}^{\dagger}a_{j+s}\right)  \right]  g\left(  \mathbf{\eta
},\mathbf{\alpha},\mathbf{\theta}\right)  ^{N}\right\rangle \nonumber\\
&  \qquad\qquad\qquad\qquad\qquad\left.  =\right.  \left\langle \left.
g\left(  \mathbf{\eta},\mathbf{\alpha},\mathbf{\theta}\right)  ^{N}\right\vert
\left[  H,a_{j}^{\dagger}a_{j+s}\right]  g\left(  \mathbf{\eta},\mathbf{\alpha
},\mathbf{\theta}\right)  ^{N}\right\rangle \nonumber\\
&  \qquad\qquad\qquad\qquad\qquad\left.  =\right.  \left\langle \left.
g\left(  \mathbf{\eta},\mathbf{\alpha},\mathbf{\theta}\right)  ^{N}\right\vert
\left[  H,a_{j+s}^{\dagger}a_{j}\right]  g\left(  \mathbf{\eta},\mathbf{\alpha
},\mathbf{\theta}\right)  ^{N}\right\rangle \left.  =\right.  0
\end{align}
give
\begin{equation}
\left\langle \left.  g\left(  \mathbf{\eta},\mathbf{\alpha},\mathbf{\theta
...
\begin{equation}
\left(  a_{j}^{\dagger}a_{j}+a_{j+s}^{\dagger}a_{j+s}\right)  \left\vert
g\left(  \mathbf{\eta},\mathbf{\alpha},\mathbf{\theta}\right)  ^{N}%
...
},\mathbf{\alpha},\mathbf{\theta}\right)  ^{N}\right\rangle =\mathcal{E}%
\left(  \mathbf{\eta},\mathbf{\alpha},\mathbf{\theta}\right)  \left\langle
\left.  g\left(  \mathbf{\eta},\mathbf{\alpha},\mathbf{\theta}\right)
^{N}\right\vert a_{j}^{\dagger}a_{j}g\left(  \mathbf{\eta},\mathbf{\alpha
...
\left(  \mathbf{\eta},\mathbf{\alpha},\mathbf{\theta}\right)  N_{j}.
\end{equation}
...
\begin{equation}
v_{ab}\left\vert g\left(  \mathbf{\eta},\mathbf{\varphi}\right)
^{N}\right\rangle =0,\quad u_{ab}\left\vert g\left(  \mathbf{\eta
...
\left\{  \mathbb{H}_{1}\left(  \mathbf{\varphi}\right)  \mathbb{D}%
^{1}\right\}  +\frac{1}{4}\operatorname{Tr}\left\{  \mathbb{V}_{2}\left(
\mathbf{\varphi}\right)  \mathbb{D}^{2}\right\}  \equiv\mathcal{E}_{1}\left(
\mathbb{D}^{1},\mathbf{\varphi}\right)  +\mathcal{E}_{2}\left(  \mathbb{D}%
^{2},\mathbf{\varphi}\right)  ,
\end{equation}
where
\begin{equation}
\mathcal{E}_{1}\left(  \mathbb{D}^{1},\mathbf{\varphi}\right)
=\operatorname{Tr}\left\{  \mathbb{H}_{1}\left(  \mathbf{\varphi}\right)
\mathbb{D}^{1}\right\}
\end{equation}
and
\begin{equation}
\mathcal{E}_{2}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  =\frac{1}%
...
{\delta\mathcal{E}_{1}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  }%
{\delta\varphi_{j}^{\ast}\left(  \mathbf{r,\xi}\right)  }+\frac{\delta
\mathcal{E}_{2}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  }{\delta
...
\begin{equation}
\operatorname{Tr}\left\{  F\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)
\right\}  =\mathcal{E}_{1}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)
+2\mathcal{E}_{2}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  .
\end{equation}
After noting that
\begin{equation}
\mathcal{E}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  =\mathcal{E}%
_{1}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  +\mathcal{E}_{2}\left(
\mathbb{D}^{2},\mathbf{\varphi}\right)
\end{equation}
one obtains
\begin{equation}
...
+\mathcal{E}_{2}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  \right\}
=\left\{  2\mathcal{E}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)
-\mathcal{E}_{1}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  \right\}  .
\end{equation}
...
=\operatorname{Tr}\left\{  F\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)
\right\}  -\mathcal{E}_{2}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)
\label{FEa}\\
\mathcal{E}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)   &  =\frac{1}%
{2}\left\{  \overset{}{\operatorname{Tr}\left\{  F\left(  \mathbb{D}%
^{2},\mathbf{\varphi}\right)  \right\}  +\mathcal{E}_{1}\left(  \mathbb{D}%
...
\mathbb{D}^{2},\mathbf{\varphi}\right)  \frac{\operatorname{Tr}\left\{
...
}\right)  I\left(  \mathbb{D}^{2},\varphi_{p}\right)  +\mathcal{E}_{1}\left(
...
{\textstyle\sum\limits_{1\leq p\leq2s}}
%EndExpansion
N_{p}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  \kappa\left(
...
\sum_{1\leq p\leq2s}N_{p}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)
\mathcal{E}_{p}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  -\mathcal{E}%
...
,\mathbf{\varphi}\right)  -\mathcal{E}_{1}\left(  \mathbb{D}^{2}%
...
\begin{align}
\frac{\delta\mathcal{E}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)
}{\delta\varphi_{j}^{\ast}}  &  =F\left(  \mathbb{D}^{2},\mathbf{\varphi
...
\mathbb{D}^{2},\mathbf{\varphi}\right)  \frac{\delta\mathcal{E}_{p}\left(
\mathbb{D}^{2},\mathbf{\varphi}\right)  }{\delta\varphi_{j}^{\ast}};\quad1\leq
j\leq2s,
\end{align}

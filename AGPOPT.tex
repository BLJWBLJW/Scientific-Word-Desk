...
%TCIDATA{Created=Thursday, December 27, 2001 10:54:41}
%TCIDATA{LastRevised=Tuesday, August 12, 2003 11:48:49}
...
spin symmetry one can obtain an unbiased description the electronic states
of a molecule by finding an IPS\ that minimizes the energy. These
...
is that it does not describe electron - electron correlation effects, which
...
initio manner, invariably increase the computational cost of any calculation
and scale at best as $r^{5}$ and usually involve constraints on the
molecular state description that reflect some bias beyond the choice of the
atomic orbital basis.

There is, however, another state ansatz that preserves most of the benefits
of IPS states while introducing a description of correlation effects, these
are the Antisymmetrized Geminal Powers (AGP) states that were first
introduced to quantum chemistry by \cite{Coleman1}\cite{Coleman2} in the
context of the analysis of the N-representability problem. In the following
decade investigators \cite{Kutzenlig1}\cite{Silver1} examined the use of
such states and Bratoz and Durand \cite{Bratoz1} constructed an energy
optimization procedure. However it was not numerically competitive with
Hartree-Fock Self Consistent Field Methods nor with methods that described
correlation by other means. In the late 1970's it was found \cite{Lindeberg1}
that AGP states produce the best possible self consistent reference state
for the Random Phase Approximation (RPA)\ with exchange. This lead to a
series of calculations, using real restricted AGPs on diatomic molecules
that produced potential energy curves for several states simultaneously and
the oscillator strengths for transitions amongst them over a range of
internuclear distances from equilibrium position to the dissociation limit. 
\cite{Kurtz1} \cite{Weiner1}\cite{Weiner2}\cite{Weiner3}\cite{Weiner4}\cite%
{Weiner5}\cite{Sangfelt87}\cite{Edwards1}\cite{Jensen1}\cite{Jensen2}. The
results of these calculations were in overall good agreement with
experiment, however the method was plagued with numerical problems that
became worse when the method was extended to larger molecules.

In this paper we present a parameterization of AGP\ states and an
optimization method that overcomes these difficulties and at the same time
is competitive with IPS\ SCF ones with respect to the number of floating
point operations and scaling with size of the basis set, while introducing
correlation effects that usually demand greater computer resources. The AGP\
state is determined by a geminal, which in a given atomic orbital basis is
determined by $r\left( r-1\right) /2$ complex variables, in comparisson an
IPS\ is determined by $Nr$ complex variables where $N$ is the number of
electrons.

\section{Bases and Geminals}
...
\right\rangle =0
\end{eqnarray}%
where the inner product between two arbitray spin functions $\sigma $ and $%
\sigma ^{\prime }$ is 
\begin{equation}
...
^{1}$. Orthonormal molecular orbitals $\left\{ \psi _{j}:1\leq j\leq
...
\psi _{j}:1\leq j\leq s\right\} $ can be used to form \emph{restricted spin }%
...
\right)
\end{equation}%
where in general $\mathbb{W}_{\alpha }\neq \mathbb{W}_{\beta }$ but 
\begin{equation}
...
\end{eqnarray}%
where $\mathbf{c}$ and $\mathbf{\gamma }$ are viewed either as vectors or
matrices. As the coordinate transformation is linear one has the following
relationship between partial derrivatives w.r.t the variables $\mathbf{c}$
and $\mathbf{\gamma }$ 
\begin{eqnarray}
...
\partial }{\partial \mathbf{c}}$ are viewed as either vectors or matrices.

\section{The AGP State}
...
g\right\rangle $, by 
\begin{equation}
G^{\dagger }=\sum_{1\leq j<k\leq r}\gamma _{jk}a_{j}^{\dagger
}a_{k}^{\dagger }=\sum_{1\leq j<k\leq r}c_{jk}b_{j}^{\dagger }b_{k}^{\dagger
}
\end{equation}%
where the second quantized creators have the properties 
\begin{eqnarray}
...
\gamma }\right) ^{\dagger }\left| \phi \right\rangle =\sum_{1\leq j<k\leq
r}\gamma _{jk}a_{j}^{\dagger }a_{k}^{\dagger }\left| \phi \right\rangle 
\notag \\
\left| g\left( \mathbf{c}\right) \right\rangle &=&G\left( \mathbf{c}\right)
^{\dagger }\left| \phi \right\rangle =\sum_{1\leq j<k\leq
...
The 2N-electron \emph{unnormalized }AGP state, $\left| g^{N}\right\rangle $,
is defined to be 
\begin{equation}
\left| g^{N}\right\rangle =\left( G^{\dagger }\right) ^{N}\left| \phi
\right\rangle
\end{equation}%
which can be expanded in coordinates as 
\begin{eqnarray}
\left| g^{N}\right\rangle &=&N!\sum_{1\leq j_{1}<k_{1}<\cdots
<j_{N}<k_{N}\leq r}c_{j_{1}k_{1}}\cdots c_{j_{N}k_{N}}\left| \psi
_{j_{1}}\psi _{k_{1}}\cdots \psi _{j_{N}}\psi _{k_{N}}\right\rangle  \notag
\\
&=&N!\sum_{1\leq j_{1}<k_{1}<\cdots <j_{N}<k_{N}\leq r}\gamma
_{j_{1}k_{1}}\cdots \gamma _{j_{N}k_{N}}\left| \varphi _{j_{1}}\varphi
_{k_{1}}\cdots \varphi _{j_{N}}\varphi _{k_{N}}\right\rangle  \label{AGPEX}
\end{eqnarray}%
The factor $N!$ comes from the $N!$ arrangements of the ordered pairs that
is produced by taking the $N^{th}$ power of $G^{\dagger }$.

\subsection{State Energy}

The energy of an 2N-electron AGP state is given by the expression 
\begin{equation}
...
\end{equation*}%
\begin{equation*}
h_{0}=\sum_{1\leq j<k\leq N_{nuc}}\frac{Z_{j}Z_{k}}{\left\| \mathbf{R}_{j}-%
\mathbf{R}_{k}\right\| }\qquad \qquad \qquad \qquad \qquad \quad
\end{equation*}%
\begin{equation*}
h_{1}=-\left( \sum_{_{1\leq j\leq N_{nuc};1\leq k\leq N}}\frac{Z_{j}}{%
\left\| \mathbf{R}_{j}-\mathbf{r}_{k}\right\| }-\frac{1}{2m_{e}}\sum_{1\leq
j\leq N}\nabla _{\mathbf{r}_{j}}^{2}\right) \,
\end{equation*}%
\begin{equation}
h_{2}=\sum_{1\leq j<k\leq N}\frac{1}{\left\| \mathbf{r}_{j}-\mathbf{r}%
_{k}\right\| }\qquad \qquad \qquad \qquad \qquad \qquad \quad
\end{equation}%
and the normalization factor 
\begin{equation}
\mathcal{N}\left( g^{N}\right) =\left\langle g^{N}|g^{N}\right\rangle
\end{equation}%
In terms of coordinates we can write these functions as 
\begin{eqnarray}
...
where%
\begin{eqnarray}
\mathcal{H}\left( \mathbf{c},\mathbf{c}^{\ast }\right) &=&\left( N!\right)
^{2}\sum c_{j_{1}^{\prime }k_{1}^{\prime }}^{\ast }\cdots c_{j_{N}^{\prime
}k_{N}^{\prime }}^{\ast }c_{j_{1}k_{1}}\cdots c_{j_{N}k_{N}}\langle \psi
_{j_{1}^{\prime }}\psi _{k_{1}^{\prime }}\cdots \psi _{j_{N}^{\prime }}\psi
_{k_{N}^{\prime }}\left| H\psi _{j_{1}}\psi _{k_{1}}\cdots \psi _{j_{N}}\psi
_{k_{N}}\right\rangle  \notag \\
...
and 
\begin{eqnarray}
\mathcal{H}\left( \mathbf{\gamma }^{\ast },\mathbf{\gamma }\right) &=&\left(
N!\right) ^{2}\sum \gamma _{j_{1}^{\prime }k_{1}^{\prime }}^{\ast }\cdots
\gamma _{j_{N}^{\prime }k_{N}^{\prime }}^{\ast }\gamma _{j_{1}k_{1}}\cdots
\gamma _{j_{N}k_{N}}\langle \varphi _{j_{1}^{\prime }}\varphi
_{k_{1}^{\prime }}\cdots \varphi _{j_{N}^{\prime }}\varphi _{k_{N}^{\prime
}}\left| H\varphi _{j_{1}}\varphi _{k_{1}}\cdots \varphi _{j_{N}}\varphi
_{k_{N}}\right\rangle  \notag \\
&&\qquad \qquad \qquad \qquad \quad _{1\leq j_{1}<k_{1}<j_{2}<k_{2}<\cdots
<j_{N}<k_{N}\leq r;\,1\leq j_{1}^{\prime }<k_{1}^{\prime }<j_{2}^{\prime
}<k_{2}^{\prime }<\cdots <j_{N}^{\prime }<k_{N}^{\prime }\leq r}
\end{eqnarray}%
and the norm functions $\mathcal{N}\left( \mathbf{c}^{\ast },\mathbf{c}%
\right) $ and $\mathcal{N}\left( \mathbf{\gamma }^{\ast },\mathbf{\gamma }%
\right) $ are obtained by setting $H=I$ in these expressions.

\subsection{Canonical Coordinates}

It is always possible to expand the AGP state in terms of $s$ complex
canonical geminal coordinates\cite{Bloch1}\cite{Zumino1} $\left\{ \zeta
_{j};1\leq j\leq s\right\} $ instead of the $s\left( 2s-1\right) $ complex
coordinates used in eq. $\left( \ref{AGPEX}\right) $ as 
\begin{equation}
...
orbitals of the geminal $\left| g\right\rangle ,$ which has the expansion 
\begin{equation}
\left| g\right\rangle =\sum_{1\leq j\leq s}\zeta _{j}\left| \eta _{j}\eta
_{j+s}\right\rangle
\end{equation}%
and the canonical coordinates have the values 
\begin{eqnarray}
\zeta _{j} &=&n_{j}e^{i\theta _{j}}  \notag \\
n_{j} &=&\left| \zeta _{j}\right| ^{2}
\end{eqnarray}%
The canonical coordinates $\left\{ \zeta _{j}\right\} $ can be constructed $%
\left( \text{see Appendix \ref{Appenccc}}\right) $ by diagonalizing the real
symmetric matrix $\mathbf{c}^{\dagger }\mathbf{c}$ formed from the complex
antisymmetric matrix $\mathbf{c}$ with a real orthogonal transformation $%
\mathbb{T}$ to obtain 
\begin{equation}
\mathbb{T}^{t}\mathbf{c}^{\dagger }\mathbf{c}\mathbb{T=}\left( 
\begin{array}{cc}
\mathbf{n} & \mathbf{0} \\ 
\mathbf{0} & \mathbf{n}%
\end{array}%
...
\end{equation}%
then use $\mathbb{U}$ as a coordinate transformation matrix to obtain 
\begin{equation}
\mathbb{T}^{t}\mathbf{c}\mathbb{U}=\left( 
\begin{array}{cc}
...
\right)
\end{equation}%
Thus the canonical coordinates have the values 
\begin{equation}
\zeta _{j}=n_{j}e^{i\theta _{j}}
\end{equation}%
and 
...
n_{j}=\left| \zeta _{j}\right| ^{2}
\end{equation}%
for a specific geminal $\left| g\right\rangle $. The matrix $\mathbf{c}%
^{\dagger }\mathbf{c}$ is actually the one particle reduced density matrix
associated with the two particle state $\left| g\right\rangle $ represented
on the basis $\left\{ \psi _{j};1\leq j\leq r\right\} $.

One can also construct the canonical coordinates directly from the non
orthogonal $\mathbf{\gamma }$ coordinate system by finding the non unitary
transformation $\mathbb{V}$ that has the properties 
\begin{eqnarray}
\mathbb{V}^{\dagger }\,\mathbf{\gamma }^{\dagger }\,\mathbb{S}_{f}\,\mathbf{%
\gamma \,}\mathbb{V} &=&\left( 
\begin{array}{cc}
\mathbf{n} & \mathbf{0} \\ 
\mathbf{0} & \mathbf{n}%
\end{array}%
\right)  \notag \\
\mathbb{V}^{\dagger }\mathbb{S}_{f}\mathbb{V} &\mathbb{=}&\mathbb{I}_{r}
\end{eqnarray}%
and 
\begin{equation}
\mathbb{V\,}\mathbf{\gamma \,}\mathbb{V}^{t}=\left( 
\begin{array}{cc}
\mathbf{0} & \mathbf{\zeta } \\ 
-\mathbf{\zeta } & \mathbf{0}%
\end{array}%
\right)
\end{equation}%
by solving the generalized hermitian eigenvalue equation 
\begin{equation}
\mathbf{\gamma }^{\dagger }\,\mathbb{S}_{f}\,\mathbf{\gamma \,\,v}_{j}=n_{j}%
\mathbb{S}_{f}\mathbf{v}_{j};\qquad 1\leq j\leq r
\end{equation}%
...
side of eq. $\left( \ref{cancoef}\right) $ non skew diagonal if the axes,
i.e. basis functions, are not changed correspondingly.

The proof of the relationships appearing in eqs.\ $\left( \ref{ucu}\right) $
and $\left( \ref{uccu}\right) $due to \cite{Bloch1}\cite{Zumino1}\cite%
{Coleman2}. In Appendix \ref{Appenccc} we present a theorem based on their
work and derive further properties of $\mathbf{c}^{\dagger }\mathbf{c}$.

\subsection{State Energy in Canonical Coordinates}

A. J. Coleman showed \cite{Coleman2} that the energy of a $2N$-electron AGP\
state (see eq.$\ref{Energy})$ has a particularly succinct form when
expressed in the canonical coordinates $\left( \mathbf{\zeta }^{\ast },%
\mathbf{\zeta }\right) $. The normalization $\mathcal{N}\left( g^{N}\right) $
becomes 
\begin{equation}
\mathcal{N}\left( \mathbf{\zeta }^{\ast },\mathbf{\zeta }\right) =\left(
N\right) !^{2}S_{N}
\end{equation}%
where $S_{N}$ is the $N^{th}$ order elementary symmetric function is defined
in terms of the eigenvalues $\left\{ n_{j};1\leq j\leq s\right\} $ of the
one particle reduced density operator associated with the geminal $\left|
g\right\rangle $ as 
\begin{equation}
S_{N}=\sum_{1\leq j_{1}<j_{2}<\cdots <j_{N}\leq s}n_{j_{1}}\cdots
n_{_{j_{N}}}
\end{equation}%
and the unnormalized energy $\mathcal{H}\left( g^{N}\right) $ in the
canonical coordinates $\left( \mathbf{\zeta }^{\ast },\mathbf{\zeta }\right) 
$ becomes%
\begin{eqnarray}
...
where 
\begin{eqnarray}
&&S_{N-1}\left( \hat{\jmath}\right) =\sum_{1\leq j_{1}<j_{2}<\cdots
<j_{N-1}\leq s}n_{j_{1}}\cdots n_{_{j_{N-1}}};\qquad \qquad \qquad \qquad
j_{1},j_{2},\cdots ,j_{N-1}\neq j \\
&&h_{jk}=-\int \eta _{j}^{\ast }\left( \mathbf{r,\xi }\right) \left\{
\sum_{_{1\leq j\leq N_{nuc}}}\frac{Z_{j}}{\left\| \mathbf{R}_{j}-\mathbf{r}%
\right\| }+\frac{1}{2m_{e}}\nabla _{\mathbf{r}}^{2}\right\} \eta _{k}\left( 
\mathbf{r,\xi }\right) d\mathbf{r}d\xi ;\quad 1\leq j,k\leq 2s \\
...
&&\qquad -\left. \eta _{j}^{\ast }\left( \mathbf{r}_{1}\mathbf{,\xi }%
_{1}\right) \eta _{k}^{\ast }\left( \mathbf{r}_{2}\mathbf{,\xi }_{2}\right) 
\frac{1}{\left\| \mathbf{r}_{1}-\mathbf{r}_{2}\right\| }\eta _{l}\left( 
\mathbf{r}_{2}\mathbf{,\xi }_{1}\right) \eta _{m}\left( \mathbf{r}_{1}%
\mathbf{,\xi }_{2}\right) \right\} d\mathbf{r}_{1}d\mathbf{r}_{2}d\xi
_{1}d\xi _{2};  \notag \\
&&\left. {}\right. \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad
\qquad \qquad \quad 1\leq j,k\leq 2s;1\leq l,m\leq 2s \\
...
\notag \\
&&\left. {}\right. \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad
\qquad \qquad \qquad \qquad \qquad \quad 1\leq j,k\leq s
\end{eqnarray}%
...
the non linear manifold of geminals. However they \emph{do }provide a
procedure for the calculation of the energy of the N-electron AGP state and
its derivatives at a specific point in this geminal manifold.
...
and }canonical coordinates as (see Appendix \ref{aoen}) 
\begin{eqnarray}
&&\mathcal{H}\left( \mathbf{\zeta }^{\ast },\mathbf{\zeta }\right) =h_{0}%
\mathcal{N}\left( g^{N}\right) +\left( N\right) !^{2}  \notag \\
&&\left\{ \sum_{j_{1,}j_{2}=1}^{r}\sum_{1\leq j\leq s}n_{j}S_{N-1}\left( 
\hat{\jmath}\right) \left\{ W_{f\,j_{1}j}^{\ast
}W_{f\,j_{2}j}+W_{f\,j_{1}j+s}^{\ast }W_{f\,j_{2}j+s}\right\}
...
W_{f\,j_{1}j}^{\ast }W_{f\,j_{2}j+s}^{\ast
}W_{f\,j_{3}j}W_{f\,j_{4}j+s}\right.  \notag \\
&&+\sum_{1\leq j\neq k\leq s}\zeta _{j}^{\ast }\zeta _{k}S_{N-1}\left( \hat{%
\jmath}\hat{k}\right) W_{f\,j_{1}j}^{\ast }W_{f\,j_{2}j+s}^{\ast
}W_{f\,j_{3}k}W_{f\,j_{4}k+s}  \notag \\
&&\left. \left. +\sum_{1\leq j,k\leq s}n_{j}n_{k}S_{N-2}\left( \hat{\jmath}%
\hat{k}\right) W_{f\,j_{1}j}^{\ast }W_{f\,j_{2}k}^{\ast
}W_{f\,j_{3}j}W_{f\,j_{4}k}\right\} \left\langle
j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}\right\}
\end{eqnarray}%
where now the matrix elements of the Hamiltonian are computed in the atomic
orbital basis and spin can be integrated out as displayed in Appendix \ref%
{spinint}

\section{Derivatives of the AGP\ State Energy}

\subsection{First Derivatives}

The derivatives of the AGP energy, $\mathcal{E}\left( g^{N}\right) $, in
terms of the $\mathbf{\gamma }$ coordinate system evaluated at a specific
geminal $\left| g\right\rangle $ can be expressed in terms of the
derivatives of $\mathcal{E}\left( g^{N}\right) $ in the $\mathbf{c}$
coordinate system using the coordinate transformation of eqs. $\left( \ref%
{coordtrans}\right) $ and $\left( \ref{d1trans}\right) $. When evaluating
these derivatives at a particular point $\left| g\right\rangle $ it is
convenient to \emph{choose} the coordinate system $\mathbf{c}$ \emph{to be }%
the canonical coordinate system $\mathbf{\zeta }\left( g\right) $ for the
...
\begin{equation}
\frac{\partial G\left( \mathbf{\gamma }\right) ^{N}}{\partial \gamma _{pq}}%
=Na_{p}^{\dagger }a_{q}^{\dagger }G\left( \mathbf{\gamma }\right) ^{\dagger
N-1}=Na_{p}^{\dagger }a_{q}^{\dagger }G^{\dagger N-1}  \label{GPderiv}
\end{equation}%
...
of $\left| g\right\rangle $ and obtain 
\begin{eqnarray}
...
orbital basis can then be obtained by using the transformation displayed in
...
&&\overset{1\leq j_{1}<\cdots <j_{N}\leq s;\quad 1\leq k_{2}<\cdots
...
\equiv $ paired GSO\ of $p$ i.e. $\bar{p}=p\pm s$ and $\left| p\right| $ to
denote the index of $p$ defined as 
\begin{equation}
\left| p\right| =\left\{ 
\begin{array}{c}
p\quad \text{if \ }1\leq p\leq s\qquad \\ 
p\text{ \ if \ }s+1\leq p\leq 2s%
\end{array}%
\right.
\end{equation}%
...
&&\qquad \qquad N!\left( N-1\right) !\left\{ S_{N-1}\left( \hat{p}\right)
\zeta _{p}^{\ast }\left\{ h_{0}+h_{pp}+h_{p+sp+s}+\left\langle
...
\left. +\zeta _{p}^{\ast }\sum_{j<k\neq p}S_{N-3}\left( \hat{p}\hat{\jmath}%
\hat{k}\right) n_{j}n_{k}\left\langle jk|||jk\right\rangle \right\}
\label{pps}
\end{eqnarray}%
or from the expansion eq. $\left( \ref{Derexp}\right) $ and evaluating
matrix elements between Independent Particle States (IPS). (See Appendix D
for details)
...
+\left\langle \bar{q}\kappa ||p\kappa \right\rangle n_{\kappa }\zeta
_{q}^{\ast }+\left\langle \bar{p}\kappa ||q\kappa \right\rangle n_{\kappa
}\zeta _{p}^{\ast }\right\} +\zeta _{p}^{\ast }\left\langle p\bar{p}%
||pq\right\rangle S_{N-1}\left( \hat{p}\hat{q}\right)  \notag \\
&&\quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \,\left. 
\overset{}{\underset{}{}}+\zeta _{q}^{\ast }\left\langle q\bar{q}%
||pq\right\rangle S_{N-1}\left( \hat{p}\hat{q}\right) +\left\{ \zeta
_{q}^{\ast }h_{\bar{q}p}+\zeta _{p}^{\ast }h_{\bar{p}q}\right\}
S_{N-1}\left( \hat{p}\hat{q}\right) \right\}  \label{pq}
\end{eqnarray}
...
N-1}\phi \right\rangle ;1\leq i,j\leq r
\end{equation}
...
=\sum_{p,q=1}^{r}W_{f\,j_{1}p}W_{f\,j_{2}q}\left\langle G^{\dagger N}\phi
|Hb_{p}^{\dagger }b_{q}^{\dagger }G^{\dagger N-1}\phi \right\rangle
\label{ftrm}
\end{equation}%
a sum that goes over the two types of matrix elements paired terms $p=\left|
q\right| $ computed in eq. $\left( \ref{pq}\right) $ and unpaired terms $%
p\neq \left| q\right| $ computed in eq. $\left( \ref{pps}\right) $. The
paired term gives 
\begin{eqnarray}
&&\left\langle G^{\dagger N}\phi |Hb_{p}^{\dagger }b_{p+s}^{\dagger
...
\hat{p}\right) \zeta _{p}^{\ast }\left\{ W_{f\,j_{1}p}^{\ast
}W_{f\,j_{2}p}+W_{f\,j_{1}p+s}^{\ast }W_{f\,j_{2}p+s}\right\}
h_{j_{1}j_{2}}^{a}\right. \right.  \notag \\
&&+\left. \sum_{j\neq p}^{s}S_{N-2}\left( \hat{p}\hat{\jmath}\right) \zeta
_{p}^{\ast }n_{j}\left\{ W_{f\,j_{1}j}^{\ast
}W_{f\,j_{2}j}+W_{f\,j_{1}j+s}^{\ast }W_{f\,j_{2}j+s}\right\}
h_{j_{1}j_{2}}^{a}\right\} \qquad \qquad \qquad \qquad
\end{eqnarray}%
\begin{eqnarray}
&&+\sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}\left\{ \underset{}{\overset{}{}}%
S_{N-1}\left( \hat{p}\right) \zeta _{p}^{\ast }W_{f\,j_{1}p}^{\ast
}W_{f\,j_{2}p+s}^{\ast }W_{f\,j_{3}p}W_{f\,j_{4}p+s}\right.  \notag \\
&&+\sum_{j\neq p}^{s}S_{N-2}\left( \hat{p}\hat{\jmath}\right) \left\{ \zeta
_{p}^{\ast }n_{j}\left\{ W_{f\,j_{1}j}^{\ast }W_{f\,j_{2}j+s}^{\ast
}W_{f\,j_{3}j}W_{f\,j_{4}j+s}+W_{f\,j_{1}j}^{\ast }W_{f\,j_{2}p}^{\ast
}W_{f\,j_{3}p}W_{f\,j_{4}j}\right. \right.  \notag \\
&&\qquad \qquad \qquad \qquad \qquad +W_{f\,j_{1}j}^{\ast
}W_{f\,j_{2}p}^{\ast }W_{f\,j_{3}p}W_{f\,j_{4}j}+W_{f\,j_{1}j}^{\ast
}W_{f\,j_{2}p+s}^{\ast }W_{f\,j_{3}p+s}W_{f\,j_{4}j}  \notag \\
&&\qquad \qquad \qquad \qquad +\left. W_{f\,j_{1}j+s}^{\ast
}W_{f\,j_{2}p}^{\ast }W_{f\,j_{3}p}W_{f\,j_{4}j+s}+W_{f\,j_{1}j+s}^{\ast
}W_{f\,j_{2}p+s}^{\ast }W_{f\,j_{3}p+s}W_{f\,j_{4}j+s}\right\}  \notag \\
&&\qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad \left.
+\zeta _{j}^{\ast }W_{f\,j_{1}j}^{\ast }W_{f\,j_{2}j+s}^{\ast
}W_{f\,j_{3}p}W_{f\,j_{4}p+s}\right\}
\end{eqnarray}%
\begin{eqnarray}
&&  \notag \\
&&+\sum_{j\neq k\neq p}S_{N-2}\left( \hat{p}\hat{\jmath}\hat{k}\right) \zeta
_{p}^{\ast }\zeta _{j}^{\ast }\zeta _{k}W_{f\,j_{1}j}^{\ast
}W_{f\,j_{2}j+s}^{\ast }W_{f\,j_{3}k}W_{f\,j_{4}k+s}\qquad \qquad \qquad 
\notag \\
&&\qquad \quad +\frac{1}{2}\zeta _{p}^{\ast }\sum_{j,k\neq p}S_{N-3}\left( 
\hat{p}\hat{\jmath}\hat{k}\right) n_{j}n_{k}\left\{ W_{f\,j_{1}j}^{\ast
}W_{f\,j_{2}k}^{\ast }W_{f\,j_{3}k}W_{f\,j_{4}pj}\right.  \notag \\
&&\qquad \qquad \qquad +W_{f\,j_{1}j}^{\ast }W_{f\,j_{2}k+s}^{\ast
}W_{f\,j_{3}k+s}W_{f\,j_{4}pj}+W_{f\,j_{1}j+s}^{\ast }W_{f\,j_{2}k}^{\ast
}W_{f\,j_{3}k}W_{f\,j_{4}pj+s}  \notag \\
&&\qquad \qquad \qquad \qquad \qquad \qquad \left. \left. \left.
+W_{f\,j_{1}j+s}^{\ast }W_{f\,j_{2}k+s}^{\ast
}W_{f\,j_{3}k+s}W_{f\,j_{4}pj+s}\right\} \underset{}{\overset{}{}}\right\}
...
\sum_{j_{1},j_{2}=1}^{r}\left\{ W_{f\,j_{1}\bar{q}}^{\ast
}W_{f\,j_{2}p}\zeta _{q}^{\ast }+W_{f\,j_{1}\bar{p}}^{\ast
}W_{f\,j_{2}q}\zeta _{p}^{\ast }\right\} h_{j_{1}j_{2}}^{a}\right.  \notag \\
&&+\sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}\left\{ \sum_{\kappa \neq \left|
p\right| \neq \left| q\right| }^{s}S_{N-1}\left( \hat{\kappa}\hat{p}\hat{q}%
\right) \left\{ W_{f\,j_{1}\kappa }^{\ast }W_{f\,j_{2}\kappa +s}^{\ast
}W_{f\,j_{3}p}W_{f\,j_{4}q}\zeta _{\kappa }^{\ast }\right. \right.  \notag \\
&&\quad \qquad \quad +W_{f\,j_{1}\bar{p}}^{\ast }W_{f\,j_{2}\bar{q}}^{\ast
}W_{f\,j_{3}\kappa }W_{f\,j_{4}\kappa +s}\zeta _{p}^{\ast }\zeta _{q}^{\ast
}\zeta _{\kappa }+W_{f\,j_{1}\bar{q}}^{\ast }W_{f\,j_{2}\kappa }^{\ast
}W_{f\,j_{3}p}W_{f\,j_{4}\kappa }n_{\kappa }\zeta _{q}^{\ast }  \notag \\
&&\qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad
\left. +W_{f\,j_{1}\bar{p}}^{\ast }W_{f\,j_{2}\kappa }^{\ast
}W_{f\,j_{3}q}W_{f\,j_{4}\kappa }n_{\kappa }\zeta _{p}^{\ast }\right\} 
\notag \\
&&+\zeta _{p}^{\ast }S_{N-1}\left( \hat{p}\hat{q}\right) W_{f\,j_{1}p}^{\ast
}W_{f\,j_{2}p+s}^{\ast }W_{f\,j_{3}p}W_{f\,j_{4}q}  \notag \\
&&\qquad \qquad \qquad \qquad \left. \left. +\zeta _{q}^{\ast }S_{N-1}\left( 
\hat{p}\hat{q}\right) W_{f\,j_{1}q}^{\ast }W_{f\,j_{2}q+s}^{\ast
}W_{f\,j_{3}p}W_{f\,j_{4}q}\underset{}{\overset{}{}}\right\} \left\langle
j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}\underset{}{\overset{}{}}\right\}
\end{eqnarray}

\section{Methods of Optimization}

\FRAME{ftbp xF }{}{}{}{\Qct{Flow Diagram}}{}{}{}
...
We consider this construction in orthonormal basis $\left\{ \psi _{j};1\leq
j\leq r\right\} $ where the geminal is represented by the complex
antisymmetric matrix \textbf{c}. We first note some relationships implied by
the antisymmetry of \textbf{c}%
\begin{eqnarray}
\mathbf{c}^{\dagger }\mathbf{c\,} &=&\mathbf{\,}\left( \mathbf{cc}^{\dagger
}\right) ^{\ast }  \label{cc1} \\
\mathbf{c}^{\dagger }\mathbf{c\,} &\mathbf{=}&\left( \mathbf{cc}^{\dagger
}\right) ^{t}
\end{eqnarray}%
and one by the associativity of matrix product 
\begin{equation}
\mathbf{c}\left( \mathbf{c}^{\dagger }\mathbf{c}\right) =\left( \mathbf{cc}%
^{\dagger }\right) \mathbf{c}
\end{equation}%
which by using eq. $\left( \ref{cc1}\right) $ gives 
\begin{equation}
\mathbf{c}\left( \mathbf{c}^{\dagger }\mathbf{c}\right) =\mathbf{\,}\left( 
\mathbf{c}^{\dagger }\mathbf{c}\right) ^{\ast }\mathbf{c}  \label{cr}
\end{equation}%
We also assume the existence of a basis in which $\mathbf{c}$ can be
...
\right)
\end{equation}%
which is shown in references \cite{Bloch1}\cite{Zumino1}. Thus we know a
unitary transformation $\mathbb{T}$ exists such that 
\begin{equation}
...
\right)
\end{equation}%
and simultaneously diagonalizes $\mathbf{c}^{\dagger }\mathbf{c}$ and $%
\mathbf{c}^{\dagger }\mathbf{c}$ i.e. 
\begin{equation}
\mathbb{T}^{\dagger }\mathbf{c}^{\dagger }\mathbf{c}\mathbb{T=T}^{\dagger }%
\mathbf{cc}^{\dagger }\mathbb{T=}\left( 
\begin{array}{cc}
\mathbf{n} & \mathbf{0} \\ 
\mathbf{0} & \mathbf{n}%
\end{array}%
\right)
\end{equation}%
Moreover since $\mathbf{c}^{\dagger }\mathbf{c=\,}\left( \mathbf{cc}%
^{\dagger }\right) ^{\ast }$ we have that if a unitary transformation
diagonalizes $\mathbf{c}^{\dagger }\mathbf{c}$ then the complex conjugate
transformation must diagonalize $\mathbf{cc}^{\dagger }$ thus $\mathbb{T=T}%
^{\ast }$ and hence must be a real orthogonal transformation. So that 
\begin{eqnarray}
\mathbf{c}^{\dagger }\mathbf{c}\mathbf{=\,} &&\mathbb{T}\left( 
\begin{array}{cc}
\mathbf{n} & \mathbf{0} \\ 
\mathbf{0} & \mathbf{n}%
\end{array}%
...
\right) \mathbb{T}^{t}
\end{eqnarray}%
so that $\mathbf{c}^{\dagger }\mathbf{c=\,cc}^{\dagger }$ and are \emph{real 
}matrices. So we have proved the following

\begin{lemma}
If $\mathbf{c}$ is a complex symmetric matrix then $\mathbf{c}^{\dagger }%
\mathbf{c=\,cc}^{\dagger }$ and are real symmetric matrices.
\end{lemma}

We are now in position to prove for \emph{non degenerate }$\mathbf{n}$

\begin{theorem}
If and only if 
\begin{equation}
\mathbb{T}^{\dagger }\mathbf{c}^{\dagger }\mathbf{c}\mathbb{T=}\left( 
\begin{array}{cc}
\mathbf{n} & \mathbf{0} \\ 
\mathbf{0} & \mathbf{n}%
\end{array}%
...
0 & 0 & \zeta _{s}%
\end{array}%
\right) ;\qquad \zeta _{j}\in \mathbb{C}
\end{equation}%
and 
\begin{equation}
\mathbf{n=}\left( 
...
If $\mathbf{n}$ is degenerate one can only conclude by eq. $\left( \ref{cr}%
\right) $ that $\mathbb{T}^{t}\mathbf{c}\mathbb{T}$ is blocked according the
level of degeneracy of $\mathbf{n}$, this is not surprising as the canonical
GSO's associated with such degenerate occupation numbers are determined up
transformations between such GSO's.
...
^{\ast },\mathbf{z}\right) \bullet \mathbf{y}+\frac{1}{2}\mathbf{y}^{\dagger
}\bullet D^{2}f\left( \mathbf{z}^{\ast },\mathbf{z}\right) \bullet \mathbf{y}
\end{equation}%
where%
\begin{eqnarray}
...
\section{Energy of the 2N-Electron Unnormalized AGP State}
...
\\
&&\qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad
\qquad \qquad \qquad \qquad _{1\leq j_{1},\cdots ,j_{N}\leq s;\,1\leq
k_{1},\cdots ,k_{N}\leq s\,}
\end{eqnarray}%
...
&&\left. {}\right. \qquad \qquad \qquad \qquad \qquad \qquad \qquad =\left(
N!\right) ^{2}\sum_{i=1}^{s}\left\{ h_{ii}+h_{i+si+s}\right\}
...
\right\rangle  \notag \\
&&\qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad
\qquad \qquad \qquad \quad _{1\leq j_{1},\cdots ,j_{N}\leq s;\,1\leq
k_{1},\cdots ,k_{N}\leq s\,}
\end{eqnarray}%
...
\notag \\
&&+\sum_{1\leq i\neq j\leq s}\zeta _{i}^{\ast }\zeta _{j}S_{N-1}\left( \hat{%
\imath}\hat{\jmath}\right) \left\langle ii+s||jj+s\right\rangle \left.
+\sum_{1\leq i,j\leq s}n_{i}n_{j}S_{N-2}\left( \hat{\imath}\hat{\jmath}%
\right) \left\langle ij|||ij\right\rangle \right\}
\end{eqnarray}

\section{Computation of First Derivatives}
...
obtain 
\begin{eqnarray}
4 &&N!\left( N-1\right) !\left\{ \underset{i}{\underset{}{\zeta _{p}^{\ast
...
&&\left. {}\right. \qquad \qquad \qquad \left. \underset{iii}{\sum_{i\neq
...
&&\left. {}\right. \qquad \qquad \qquad \sum_{i\neq p}^{s}\zeta _{p}^{\ast
...
&&\qquad \qquad \qquad \qquad \qquad \left. \sum_{i,j\neq p}^{s}\zeta
_{p}^{\ast }n_{i}n_{j}S_{N-3}\left( \hat{p}\hat{\imath}\hat{\jmath}\right)
\left\langle ij|||ij\right\rangle +\sum_{i\neq p}^{s}n_{i}\zeta _{p}^{\ast
}S_{N-2}\left( \hat{p}\hat{\imath}\right) \left\langle ip|||ip\right\rangle
\right\}  \label{Der}
\end{eqnarray}%
...
\right.  \notag \\
&&\quad \quad \quad \quad \quad \quad \quad \quad \quad \left. +\left\langle 
\bar{q}\kappa ||p\kappa \right\rangle n_{\kappa }\zeta _{q}^{\ast
}+\left\langle \bar{p}\kappa ||q\kappa \right\rangle n_{\kappa }\zeta
_{p}^{\ast }\right\} +\zeta _{p}^{\ast }\left\langle pp+s||pq\right\rangle
S_{N-1}\left( \hat{p}\hat{q}\right)  \notag \\
&&\quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \left. +\zeta
_{q}^{\ast }\left\langle qq+s||pq\right\rangle S_{N-1}\left( \hat{p}\hat{q}%
\right) +\left\{ \zeta _{q}^{\ast }h_{\bar{q}p}+\zeta _{p}^{\ast }h_{\bar{p}%
q}\right\} S_{N-1}\left( \hat{p}\hat{q}\right) \right\}
\end{eqnarray}

\section{Transformation of the Energy of the 2N-Electron Unnormalized AGP
State to the Atomic Orbital Basis \label{aoen}}
...
\mathcal{N}\left( g^{N}\right) +\left( N\right) !^{2}  \notag \\
&&\left\{ \sum_{j_{1,}j_{2}=1}^{r}\sum_{1\leq j\leq s}n_{j}S_{N-1}\left( 
\hat{\jmath}\right) \left\{ \left\{ W_{f\,j_{1}j}^{\ast
}W_{f\,j_{2}j}+W_{f\,j_{1}j+s}^{\ast }W_{f\,j_{2}j+s}\right\}
h_{j_{1}j_{2}}^{a}\right. \right.  \notag \\
&&\qquad \qquad \qquad \qquad \left.
+\sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}W_{f\,j_{1}j}^{\ast
}W_{f\,j_{2}j+s}^{\ast }W_{f\,j_{3}j}W_{f\,j_{4}j+s}\left\langle
j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}\right\}  \notag \\
&&+\sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}\sum_{1\leq j\neq k\leq s}\zeta
_{j}^{\ast }\zeta _{k}S_{N-1}\left( \hat{\jmath}\hat{k}\right)
W_{f\,j_{1}j}^{\ast }W_{f\,j_{2}j+s}^{\ast
}W_{f\,j_{3}k}W_{f\,j_{4}k+s}\left\langle
j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}  \notag \\
&&\left. +\sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}\sum_{1\leq j,k\leq
s}n_{j}n_{k}S_{N-2}\left( \hat{\jmath}\hat{k}\right) W_{f\,j_{1}j}^{\ast
}W_{f\,j_{2}k}^{\ast }W_{f\,j_{3}j}W_{f\,j_{4}k}\left\langle
j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}\right\}
\end{eqnarray}%
...
\section{Spin Integration of the Hamiltonian Matrix Elements\label{spinint}}
...
\left( \ref{pq}\right) $ and eq. $\left( \ref{pps}\right) $ and each of
...
h_{0}+\sum_{j_{1,}j_{2}=1}^{r}\left\{ W_{f\,j_{1}p}^{\ast
}W_{f\,j_{2}p}+W_{f\,j_{1}p+s}^{\ast }W_{f\,j_{2}p+s}\right\}
h_{j_{1}j_{2}}^{a}\right. \right.  \notag \\
&&+\left. {}\right. \qquad \qquad \qquad \qquad \left.
\sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}W_{f\,j_{1}p}^{\ast
}W_{f\,j_{2}p+s}^{\ast }W_{f\,j_{3}p}W_{f\,j_{4}p+s}\left\langle
j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}\right\}
\end{eqnarray}%
\begin{eqnarray}
&&+\sum_{j\neq p}^{s}S_{N-2}\left( \hat{p}\hat{\jmath}\right) \left\{ \zeta
_{p}^{\ast }n_{j}\left\{ \sum_{j_{1},j_{2}}^{r}\left\{ W_{f\,j_{1}j}^{\ast
}W_{f\,j_{2}j}+W_{f\,j_{1}j+s}^{\ast }W_{f\,j_{2}j+s}\right\}
h_{j_{1}j_{2}}^{a}\right. \right. \qquad  \notag \\
&&+\sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}\left\{ W_{f\,j_{1}j}^{\ast
}W_{f\,j_{2}j+s}^{\ast }W_{f\,j_{3}j}W_{f\,j_{4}j+s}+W_{f\,j_{1}j}^{\ast
}W_{f\,j_{2}p}^{\ast }W_{f\,j_{3}p}W_{f\,j_{4}j}\right\} \left\langle
j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}
\end{eqnarray}%
\begin{eqnarray}
&&+\sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}W_{f\,j_{1}j}^{\ast
...
&&+\left. \left. W_{f\,j_{1}j+s}^{\ast }W_{f\,j_{2}p}^{\ast
}W_{f\,j_{3}p}W_{f\,j_{4}j+s}+W_{f\,j_{1}j+s}^{\ast }W_{f\,j_{2}p+s}^{\ast
}W_{f\,j_{3}p+s}W_{f\,j_{4}j+s}\right\} \left\langle
j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}\right\}  \notag \\
&&\qquad \qquad \qquad \qquad +\left. \zeta _{j}^{\ast
}\sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}W_{f\,j_{1}j}^{\ast
}W_{f\,j_{2}j+s}^{\ast }W_{f\,j_{3}p}W_{f\,j_{4}p+s}\left\langle
j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}\right\}
\end{eqnarray}%
\begin{eqnarray}
&&+\sum_{j\neq k\neq p}S_{N-2}\left( \hat{p}\hat{\jmath}\hat{k}\right) \zeta
_{p}^{\ast }\zeta _{j}^{\ast }\zeta
_{k}\sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}W_{f\,j_{1}j}^{\ast
}W_{f\,j_{2}j+s}^{\ast }W_{f\,j_{3}k}W_{f\,j_{4}k+s}\left\langle
j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}  \notag \\
&&+\frac{1}{2}\zeta _{p}^{\ast }\sum_{j,k\neq p}S_{N-3}\left( \hat{p}\hat{%
\jmath}\hat{k}\right) n_{j}n_{k}\sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}\left\{
W_{f\,j_{1}j}^{\ast }W_{f\,j_{2}k}^{\ast }W_{f\,j_{3}k}W_{f\,j_{4}pj}\right.
\notag \\
&&\qquad \qquad \qquad +W_{f\,j_{1}j}^{\ast }W_{f\,j_{2}k+s}^{\ast
}W_{f\,j_{3}k+s}W_{f\,j_{4}pj}+W_{f\,j_{1}j+s}^{\ast }W_{f\,j_{2}k}^{\ast
}W_{f\,j_{3}k}W_{f\,j_{4}pj+s}+  \notag \\
...
+W_{f\,j_{1}j+s}^{\ast }W_{f\,j_{2}k+s}^{\ast
}W_{f\,j_{3}k+s}W_{f\,j_{4}pj+s}\right\} \left\langle
j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}\right\}
\end{eqnarray}%
...
&&\left\{ h_{0}S_{N-1}\left( \hat{p}\right) \zeta _{p}^{\ast
}+\sum_{j_{1,}j_{2}=1}^{r}\left\{ S_{N-1}\left( \hat{p}\right) \zeta
_{p}^{\ast }\left\{ W_{f\,j_{1}p}^{\ast }W_{f\,j_{2}p}+W_{f\,j_{1}p+s}^{\ast
}W_{f\,j_{2}p+s}\right\} h_{j_{1}j_{2}}^{a}\right. \right.  \notag \\
&&+\left. \sum_{j\neq p}^{s}S_{N-2}\left( \hat{p}\hat{\jmath}\right) \zeta
_{p}^{\ast }n_{j}\left\{ W_{f\,j_{1}j}^{\ast
}W_{f\,j_{2}j}+W_{f\,j_{1}j+s}^{\ast }W_{f\,j_{2}j+s}\right\}
...
\end{eqnarray}%
\begin{eqnarray}
&&+\sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}\left\{ S_{N-1}\left( \hat{p}\right)
\zeta _{p}^{\ast }W_{f\,j_{1}p}^{\ast }W_{f\,j_{2}p+s}^{\ast
}W_{f\,j_{3}p}W_{f\,j_{4}p+s}\right.  \notag \\
&&+\sum_{j\neq p}^{s}S_{N-2}\left( \hat{p}\hat{\jmath}\right) \left\{ \zeta
_{p}^{\ast }n_{j}\left\{ W_{f\,j_{1}j}^{\ast }W_{f\,j_{2}j+s}^{\ast
}W_{f\,j_{3}j}W_{f\,j_{4}j+s}+W_{f\,j_{1}j}^{\ast }W_{f\,j_{2}p}^{\ast
...
&&\qquad \qquad \qquad \qquad \qquad +\left. W_{f\,j_{1}j+s}^{\ast
}W_{f\,j_{2}p}^{\ast }W_{f\,j_{3}p}W_{f\,j_{4}j+s}+W_{f\,j_{1}j+s}^{\ast
}W_{f\,j_{2}p+s}^{\ast }W_{f\,j_{3}p+s}W_{f\,j_{4}j+s}\right\}  \notag \\
&&\qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad \left.
+\zeta _{j}^{\ast }W_{f\,j_{1}j}^{\ast }W_{f\,j_{2}j+s}^{\ast
...
&&\qquad \qquad \qquad \qquad \qquad \qquad \qquad \left. \left. \left.
+W_{f\,j_{1}j+s}^{\ast }W_{f\,j_{2}k+s}^{\ast
}W_{f\,j_{3}k+s}W_{f\,j_{4}pj+s}\right\} \underset{}{\overset{}{}}\right\}
\left\langle j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}\right\}
\end{eqnarray}%
...
\sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}W_{f\,j_{1}\kappa }^{\ast
}W_{f\,j_{2}\kappa +s}^{\ast }W_{f\,j_{3}p}W_{f\,j_{4}q}\zeta _{\kappa
}^{\ast }\left\langle j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}\right.
\right. \right.  \notag \\
&&\quad \qquad \quad \quad \quad \qquad +\left.
\sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}W_{f\,j_{1}\bar{p}}^{\ast }W_{f\,j_{2}%
\bar{q}}^{\ast }W_{f\,j_{3}\kappa }W_{f\,j_{4}\kappa +s}\left\langle
j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}\zeta _{p}^{\ast }\zeta _{q}^{\ast
}\zeta _{\kappa }\right\}  \notag \\
&&\qquad \qquad \quad \quad \qquad
+\sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}W_{f\,j_{1}\bar{q}}^{\ast
}W_{f\,j_{2}\kappa }^{\ast }W_{f\,j_{3}p}W_{f\,j_{4}\kappa }\left\langle
j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}n_{\kappa }\zeta _{q}^{\ast }  \notag
\\
&&\qquad \qquad \quad \quad \qquad \left.
+\sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}W_{f\,j_{1}\bar{p}}^{\ast
}W_{f\,j_{2}\kappa }^{\ast }W_{f\,j_{3}q}W_{f\,j_{4}\kappa }\left\langle
j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}n_{\kappa }\zeta _{p}^{\ast }\right\}
\notag \\
&&\qquad \qquad \quad \quad \quad +\zeta _{p}^{\ast }S_{N-1}\left( \hat{p}%
\hat{q}\right) \sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}W_{f\,j_{1}p}^{\ast
}W_{f\,j_{2}p+s}^{\ast }W_{f\,j_{3}p}W_{f\,j_{4}q}\left\langle
j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}  \notag \\
&&\qquad \qquad \quad \quad \quad +\zeta _{q}^{\ast }S_{N-1}\left( \hat{p}%
\hat{q}\right) \sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}W_{f\,j_{1}q}^{\ast
}W_{f\,j_{2}q+s}^{\ast }W_{f\,j_{3}p}W_{f\,j_{4}q}\left\langle
j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}  \notag \\
&&\qquad \quad \quad \quad \qquad \left. +S_{N-1}\left( \hat{p}\hat{q}%
\right) \sum_{j_{1},j_{2}=1}^{r}\left\{ W_{f\,j_{1}\bar{q}}^{\ast
}W_{f\,j_{2}p}\zeta _{q}^{\ast }+W_{f\,j_{1}\bar{p}}^{\ast
}W_{f\,j_{2}q}\zeta _{p}^{\ast }\right\} h_{j_{1}j_{2}}^{a}\right\}
\end{eqnarray}%
...
}W_{f\,j_{2}q}\zeta _{p}^{\ast }\right\} h_{j_{1}j_{2}}\right.  \notag \\
&&+\sum_{j_{1,}j_{2,}j_{3,}j_{4}=1}^{r}\left\{ \sum_{\kappa \neq \left|
p\right| \neq \left| q\right| }^{s}S_{N-1}\left( \hat{\kappa}\hat{p}\hat{q}%
\right) \left\{ W_{f\,j_{1}\kappa }^{\ast }W_{f\,j_{2}\kappa +s}^{\ast
}W_{f\,j_{3}p}W_{f\,j_{4}q}\zeta _{\kappa }^{\ast }\right. \right.  \notag \\
...
&&\qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad \left.
+W_{f\,j_{1}\bar{p}}^{\ast }W_{f\,j_{2}\kappa }^{\ast
}W_{f\,j_{3}q}W_{f\,j_{4}\kappa }n_{\kappa }\zeta _{p}^{\ast }\right\} 
\notag \\
&&+\zeta _{p}^{\ast }S_{N-1}\left( \hat{p}\hat{q}\right) W_{f\,j_{1}p}^{\ast
}W_{f\,j_{2}p+s}^{\ast }W_{f\,j_{3}p}W_{f\,j_{4}q}  \notag \\
&&\qquad \qquad \qquad \left. \left. +\zeta _{q}^{\ast }S_{N-1}\left( \hat{p}%
\hat{q}\right) W_{f\,j_{1}q}^{\ast }W_{f\,j_{2}q+s}^{\ast
}W_{f\,j_{3}p}W_{f\,j_{4}q}\right\} \left\langle
j_{1}j_{2}||j_{3}j_{4}\right\rangle ^{a}\underset{}{\overset{}{}}\right\}
\end{eqnarray}

\begin{thebibliography}{99}
\bibitem{Jensen1} H. J. Aa. Jensen, B. Weiner, J. V. Ortiz and Y. Ohrn, A
Powerful Procedure for Optimizing AGP States, Int. J. Quantum Chem. S16,
615-128 (1982).

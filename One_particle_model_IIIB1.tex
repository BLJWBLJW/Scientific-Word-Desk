...
%TCIDATA{LastRevised=Monday, September 06, 2004 14:50:22}
...
orbitals and a commutative non unitary group of positive transformations,
...
\begin{equation}
\psi_{j+s}\left(  \bm{r,}\zeta\right)  =\psi_{j}\left(  \bm{r,}\zeta\right)
^{\ast}.
\end{equation}
...
$, given by \cite{Coleman97}\cite{Yukalov00}
\begin{align}
...
functional in Density Matrix Functional Theory (DMFT) \cite{cioslowski:4156}%
\cite{staroverov:11107}\cite{cioslowski:67} and it clearly demonstrates the
...
\bm{r}_{1}-\bm{r}_{2}\right\Vert }\psi_{l}\left(  \bm{r}_{2}\bm{,\xi}_{1}%
\right)  \psi_{m}\left(  \bm{r}_{1}\bm{,\xi}_{2}\right)  \right\}
...
j\leq s}\eta_{j}\Upsilon_{j}\right\}  \right\vert \eta_{j}\in\mathbb{R}%
\right\}  , \label{occup}%
\end{equation}
where
\begin{equation}
...
as
\begin{align}
\mathcal{M}  &  \mathcal{=}\left\{  \left.  k\left(  \bm{\eta}\right)
p\left(  \bm{\theta}\right)  u\left(  \bm{\alpha}\right)  \left\vert g_{0}%
...
a generalized Fock operator $F\left(  \bm{n,\psi}\right)  $ (see section
...
$ this becomes
\begin{equation}
F\left(  \bm{n},\bm{\varphi}\right)  _{qp}=\frac{1}{\left\langle \left.
\Psi\right\vert \Psi\right\rangle }\left\langle \left.  \Psi\right\vert
\left\{  a_{p}^{\dagger}a_{q}H-a_{p}^{\dagger}Ha_{q}\right\}  \Psi
\right\rangle .
\end{equation}
...
\begin{equation}
F\left(  \mathbb{D}^{2},\bm{\varphi}\right)  _{pp}=\frac{1}{\left\langle
\left.  \Psi\right\vert \Psi\right\rangle }\left\langle \left.  \Psi
\right\vert \left\{  a_{p}^{\dagger}a_{q}H-a_{p}^{\dagger}Ha_{q}\right\}
\Psi\right\rangle =\mathcal{E}\left(  \mathbb{D}^{2},\bm{\varphi}\right)
...
^{2},\bm{\varphi}\right)  _{pp}=0$ as the GSO $\left\vert \varphi
...
$\mathcal{E}\left(  \mathbb{D}^{2},\bm{\varphi}\right)  $ can be expressed as
\begin{equation}
...
\left(  \bm{n},\bm{\psi}\right)  $ can be expressed as
\begin{equation}
...
\begin{equation}
f\left(  \bm{\varphi}\right)  _{mq}=\left(  h_{1mq}\left(
\bm{\varphi }\right)  \right)  +J_{mq}\left(  \bm{\varphi}\right)
-K_{mq}\left(  \bm{\varphi}\right)
\end{equation}
...
$\bm{N}_{0}$ to $\bm{N}_{0}+\delta\bm{N}_{1}+\delta\bm{N}_{2}$
\begin{equation}
\bm{n}_{2}=\bm{n}_{0}+\left[  d\mathcal{N}\left(  \bm{n}_{0}\right)  \right]
^{-1}\left(  \delta\bm{N}_{1}\right)  +\left[  d\mathcal{N}\left(
\bm{n}_{0}+\left[  d\mathcal{N}\left(  \bm{n}_{0}\right)  \right]
^{-1}\left(  \delta\bm{N}_{1}\right)  \right)  \right]  ^{-1}\left(  \left(
\delta\bm{N}_{2}\right)  \right)
\end{equation}
producing the change of geminal occupation numbers $\bm{n}_{2}-\bm{n}_{0},$
where
\begin{equation}
\bm{N}_{1}=\left(
\begin{array}
...
\right)  .
\end{equation}
The next incremental change of the AGP occupation numbers, $\delta\bm{N}_{2}$,
added to $\delta\bm{N}_{1}$ gives
\[
\bm{n}_{2}=\mathcal{N}^{-1}\left(
\begin{array}
[c]{c}%
.9\\
.8\\
...
\right)  .
\end{equation}
At each step one can calculate $\bm{N}$ from $\bm{n}$ to check whether the
increment $\delta\bm{N}$ was small enough to produce consistency.
...
\end{align}
Each new non zero value of $\bm{\alpha}$ determines a new state $D^{2N}\left(
...
\end{equation}
where $\mathbb{H}_{2}\left(  \bm{\varphi}\right)  $ is the \emph{non
...
terms of the non antisymmetric matrix $\mathbb{H}_{2}\left(
...
\bm{n}\right)  \right\} \\
\partial_{\bm{\psi}^{\ast}}^{1}\mathcal{E}\left(
\bm{n},\bm{\varphi},\bm{\theta}\right)   &  =\frac{1}{S_{N}\left(
\bm{n}\right)  }\left\{  \partial_{\bm{\psi}^{\ast}}^{1}\mathcal{H}\left(
\bm{n},\bm{\varphi},\bm{\theta}\right)  -\mathcal{E}\left(
\bm{n},\bm{\varphi},\bm{\theta}\right)  \partial_{\bm{\psi}^{\ast}}^{1}%
S_{N}\left(  \bm{n}\right)  \right\}  =\left[  \partial_{\bm{\psi}}%
...
\partial_{\bm{\psi}^{\ast}}^{1}\mathcal{E}\left(
\bm{n},\bm{\varphi},\bm{\theta}\right)   &  =\frac{1}{S_{N}\left(
\bm{n}\right)  }\partial_{\bm{\psi}^{\ast}}^{1}\mathcal{H}\left(
...
\bm{\varphi}\right)  \frac{\operatorname{Tr}\left\{  a_{p}Ha_{p}^{\dagger
...
,\bm{\varphi }\right)  \kappa_{p}\left(  \mathbb{D}^{2},\bm{\varphi}\right)  ,
\end{equation}
where
\begin{equation}
\kappa_{p}\left(  \mathbb{D}^{2},\bm{\varphi}\right)  =\mathcal{E}_{p}\left(

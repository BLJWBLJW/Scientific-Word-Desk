...
%TCIDATA{LastRevised=Wednesday, March 12, 2008 08:36:43}
...
\begin{document}
\preprint{ }
\title[One Electron Models]{Correlated One Electron and Models and Density Matrix Functionals, some simple
examples }
\author{Brian Weiner}
...
human scientific endeveour. Perhaps one of the most unbiquitous examples of
this is the use of molecular oribtals in understanding structure, properties
and reactivity in chemistry and material science. This efficacy is due to the
approximate validity of the Independent Partcle State (IPS) model, where  
...
hence the observable properties of these orbitals (e.g. energy, ionization
potential, electron affinity etc..) 
...
utilized that contain an explicit description of corellation. The resutling
improved multi electron states still determine sets of orbitals, now greater
than the number of electrons, weighted with occupation numbers, through the
First Order Reduced Density Operator (FORDO). However these orbitals and their
occupation numbers \emph{do not }not, in general, determine a unique multi
...
Maintaining a one electron picture while including electron corellation
...
feature in all of these methods is that it is not known how to express the
multi electron state as a functional of orbitals and occupation numbers.
However there is a class of multi electron states that can be expressed
explicitly as known functionals of orbitals and occupation numbers
\cite{weiner_ortiz2005}, they are the Antisymmetried Geminal Power (AGP)
states. The analysis of this class of states also shows that an orbital theory
is more general than a FORDO theory or the equivalent FDMT. In this paper we
review the definition of a corellated one particle theory (section 2), the
structure and parameterization of AGP\ states and the N-electron energy
functional (section 3), the expression of the energy in terms of one electron
quantities (section 4)

\section{Antisymmetrized Geminal Power States\label{AGP States}}

\section{N-electron Energy Functional}

\section{One Electron Quantities}
...
\]
repectively, where $n_{j}=c_{j}^{2}$ and the canonical form of the geminal is
\[
\left\vert g\right\rangle =\sum_{1\leq j\leq s}c_{j}\left\vert \tilde{\psi
}_{j}\tilde{\psi}_{j+s}\right\rangle =\sum_{1\leq j\leq s}c_{j}e^{i\theta_{j}%
}\left\vert \psi_{j}\psi_{j+s}\right\rangle
\]
$\left\{  \left\vert \psi_{j}\right\rangle ;1\leq j\leq r\right\}  $ is a
conb. Geminals can be parameterized by
\begin{align*}
\left\vert g\right\rangle  &  =\sum_{1\leq j\leq s}c_{j}\left\vert \tilde
{\psi}_{j}\tilde{\psi}_{j+s}\right\rangle =\sum_{1\leq j\leq s}c_{j}%
e^{i\theta_{j}}\left\vert \psi_{j}\psi_{j+s}\right\rangle \\
\left\vert g\left(  \mathbf{\psi,\theta}\right)  \right\rangle  &
=\frac{U\left(  2\right)  }{SU\left(  2\right)  }SU\left(  2\right)
\left\vert g_{\psi}\right\rangle =U\left(  1\right)  SU\left(  2\right)
\left\vert g_{\psi}\right\rangle =U\left(  1\right)  \left\vert g_{\psi
}\right\rangle
\end{align*}
...
+\left\vert \psi_{2}\psi_{4}\right\rangle \\
&  =\left\vert \varphi_{1}\alpha\varphi_{3}\beta\right\rangle +\left\vert
\varphi_{2}\alpha\varphi_{4}\beta\right\rangle \equiv\left\vert 1\alpha
3\beta\right\rangle +\left\vert 2\alpha4\beta\right\rangle
\end{align*}%
\begin{align}
...
\right\}
\end{align}
while the SORDO's are If we further set%
\begin{align}
...
\end{align*}
and the energy for fixed $\left(  \mathbf{c,\psi}\right)  $ is given by%
\begin{align*}
...
\]
which can be seperated into a product of space and spin factors as%
\begin{align}
\left\vert g_{10}\right\rangle  &  =\left(  \sqrt{2}\right)  ^{-1}\left\{
...
Thus
\begin{align*}
&  \mathcal{S}^{2}\left\vert g_{10}\right\rangle =\mathcal{S}^{2}\left(
\sqrt{2}\right)  ^{-1}\left\{  \left\vert \varphi_{1}\alpha\varphi_{2}%
\beta\right\rangle +\left\vert \varphi_{1}\beta\varphi_{2}\alpha\right\rangle
\right\}  =\mathcal{S}_{-}\mathcal{S}_{+}\left(  \sqrt{2}\right)
...
\varphi_{1}\alpha\varphi_{2}\alpha\right\rangle +\left\vert \varphi_{1}%
\alpha\varphi_{2}\alpha\right\rangle \right\}  =2\left(  \sqrt{2}\right)
^{-1}\left\{  \left\vert \varphi_{1}\beta\varphi_{2}\alpha\right\rangle
+\left\vert \varphi_{1}\alpha\varphi_{2}\beta\right\rangle \right\}
=2\left\vert g_{10}\right\rangle
\end{align*}
and
\begin{align*}
&  \mathcal{S}^{2}\left\vert g_{00}\right\rangle =\mathcal{S}^{2}\left(
\sqrt{2}\right)  ^{-1}\left\{  \left\vert \varphi_{1}\alpha\varphi_{2}%
\beta\right\rangle -\left\vert \varphi_{1}\beta\varphi_{2}\alpha\right\rangle
\right\}  =\mathcal{S}_{-}\mathcal{S}_{+}\left(  \sqrt{2}\right)
^{-1}\left\{  \left\vert \varphi_{1}\alpha\varphi_{2}\beta\right\rangle
...
&  =\mathcal{S}_{-}\left(  \sqrt{2}\right)  ^{-1}\left\{  \left\vert
\varphi_{1}\alpha\varphi_{2}\alpha\right\rangle -\left\vert \varphi_{1}%
\alpha\varphi_{2}\alpha\right\rangle \right\} \\
...
\begin{align*}
\mathcal{S}^{2}\left\vert g_{+}\right\rangle =  &  \mathcal{S}^{2}\left(
\sqrt{2}\right)  ^{-1}\left\{  \left\vert \varphi_{1}\alpha\varphi_{3}%
\beta\right\rangle +\left\vert \varphi_{2}\alpha\varphi_{4}\beta\right\rangle
\right\}  =\mathcal{S}_{-}\mathcal{S}_{+}\left(  \sqrt{2}\right)
^{-1}\left\{  \left\vert \varphi_{1}\alpha\varphi_{3}\beta\right\rangle
+\left\vert \varphi_{2}\alpha\varphi_{4}\beta\right\rangle \right\} \\
&  =\mathcal{S}_{-}\left(  \sqrt{2}\right)  ^{-1}\left\{  \left\vert
\varphi_{1}\alpha\varphi_{3}\alpha\right\rangle +\left\vert \varphi_{2}%
\alpha\varphi_{4}\alpha\right\rangle \right\} \\
&  =\left(  \sqrt{2}\right)  ^{-1}\left\{  \left\vert \varphi_{1}\beta
...
\[
\mathcal{S}^{2}\left\vert g_{+}\right\rangle =\left\vert 1\beta1\alpha
\right\rangle +\left\vert 1\alpha1\beta\right\rangle +\left\vert 2\beta
2\alpha\right\rangle +\left\vert 2\alpha2\beta\right\rangle =0,
\]
...
\[
\mathcal{F}\left(  D^{1}\right)  =\min_{\mathbf{\theta}}\mathcal{E}\left(
\mathbf{\theta}\right)
\]
...
hypersphere, (this comes from $\mathcal{E}\left(  g^{\frac{N}{2}}\right)
=\mathcal{E}\left(  \lambda g^{\frac{N}{2}}\right)  $ for all $\lambda
\in\mathbb{R}$),
\[
...
\[
\left(  \mathbf{c,\varphi}\right)  =\left(  \frac{1}{\sqrt{2}},\frac{1}%
{\sqrt{2}},\varphi_{1},\varphi_{2}\right)
\]
as
\[
\mathcal{F}\left(  \left(  \frac{1}{\sqrt{2}},\frac{1}{\sqrt{2}}\right)
,\mathbf{\varphi}\right)  =\min_{\mathbf{\theta}}\left\{  \left\langle
1\left\vert h_{1}1\right.  \right\rangle +\left\langle 2\left\vert
h_{1}2\right.  \right\rangle +\left(  \left.  11\right\vert 11\right)
+\left(  \left.  22\right\vert 22\right)  +2\operatorname{Re}\left\{
e^{i\theta}\left(  \left.  11\right\vert 22\right)  \right\}  \right\}
\]
Note that
\begin{align*}
z  &  =re^{i\omega}=r\left(  \cos\omega+i\sin\omega\right) \\
r  &  =+\sqrt{\left(  \operatorname{Re}z\right)  ^{2}+\left(
\operatorname{Im}z\right)  ^{2}}\\
...
\[
\mathcal{F}\left(  \left(  \frac{1}{\sqrt{2}},\frac{1}{\sqrt{2}}\right)
,\mathbf{\varphi}\right)  =\left\langle 1\left\vert h_{1}1\right.
\right\rangle +\left\langle 2\left\vert h_{1}2\right.  \right\rangle +\left(
\left.  11\right\vert 11\right)  +\left(  \left.  22\right\vert 22\right)
...
\section{Generalized Fock Operator}

The generalized Fock operator for an AGP\ state (using a redundent
parameterization $\left(  \mathbf{c,\psi}\right)  $, where $\mathbf{\psi}$
contains the phase factors) can be defined as
\[
F\left(  \mathbf{c,\psi}\right)  =\sum_{1\leq j_{1}j_{2}\leq2s}\left\{
\left(  \mathbb{H}_{1}\left(  \mathbf{\psi}\right)  \mathbb{D}^{1}\left(
\mathbf{c}\right)  \right)  _{mn}+\frac{1}{2}\left(  L_{2}^{1}\left(
\mathbb{V}_{2}\left(  \mathbf{\psi}\right)  \mathbb{D}^{2}\left(
\mathbf{c}\right)  \right)  \right)  _{mn}\right\}  \left\vert \psi
_{m}\right\rangle \left\langle \psi_{n}\right\vert
\]
where%
\[
L_{2}^{1}\left(  \mathbb{V}_{2}\left(  \mathbf{\psi}\right)  \mathbb{D}%
^{2}\left(  \mathbf{c}\right)  \right)  _{j_{1}j_{2}}=\sum_{1\leq l_{1}%
,l_{2},j_{2}\leq r}\mathbb{V}_{2}\left(  \mathbf{\psi}\right)  _{j_{1}%
j_{2}l_{1}l_{2}}\mathbb{D}^{2}\left(  \mathbf{c}\right)  _{l_{1}l_{2}%
k_{1}j_{2}}%
\]
and
\begin{align*}
\mathbb{D}^{2}\left(  \mathbf{c}\right)  _{l_{1}l_{2}k_{1}j_{2}}  &
=\text{coeffs in eq. }\left(  \ref{D2}\right) \\
\mathbb{V}_{2}\left(  \mathbf{\psi}\right)  _{j_{1}j_{2}l_{1}l_{2}}  &
=\left\langle \psi_{j_{1}}\psi_{j_{2}}||\psi_{l_{1}}\psi_{l_{2}}\right\rangle
.
\end{align*}%
\[
L_{2}^{1}\left(  \mathbb{V}_{2}\left(  \mathbf{\psi}\right)  \mathbb{D}%
^{2}\left(  \mathbf{c}\right)  \right)  _{j_{1}k_{1}}=\sum_{1\leq l_{1}%
,l_{2},j_{2}\leq r}\left\{  \mathbb{V}_{2}\left(  \mathbf{\psi}\right)
_{j_{1}j_{2}k_{1}j_{2}}\mathbb{D}^{2}\left(  \mathbf{c}\right)  _{k_{1}%
j_{2}k_{1}j_{2}}+\mathbb{V}_{2}\left(  \mathbf{\psi}\right)  _{j_{1}%
k_{1}+sl_{1}l_{1}+s}\mathbb{D}^{2}\left(  \mathbf{c}\right)  _{l_{1}%
l_{1}+sk_{1}k_{1}+s}\right\}
\]%
\begin{align}
D^{2}\left(  \left\vert g^{\frac{N}{2}}\right\rangle \left\langle g^{\frac
{N}{2}}\right\vert \right)   &  =\left(  S_{\frac{N}{2}}\right)  ^{-1}\left\{
...
_{k}\right\rangle \left\langle \psi_{j}\psi_{k}\right\vert \right\}  .
\end{align}
The Fock operator acts as a differential operator producing the differential
of the energy i.e.
\[
\frac{\delta\mathcal{E}\left(  \mathbf{c,\psi}\right)  }{\delta\psi_{j}^{\ast
}}=F\left(  \mathbf{c,\psi}\right)  \left\vert \psi_{j}\right\rangle \text{
and }\frac{\delta\mathcal{E}\left(  \mathbf{c,\psi}\right)  }{\delta\psi_{j}%
}=\left\langle \psi_{j}\right\vert F\left(  \mathbf{c,\psi}\right)  ^{\dagger}%
\]
The Lagrangian for the minimization of the energy functional wrt GSO
variations subject to orthonormal constraints is
\[
...
\psi_{j}\right\vert \psi_{k}\right\rangle -\delta_{jk}\right)
\]%
\begin{align*}
&  \mathcal{E}\left(  \mathbf{c,\psi}\right)  =\frac{\mathcal{H}\left(
\mathbf{c,\psi}\right)  }{S_{\frac{N}{2}}\left(  \mathbf{c}\right)  }\\
&  =S_{1}\left(  \mathbf{c}\right)  ^{-1}\left\{  \sum_{1\leq j\leq2}%
n_{j}S_{0}\left(  \hat{\jmath}\right)  \left\{  \overset{}{\left\langle
\psi_{j}\left\vert h_{1}\psi_{j}\right.  \right\rangle +\left\langle
\psi_{j+s}\left\vert h_{1}\psi_{j+s}\right.  \right\rangle }+\,\left\langle
\psi_{j}\psi_{j+s}\right\vert \left\vert \psi_{j}\psi_{j+s}\right\rangle
\right\}  \right. \\
&  +\sum_{1\leq j\neq k\leq2}\left\{  c_{j}c_{k}S_{0}\left(  \hat{\jmath}%
\hat{k}\right)  \left\langle \psi_{j}\psi_{j+s}\right\vert \left\vert \psi
_{k}\psi_{k+s}\right\rangle +\frac{1}{2}\overset{}{n_{j}n_{k}S_{-1}\left(
\hat{\jmath}\hat{k}\right)  \left\langle \psi_{j}\psi_{k}|||\psi_{j}\psi
_{k}\right\rangle }\right\}
\end{align*}
leading to
\[
F\left(  \mathbf{c}_{\#}\mathbf{,\psi}_{\#}\right)  ^{\dagger}=F\left(
\mathbf{c}_{\#}\mathbf{,\psi}_{\#}\right)
\]
and
\[
\frac{\delta\mathcal{E}\left(  \mathbf{c}_{\#}\mathbf{,\psi}_{\#}\right)
}{\delta\psi_{j}^{\ast}}=F\left(  \mathbf{c}_{\#}\mathbf{,\psi}_{\#}\right)
\left\vert \psi_{\#j}\right\rangle =\sum_{1\leq k\leq2s}\left\vert \psi
_{\#k}\right\rangle \varepsilon_{_{\#}kj}%
\]
at stationary points $\left(  \mathbf{c}_{\#}\mathbf{,\psi}_{\#}\right)  ,$
where
\[
\frac{\delta\mathcal{L}\left(  \mathbf{c}_{\#}\mathbf{,\psi}_{\#}\right)
}{\delta\psi_{j}^{\ast}}=\frac{\delta\mathcal{L}\left(  \mathbf{c}%
_{\#}\mathbf{,\psi}_{\#}\right)  }{\delta\psi_{j}}=0
\]
and
\[
\frac{\partial\mathcal{L}\left(  \mathbf{c}_{\#}\mathbf{,\psi}_{\#}\right)
}{\partial c_{j}}=0.
\]
The variation of the canonical coeffcients could also be included in the
variation of the GSO's by considering unnormalized, but orthogonal GSO's. This
results in a Lagrangian
\[
\mathcal{L}\left(  \mathbf{c,\chi}\right)  =\mathcal{E}\left(  \mathbf{c,\chi
}\right)  -\sum_{1\leq j,k\leq2s}\varepsilon_{kj}\left(  \left\langle \left.
\mathbf{\chi}_{j}\right\vert \mathbf{\chi}_{k}\right\rangle -c_{j}\delta
_{jk}\right)  ,
\]
(so that in the expressions for the energy and symmetric functions
$n_{j}\equiv1$ or $0$)%
\begin{align*}
&  \mathcal{E}\left(  \mathbf{c,\chi}\right)  =\frac{\mathcal{H}\left(
\mathbf{c,c}^{-1}\mathbf{\chi}\right)  }{S_{\frac{N}{2}}\left(  \mathbf{c}%
\right)  }\\
&  =S_{1}\left(  \mathbf{c}\right)  ^{-1}\left\{  \sum_{1\leq j\leq2}%
c_{j}S_{0}\left(  \hat{\jmath}\right)  \left\{  \overset{}{\left\langle
\chi_{j}\left\vert h_{1}\chi_{j}\right.  \right\rangle +\left\langle
\chi_{j+s}\left\vert h_{1}\chi_{j+s}\right.  \right\rangle }+c_{j}%
^{-1}\,\left\langle \chi_{j}\chi_{j+s}\right\vert \left\vert \chi_{j}%
\chi_{j+s}\right\rangle \right\}  \right. \\
&  +\sum_{1\leq j\neq k\leq2}\left\{  S_{0}\left(  \hat{\jmath}\hat{k}\right)
\left\langle \chi_{j}\chi_{j+s}\right\vert \left\vert \chi_{k}\chi
_{k+s}\right\rangle +\frac{1}{2}\overset{}{c_{j}c_{k}S_{-1}\left(  \hat
{\jmath}\hat{k}\right)  \left\langle \chi_{j}\chi_{k}|||\chi_{j}\chi
_{k}\right\rangle }\right\}
\end{align*}
...
\[
\frac{\partial\mathcal{L}\left(  \mathbf{c}_{\#}\mathbf{,\psi}_{\#}\right)
}{\partial c_{j}}=\frac{\partial\mathcal{E}\left(  \mathbf{c}_{\#}%
\mathbf{,\psi}_{\#}\right)  }{\partial c_{j}}+\varepsilon_{_{\#}jj}=0
\]
the last equation is a form of Janaks theorem.

At such stationary points one can observe that the diagonal matrix elements of
the Fock operator satisfy
\begin{align*}
F\left(  \mathbf{c}_{\#},\mathbf{\psi}_{\#}\right)  _{jj}  &  =\Xi_{j}\left(
\mathbf{\psi}_{\#}\right)  \left\{  \mathcal{E}\left(  \mathbf{c}%
_{\#},\mathbf{\psi}_{\#}\right)  -\mathcal{E}_{N-1}\left(  \mathbf{c}%
_{\#},\mathbf{\psi}_{\#}\mathbf{,}\psi_{\#j}\right)  \right\} \\
&  =\Xi_{j}\left(  \mathbf{\psi}_{\#}\right)  I\left(  \psi_{\#j}\right)
;\qquad1\leq j\leq2s
\end{align*}
where%
\[
\Xi_{j}\left(  \mathbf{\psi}_{\#}\right)
\]
is the occupation number of the GSO $\psi_{\#j}$ and
\[
I\left(  \psi_{\#j}\right)  =\mathcal{E}\left(  \mathbf{c}_{\#},\mathbf{\psi
}_{\#}\right)  -\mathcal{E}_{N\,-1}\left(  \mathbf{c}_{\#},\mathbf{\psi}%
_{\#}\mathbf{,}\psi_{\#j}\right)
\]
is the "frozen" IP of the GSO $\psi_{\#j}.$

The total energy can be expressed in terms of one particle quantities as\
\[
\mathcal{E}\left(  \mathbf{c}_{\#},\mathbf{\psi}_{\#}\right)  =\frac{1}%
{2}\left\{  \overset{}{\operatorname{Tr}\left\{  F\left(  \mathbf{c}%
_{\#},\mathbf{\psi}_{\#}\right)  \right\}  +\mathcal{E}_{1}\left(
\mathbf{c}_{\#},\mathbf{\psi}_{\#}\right)  }\right\}
\]
where
\[
\mathcal{E}_{1}\left(  \mathbf{c}_{\#},\mathbf{\psi}_{\#}\right)  =\sum_{1\leq
j\leq2s}D_{jk}^{1}\left(  \mathbf{c}_{\#},\mathbf{\psi}_{\#}\right)
\left\langle \left.  \psi_{\#k}\right\vert h_{1}\psi_{\#j}\right\rangle
\]
as the CSO's \textbf{$\psi$}$_{\#}$ are also NGSO's one obtains
\[
\mathcal{E}_{1}\left(  \mathbf{c}_{\#},\mathbf{\psi}_{\#}\right)  =\sum_{1\leq
j\leq2s}N_{j}\left(  \mathbf{c}_{\#}\right)  \left\langle \left.  \psi
_{\#j}\right\vert h_{1}\psi_{\#j}\right\rangle
\]
and hence
\[
\mathcal{E}\left(  \mathbf{c}_{\#},\mathbf{\psi}_{\#}\right)  =\frac{1}{2}%
\sum_{1\leq j\leq2s}N_{j}\left(  \mathbf{c}_{\#}\right)  \kappa\left(
\psi_{\#j}\right)
\]
where
\[
\kappa\left(  \psi_{\#j}\right)  =I\left(  \psi_{\#j}\right)  +h_{1}\left(
\psi_{\#j}\right)
\]
and
\[
h_{1}\left(  \psi_{\#j}\right)  =\left\langle \left.  \psi_{\#j}\right\vert
h_{1}\psi_{\#j}\right\rangle .
\]

...
%TCIDATA{LastRevised=Wednesday, June 25, 2003 21:43:12}
...
expectation value expressions, and the various reduced density operators and
...
so that
\begin{align}
T\left(  \left\vert \underline{\chi}\right\rangle \right)   &  =\left(
\left\vert \underline{\varphi}\right\rangle \right)  \nonumber\\
T^{\dagger}\left(  \left\vert \underline{\chi}\right\rangle \right)   &
=\left(  \left\vert \underline{\varphi}\right\rangle \right)  \mathbb{T}%
^{\dagger}=\left(  \left\vert \underline{\chi}\right\rangle \right)
\end{align}
The non uniqueness of the complex canonical coefficients is then easily seen
by noting that
\begin{equation}
\left\vert g\right\rangle =\sum_{1\leq k\leq s}n_{k}^{\frac{1}{2}}%
...
\mathbf{s}_{j}\right\rangle ;1\leq j\leq r\right\}  $ we can produce the

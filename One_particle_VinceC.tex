...
%TCIDATA{LastRevised=Tuesday, January 11, 2005 09:09:03}
...
use the symbol $\bm{\psi}$ and when we refer to NGSOs we will use the symbol
$\bm{\varphi.}$
...
\bm{n},\bm{\psi}\right)  $ (first introduced by Hinze \cite{Hinze}), which is
\emph{self--adjoint only} at stationary points $\bm{\psi}_{s}$ of the orbital
variation. Further if $\bm{n}_{s}$ is a stationary point of the occupation
number variation, then the diagonal elements of the matrix of $F\left(
\bm{n}_{s},\bm{\psi}_{s}\right)  $ in the CGSO basis $\left\{  \left\vert
...
energies associated with the orbitals $\left\{  \left\vert \chi_{j}%
\right\rangle \right\}  $, but then the geminal $\left\vert g\left(
...
\bm{0}\right)  \right)  \right]  $ also can be expressed in canonical form as
\begin{equation}
...
expanded in terms of $\bm{\theta}$--dependent NGSOs $\left\{  \left.
...
significant in two respects. It can serve as a rigorous explicitly known model
...
difference between CGSOs and NGSOs.
...
\bm{R}_{1},\cdots,\bm{R}_{N_{nuc}}\right)  $ in terms of the CGSOs amplitudes
...
$\bm{\psi}$ determines $\bm{\psi}^{\ast}$.) The one--electron matrix elements
$\left\{  \left\langle \psi_{j}\left\vert h_{1}\psi_{k}\right.  \right\rangle
\right\}  $ wrt the CGSOs are
\begin{equation}
...
\end{align}
where $\left\vert \bm{\varphi}\right\rangle $ is a representative set of NGSOs
for the class $\left[  D^{1}\left(  g^{N}\left(  \bm{0}\right)  \right)
\right]  $. As $\left(  \bm{n},\bm{\varphi,\theta}\right)  $ uniquely defines
a $2N$-electron AGP-FORDO, the expression in eq.\ $\left(  \ref{thetaen}%
...
where
\begin{equation}
\mathcal{H}\left(  \bm{\eta},\bm{\alpha},\bm{\theta}\right)  =\left\langle
...
operator $F\left(  \bm{n,\psi}\right)  $ must be self--adjoint when based on
an optimized state. The details of the derivations are contained in Appendices
...
stationary point of the orbital variation, $F\left(  \bm{n,\psi}\right)  $
...
by $\left(  \bm{\eta,\alpha,\theta}\right)  $. Setting
\begin{equation}
...
\begin{equation}
U_{orb}=\left\{  u\left(  \bm{\beta}\right)  =\exp\sum_{1\leq\left\vert
...
of the two--particle Hamiltonian $h_{2}$, $\mathbb{D}^{1}\left(
\bm{n}\right)  $ the FORDM and $\mathbb{D}^{2}\left(  \bm{n}\right)  $ the
SORDM of the $2N$-electron AGP\ state (see eq.\ $\left(  \ref{D2C}\right)  ),$
wrt the basis $\left\{  \left\vert \psi_{j}\right\rangle ;1\leq j\leq
2s\right\}  .$ One should recall that in L\"{o}wdin normalization and using an
unordered basis of two--electron space
\begin{equation}
...
\Psi\right\vert \left\{  a_{p}^{\dagger}a_{p}H-a_{p}^{\dagger}Ha_{p}\right\}
\Psi\right\rangle }{\left\langle \left.  \Psi\right\vert \Psi\right\rangle
...
$=0$ then $F\left(  \mathbb{D}^{2},\bm{\varphi}\right)  _{pp}=0$ as the GSO
$\left\vert \varphi_{p}\right\rangle $ is unoccupied. In Appendix
\ref{Fock&Energy} we show that the total energy can be expressed in terms of
the Fock operator $F\left(  \mathbb{D}^{2},\bm{\varphi}\right)  \ $as
\begin{equation}
...
\end{equation}
where $\mathcal{E}_{1}\left(  \mathbb{D}^{2},\bm{\varphi}\right)  $ is the
...
$D^{1}\left(  \mathbb{D}^{2}\left(  \bm{\varphi}\right)  \right)  \equiv
D^{1}\left(  \mathbb{D}^{2}\left(  \bm{\varsigma}\right)  \right)  $ is still
not in general diagonal in the basis $\left\{  \left\vert \varsigma
...
\mathbb{D}^{2},\bm{\varphi}\right)  $ in terms of the NGSOs $\left\{
...
\bm{\varphi}\right)  $ with matrix elements
\begin{equation}
...
\begin{equation}
\mathfrak{f}\left(  \bm{\varphi}\right)  \left\vert \psi_{j}\right\rangle
...
matrix $\bm{\alpha
}$ in terms of $r^{2}$ unconstrained real parameters. We show that this
formulation of the energy is equivalent to one in terms of a
...
\bm{\alpha }\right)  $, of orthonormal GSOs by considering a fixed reference
state $D_{0}^{2N}$ and one--electron unitary transformations $U\left(
\bm{\alpha
}\right)  $ by setting%
\begin{equation}
...
,\delta\bm{\varphi}\right\}  $ in the GSOs is given by the Taylors series as
\begin{align}
...
\mathcal{E}\left(  \mathbb{D}^{2},\bm{\varphi}\right)  ^{t}\delta
\bm{\varphi}^{\ast}\right\}  ,
\end{align}
where
\begin{equation}
...
$\bm{\alpha}$ evaluated around $\bm{\alpha}$=$\bm{0}$ is
\begin{align}
...
^{2},\bm{0}\right)  }{\partial\alpha_{p}q}\right\}  $ can also be expressed in
...
\bm{\varphi}\right\}  $ maintain the orthonormality of the NGSOs i.e.\
\end{subequations}
\begin{equation}
...
\bm{\eta },\bm{\alpha},\bm{\theta}\right\}  $ and the energy functional
\end{subequations}
\begin{equation}
\mathcal{E}\left(  \bm{\eta},\bm{\alpha},\bm{\theta}\right)  =\frac
{\mathcal{H}\left(  \bm{\eta},\bm{\alpha},\bm{\theta }\right)  }{S_{N}\left(
...
\bm{\eta},\bm{\alpha },\bm{\theta}\right)   &  =\frac{1}{S_{N}\left(
\bm{\eta}\right)  }\left\{  \partial_{\bm{\eta}}^{1}\mathcal{H}\left(
\bm{\eta },\bm{\alpha},\bm{\theta}\right)  -\mathcal{E}\left(
\bm{\eta },\bm{\alpha},\bm{\theta}\right)  \partial_{\bm{\eta}}^{1}%
S_{N}\left(  \bm{\eta}\right)  \right\} \\
\partial_{\bm{\alpha}}^{1}\mathcal{E}\left(
\bm{\eta},\bm{\alpha},\bm{\theta}\right)   &  =\frac{1}{S_{N}\left(
\bm{\eta}\right)  }\partial_{\bm{\alpha}}^{1}\mathcal{H}\left(
\bm{\eta},\bm{\alpha},\bm{\theta}\right) \\
\partial_{\bm{\theta}}^{1}\mathcal{E}\left(
\bm{\eta},\bm{\alpha },\bm{\theta}\right)   &  =\frac{1}{S_{N}\left(
...
\end{subequations}
\begin{equation}
\partial_{\bm{\alpha}_{jk}}^{1}\mathcal{E}\left(
\bm{\eta },\bm{\alpha},\bm{\theta}\right)  =\frac{i}{S_{N}\left(
\bm{\eta}\right)  }\left\langle \left.  g\left(
\bm{\eta },\bm{\alpha},\bm{\theta}\right)  ^{N}\right\vert \left[
H,a_{j}^{\dagger}a_{k}\right]  g\left(
\bm{\eta},\bm{\alpha },\bm{\theta}\right)  ^{N}\right\rangle ;\quad1\leq
...
\bm{\eta },\bm{\alpha},\bm{\theta}\right)  =\frac{i}{S_{N}\left(
\bm{\eta}\right)  }\left\langle \left.  g\left(
\bm{\eta },\bm{\alpha},\bm{\theta}\right)  ^{N}\right\vert \left[  H,\left(
a_{j}^{\dagger}a_{j}+a_{j+s}^{\dagger}a_{j+s}\right)  \right]  g\left(
\bm{\eta},\bm{\alpha},\bm{\theta}\right)  ^{N}\right\rangle ;\quad1\leq j\leq
s, \label{Dtheta}%
\end{equation}
...
\bm{\eta},\bm{\alpha},\bm{\theta}\right)  ^{N}\right\rangle \right.
\nonumber\\
&  -\left.  2\mathcal{E}\left(  \bm{\eta},\bm{\alpha},\bm{\theta }\right)
\left\langle \left.  g\left(  \bm{\eta},\bm{\alpha },\bm{\theta}\right)
^{N}\right\vert \left(  a_{j}^{\dagger}a_{j}+a_{j+s}^{\dagger}a_{j+s}\right)
g\left(  \bm{\eta},\bm{\alpha },\bm{\theta}\right)  ^{N}\right\rangle
...
\bm{\eta },\bm{\alpha},\bm{\theta}\right)  ^{N}\right\vert \left[  H,\left(
a_{j}^{\dagger}a_{j}+a_{j+s}^{\dagger}a_{j+s}\right)  \right]  g\left(
\bm{\eta},\bm{\alpha},\bm{\theta}\right)  ^{N}\right\rangle =0;\quad1\leq
j\leq s, \label{s1}%
\end{equation}
and%
\begin{equation}
...
}a_{j+s}\right)  \right]  g\left(  \bm{\eta },\bm{\alpha},\bm{\theta}\right)
^{N}\right\rangle \nonumber\\
&  \qquad\qquad\qquad\qquad\qquad\left.  =\right.  \left\langle \left.
g\left(  \bm{\eta},\bm{\alpha},\bm{\theta}\right)  ^{N}\right\vert \left[
H,a_{j}^{\dagger}a_{j+s}\right]  g\left(
\bm{\eta },\bm{\alpha},\bm{\theta}\right)  ^{N}\right\rangle \nonumber\\
&  \qquad\qquad\qquad\qquad\qquad\left.  =\right.  \left\langle \left.
g\left(  \bm{\eta},\bm{\alpha},\bm{\theta}\right)  ^{N}\right\vert \left[
H,a_{j+s}^{\dagger}a_{j}\right]  g\left(
\bm{\eta },\bm{\alpha},\bm{\theta}\right)  ^{N}\right\rangle \left.
=\right.  0
\end{align}
give
\begin{equation}
...
\bm{\eta },\bm{\alpha},\bm{\theta}\right)  ^{N}\right\rangle =\mathcal{E}%
\left(  \bm{\eta},\bm{\alpha},\bm{\theta}\right)  \left\langle \left.
g\left(  \bm{\eta},\bm{\alpha},\bm{\theta}\right)  ^{N}\right\vert
a_{j}^{\dagger}a_{j}g\left(  \bm{\eta},\bm{\alpha },\bm{\theta}\right)
...
^{N}\right\vert Ha_{j}^{\dagger}a_{j}g\left(
\bm{\eta },\bm{\alpha},\bm{\theta}\right)  ^{N}\right\rangle =\mathcal{E}%
\left(  \bm{\eta},\bm{\alpha},\bm{\theta}\right)  N_{j}.
\end{equation}

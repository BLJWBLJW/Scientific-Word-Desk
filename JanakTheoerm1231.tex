...
%TCIDATA{LastRevised=Thursday, April 03, 2008 11:16:35}
...
nonsingular then the values of Lagrange parameters correspond to the
sensitivity of the object function with respect to the control parameters,
...
Hence Janaks theorem implies that the normalization of the orbitals must be a
function of the occupation numbers. The form of the dependence of the energy
on the occupation numbers can be factored through the dependency of the energy
on the orbitals, which, in particular, contains the dependence on the norm of
the orbitals, and thence on the occupation numbers. This factoriztion is
especially important as the relationship between the form of the orbitals and
the total energy is crucial in the construction of visual models and hence in
the design of new materials and molecules.
...
$U\left(  1\right)  $, e.g. Density Matrix and KS\ Functionals

\item not invariant to $U\left(  1\right)  $ e.g. correlated AGP\ functionals
\cite{Weiner&Ortiz2005}.
\end{itemize}

In order to answer the preceding questions and analyze the form of the energy
dependence on occupation numbers we will review the basic principles of
Hohenburg-Kohn (HK) and KS DFT (section 2), Janaks Theorem (section 3) and
Density Matrix Functional Theory (DMFT) in the context of a constrained
variational approach and sensitivity theory. The KS\ density functional,
density matrix functional and the AGP functional determine the total energy in
terms of a set of occupations numbers and one particle orbitals. The first two
have only been defined implicitly while the AGP functional has been explicitly
constructed \cite{Weiner&Ortiz2005} in terms of General Spin Orbitals (GSO's)
and parameters that determine the FORDO occupation numbers (see section 4). In
section 5 we investigate the form of the derivatives of this functional with
respect to occupation numbers and the special case of its value at integer
occupation. In section 6 we conclude with a discussion and summary.
...
$\mathcal{\tilde{F}}\left(  \rho\right)  $ \cite{HK} of the charge density
$\rho\left(  \boldsymbol{r}\right)  ,$ where
\begin{equation}
...
$D^{1}$ \cite{Coleman} (see Appendix \ref{DenMat} for details of density and
...
and a family of $SU(2)$-coherent spin states $\left\{  \left\vert
\boldsymbol{\xi}\right\rangle \right\}  $ \cite{weiner2005}. In order to
consider spin dependent effects it is convenient to extend the original
formulation of DFT in terms of $\rho$ and consider a universal functional,
$\mathcal{F}\left(  \gamma\right)  $, of the space-spin density $\gamma\left(
\boldsymbol{r,\xi}\right)  =$ $D^{1}\left(  \boldsymbol{r,\xi;r,\xi}\right)  $
\cite{weiner-trickey} and local external potentials of the position variable
$\boldsymbol{r}$ and spin space variable $\boldsymbol{\xi}$ that describe the
interaction with both external electric and magnetic fields.

For a system that contains on average $N$-electrons the space spin density
must satisfy the condition
\begin{equation}
\int\gamma\left(  \boldsymbol{r,\xi}\right)  d\boldsymbol{r}d\boldsymbol{\xi
}=N, \label{Number}%
\end{equation}
...
$\mathcal{B}_{1}\left(  \mathcal{H}^{N}\right)  $). The constraining
functional $\Xi_{1}$ can be written in terms of the set of density operators
$\left\{  \left.  \Phi\left(  \boldsymbol{x}\right)  ^{\dagger}\Phi\left(
\boldsymbol{x}\right)  \right\vert \boldsymbol{x\in}\mathbb{X}\right\}  $, as
\begin{equation}
\Xi_{1}\left(  D\right)  \left(  \boldsymbol{x}\right)  =\operatorname{Tr}%
\left\{  \Phi\left(  \boldsymbol{x}\right)  ^{\dagger}\Phi\left(
\boldsymbol{x}\right)  D\right\}  =\gamma\left(  \boldsymbol{x}\right)  .
\end{equation}
As $D$ is a positive operator it can always be expressed as the product
\begin{equation}
D=QQ^{\dagger},
\end{equation}
where $\left(  \text{Appendix \ref{Denmat}}\right)  $
\begin{equation}
...
\ref{Denmat} but do not form a linear space. However, the multi valuedness of
...
j\leq r_{N}\right\}  $ is any complete ortho normal basis for $\mathcal{H}%
...
\mathbb{X}\right)  ,$ i.e. the set of positive absolute integrable
functions$,$ and the real symmetric inner product $\left(  \cdot\left\vert
\cdot\right.  \right)  $ in $\mathcal{B}_{2sa}\left(  \mathcal{H}^{N}\right)
$ is defined as
\begin{equation}
...
\end{equation}
which can be defined in terms of quadratic forms based on the inner product
eq. $\left(  \ref{Opinner}\right)  $ (Appendix \ref{super}). The action of
such super operators can be extended to the larger Hilbert-Schmidt space
$\mathcal{B}_{2}\left(  \mathcal{H}^{N}\right)  ,$ which can be expressed as
the direct sum of the orthogonal subspaces formed by the self adjoint and anti
...
\end{equation}
and $\left\vert \left[  TY\right]  \right)  $ is the othogonal projection of
$TY$ onto $\mathcal{B}_{2sa}\left(  \mathcal{H}^{N}\right)  $ i.e. the self
adjoint part of $TY,$ given by
\[
\left[  TY\right]  _{+}=\frac{1}{2}\left\{  TY+YT\right\}  =\frac{1}{2}\left[
T,Y\right]  _{+}.
\]
This definition is sufficiently general for our purposes as we only consider
expressions of the form $\operatorname{Tr}\left\{  XTY\right\}  $ and as
$X\in\mathcal{B}_{2sa}\left(  \mathcal{H}^{N}\right)  $ it is orthogonal to
the subspace of anti self adjoint operators giving
\begin{equation}
\operatorname{Tr}\left\{  XTY\right\}  =\operatorname{Tr}\left\{  X\left[
TY\right]  _{+}\right\}  .
\end{equation}
The constraint eq. $\left(  \ref{QuadCon}\right)  $ can thus be formulated in
terms of the generalized \emph{vector} super operator $\widehat{\Phi^{\dagger
}\Phi}$%
\begin{equation}
\Xi_{2}\left(  A\right)  =\left(  A\left\vert \widehat{\Phi^{\dagger}\Phi
}A\right.  \right)  =\gamma,
\end{equation}
which has components, $\widehat{\Phi\left(  \boldsymbol{x}\right)  ^{\dagger
}\Phi\left(  \boldsymbol{x}\right)  },$ that are multiplication super operators.

As the operators $\left\{  \widehat{\Phi\left(  \boldsymbol{x}\right)
^{\dagger}\Phi\left(  \boldsymbol{x}\right)  }\right\}  $ are bounded one can
...
has components
\begin{equation}
\left[  d\Xi_{2}\left(  A\right)  \right]  \left(  \boldsymbol{x}\right)
=\left(  \Phi\left(  \boldsymbol{x}\right)  ^{\dagger}\Phi\left(
\boldsymbol{x}\right)  A\right\vert .
\end{equation}
The set of opeators $\left\{  \left.  \left\vert \Phi\left(  \boldsymbol{x}%
\right)  ^{\dagger}\Phi\left(  \boldsymbol{x}\right)  A\right)  \right\vert
\boldsymbol{x\in}\mathbb{X}\right\}  $ forms a generalized basis $\left(
\text{Appendix \ref{Bases}}\right)  $ for the normal spaces of the surfaces
$\Xi_{2}\left(  A\right)  =\gamma$ for all points $A$ $\left(  \text{Appendix
\ref{Regular}}\right)  $ which shows that the constraint $\Xi_{2}\left(
A\right)  =\gamma$ is regular everywhere.
...
K+V_{2}\right)  A_{\ast}\left(  \gamma\right)  ^{2}\right\}  =E_{U}\left(
A_{\ast}\left(  \gamma\right)  \right)
\end{equation}
in terms of the minima, $E_{U}\left(  A_{\ast}\left(  \gamma\right)  \right)
...
\begin{equation}
\left\langle \left.  \lambda\right\vert \left\{  \Xi_{2}\left(  A\right)
-\gamma\right\}  \right\rangle =\int\lambda\left(  \boldsymbol{x}\right)
\left\{  \Xi_{2}\left(  A\right)  \left(  \boldsymbol{x}\right)
-\gamma\left(  \boldsymbol{x}\right)  \right\}  d\boldsymbol{x,\,}\lambda\in
L_{\infty}\left(  \mathbb{X}\right)  ,\gamma\in L_{1}\left(  \mathbb{X}%
\right)  ,
\end{equation}
$\left\langle \cdot|\cdot\right\rangle $ is the scalar product between the
dual spaces $\left\langle L_{\infty}\left(  \mathbb{X}\right)  ,L_{1}\left(
\mathbb{X}\right)  \right\rangle $ i.e.
\begin{equation}
...
$\mathcal{D}$ given by
\begin{equation}
\mathcal{D}=\left\{  f\left\vert f\in L_{1}\left(  \mathbb{X}\right)
;\,f\left(  \boldsymbol{x}\right)  \geq0;\int f\left(  \boldsymbol{x}\right)
d\boldsymbol{x}=N\right.  \right\}  .
\end{equation}
This choice of the density $\gamma$ coupled while insisting that
...
\end{equation}
thus the normalization constraint is not an active constraint for such
densities and the Lagrange parameter associated with it is zero.
...
as
\begin{equation}
\Omega\left(  \lambda\right)  =\int\lambda\left(  \boldsymbol{x}\right)
\boldsymbol{\Phi}\left(  \boldsymbol{x}\right)  ^{\dagger}\boldsymbol{\Phi
}\left(  \boldsymbol{x}\right)  d\boldsymbol{x}%
\end{equation}
and
\begin{equation}
\left(  A\left\vert \widehat{\Omega\left(  \lambda\right)  }\right.  A\right)
=\int\lambda\left(  \boldsymbol{x}\right)  \Xi_{2}\left(  A\right)  \left(
\boldsymbol{x}\right)  d\boldsymbol{x.}%
\end{equation}
The Lagrangian for a fixed $N$-particle density can then be recast as
\begin{equation}
\mathcal{L}\left(  A,\lambda,\gamma\right)  =\left(  A\left\vert
\widehat{\left(  K+V_{2}-\Omega\left(  \lambda\right)  \right)  }A\right.
\right)  +\left\langle \lambda\left\vert \gamma\right.  \right\rangle .
\end{equation}
...
\end{equation}
As the constraint is regular $\lambda_{\ast}$ is unique \cite{unique}, but
...
\end{equation}
Moreover $\gamma$ is the density of the ground state $D_{\ast}\left(
\gamma\right)  =A_{\ast}\left(  \gamma\right)  ^{2}$ of the Hamiltonian
$K+V_{2}-\Omega\left(  \lambda_{\ast}\left(  \gamma\right)  \right)  $ i.e. of
an electronic system subject to an external potential $\lambda_{\ast}\left(
\gamma\right)  $. \textrm{Insert variation over local potentials --Legendre. }
...
\boldsymbol{r,\xi}\right)  \gamma\left(  \boldsymbol{r}^{\prime}%
\boldsymbol{,\xi}\right)  }{\left\Vert \boldsymbol{r-r}^{\prime}\right\Vert
}d\boldsymbol{r}d\boldsymbol{r}^{\prime}d\boldsymbol{\xi},\nonumber\\
...
is
\begin{equation}
E\left(  \gamma\boldsymbol{,}\mathrm{v}\right)  =\mathcal{F}_{HK}\left(
\gamma\right)  +f_{\gamma}\left(  \mathrm{v}\right)  ,
\end{equation}
where the explicit effect of the environment is described by the functional
\begin{equation}
f_{\gamma}\left(  \mathrm{v}\right)  =\int\mathrm{v}\left(  \boldsymbol{r,\xi
}\right)  \gamma\left(  \boldsymbol{r,\xi}\right)  d\boldsymbol{r}%
d\boldsymbol{\xi.}%
\end{equation}
The ground state energy $E_{GS}\left(  \mathrm{v}\right)  $ and the
corresponding space-spin density $\gamma_{GS}$ $\left(  \mathrm{v}\right)  $
are then determined by the solution to the constrained optimization problem
\begin{equation}
E_{GS}\left(  \mathrm{v}\right)  =E\left(  \gamma_{GS}\boldsymbol{,}%
\mathrm{v}\right)  =\min_{\boldsymbol{\gamma\in}\mathcal{D}}E\left(
\gamma\boldsymbol{,}\mathrm{v}\right)  , \label{GS}%
\end{equation}
which one can make unconstrained in terms of square integrable functions $f$
by setting
\begin{equation}
\gamma\left(  \boldsymbol{x}\right)  =\frac{Nf\left(  \boldsymbol{x}\right)
^{2}}{\left\langle f|f\right\rangle }=\frac{N\left\langle f\left\vert
\Phi\left(  \boldsymbol{x}\right)  ^{\dagger}\Phi\left(  \boldsymbol{x}%
\right)  f\right.  \right\rangle }{\left\langle f|f\right\rangle },\,f\in
L_{2}\left(  \mathbb{X}\right)  .
\end{equation}
The minimizer $f$ $_{GS}$ of $E\left(  f\boldsymbol{,}\mathrm{v}\right)  $
then satisfies
\begin{equation}
\frac{\delta E\left(  f_{GS}\boldsymbol{,}\mathrm{v}\right)  }{\delta f\left(
\boldsymbol{x}\right)  }=2f_{GS}\left(  \boldsymbol{x}\right)  \left\{
\frac{N}{\left\langle f_{GS}|f_{GS}\right\rangle }\frac{\delta E\left(
f_{GS}\boldsymbol{,}\mathrm{v}\right)  }{\delta\gamma\left(  \boldsymbol{x}%
\right)  }-\int\frac{\delta E\left(  f_{GS}\boldsymbol{,}\mathrm{v}\right)
}{\delta\gamma\left(  \boldsymbol{x}^{\prime}\right)  }\gamma_{GS}\left(
\boldsymbol{x}^{\prime}\right)  d\boldsymbol{x}^{\prime}\right\}
\overset{a.e.}{=}0
\end{equation}
which implies that
\begin{equation}
f_{GS}\left(  \boldsymbol{x}\right)  \overset{a.e.}{=}0\Leftrightarrow
\gamma_{GS}\left(  \boldsymbol{x}\right)  \overset{a.e.}{=}0
\end{equation}
or
\begin{equation}
\frac{\delta E\left(  f_{GS}\boldsymbol{,}\mathrm{v}\right)  }{\delta
\gamma\left(  \boldsymbol{x}\right)  }\overset{a.e.}{=}\alpha\left(
\gamma_{GS}\right)  , \label{fmin}%
\end{equation}
where%
\begin{equation}
\alpha\left(  \gamma_{GS}\right)  =\frac{\left\langle f_{GS}|f_{GS}%
\right\rangle }{N}\int\frac{\delta E\left(  f_{GS}\boldsymbol{,}%
\mathrm{v}\right)  }{\delta\gamma\left(  \boldsymbol{x}^{\prime}\right)
}\gamma_{GS}\left(  \boldsymbol{x}^{\prime}\right)  d\boldsymbol{x}^{\prime}.
\label{alpha}%
\end{equation}
Hence
\begin{align}
\alpha\left(  \gamma_{GS}\right)   &  =\frac{\left\langle f_{GS}%
|f_{GS}\right\rangle }{N}\alpha\left(  \gamma_{GS}\right)  \int\gamma
_{GS}\left(  \boldsymbol{x}^{\prime}\right)  d\boldsymbol{x}^{\prime}%
=\frac{\left\langle f_{GS}|f_{GS}\right\rangle }{N}\alpha\left(  \gamma
_{GS}\right) \\
&  \Rightarrow\alpha\left(  \gamma_{GS}\right)  =0\text{ or }f_{GS}\left(
\boldsymbol{x}\right)  \overset{a.e.}{=}0.
\end{align}
So as $f_{GS}\left(  \boldsymbol{x}\right)  \overset{a.e.}{\neq}0$ we obtain
the well known result
\begin{equation}
\frac{\delta E\left(  f_{GS}\boldsymbol{,}\mathrm{v}\right)  }{\delta
\gamma\left(  \boldsymbol{x}\right)  }\overset{a.e.}{=}0\Leftrightarrow
\frac{\delta\mathcal{F}_{HK}\left(  \gamma_{GS}\right)  }{\delta\gamma\left(
\boldsymbol{x}\right)  }\overset{a.e.}{=}-\mathrm{v}\left(  \boldsymbol{x}%
\right)  .
\end{equation}
...
\begin{equation}
\left[  \gamma_{GS}\left(  \mathrm{v}\right)  \right]  \left(  \boldsymbol{x}%
\right)  =\left[  D_{\ast}^{1}\left(  \gamma_{GS}\left(  \mathrm{v}\right)
\right)  \right]  \left(  \boldsymbol{x,x}\right)  =\sum_{1\leq j\leq r_{1}%
}N_{j}\left\vert \tilde{\psi}_{j}\left(  \boldsymbol{x}\right)  \right\vert
^{2}, \label{NGSOexpan}%
\end{equation}
...
\left[  \gamma_{GS}\left(  \mathrm{v}\right)  \right]  \left(  \boldsymbol{x}%
\right)  =\left[  D_{\ast}^{1}\left(  \gamma_{GS}\left(  \mathrm{v}\right)
\right)  \right]  \left(  \boldsymbol{x,x}\right)  =\sum_{1\leq j\leq
N}\left\vert \varphi_{j}\left(  \boldsymbol{x}\right)  \right\vert ^{2}
\label{Genexp}%
\end{equation}
...
unnormalized orthogonal sets of GSO's $\boldsymbol{\varphi,}$ treating the
occupation numbers $\boldsymbol{N}$ as control parameters, the other in terms
of orthonormal sets $\boldsymbol{\tilde{\varphi}}$ and occupation numbers
$\boldsymbol{N,}$ viewed as constrained independent variables. This is made
explicit through the introduction of two functionals $\Gamma_{1}$ and
$\Gamma_{2}.$ The first functional $\Gamma_{1}$ is defined by
\begin{equation}
\Gamma_{1}:\mathfrak{S}_{1}\left(  \boldsymbol{N}\left(  N\right)  \right)
\subset\mathcal{H}^{1}\rightarrow L_{1+}\left(  \mathbb{X}\right)  \subset
L_{1}\left(  \mathbb{X}\right)  ,
\end{equation}
where%
\begin{align}
\gamma &  =\Gamma_{1}\left(  \boldsymbol{\varphi}\right)  ,\nonumber\\
\left[  \Gamma_{1}\left(  \boldsymbol{\varphi}\right)  \right]  \left(
\boldsymbol{x}\right)   &  =\sum_{1\leq j\leq r_{1}}\left\vert \varphi
_{j}\left(  \boldsymbol{x}\right)  \right\vert ^{2}=\gamma\left(
\boldsymbol{x}\right)  . \label{Gamma}%
\end{align}
and the domain $\mathfrak{S}_{1}\left(  \boldsymbol{N}\left(  N\right)
\right)  $ is given by
\begin{equation}
\mathfrak{S}_{1}\left(  \boldsymbol{N}\left(  N\right)  \right)  =\left\{
\boldsymbol{\varphi\in}\mathfrak{S}_{1}\left(  \boldsymbol{N}\right)
\left\vert \sum_{1\leq j\leq r_{1}}N_{j}=N\right.  \right\}  ,
\end{equation}
where
\begin{equation}
\mathfrak{S}_{1}\left(  \boldsymbol{N}\right)  =\left\{  \boldsymbol{\varphi
...
where
\begin{equation}
\Gamma_{2}\left(  \boldsymbol{\tilde{\varphi},N}\right)  =\sum_{1\leq j\leq
r_{1}}\left\vert \tilde{\varphi}_{j}\left(  \boldsymbol{x}\right)  \right\vert
^{2}N_{j}=\gamma\left(  \boldsymbol{x}\right)  , \label{GammaN}%
\end{equation}
$\mathfrak{S}_{2}\left(  N\right)  $ is composed of sets of orthonormalized
GSO's
\begin{equation}
\mathfrak{S}_{2}=\left\{  \boldsymbol{\tilde{\varphi}}\left\vert \left\langle
...
N_{j}\leq1,1\leq j\leq r_{1};\sum_{1\leq j\leq r_{1}}N_{j}=N\right.  \right\}
.
\end{equation}
Clearly
\begin{equation}
\gamma=\Gamma_{1}\left(  \boldsymbol{\varphi}\right)  =\Gamma_{2}\left(
\boldsymbol{\tilde{\varphi},N}\right)  ,
\end{equation}
where $\boldsymbol{\varphi}$ and $\left\{  \boldsymbol{\tilde{\varphi}%
,N}\right\}  $ determine the same FORDO $D^{1}$ i.e.
\begin{equation}
D^{1}\left(  \boldsymbol{\varphi}\right)  =D^{1}\left(  \boldsymbol{\tilde
{\varphi},N}\right)  =\sum_{1\leq j\leq r_{1}}\left\vert \varphi
_{j}\right\rangle \left\langle \varphi_{j}\right\vert =\sum_{1\leq j\leq
r_{1}}\left\vert \tilde{\varphi}_{j}\right\rangle \left\langle \tilde{\varphi
}_{j}\right\vert N_{j}.
\end{equation}
The optimization wrt $\gamma$ can thus be expressed in terms of
$\boldsymbol{\varphi}$ via $\Gamma_{1}$ or $\left\{  \boldsymbol{\tilde
{\varphi},N}\right\}  $ via $\Gamma_{2}.$ We will base the following analysis
on $\Gamma_{1},$ but describe and compare in Appendix \ref{GSO&Occ} the
analogous analysis in terms of $\Gamma_{2}.$

The functional $\Gamma_{1}$ is not one to one and is constant on equivalence
classes, $\mathfrak{S}\left(  \boldsymbol{N}\left(  N\right)
\boldsymbol{,\gamma}\right)  ,$ of sets of GSO's $\boldsymbol{\varphi}$
corresponding to the equivalence classes $\left[  \gamma\right]  _{1}$ of
FORDO's, that produce the same density. Hence an inverse functional
$\Gamma_{1}^{-1}$ does not exist. We will, however, discuss later a procedure
that essentially provides a one to one relationship between densities and
GSO's and thus leads to a generalized inverse.
...
_{HK}\left(  \gamma\right)  . \label{FO}%
\end{equation}
...
\begin{equation}
\mathcal{F}_{T}\left(  \boldsymbol{\varphi}\right)  =\sum_{1\leq j\leq r_{1}%
...
\emph{constant} for all $\boldsymbol{\varphi}$ that satisfy $\gamma=\Gamma
_{1}\left(  \boldsymbol{\varphi}\right)  $, while the functional
$\mathcal{F}_{T}\left(  \boldsymbol{\varphi}\right)  $ can have
\emph{different} values for values of $\boldsymbol{\varphi}$ that correspond
to a fixed values of $\gamma$
...
is
\begin{equation}
E\left(  \Gamma_{1}\left(  \boldsymbol{\varphi}_{GS}\right)  \boldsymbol{,}%
\mathrm{v}\right)  =\min_{\boldsymbol{\varphi\in}\mathfrak{S}\left(
\boldsymbol{N}\left(  N\right)  \right)  }\mathcal{E}_{O}\left(
...
\end{equation}
The feasible set $\mathfrak{S}\left(  \boldsymbol{N}\left(  N\right)  \right)
...
,\mathrm{v}\right)  $ of the Lagrangian
\begin{equation}
\mathcal{L}_{\boldsymbol{\varphi}}\left(  \boldsymbol{\varphi}%
,\boldsymbol{\varepsilon,N,}\mathrm{v}\right)  =\mathcal{E}_{O}\left(
\boldsymbol{\varphi,}\mathrm{v}\right)  -\sum_{1\leq j,k\leq r_{1}}%
\varepsilon_{jk}\left\{  \overset{}{\left\langle \left.  \varphi
...
\mathcal{L}_{\boldsymbol{\varphi}}\left(  \boldsymbol{\varphi}\left(
\boldsymbol{N}\right)  ,\boldsymbol{\varepsilon}\left(  \boldsymbol{N}\right)
,\mathrm{v}\right)  =\max_{\boldsymbol{\varepsilon}}\min_{\boldsymbol{\varphi
}}\mathcal{L}_{\boldsymbol{\varphi}}\left(  \boldsymbol{\varphi}%
,\boldsymbol{\varepsilon,N,}\mathrm{v}\right)  , \label{KSM1}%
\end{equation}
which leads to optimal values $\mathcal{E}_{O}\left(  \boldsymbol{\varphi
}\left(  \boldsymbol{N}\right)  \boldsymbol{,}\mathrm{v}\right)  =$
$\mathcal{L}_{\boldsymbol{\varphi}}\left(  \boldsymbol{\varphi}\left(
\boldsymbol{N}\right)  ,\boldsymbol{\varepsilon}\left(  \boldsymbol{N}\right)
,\boldsymbol{N,}\mathrm{v}\right)  $ of the GSO energy functional for fixed
values of the control parameters $\boldsymbol{N}$. In the second optimization
one finds minima of $\mathcal{E}_{O}\left(  \boldsymbol{\varphi}\left(
\boldsymbol{N}\right)  \boldsymbol{,}\mathrm{v}\right)  $ with respect to a
constrained variation of the normalization control parameters. The
constraints
\begin{equation}
0\leq N_{j}\leq1,1\leq j\leq r_{1}%
\end{equation}
on the control parameter $\boldsymbol{N}$ can be held by setting
\begin{equation}
N_{j}\left(  \boldsymbol{\theta}\right)  =\sin^{2}\theta_{j},
\end{equation}
and the introduction of a Lagrange parameter $\eta$ in order to enforce the
constraint
\begin{equation}
\sum_{1\leq j\leq r_{1}}N_{j}\left(  \boldsymbol{\theta}\right)  =N.
\end{equation}
This leads to a Lagrangian
\begin{equation}
\mathcal{L}_{\boldsymbol{\theta}}\left(  \boldsymbol{\varphi}\left(
\boldsymbol{\theta}\right)  ,\eta,N,\mathrm{v}\right)  =\mathcal{E}_{O}\left(
\boldsymbol{\varphi}\left(  \boldsymbol{\theta}\right)  \boldsymbol{,}%
\mathrm{v}\right)  -\eta\left\{  \sum_{\,1\leq j\leq r_{1}}N_{j}\left(
\boldsymbol{\theta}\right)  -N\right\}  .
\end{equation}
The saddle point $\mathcal{L}_{\boldsymbol{\theta}}\left(  \boldsymbol{\varphi
}\left(  \boldsymbol{\theta}_{\#}\left(  N\right)  \right)  ,\eta\left(
N\right)  ,\mathrm{v}\right)  $ is given by
\begin{equation}
\mathcal{L}_{\boldsymbol{\theta}}\left(  \boldsymbol{\varphi}\left(
\boldsymbol{\theta}_{\#}\left(  N\right)  \right)  ,\eta,N,\mathrm{v}\right)
=\max_{\eta}\min_{\boldsymbol{\theta}}\mathcal{L}_{\boldsymbol{\theta}}\left(
\boldsymbol{\varphi}\left(  \boldsymbol{\theta}\right)  ,\eta,N,\mathrm{v}%
\right)  ,
\end{equation}
which determines the ground state energy $E\left(  \gamma_{GS}\boldsymbol{,}%
\mathrm{v}\right)  =\mathcal{E}_{O}\left(  \boldsymbol{\varphi}\left(
\boldsymbol{\theta}_{\#}\left(  N\right)  \right)  ,\mathrm{v}\right)  .$The
ground state density can then expanded as
\begin{equation}
\gamma_{GS}\left(  \boldsymbol{x}\right)  =\sum_{1\leq j\leq r_{1}}\left\vert
\overset{}{\left[  \varphi_{j}\left(  \boldsymbol{\theta}_{\#}\left(
N\right)  \right)  \right]  \left(  \boldsymbol{x}\right)  }\right\vert ^{2}.
\end{equation}


The saddle points of $\mathcal{L}_{\boldsymbol{\varphi}}\left(
\boldsymbol{\varphi},\boldsymbol{\varepsilon,N,}\mathrm{v}\right)  $ are
characterized by the necessary conditions
\begin{subequations}
\begin{align}
\frac{\delta\mathcal{L}_{\boldsymbol{\varphi}}\left(  \boldsymbol{\varphi
},\boldsymbol{\varepsilon,N,}\mathrm{v}\right)  }{\delta\varphi_{j}^{\ast}}
&  =0;\,1\leq j\leq r_{1},\label{KSC1}\\
\frac{\partial\mathcal{L}_{\boldsymbol{\varphi}}\left(  \boldsymbol{\varphi
},\boldsymbol{\varepsilon,N,}\mathrm{v}\right)  }{\partial\varepsilon_{jk}}
...
multipliers $\boldsymbol{\varepsilon}\left(  \boldsymbol{N}\right)  $ is
hermitian i.e.
\end{subequations}
\begin{equation}
\varepsilon\left(  \boldsymbol{N}\right)  _{jk}=\varepsilon\left(
\boldsymbol{N}\right)  _{kj}^{\ast},\quad1\leq j,k\leq r_{1}%
\end{equation}
and
\begin{equation}
\frac{\delta\mathcal{E}_{O}\left(  \boldsymbol{\varphi}\left(  \boldsymbol{N}%
\right)  ,\boldsymbol{\varepsilon}\left(  \boldsymbol{N}\right)
,\mathrm{v}\right)  }{\delta\varphi_{j}^{\ast}}-\sum_{1\leq k\leq r_{1}%
}\varepsilon_{jk}\left(  \boldsymbol{N}\right)  \left\vert \varphi_{k}\left(
\boldsymbol{N}\right)  \right\rangle =0;\,1\leq j\leq r_{1}.
\end{equation}
Which after noting that if one factors the energy functional through the
density, (denoted by $\mathcal{E}_{O}\left(  \gamma\left(  \boldsymbol{\varphi
}\right)  \boldsymbol{,}\mathrm{v}\right)  $ in contrast to $\mathcal{E}%
_{O}\left(  \boldsymbol{\varphi,}\mathrm{v}\right)  $), one obtains
\begin{align}
\frac{\delta\mathcal{E}_{O}\left(  \gamma\left(  \boldsymbol{\varphi}\right)
\boldsymbol{,}\mathrm{v}\right)  }{\delta\varphi_{j}^{\ast}\left(
\boldsymbol{x}\right)  }  &  =\int\frac{\delta\mathcal{E}_{O}\left(
\gamma\left(  \boldsymbol{\varphi}\right)  \boldsymbol{,}\mathrm{v}\right)
}{\delta\gamma\left(  \boldsymbol{x}^{\prime}\right)  }\frac{\delta
\gamma\left(  \boldsymbol{x}^{\prime}\right)  }{\delta\varphi_{j}^{\ast
}\left(  \boldsymbol{x}\right)  }d\boldsymbol{x}^{\prime}\nonumber\\
&  =\int\frac{\delta\mathcal{E}_{O}\left(  \gamma\left(  \boldsymbol{\varphi
}\right)  \boldsymbol{,}\mathrm{v}\right)  }{\delta\gamma\left(
\boldsymbol{x}\right)  }\varphi_{j}\left(  \boldsymbol{x}^{\prime}\right)
\delta\left(  \boldsymbol{x-x}^{\prime}\right)  d\boldsymbol{x}^{\prime}%
=\frac{\delta\mathcal{E}_{O}\left(  \gamma\left(  \boldsymbol{\varphi}\right)
\boldsymbol{,}\mathrm{v}\right)  }{\delta\gamma\left(  \boldsymbol{x}\right)
}\varphi_{j}\left(  \boldsymbol{x}\right)
\end{align}
that gives%
\begin{equation}
\frac{\delta\mathcal{E}_{O}\left(  \left[  \gamma\left(  \boldsymbol{\varphi
}\right)  \right]  \boldsymbol{\left(  \boldsymbol{N}\right)  ,}%
\mathrm{v}\right)  }{\delta\gamma}\left\vert \varphi_{j}\left(  \boldsymbol{N}%
\right)  \right\rangle -\sum_{1\leq k\leq r_{1}}\varepsilon_{jk}\left(
\boldsymbol{N}\right)  \left\vert \varphi_{k}\left(  \boldsymbol{N}\right)
\right\rangle =0;\,1\leq j\leq r_{1}.
\end{equation}
The operator $\frac{\delta\mathcal{E}_{O}\left(  \left[  \gamma\left(
\boldsymbol{\varphi}\right)  \right]  \boldsymbol{\left(  \boldsymbol{N}%
\right)  ,}\mathrm{v}\right)  }{\delta\gamma}$ is a multiplication operator in
the coordinate representation given by
\begin{equation}
\frac{\delta\mathcal{E}_{O}\left(  \left[  \gamma\left(  \boldsymbol{\varphi
}\right)  \right]  \boldsymbol{\left(  \boldsymbol{N}\right)  ,}%
\mathrm{v}\right)  }{\delta\gamma}\left\vert \varphi_{j}\left(  \boldsymbol{N}%
\right)  \right\rangle =\left\vert \frac{\delta\mathcal{E}_{O}\left(  \left[
\gamma\left(  \boldsymbol{\varphi}\right)  \right]  \boldsymbol{\left(
\boldsymbol{N}\right)  ,}\mathrm{v}\right)  }{\delta\gamma}\varphi_{j}\left(
\boldsymbol{N}\right)  \right\rangle ,
\end{equation}
where
\begin{equation}
\left[  \frac{\delta\mathcal{E}_{O}\left(  \left[  \gamma\left(
\boldsymbol{\varphi}\right)  \right]  \boldsymbol{\left(  \boldsymbol{N}%
\right)  ,}\mathrm{v}\right)  }{\delta\gamma}\varphi_{j}\left(  \boldsymbol{N}%
\right)  \right]  \left(  \boldsymbol{x}\right)  =\left(  \frac{\delta
\mathcal{E}_{O}\left(  \left[  \gamma\left(  \boldsymbol{\varphi}\right)
\right]  \boldsymbol{\left(  \boldsymbol{N}\right)  ,}\mathrm{v}\right)
}{\delta\gamma\left(  \boldsymbol{x}\right)  }\right)  \left(  \left[
\varphi_{j}\left(  \boldsymbol{N}\right)  \right]  \left(  \boldsymbol{x}%
\right)  \right)  .
\end{equation}
...
_{j}\left(  \boldsymbol{N}\right)  \right\rangle \right\}  $ are the
occupation numbers and eq. $\left(  \ref{FockEq}\right)  $ is not an
eigenvalue equation unless $\mathcal{E}_{O}\left(  \boldsymbol{\varphi}\left(
\boldsymbol{N}\right)  ,\mathrm{v}\right)  $ is invariant to unitary mixing of
$\boldsymbol{\varphi}\left(  \boldsymbol{N}\right)  $.
...
control parameters $\boldsymbol{N}$, is
\begin{equation}
\frac{\partial\mathcal{E}_{O}\left(  \varphi\boldsymbol{\left(  \boldsymbol{N}%
\right)  },\mathrm{v}\right)  }{\partial N_{j}}=\varepsilon_{jj}\left(
\boldsymbol{N}\right)  , \label{Janak}%
\end{equation}
...
\left[  \gamma\left(  \boldsymbol{\varphi}\right)  \right]
\boldsymbol{\left(  \boldsymbol{N}\right)  ,}\mathrm{v}\right)  }{\delta
\gamma}$ i.e.
\begin{equation}
\frac{\partial\mathcal{E}_{O}\left(  \left[  \gamma\left(  \boldsymbol{\varphi
}\right)  \right]  \boldsymbol{\left(  \boldsymbol{N}\right)  },\mathrm{v}%
\right)  }{\partial N_{j}}=\left\langle \tilde{\varphi}_{j}\left(
\boldsymbol{N}\right)  \left\vert \frac{\delta\mathcal{E}_{O}\left(  \left[
\gamma\left(  \boldsymbol{\varphi}\right)  \right]  \boldsymbol{\left(
\boldsymbol{N}\right)  ,}\mathrm{v}\right)  }{\delta\gamma}\right.
\tilde{\varphi}_{j}\left(  \boldsymbol{N}\right)  \right\rangle =\tilde
{\varepsilon}_{jj}\left(  \boldsymbol{N}\right)  =\varepsilon_{jj}\left(
\boldsymbol{N}\right)  ,
\end{equation}
...
}\left(  \boldsymbol{\theta}\right)  ,\eta,N,\mathrm{v}\right)  }%
{\partial\theta_{j}}=\frac{\partial\mathcal{E}_{O}\left(  \varphi\left(
\boldsymbol{\theta}\right)  ,\mathrm{v}\right)  }{\partial\theta_{j}}-\eta
\sin\left\{  2\theta_{j}\right\}  =\left\{  \frac{\partial\mathcal{E}%
_{O}\left(  \varphi\left(  \boldsymbol{N}\left(  \boldsymbol{\theta}\right)
\right)  ,\mathrm{v}\right)  }{\partial N_{j}}-\eta\right\}  \sin\left\{
2\theta_{j}\right\}  \,;\,1\leq j\leq r_{1},
\end{equation}
one observes that the stationarity conditions eq. $\left(  \ref{KSC2}\right)
$
\begin{equation}
\frac{\partial\mathcal{L}_{\boldsymbol{\theta}}\left(  \boldsymbol{\varphi
}\left(  \boldsymbol{\theta}\left(  N\right)  \right)  ,\eta\boldsymbol{,}%
N\boldsymbol{,}\mathrm{v}\right)  }{\partial\theta_{j}}=0\text{ }%
\end{equation}
for the outer optimization give%
\begin{equation}
\frac{\partial\mathcal{E}_{O}\left(  \varphi\left(  \boldsymbol{N}\left(
\boldsymbol{\theta}\left(  N\right)  \right)  \right)  ,\mathrm{v}\right)
}{\partial N_{j}}=\eta\left(  N\right)  =\varepsilon_{jj}\left(
\boldsymbol{\boldsymbol{N}}\left(  \boldsymbol{\theta}\left(  N\right)
\right)  \right)  \label{out1}%
\end{equation}
or%
\begin{equation}
...
\boldsymbol{\theta}\left(  N\right)  \right)  =0\text{ or }1
\end{equation}
...
\right)  \boldsymbol{,}\mathrm{v}\right)  }{\partial N_{j}}=\eta\left(
N\right)  \text{ or }N_{j}\left(  \boldsymbol{\theta}_{\#}\left(  N\right)
\right)  =0\text{ or }1;\,1\leq j\leq r_{1}.
\end{equation}
...
The minima of $\mathcal{E}_{O}\left(  \boldsymbol{\varphi,}\mathrm{v}\right)
,$ for both the inner and outer minimizations, are highly degenerate e.g.
there are many minimizers $\boldsymbol{\varphi}\left(  \boldsymbol{\theta
}_{\#}\left(  N\right)  \right)  $ that correspond to the constrained minimum
value of $\mathcal{E}_{O}\left(  \boldsymbol{\varphi}\left(
\boldsymbol{\theta}_{\#}\left(  N\right)  \right)  \boldsymbol{,}%
\mathrm{v}\right)  ,$ even if the ground state density $\gamma_{GS}$ is
unique$,$ as
\begin{equation}
\mathcal{E}_{O}\left(  \boldsymbol{\varphi,}\mathrm{v}\right)
=const.\,\forall\boldsymbol{\varphi\in}\mathfrak{S}\left(  \boldsymbol{N}%
\left(  N\right)  \boldsymbol{,\gamma}\right)  ,
\end{equation}
where $\mathfrak{S}\left(  \boldsymbol{N}\left(  N\right)  \boldsymbol{,\gamma
}\right)  $ is the equivalence class
\begin{equation}
\mathfrak{S}\left(  \boldsymbol{N}\left(  N\right)  \boldsymbol{,\gamma
}\right)  =\mathfrak{S}\left(  \boldsymbol{N}\left(  N\right)  \right)
\cap\mathfrak{S}\left(  \boldsymbol{N,\gamma}\right)
\end{equation}
...
constraining $\boldsymbol{\varphi}$ to be a specific representative of the
equivalence class $\mathfrak{S}\left(  \boldsymbol{N}\left(  N\right)
\boldsymbol{,\gamma}\right)  .$ In order to obtain such a representative one
...
results in
\begin{equation}
\gamma\left(  \boldsymbol{x}\right)  =\sum_{1\leq j\leq r_{1}}\left\vert
\left[  \varphi_{0j}\left(  \gamma\right)  \right]  \left(  \boldsymbol{x}%
\right)  \right\vert ^{2}. \label{gammaKS}%
\end{equation}
...
^{1}\left(  \gamma\right)  ,$ between $\gamma$ and $\left\{
\boldsymbol{\varphi}_{0}\right\}  ,$ thus modulo this unitary mixing one can
define an inverse functional $\Gamma_{0}^{-1}$ as
\begin{equation}
\boldsymbol{\varphi}_{0}=\Gamma_{0}^{-1}\left(  \gamma\right)  .
\end{equation}
...
\left\vert \boldsymbol{\varphi}_{0}\left(  \gamma\right)  \right\rangle
\right\}  $ but normalized to different occupation numbers $\left\{
\boldsymbol{N}_{0}\left(  \gamma\right)  \right\}  ,$ where $\boldsymbol{N}%
_{0}\left(  \gamma\right)  $ are convex combinations of $\left\{  \left.
\boldsymbol{N}_{0\kappa}\left(  \gamma\right)  \right\vert 1\leq\kappa\leq
m\right\}  $. Hence if degeneracies occur these are reflected in multiple
optimal values for $\boldsymbol{\boldsymbol{N}}_{0}$ for fixed $\left\{
\boldsymbol{\varphi}_{0}\left(  \gamma\right)  \right\}  \boldsymbol{.}$
...
\boldsymbol{N}\left(  N\right)  \boldsymbol{,}\gamma\right)  $ whose values
are given by
\begin{equation}
...
the HK energy functional $\mathcal{F}_{HK}\left(  \gamma\right)  $ in terms of
GSO's as
\begin{equation}
...
\begin{equation}
\mathcal{F}_{KS}\left(  \boldsymbol{\varphi}_{0}\right)  =\mathcal{F}%
_{HK}\left(  \Gamma\left(  \boldsymbol{\varphi}_{0}\right)  \right)  .
\end{equation}


The relationships described in subsection \ref{GSO} derived for stationary
points $\boldsymbol{\varphi}\left(  \boldsymbol{N}\left(  \boldsymbol{\theta
}\right)  \right)  ,$ $\boldsymbol{\varphi}\left(  \boldsymbol{N}\left(
\boldsymbol{\theta}_{\#}\left(  N\right)  \right)  \right)  $ are valid for
any points contained in the equivalence classes that these points belong to,
in particular $\boldsymbol{\varphi}_{0}\left(  \boldsymbol{N}\left(
\boldsymbol{\theta}\right)  \right)  $ and $\boldsymbol{\varphi}_{0}\left(
\boldsymbol{N}\left(  \boldsymbol{\theta}_{\#}\left(  N\right)  \right)
\right)  .$

The reference $N$-electron IPS, $D_{0}\left(  \boldsymbol{N}\left(
\boldsymbol{\theta}_{\#}\left(  N\right)  \right)  \right)  $ formed from the
GSO's $\boldsymbol{\varphi}_{0}$ has two energies associated with it. One,
$E_{0}\left(  \boldsymbol{\varphi}_{0}\left(  \boldsymbol{N}\left(
\boldsymbol{\theta}_{\#}\left(  N\right)  \right)  \right)  \right)  $, is the
...
\begin{equation}
E_{0}\left(  \boldsymbol{\varphi}_{0}\left(  \boldsymbol{N}\left(
\boldsymbol{\theta}_{\#}\left(  N\right)  \right)  \right)  \right)
=\sum_{1\leq j\leq r_{1}}\left\langle \tilde{\varphi}_{0j}\left\vert \left(
K+\mathrm{v}\right)  \right.  \tilde{\varphi}_{0j}\right\rangle N_{j}\left(
\boldsymbol{\theta}_{\#}\left(  N\right)  \right)  .
\end{equation}
The other is the energy of the Hamiltonian
\begin{equation}
H=H_{0}+V_{2}%
\end{equation}
wrt the IPS $D_{0}\left(  \boldsymbol{N}\left(  \boldsymbol{\theta}%
_{\#}\left(  N\right)  \right)  \right)  $ given by
\begin{equation}
E_{IP}\left(  D_{0}\left(  \boldsymbol{N}\left(  \boldsymbol{\theta}%
_{\#}\left(  N\right)  \right)  \right)  \right)  =\operatorname{Tr}\left\{
HD_{0}\left(  \boldsymbol{N}\left(  \boldsymbol{\theta}_{\#}\left(  N\right)
\right)  \right)  \right\}  .
\end{equation}
...
fashion to the ground state energy $E\left(  \gamma_{GS}\boldsymbol{,}%
\mathrm{v}\right)  .$
...
\boldsymbol{N}\left(  \boldsymbol{\theta}\left(  N\right)  \right)  \right)
,\mathrm{v}\right)  }{\partial N_{j}}\equiv\frac{\partial\mathcal{E}%
_{KS}\left(  \boldsymbol{\varphi}_{0}\left(  \boldsymbol{N}\left(
\boldsymbol{\theta}\left(  N\right)  \right)  \right)  ,\mathrm{v}\right)
}{\partial N_{j}}=\varepsilon_{jj}\left(  \boldsymbol{N}\left(
\boldsymbol{\theta}\left(  N\right)  \right)  \right)  , \label{ClassJanak}%
\end{equation}
...
_{jj}\left(  \boldsymbol{N}\left(  \boldsymbol{\theta}\left(  N\right)
\right)  \right)  \right\}  $ in this equation \emph{are not }associated with
the minimization of the universal kinetic energy.\emph{ }
...
\emph{ }
\begin{align}
\mathcal{E}_{1}\left(  \boldsymbol{\tilde{\varphi},n}\right)   &
=\frac{\mathcal{H}\left(  \boldsymbol{\tilde{\varphi},n}\right)  }%
{S_{M}\left(  \boldsymbol{n}\right)  }\nonumber\\
...
linear group parameters $\left\{  \boldsymbol{\eta},\boldsymbol{\lambda
}\right\}  $ as
\begin{equation}
\mathcal{E}_{2}\left(  \boldsymbol{\eta},\boldsymbol{\lambda}\right)
=\frac{\mathcal{H}\left(  \boldsymbol{\eta},\boldsymbol{\lambda}\right)
}{S_{M}\left(  \boldsymbol{\eta}\right)  }=\frac{\left\langle \left.  g\left(
\boldsymbol{\eta},\boldsymbol{\lambda}\right)  ^{M}\right\vert Hg\left(
\boldsymbol{\eta},\boldsymbol{\lambda}\right)  ^{M}\right\rangle }%
{S_{M}\left(  \boldsymbol{\eta}\right)  }, \label{gp_funct}%
\end{equation}
where%
\begin{equation}
S_{M}\left(  \boldsymbol{\eta}\right)  =\left\langle \left.  k\left(
\boldsymbol{\eta}\right)  g_{0}^{M}\right\vert k\left(  \boldsymbol{\eta
}\right)  g_{0}^{M}\right\rangle =\sum_{1\leq j_{1}<\cdots<j_{M}\leq
s}e^{2\left(  \eta_{j_{1}}+\cdots+\eta_{j_{M}}\right)  }n_{0j_{1}}\cdots
n_{0j_{N}}%
\end{equation}
and $\boldsymbol{\lambda\in}\mathbb{R}^{r_{1}\left(  r_{1}-1\right)  /2}.$
...
either constrain the GSO's $\boldsymbol{\varphi}$ to be orthonormal, in which
case we denote the energy functional as $\mathcal{E}_{1}\left(
\boldsymbol{\tilde{\varphi},n}\right)  ,$ or orthogonal and normalized to the
two electron geminal occupation numbers, in which case we denote the energy
functional as $\mathcal{E}_{1}\left(  \boldsymbol{\varphi}\right)  ,$ that
satisfy the constraints
\begin{align}
0  &  \leq n_{j}\leq1;1\leq j\leq s\nonumber\\
&  \sum_{1\leq j\leq s}n_{j}=1,
\end{align}
i.e. $\boldsymbol{n\in}\mathfrak{N}\left(  1\right)  \subset\mathbb{R}^{s}.$
This corresponds to treating the geminal occupation numbers either as
independent variables or control parameters in an analogous fashion to section
\ref{GSO}. To this end one can define Lagrangians
\begin{equation}
\mathcal{L}_{I}\left(  \boldsymbol{\tilde{\varphi},\theta},\boldsymbol{\tilde
{\varepsilon},}\eta\right)  =\mathcal{E}_{1I}\left(  \boldsymbol{\tilde
{\varphi},\boldsymbol{\theta}}\right)  -\sum_{1\leq j,k\leq2s}\tilde
{\varepsilon}_{kj}\left(  \left\langle \left.  \tilde{\varphi}_{j}\right\vert
\tilde{\varphi}_{k}\right\rangle -\delta_{jk}\right)  -\eta\left\{
\sum_{1\leq j\leq2s}n_{j}\boldsymbol{\left(  \boldsymbol{\theta}\right)
}-1\right\}  \label{LI1}%
\end{equation}
or%
\begin{equation}
\mathcal{L}_{C}\left(  \boldsymbol{\varphi,\theta},\boldsymbol{\varepsilon
,}\eta\right)  =\mathcal{E}_{1C}\left(  \boldsymbol{\varphi}\right)
-\sum_{1\leq j,k\leq2s}\varepsilon_{kj}\left(  \left\langle \left.
\varphi_{j}\right\vert \varphi_{k}\right\rangle -n_{j}\boldsymbol{\left(
\boldsymbol{\theta}\right)  }\delta_{jk}\right)  -\eta\left\{  \sum_{1\leq
j\leq2s}n_{j}\boldsymbol{\left(  \boldsymbol{\theta}\right)  }-1\right\}
\label{LI2}%
\end{equation}
respectively, where $\boldsymbol{n}$ is expressed in terms of the variable
$\boldsymbol{\theta}$ as
\begin{equation}
n_{j}\boldsymbol{\left(  \boldsymbol{\theta}\right)  }=\sin^{2}\theta_{j}.
\end{equation}
One carries out sequential optimizations over $\left(  \boldsymbol{\tilde
{\varphi},\tilde{\varepsilon}}\right)  $ and $\left(  \boldsymbol{\theta,}%
\eta\right)  ,$ giving
\begin{align}
\mathcal{L}\left(  \boldsymbol{\chi\left(  \boldsymbol{\theta}\right)
,\epsilon\left(  \boldsymbol{\theta}\right)  ,}\eta\right)   &  =\max
_{\boldsymbol{\epsilon}}\min_{\boldsymbol{\chi}}\mathcal{L}\left(
\boldsymbol{\chi,\theta},\boldsymbol{\epsilon,}\eta\right) \nonumber\\
\frac{\delta\mathcal{L}\left(  \boldsymbol{\chi\left(  \boldsymbol{\theta
}\right)  ,\boldsymbol{\theta},\epsilon\left(  \boldsymbol{\theta}\right)
,}\eta\right)  }{\delta\boldsymbol{\chi}}  &  =\boldsymbol{0};\quad
\frac{\partial\mathcal{L}\left(  \boldsymbol{\chi\left(  \boldsymbol{\theta
}\right)  ,\boldsymbol{\theta},\epsilon\left(  \boldsymbol{\theta}\right)
,}\eta\right)  }{\partial\boldsymbol{\epsilon}}=\boldsymbol{0} \label{AGPL1}%
\end{align}
and
\begin{align}
\mathcal{L}\left(  \boldsymbol{\chi}\left(  \boldsymbol{\theta}_{\#}\left(
\eta_{\#}\right)  \right)  \boldsymbol{,\epsilon}\left(  \boldsymbol{\theta
}_{\#}\left(  \eta_{\#}\right)  \right)  \right)   &  =\max_{\boldsymbol{\eta
}}\min_{\boldsymbol{\theta}}\mathcal{L}\left(  \boldsymbol{\chi\left(
\boldsymbol{\theta}\right)  ,\epsilon\left(  \boldsymbol{\theta}\right)
,}\eta\right) \nonumber\\
\frac{\partial\mathcal{L}\left(  \boldsymbol{\chi}\left(  \boldsymbol{\theta
}_{\#}\left(  \eta_{\#}\right)  \right)  ,\boldsymbol{\epsilon}\left(
\boldsymbol{\theta}_{\#}\left(  \eta_{\#}\right)  \right)  \boldsymbol{,}%
\eta_{\#}\right)  }{\partial\boldsymbol{\theta}}  &  =\boldsymbol{0}%
;\quad\frac{\partial\mathcal{L}\left(  \boldsymbol{\chi}\left(
\boldsymbol{\theta}_{\#}\left(  \eta_{\#}\right)  \right)
,\boldsymbol{\epsilon}\left(  \boldsymbol{\theta}_{\#}\left(  \eta
_{\#}\right)  \right)  \boldsymbol{,}\eta_{\#}\right)  }{\partial\eta
}=\boldsymbol{0} \label{AGPL2}%
\end{align}
for $\boldsymbol{\chi=\tilde{\varphi},\varphi}$ and $\boldsymbol{\epsilon
=\tilde{\varepsilon},\varepsilon.}$ The stationarity condition in eq. $\left(
\ref{AGPL1}\right)  $ gives
\begin{equation}
\frac{\delta\mathcal{E}_{1}\left(  \boldsymbol{\chi\left(  \boldsymbol{\theta
}\right)  }\right)  }{\delta\chi_{j}}=\sum_{1\leq jk\leq2s}\epsilon
\boldsymbol{\left(  \boldsymbol{\theta}\right)  }_{kj}\left\vert \varphi
_{k}\boldsymbol{\left(  \boldsymbol{\theta}\right)  }\right\rangle ,
\end{equation}
which can be expressed \cite{WeinerOrtiz05} in terms of a generalized Fock
operator as
\begin{equation}
F\left(  \boldsymbol{\chi\left(  \boldsymbol{\theta}\right)  }\right)
\left\vert \varphi_{j}\boldsymbol{\left(  \boldsymbol{\theta}\right)
}\right\rangle =\sum_{1\leq jk\leq2s}\epsilon\boldsymbol{\left(
\boldsymbol{\theta}\right)  }_{kj}\left\vert \varphi_{k}\boldsymbol{\left(
\boldsymbol{\theta}\right)  }\right\rangle ,
\end{equation}
...
\boldsymbol{\theta}_{\#}\left(  \eta_{\#}\right)  \right)  \boldsymbol{,}%
\eta_{\#}\right)  }{\partial\theta_{j}}=\frac{\partial\mathcal{L}\left(
\boldsymbol{\chi}\left(  \boldsymbol{\theta}_{\#}\left(  \eta_{\#}\right)
\right)  ,\boldsymbol{\epsilon}\left(  \boldsymbol{\theta}_{\#}\left(
\eta_{\#}\right)  \right)  \boldsymbol{,}\eta_{\#}\right)  }{\partial n_{j}%
}\sin\left\{  2\theta_{j}\right\}  .
\end{equation}
This leads to
\begin{equation}
\left\{  \frac{\partial\mathcal{E}_{1}\left(  \boldsymbol{\chi}\left(
\boldsymbol{n}\left(  \boldsymbol{\theta}_{\#}\left(  \eta_{\#}\right)
\right)  \right)  \right)  }{\partial n_{j}}-\eta_{\#}\right\}  \sin\left\{
2\theta_{j}\left(  \eta_{\#}\right)  \right\}  =0,
\end{equation}
which gives
\begin{equation}
\frac{\partial\mathcal{E}_{1}\left(  \boldsymbol{\chi}\left(  \boldsymbol{n}%
\left(  \boldsymbol{\theta}_{\#}\left(  \eta_{\#}\right)  \right)  \right)
\right)  }{\partial n_{j}}=\eta_{\#}%
\end{equation}
or
\begin{equation}
\sin\left\{  2\theta_{j}\left(  \eta_{\#}\right)  \right\}  =0\Rightarrow
\theta_{j}\left(  \eta_{\#}\right)  =0\text{ or }\frac{\pi}{2}\Rightarrow
n_{j}\left(  \boldsymbol{\theta}\left(  \eta_{\#}\right)  \right)  =0\text{ or
}1.
\end{equation}
On the other hand sensitivity theory gives%
\begin{equation}
\frac{\partial\mathcal{E}_{1}\left(  \boldsymbol{\varphi}\left(
\boldsymbol{n}\left(  \boldsymbol{\theta}\right)  \right)  \right)  }{\partial
n_{j}}=\varepsilon_{jj}\boldsymbol{\left(  \boldsymbol{\theta}\right)  }%
\end{equation}
for all values of $\boldsymbol{\theta.}$
...
\begin{align}
&  \frac{\partial\mathcal{E}\left(  \boldsymbol{n,\varphi}\right)  }{\partial
N_{j}}=\sum_{1\leq k\leq2s}\frac{\partial\mathcal{E}\left(
\boldsymbol{n,\varphi}\right)  }{\partial n_{k}}\frac{\partial n_{k}}{\partial
N_{j}}\nonumber\\
&  \Rightarrow\frac{\partial\mathcal{E}\left(  \boldsymbol{n,\varphi}\right)
}{\partial N_{j}}=\sum_{1\leq k\leq2s}\frac{\partial\mathcal{E}\left(
\boldsymbol{n,\varphi}\right)  }{\partial n_{k}}\frac{\partial n_{k}}{\partial
N_{j}}=\sum_{1\leq k\leq2s}\frac{\partial\mathcal{E}\left(
\boldsymbol{n,\varphi}\right)  }{\partial n_{k}}\left(  \frac{\partial N_{k}%
}{\partial n_{j}}\right)  ^{-1}%
\end{align}
i.e.%
\begin{equation}
\frac{\partial\mathcal{E}\left(  \boldsymbol{n,\varphi}\right)  }%
{\partial\boldsymbol{N}}=\frac{\partial\mathcal{E}\left(
\boldsymbol{n,\varphi}\right)  }{\partial\boldsymbol{n}}\left(  \frac
{\partial\boldsymbol{N}}{\partial\boldsymbol{n}}\right)  ^{-1}.
\end{equation}
...
;\quad1\leq j\leq s.
\end{equation}
The matrix elements of the Jacobian $\frac{\partial\boldsymbol{N}}%
{\partial\boldsymbol{n}}$ are
\begin{align}
...
\begin{equation}
\mathcal{M}\mathcal{=}\left\{  \left.  \overset{}{k\left(  \boldsymbol{\eta
}\right)  u\left(  \boldsymbol{\lambda}\right)  }\left\vert g_{0}%
^{N}\right\rangle \right\vert \right.  \left.  \boldsymbol{\lambda
}=\boldsymbol{\lambda}^{\dagger}\right\}  .
\end{equation}
Explicitly the action on a reference geminal is given by
\begin{equation}
\left\vert \tilde{g}\left(  \boldsymbol{\eta},\boldsymbol{\lambda}\right)
\right\rangle =k\left(  \boldsymbol{\eta}\right)  u\left(  \boldsymbol{\lambda
}\right)  \left\vert g_{0}\right\rangle =\sum_{1\leq l\leq s}e^{\eta_{l}%
}c_{0l}\left\vert \varphi_{l}\left(  \boldsymbol{\lambda}\right)
\varphi_{l+s}\left(  \boldsymbol{\lambda}\right)  \right\rangle ,
\end{equation}
where
\begin{equation}
\left\vert \varphi_{l}\left(  \boldsymbol{\lambda}\right)  \right\rangle
...
\begin{equation}
\left\vert g\left(  \boldsymbol{\eta},\boldsymbol{\lambda}\right)
\right\rangle =\mathit{s}\left(  \boldsymbol{\eta}\right)  ^{-\frac{1}{2}%
}\left\vert \tilde{g}\left(  \boldsymbol{\eta},\boldsymbol{\lambda}\right)
\right\rangle ,
\end{equation}
where
\begin{equation}
\mathit{s}\left(  \boldsymbol{\eta}\right)  =\left\langle \left.  \tilde
{g}\left(  \boldsymbol{\eta},\boldsymbol{\lambda}\right)  \right\vert
\tilde{g}\left(  \boldsymbol{\eta},\boldsymbol{\lambda}\right)  \right\rangle
=\sum_{1\leq l\leq s}e^{2\eta_{l}}c_{0l}^{2}=\sum_{1\leq l\leq s}e^{2\eta_{l}%
}n_{0l}%
\end{equation}
giving
\begin{align}
\left\vert g\left(  \boldsymbol{\eta},\boldsymbol{\lambda}\right)
\right\rangle  &  =\mathit{s}\left(  \boldsymbol{\eta}\right)  ^{-\frac{1}{2}%
}\sum_{1\leq l\leq s}e^{\eta_{l}}c_{0l}\left\vert \varphi_{l}\left(
\boldsymbol{\lambda}\right)  \varphi_{l+s}\left(  \boldsymbol{\lambda}\right)
\right\rangle \nonumber\\
&  =\sum_{1\leq l\leq s}e^{\eta_{l}}c_{0l}\mathit{s}\left(  \boldsymbol{\eta
}\right)  ^{-\frac{1}{2}}\left\vert \varphi_{l}\left(  \boldsymbol{\lambda
}\right)  \varphi_{l+s}\left(  \boldsymbol{\lambda}\right)  \right\rangle
=\sum_{1\leq l\leq s}c_{l}\left(  \boldsymbol{\eta}\right)  \left\vert
\varphi_{l}\left(  \boldsymbol{\lambda}\right)  \varphi_{l+s}\left(
\boldsymbol{\lambda}\right)  \right\rangle ,
\end{align}
where
\begin{equation}
c_{l}\left(  \boldsymbol{\eta}\right)  =e^{\eta_{l}}c_{0l}\mathit{s}\left(
\boldsymbol{\eta}\right)  ^{-\frac{1}{2}}%
\end{equation}
leading to
\begin{equation}
n_{l}\left(  \boldsymbol{\eta}\right)  =e^{2\eta_{l}}n_{0l}\mathit{s}\left(
\boldsymbol{\eta}\right)  ^{-1}.
\end{equation}
This gives
\begin{equation}
\sum_{1\leq l\leq s}n_{l}\left(  \boldsymbol{\eta}\right)  =\mathit{s}\left(
\boldsymbol{\eta}\right)  ^{-1}\sum_{1\leq l\leq s}e^{2\eta_{l}}n_{0l}=\left(
\sum_{1\leq l\leq s}e^{2\eta_{l}}n_{0l}\right)  ^{-1}\sum_{1\leq l\leq
s}e^{2\eta_{l}}n_{0l}=1
\end{equation}
which shows that $\left\{  n_{l}\left(  \boldsymbol{\eta}\right)  \right\}  $
satisfy the geminal occupation number positivity and normalization
constraints. We can define%
\begin{equation}
\left\vert \varphi_{l}\left(  \boldsymbol{\eta,\lambda}\right)  \right\rangle
=\exp\eta_{l}\exp\left\{  i\sum_{1\leq j,k\leq2s}\lambda_{jk}a_{j}^{\dagger
}a_{k}\right\}  \left\vert \varphi_{0l}\right\rangle ,
\end{equation}
so that
\begin{equation}
\left\langle \left.  \varphi_{l}\left(  \boldsymbol{\eta,\lambda}\right)
\right\vert \varphi_{l}\left(  \boldsymbol{\eta,\lambda}\right)  \right\rangle
...
\left\vert g\left(  \boldsymbol{\eta},\boldsymbol{\lambda}\right)
\right\rangle =\mathit{s}\left(  \boldsymbol{\eta}\right)  ^{-\frac{1}{2}}%
\sum_{1\leq l\leq s}c_{0l}\left\vert \varphi_{l}\left(  \boldsymbol{\eta
,\lambda}\right)  \varphi_{l+s}\left(  \boldsymbol{\eta,\lambda}\right)
\right\rangle .
\end{equation}
...
$\boldsymbol{\eta}$
\begin{align}
n_{l}\left(  \boldsymbol{\eta}\right)   &  =e^{2\eta_{l}}n_{0l}\mathit{s}%
\left(  \boldsymbol{\eta}\right)  ^{-1}\nonumber\\
s_{l}  &  =\left\langle \left.  \varphi_{l}\left(  \boldsymbol{\eta,\lambda
}\right)  \right\vert \varphi_{l}\left(  \boldsymbol{\eta,\lambda}\right)
\right\rangle =e^{2\eta_{l}}%
\end{align}
giving
\begin{equation}
\eta_{l}=\frac{1}{2}\ln\left\langle \left.  \varphi_{l}\left(
\boldsymbol{\eta,\lambda}\right)  \right\vert \varphi_{l}\left(
\boldsymbol{\eta,\lambda}\right)  \right\rangle \equiv\frac{1}{2}%
\ln\left\langle \left.  \varphi_{l}\left(  \boldsymbol{\lambda}\right)
\right\vert \varphi_{l}\left(  \boldsymbol{\lambda}\right)  \right\rangle
=\frac{1}{2}\ln s_{l}%
\end{equation}
Hence%
\begin{equation}
\mathcal{E}\left(  \boldsymbol{n}_{\min}\boldsymbol{,\varphi}_{\min}\right)
=\mathcal{E}\left(  \boldsymbol{\eta}_{\min}\boldsymbol{,\lambda}_{\min
}\right)  =\mathcal{E}\left(  \boldsymbol{\varphi}_{\min}\right)
\end{equation}
where
\begin{align}
\mathcal{E}\left(  \boldsymbol{n}_{\min}\boldsymbol{,\varphi}_{\min}\right)
&  =\min_{\boldsymbol{n\in\mathcal{S},\varphi\in}\mathcal{O}\left(
\boldsymbol{1}_{s}\right)  }\mathcal{E}\left(  \boldsymbol{n,\varphi}\right)
\\
\mathcal{E}\left(  \boldsymbol{\eta}_{\min}\boldsymbol{,\lambda}_{\min
}\right)   &  =\min_{\boldsymbol{\eta,\lambda}}\mathcal{E}\left(
\boldsymbol{\eta,\lambda}\right) \\
\mathcal{E}\left(  \boldsymbol{\varphi}_{\min}\right)   &  =\min
_{\boldsymbol{s\in}\mathbb{R}_{+}^{s}}\min_{\boldsymbol{\varphi\in}%
\mathcal{O}\left(  \boldsymbol{s}\right)  }\mathcal{E}\left(
\boldsymbol{\varphi}\right)
\end{align}%
\begin{align}
\mathcal{S}  &  =\left\{  \left.  \boldsymbol{n}\right\vert n_{j}\geq0,1\leq
j\leq s;\sum_{1\leq j\leq s}n_{j}=1\right\} \\
\mathcal{O}\left(  \boldsymbol{s}\right)   &  =\left\{  \left.
\boldsymbol{\varphi}\right\vert \left\langle \left.  \varphi_{j}\right\vert
\varphi_{k}\right\rangle =s_{j}\delta_{jk},1\leq j,k\leq s\right\}  .
\end{align}%
\begin{equation}
\boldsymbol{\lambda}=\boldsymbol{\lambda}^{\dagger},\boldsymbol{\eta\in
}\mathbb{R}^{s}%
\end{equation}%
\begin{align}
\mathcal{L}\left(  \boldsymbol{\varphi,s}\right)   &  =\mathcal{E}\left(
\boldsymbol{n}_{0}\boldsymbol{,\varphi}\right)  -\sum_{1\leq j,k\leq
2s}\varepsilon_{kj}\left(  \left\langle \left.  \varphi_{j}\right\vert
\varphi_{k}\right\rangle -s_{j}\delta_{jk}\right) \\
\frac{\partial\mathcal{L}\left(  \boldsymbol{\varphi,s}\right)  }{\partial
s_{j}}  &  =\varepsilon_{jj}.
\end{align}%
\begin{equation}
\mathcal{L}\left(  \boldsymbol{n\left(  \boldsymbol{\eta}\right)  ,\varphi
}\right)  =\mathcal{L}\left(  \boldsymbol{\varphi,s}\left(  \boldsymbol{\eta
}\right)  \right)
\end{equation}
...
\section{Density Operators \label{Denmat}}
...
_{1}\left(  \mathcal{H}^{N}\right)  $ are Banach spaces with norms defiend by
\begin{equation}
...
$\Xi_{2}$ is given by
\begin{equation}
\mathcal{T}_{A}=\left\{  \left.  B\right\vert \left(  A\left\vert \Phi\left(
\boldsymbol{x}\right)  ^{\dagger}\Phi\left(  \boldsymbol{x}\right)  \right.
B\right)  =0\forall\boldsymbol{x}\right\}  \subseteq\ker D\Xi_{2}\left(
A\right)
\end{equation}
...
\boldsymbol{x}\right)  ^{\dagger}\Phi\left(  \boldsymbol{x}\right)  \right.
B\right)  \neq0\forall\boldsymbol{x\in}\mathbb{X}\right\}
\end{equation}
One can show that $\left\{  \left.  \left[  D\Xi_{2}\left[  \boldsymbol{x}%
\right]  \left(  A\right)  \right]  \left(  \cdot\right)  \equiv\left\vert
\Phi\left(  \boldsymbol{x}\right)  ^{\dagger}\Phi\left(  \boldsymbol{x}%
\right)  A\right)  \equiv\operatorname{Tr}\left\{  A\Phi\left(  \boldsymbol{x}%
\right)  ^{\dagger}\Phi\left(  \boldsymbol{x}\right)  \cdot\right\}
\right\vert \boldsymbol{x\in}\mathbb{X}\right\}  $ is a generalized basis,
(see Appendix basis \ref{Bases}), for $\mathcal{N}_{A}$ which shows that
\begin{equation}
...
\mathcal{H}^{N}\right)  \rightarrow L_{1}\left(  \mathbb{X}\right)
\end{equation}
...
where
\begin{equation}
\mathcal{N}=\int\Phi\left(  \boldsymbol{x}\right)  ^{\dagger}\Phi\left(
\boldsymbol{x}\right)  d\boldsymbol{x,}%
\end{equation}
as
\begin{equation}
...
$\boldsymbol{\tilde{\varphi}}$ and occupation numbers $\boldsymbol{N}$ by the
orbital functional $\mathcal{\tilde{F}}_{O}$
\begin{equation}
\mathcal{\tilde{F}}_{O}\left(  \boldsymbol{\tilde{\varphi},N}\right)
=\mathcal{F}_{HK}\left(  \Gamma_{2}\left(  \boldsymbol{\tilde{\varphi}%
,N}\right)  \right)  =\mathcal{F}_{HK}\left(  \gamma\right)  ,
\end{equation}
...
\begin{equation}
\mathcal{\tilde{F}}_{O}\left(  \boldsymbol{\tilde{\varphi},N}\right)
=\mathcal{\tilde{F}}_{T}\left(  \boldsymbol{\tilde{\varphi},N}\right)
+\mathcal{\tilde{F}}_{C}\left(  \boldsymbol{\tilde{\varphi},N}\right)
+\mathcal{\tilde{F}}_{OXC}\left(  \boldsymbol{\tilde{\varphi},N}\right)  ,
\end{equation}
where
\begin{equation}
\mathcal{\tilde{F}}_{T}\left(  \boldsymbol{\tilde{\varphi},N}\right)
=\sum_{1\leq j\leq r_{1}}\left\langle \tilde{\varphi}_{j}\left\vert
K\tilde{\varphi}_{j}\right.  \right\rangle N_{j},
\end{equation}
the orbital exchange-corellation functional is defined as
\begin{equation}
\mathcal{\tilde{F}}_{OXC}\left(  \boldsymbol{\tilde{\varphi},N}\right)
=\mathcal{F}_{XC}\left(  \Gamma_{2}\left(  \boldsymbol{\tilde{\varphi}%
,N}\right)  \right)  +\mathcal{F}_{K}\left(  \Gamma_{2}\left(
\boldsymbol{\tilde{\varphi},N}\right)  \right)  -\mathcal{\tilde{F}}%
_{T}\left(  \boldsymbol{\tilde{\varphi},N}\right)  ,
\end{equation}
and one recalls the definitions
\begin{equation}
\mathcal{F}_{XC}\left(  \Gamma_{1}\left(  \boldsymbol{\varphi}\right)
\right)  =\operatorname{Tr}\left\{  V_{2}D_{\ast}\left(  \Gamma_{1}\left(
\boldsymbol{\varphi}\right)  \right)  \right\}  -\mathcal{F}_{C}\left(
\Gamma_{1}\left(  \boldsymbol{\varphi}\right)  \right)
\end{equation}
and
\begin{equation}
\mathcal{F}_{K}\left(  \Gamma_{1}\left(  \boldsymbol{\varphi}\right)  \right)
=\operatorname{Tr}\left\{  KD_{\ast}\left(  \Gamma_{1}\left(
\boldsymbol{\varphi}\right)  \right)  \right\}  .
\end{equation}
...
is
\begin{equation}
E\left(  \Gamma_{2}\left(  \boldsymbol{\tilde{\varphi}}_{GS}\boldsymbol{,N}%
\right)  \boldsymbol{,}\mathrm{v}\right)  =\min_{\left(  \boldsymbol{\tilde
{\varphi},N}\right)  \boldsymbol{\in}\mathfrak{S}_{2}\left(  N\right)
\times\mathfrak{N}\left(  N\right)  }\mathcal{E}_{O}\left(  \boldsymbol{\tilde
{\varphi},N,}\mathrm{v}\right)  , \label{KSminA}%
\end{equation}
where
\begin{equation}
\mathcal{E}_{O}\left(  \boldsymbol{\tilde{\varphi},N,}\mathrm{v}\right)
=\mathcal{\tilde{F}}_{O}\left(  \boldsymbol{\tilde{\varphi},N}\right)
+f_{\Gamma_{2}\left(  \boldsymbol{\tilde{\varphi},N}\right)  }\left(
\mathrm{v}\right)  .
\end{equation}
...
saddle point $\mathcal{L}_{\boldsymbol{\tilde{\varphi}}}\left(
\boldsymbol{\tilde{\varphi},N},\boldsymbol{\varepsilon},\mathrm{v}\right)  $
of the Lagrangian
\begin{equation}
\mathcal{L}_{\boldsymbol{\tilde{\varphi}}}\left(  \boldsymbol{\tilde{\varphi}%
},\boldsymbol{\tilde{\varepsilon}}\left(  \boldsymbol{N}\right)
\boldsymbol{,N,}\mathrm{v}\right)  =\mathcal{E}_{O}\left(  \boldsymbol{\tilde
{\varphi},N,}\mathrm{v}\right)  -\sum_{1\leq j,k\leq r_{1}}\tilde{\varepsilon
...
\begin{equation}
\mathcal{L}_{\boldsymbol{\tilde{\varphi}}}\left(  \boldsymbol{\tilde{\varphi}%
}\left(  \boldsymbol{N}\right)  ,\boldsymbol{\tilde{\varepsilon}}\left(
\boldsymbol{N}\right)  ,\boldsymbol{N,}\mathrm{v}\right)  =\max
_{\boldsymbol{\tilde{\varepsilon}}}\min_{\boldsymbol{\tilde{\varphi}}%
}\mathcal{L}_{\boldsymbol{\tilde{\varphi}}}\left(  \boldsymbol{\tilde{\varphi
}},\boldsymbol{\tilde{\varepsilon}}\left(  \boldsymbol{N}\right)
\boldsymbol{,N,}\mathrm{v}\right)  , \label{KSM1A}%
\end{equation}
which leads to optimal values $\mathcal{E}_{O}\left(
\boldsymbol{\boldsymbol{\tilde{\varphi}}\left(  \boldsymbol{N}\right)
,N,}\mathrm{v}\right)  =$ $\mathcal{L}_{\boldsymbol{\tilde{\varphi}}}\left(
\boldsymbol{\tilde{\varphi}}\left(  \boldsymbol{N}\right)  ,\boldsymbol{\tilde
{\varepsilon}}\left(  \boldsymbol{N}\right)  ,\boldsymbol{N,}\mathrm{v}%
\right)  $ of the GSO energy functional for fixed values of the control
parameters $\boldsymbol{N}$. In the second optimization one finds minima of
$\mathcal{E}_{O}\left(  \boldsymbol{\boldsymbol{\tilde{\varphi}}\left(
\boldsymbol{N}\right)  ,N,}\mathrm{v}\right)  $ with respect the variables
$\boldsymbol{N}$ that satisfy the constraints
\begin{equation}
0\leq N_{j}\leq1,1\leq j\leq r_{1}%
\end{equation}
...
This leads to a Lagrangian
\begin{equation}
\mathcal{L}_{\boldsymbol{\theta}}\left(  \boldsymbol{\tilde{\varphi}}\left(
\boldsymbol{\theta}\right)  ,\boldsymbol{\theta},N,\eta,\mathrm{v}\right)
=\mathcal{E}_{O}\left(  \boldsymbol{\tilde{\varphi}}\left(  \boldsymbol{\theta
}\right)  \boldsymbol{,\theta},\mathrm{v}\right)  -\eta\left\{  \sum_{\,1\leq
j\leq r_{1}}N_{j}\left(  \boldsymbol{\theta}\right)  -N\right\}  .
\end{equation}
The determination of the saddle point $\mathcal{L}_{\boldsymbol{\theta}%
}\left(  \boldsymbol{\tilde{\varphi}}\left(  \boldsymbol{\theta}_{\#}\left(
N\right)  \right)  ,\boldsymbol{\theta}_{\#}\left(  N\right)  ,\eta\left(
N\right)  ,\mathrm{v}\right)  $ given by
\begin{equation}
\mathcal{L}_{\boldsymbol{\theta}}\left(  \boldsymbol{\tilde{\varphi}}\left(
\boldsymbol{\theta}_{\#}\left(  N\right)  \right)  ,\boldsymbol{\theta}%
_{\#}\left(  N\right)  ,\eta\left(  N\right)  ,\mathrm{v}\right)  =\max_{\eta
}\min_{\boldsymbol{\theta}}\mathcal{L}_{\boldsymbol{\theta}}\left(
\boldsymbol{\tilde{\varphi}}\left(  \boldsymbol{\theta}\right)
,\boldsymbol{\theta},N,\eta,\mathrm{v}\right)  .
\end{equation}
determines the ground state energy $E\left(  \gamma_{GS}\boldsymbol{,}%
\mathrm{v}\right)  =\mathcal{E}_{O}\left(  \boldsymbol{\tilde{\varphi}}\left(
\boldsymbol{\theta}_{\#}\left(  N\right)  \right)  ,\boldsymbol{\theta}%
_{\#}\left(  N\right)  ,\eta,N,\mathrm{v}\right)  .$The ground state density
can then expanded as
\begin{equation}
\gamma_{GS}\left(  \boldsymbol{x}\right)  =\sum_{1\leq j\leq r_{1}}%
N_{j}\left\vert \left[  \tilde{\varphi}_{j}\left(  \boldsymbol{\theta}%
_{\#}\left(  N\right)  \right)  \right]  \left(  \boldsymbol{x}\right)
\right\vert ^{2}.
\end{equation}
The saddle points of $\mathcal{L}_{\boldsymbol{\tilde{\varphi}}}\left(
\boldsymbol{\tilde{\varphi}},\boldsymbol{\tilde{\varepsilon},N,}%
\mathrm{v}\right)  $ are characterized by the necessary conditions
\begin{subequations}
\begin{align}
\frac{\delta\mathcal{L}_{\boldsymbol{\tilde{\varphi}}}\left(
\boldsymbol{\tilde{\varphi}},\boldsymbol{\tilde{\varepsilon},N,}%
\mathrm{v}\right)  }{\delta\varphi_{j}^{\ast}}  &  =0;\,1\leq j\leq
r_{1},\label{KSC1A}\\
\frac{\partial\mathcal{L}_{\boldsymbol{\tilde{\varphi}}}\left(
\boldsymbol{\tilde{\varphi}},\boldsymbol{\tilde{\varepsilon},N,}%
\mathrm{v}\right)  }{\partial\tilde{\varepsilon}_{jk}}  &  =0;\,1\leq j,k\leq
r_{1} \label{KSC2A}%
\end{align}
(Appendix \ref{ComplexConjugate} describes why it is sufficient to consider
derivatives wrt $\varphi_{j}^{\ast}).$ The condition eq. $\left(
\ref{KSC1A}\right)  $ implies that at an optimal point the matrix of Lagrange
multipliers $\boldsymbol{\tilde{\varepsilon}}\left(  \boldsymbol{N}\right)  $
is hermitian i.e.
\end{subequations}
\begin{equation}
\tilde{\varepsilon}\left(  \boldsymbol{N}\right)  _{jk}=\tilde{\varepsilon
}\left(  \boldsymbol{N}\right)  _{kj}^{\ast},\quad1\leq j,k\leq r_{1}%
\end{equation}
and
\begin{equation}
\frac{\delta\mathcal{E}_{O}\left(  \boldsymbol{\tilde{\varphi}}\left(
\boldsymbol{N}\right)  ,\boldsymbol{N,}\mathrm{v}\right)  }{\delta
\tilde{\varphi}_{j}^{\ast}}-\sum_{1\leq k\leq r_{1}}\tilde{\varepsilon}%
_{jk}\left(  \boldsymbol{N}\right)  \left\vert \tilde{\varphi}_{k}\left(
\boldsymbol{N}\right)  \right\rangle =0;\,1\leq j\leq r_{1}.
\end{equation}
...
\frac{\delta\mathcal{E}_{O}\left(  \boldsymbol{\tilde{\varphi}}\left(
\boldsymbol{N}\right)  ,\boldsymbol{N,}\mathrm{v}\right)  }{\delta
\tilde{\varphi}_{j}^{\ast}}  &  =\int\frac{\delta\mathcal{E}_{O}\left(
\boldsymbol{\tilde{\varphi}}\left(  \boldsymbol{N}\right)  ,\boldsymbol{N,}%
\mathrm{v}\right)  }{\delta\gamma\left(  \boldsymbol{x}^{\prime}\right)
}\frac{\delta\gamma\left(  \boldsymbol{x}^{\prime}\right)  }{\delta
\tilde{\varphi}_{j}^{\ast}}d\boldsymbol{x}^{\prime}\nonumber\\
&  =\int\frac{\delta\mathcal{E}_{O}\left(  \boldsymbol{\tilde{\varphi}}\left(
\boldsymbol{N}\right)  ,\boldsymbol{N,}\mathrm{v}\right)  }{\delta
\gamma\left(  \boldsymbol{x}\right)  }\tilde{\varphi}_{j}\left(
\boldsymbol{x}^{\prime}\right)  N_{j}\delta\left(  \boldsymbol{x-x}^{\prime
}\right)  d\boldsymbol{x}^{\prime}=\frac{\delta\mathcal{E}_{O}\left(
\boldsymbol{\tilde{\varphi}}\left(  \boldsymbol{N}\right)  ,\boldsymbol{N,}%
\mathrm{v}\right)  }{\delta\gamma\left(  \boldsymbol{x}\right)  }N_{j}%
\tilde{\varphi}_{j}\left(  \boldsymbol{x}\right)
\end{align}
gives%
\begin{equation}
\frac{\delta\mathcal{E}_{O}\left(  \boldsymbol{\tilde{\varphi}}\left(
\boldsymbol{N}\right)  ,\boldsymbol{N,}\mathrm{v}\right)  }{\delta\gamma}%
N_{j}\left\vert \tilde{\varphi}_{j}\left(  \boldsymbol{N}\right)
\right\rangle -\sum_{1\leq k\leq r_{1}}\tilde{\varepsilon}_{jk}\left(
\boldsymbol{N}\right)  \left\vert \tilde{\varphi}_{k}\left(  \boldsymbol{N}%
\right)  \right\rangle =0;\,1\leq j\leq r_{1}.
\end{equation}
The operator $\frac{\delta\mathcal{E}_{O}\left(  \boldsymbol{\tilde{\varphi}%
}\left(  \boldsymbol{N}\right)  ,\boldsymbol{N,}\mathrm{v}\right)  }%
{\delta\gamma}$ is a multiplication operator given by
\begin{equation}
\frac{\delta\mathcal{E}_{O}\left(  \boldsymbol{\tilde{\varphi}}\left(
\boldsymbol{N}\right)  ,\boldsymbol{N,}\mathrm{v}\right)  }{\delta\gamma
}\left\vert \tilde{\varphi}_{j}\left(  \boldsymbol{N}\right)  \right\rangle
=\left\vert \frac{\delta\mathcal{E}_{O}\left(  \boldsymbol{\tilde{\varphi}%
}\left(  \boldsymbol{N}\right)  ,\boldsymbol{N,}\mathrm{v}\right)  }%
{\delta\gamma}\tilde{\varphi}_{j}\left(  \boldsymbol{N}\right)  \right\rangle
,
\end{equation}
where
\begin{equation}
\left[  \frac{\delta\mathcal{E}_{O}\left(  \boldsymbol{\tilde{\varphi}}\left(
\boldsymbol{N}\right)  ,\boldsymbol{N,}\mathrm{v}\right)  }{\delta\gamma
}\tilde{\varphi}_{j}\left(  \boldsymbol{N}\right)  \right]  \left(
\boldsymbol{x}\right)  =\frac{\delta\mathcal{E}_{O}\left(  \boldsymbol{\tilde
{\varphi}}\left(  \boldsymbol{N}\right)  ,\boldsymbol{N,}\mathrm{v}\right)
}{\delta\gamma\left(  \boldsymbol{x}\right)  }\left[  \tilde{\varphi}%
_{j}\left(  \boldsymbol{N}\right)  \right]  \left(  \boldsymbol{x}\right)  .
\end{equation}
We thus obtain the intermediate GSO equations that correspond to condition eq.
...
=\sum_{1\leq k\leq r_{1}}\tilde{\varepsilon}_{jk}\left(  \boldsymbol{N}%
\right)  \left\vert \tilde{\varphi}_{k}\left(  \boldsymbol{N}\right)
\right\rangle .
\end{equation}
...
\begin{equation}
\frac{\partial\mathcal{E}_{O}\left(  \boldsymbol{\tilde{\varphi}}\left(
\boldsymbol{N}\right)  ,\boldsymbol{N,}\mathrm{v}\right)  }{\partial N_{j}%
}=\tilde{\varepsilon}_{jj}\left(  \boldsymbol{N}\right)  ,
\end{equation}
...
\end{equation}%
\[
\frac{\delta\gamma\left(  \boldsymbol{x}^{\prime}\right)  }{\delta f\left(
\boldsymbol{x}\right)  }=\frac{1}{\left\langle f|f\right\rangle }\left\{
2Nf\left(  \boldsymbol{x}^{\prime}\right)  \delta\left(  \boldsymbol{x-x}%
^{\prime}\right)  -2\gamma\left(  \boldsymbol{x}^{\prime}\right)  f\left(
\boldsymbol{x}\right)  \right\}
\]%
\begin{align*}
&  2\int\frac{\delta E\left(  f\boldsymbol{,}\mathrm{v}\right)  }{\delta
\gamma\left(  \boldsymbol{x}^{\prime}\right)  }\left\{  \frac{1}{\left\langle
f|f\right\rangle }Nf\left(  \boldsymbol{x}^{\prime}\right)  \delta\left(
\boldsymbol{x-x}^{\prime}\right)  -\gamma\left(  \boldsymbol{x}^{\prime
}\right)  f\left(  \boldsymbol{x}\right)  \right\}  d\boldsymbol{x}^{\prime
}=\\
&  2\int\frac{\delta E\left(  f\boldsymbol{,}\mathrm{v}\right)  }{\delta
\gamma\left(  \boldsymbol{x}^{\prime}\right)  }\frac{1}{\left\langle
f|f\right\rangle }Nf\left(  \boldsymbol{x}^{\prime}\right)  \delta\left(
\boldsymbol{x-x}^{\prime}\right)  d\boldsymbol{x}^{\prime}-2\int\frac{\delta
E\left(  f\boldsymbol{,}\mathrm{v}\right)  }{\delta\gamma\left(
\boldsymbol{x}^{\prime}\right)  }\gamma\left(  \boldsymbol{x}^{\prime}\right)
f\left(  \boldsymbol{x}\right)  d\boldsymbol{x}^{\prime}=\\
\frac{\delta E\left(  f\boldsymbol{,}\mathrm{v}\right)  }{\delta f\left(
\boldsymbol{x}\right)  }=  &  2\frac{Nf\left(  \boldsymbol{x}\right)
}{\left\langle f|f\right\rangle }\frac{\delta E\left(  f\boldsymbol{,}%
\mathrm{v}\right)  }{\delta\gamma\left(  \boldsymbol{x}\right)  }-2f\left(
\boldsymbol{x}\right)  \int\frac{\delta E\left(  f\boldsymbol{,}%
\mathrm{v}\right)  }{\delta\gamma\left(  \boldsymbol{x}^{\prime}\right)
}\gamma\left(  \boldsymbol{x}^{\prime}\right)  d\boldsymbol{x}^{\prime
}=2f\left(  \boldsymbol{x}\right)  \left\{  \frac{N}{\left\langle
f|f\right\rangle }\frac{\delta E\left(  f\boldsymbol{,}\mathrm{v}\right)
}{\delta\gamma\left(  \boldsymbol{x}\right)  }-\int\frac{\delta E\left(
f\boldsymbol{,}\mathrm{v}\right)  }{\delta\gamma\left(  \boldsymbol{x}%
^{\prime}\right)  }\gamma\left(  \boldsymbol{x}^{\prime}\right)
d\boldsymbol{x}^{\prime}\right\}
\end{align*}

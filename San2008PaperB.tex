...
%TCIDATA{LastRevised=Wednesday, March 26, 2008 16:35:34}
...
their coccupation numbers uniquely determine N-electron Antisymmetrized
Geminaql Power states. This provides a corellated one electron model that
generalizes the independent particle model that is universally used in
pictuting chemical behaviour. These examples also demonstrate that one
electron models, involving GSO's and occupation numbers, and ones based on a
first order density matrix are not equivalent.  This difference is shown by
displaying sets of different GSO's that have the same occupation numbers that
correspond to the same density matrix but different N-electron states. This
observation also leads to the explicit construction of a density matrix functional.
...
\item electrons can be identified with orbitals in a one to one fashion and
hence observable quantities associated with these orbitals (e.g. energy,
...
orbitals and occupation numbers, they are the Antisymmetyrized Geminal Power
...
N-electron energy functional and the expression of their energy in terms of
one electron quantities (section 3). In section 4 we define the AGP\ DMF and
in section 5 we describe simple examples of how (i) AGP states are determined
by orbitals and occupation numbers and (ii) AGP DMF's.
...
\begin{itemize}
\item N-electron state is \emph{totally determined} \ by a set of orthonormal
Generalized Spin Orbitals (GSO's) $\left\{  \left\vert \psi_{j}\right\rangle
;1\leq j\leq r\right\}  $ spanning one particle space and a set of occupation
numbers $\left\{  N_{j};1\leq j\leq r\right\}  $ of these GSO's
...
observed by considering the natural expansion
\begin{equation}
D^{1}=\sum_{1\leq j\leq r}N_{j}\left\vert \psi_{j}\right\rangle \left\langle
\psi_{j}\right\vert ,
\end{equation}
which is invariant to the action of the unitary group $U(1)$ on the GSO's i.e.%
\begin{align}
...
through a Levy-Lieb\cite{Levy1979}, \cite{Lieb1983} \ type of construction.
Simple examples of such functionals will be displayed in the proceeding.
...
antisymmetrized power of a single geminal composed of General Spin Orbitals
(GSO's). Geminals can be expanded uniquely in parametric form as
\begin{equation}
\left\vert g\left(  \mathbf{c},\bm{\psi,\theta}\right)  \right\rangle
=\sum_{1\leq j\leq s}c_{j}e^{i\theta_{j}}\left\vert \psi_{j}\psi
_{j+s}\right\rangle , \label{gem}%
\end{equation}
...
$\bm{c}$ lie on the positive portion of the hypersphere
\begin{equation}
\sum\limits_{1\leq j\leq s}c_{j}^{2}=1;\,c_{j}\geq0;1\leq j\leq s,
\end{equation}
the angles $\bm{\theta}=\left(  \theta_{2},\cdots,\theta_{s}\right)  $ satisfy%

\begin{equation}
0\leq\theta_{2},\cdots,\theta_{s}<2\pi;\theta_{1}=0 \label{Phases}%
\end{equation}
and the GSO's $\left\{  \psi_{j},\psi_{j+s}\right\}  $ belong to a set of non
equivalent pairs, $\mathcal{P},$ (see Appendix \ref{GSOpairs}).

\subsection{Density Operators\label{DenOp}}

The FORDO\ of an AGP\ is always at least doubly degenerate i.e.
\begin{equation}
D^{1}\left(  \bm{c},\bm{\psi}\right)  =\sum_{1\leq j\leq s}N_{j}\left(
\bm{c}\right)  \left\{  \left\vert \psi_{j}\right\rangle \left\langle \psi
_{j}\right\vert +\left\vert \psi_{j+s}\right\rangle \left\langle \psi
_{j+s}\right\vert \right\}  , \label{D1}%
\end{equation}
and it is uniquely parameterized by $\left\{  \bm{c},\bm{\psi
}\right\}  ,$ where $\bm{\psi\in}\mathcal{P}$ are representatives of the
equivalence classes, $\left[  \bm{\psi}\right]  $, of paired normalized GSO's,
which for non degenerate AGP's are defined by
\begin{equation}
...
\bm{c}\right)  ;1\leq j\leq s\right\}  $ are given in terms of $\left\{
n_{j};1\leq j\leq s\right\}  $ in a one-one fashion \cite{Yukalov00} by
\begin{equation}
N_{j}\left(  \bm{c}\right)  =n_{j}S_{\frac{N}{2}-1}\left(  \hat{\jmath
}\right)  \left(  S_{\frac{N}{2}}\right)  ^{-1}, \label{OccNum}%
\end{equation}
where $n_{j}=c_{j}^{2}$
\begin{equation}
S_{_{\frac{N}{2}}}\equiv S_{_{\frac{N}{2}}}\left(  \bm{n}\right)  =\sum_{1\leq
j_{1}<\cdots<j_{\frac{N}{2}}\leq s}n_{j_{1}}\cdots n_{j_{_{\frac{N}{2}}}},
\end{equation}
and
\begin{equation}
...
\begin{align}
&  D^{2}\left(  \bm{c},\bm{\psi,\theta}\right)  =\left(  S_{\frac{N}{2}%
}\right)  ^{-1}\left\{  \sum_{1\leq j\leq s}n_{j}S_{\frac{N}{2}-1}\left(
\hat{\jmath}\right)  \left\vert \psi_{j}\psi_{j+s}\right\rangle \left\langle
...
\begin{equation}
\left\vert g\left(  \bm{c,\psi,\theta}\right)  ^{\frac{N}{2}}\right\rangle
=\sum_{1\leq j_{1}<\cdots<j_{\frac{N}{2}}\leq s}c_{j_{1}}\cdots c_{j_{\frac
{N}{2}}}\exp\left\{  i\sum\limits_{1\leq\kappa\leq\frac{N}{2}}\theta
_{j_{\kappa}}\right\}  \left\vert \psi_{j_{1}}\psi_{j_{1}+s}\cdots
\psi_{j_{\frac{N}{2}}}\psi_{_{j_{\frac{N}{2}}}+s}\right\rangle ,
\end{equation}
which explicitly displays the dependence of AGP states on the phases $\left\{
\left.  \theta_{j_{\kappa}}\right\vert 1\leq\kappa\leq s\right\}  $.

\subsection{Energy Functional\label{AGPFunc}}

The energy of an AGP\ state is given by
\begin{align}
&  \mathcal{E}\left(  \bm{c},\bm{\psi,\theta}\right)  =\frac{\mathcal{H}%
\left(  \bm{c},\bm{\psi,\theta}\right)  }{S_{\frac{N}{2}}\left(
\bm{n}\right)  }=\nonumber\\
...
&  \qquad\qquad\qquad\qquad+\sum_{1\leq j\neq k\leq s}\left\{  c_{j}%
c_{k}e^{i\left(  \theta_{j}-\theta_{k}\right)  }S_{\frac{N}{2}-1}\left(
\hat{\jmath}\hat{k}\right)  \left\langle \psi_{j}\psi_{j+s}\right\vert
\left\vert \psi_{k}\psi_{k+s}\right\rangle \right. \nonumber\\
...
S_{\frac{N}{2}}\left(  \bm{n}\right)  . \label{EnFunc}%
\end{align}
...
\end{align}
and one can express the one and two electron values in terms of of space-spin
wave functions with coordinates $\left(  \bm{r,\xi}\right)  $ in atomic units
as
\begin{equation}
\left\langle \psi_{j}\left\vert h_{1}\psi_{j}\right.  \right\rangle =\int
\psi_{j}^{\ast}\left(  \bm{r,\xi}\right)  \left\{  V_{\text{ext}}\left(
\bm{r}\right)  -\nabla_{\bm{r}}^{2}\right\}  \psi_{l}\left(
\bm{r,\xi}\right)  d\bm{r}d\xi
\end{equation}
and
\begin{align}
&  \left\langle \psi_{j}\psi_{k}||\psi_{l}\psi_{m}\right\rangle =\int\left\{
\psi_{j}^{\ast}\left(  \bm{r}_{1}\bm{,\xi}_{1}\right)  \psi_{k}^{\ast}\left(
\bm{r}_{2}\bm{,\xi}_{2}\right)  \frac{1}{\left\Vert \bm{r}_{1}-\bm{r}_{2}%
\right\Vert }\psi_{l}\left(  \bm{r}_{1}\bm{,\xi}_{1}\right)  \psi_{m}\left(
...
\begin{equation}
\left\vert g\left(  \bm{c},\bm{\psi,\theta}\right)  \right\rangle =\sum_{1\leq
j\leq s}c_{j}e^{i\theta_{j}}\left\vert \psi_{j}\psi_{j+s}\right\rangle
=\sum_{1\leq j\leq s}c_{j}\left\vert \tilde{\psi}_{j}\tilde{\psi}%
_{j+s}\right\rangle ,
\end{equation}
where
\begin{align}
...
$\left(  \ref{OccNum}\right)  ).$

\subsection{One Electron Quantities\label{OEP}}

In \cite{OrtizWeiner2006} it was shown that for AGP's states, $\left\vert
g\left(  \bm{c}_{\#},\bm{\tilde{\psi}}_{\#}\right)  ^{\frac{N}{2}%
}\right\rangle $ that optimize the energy functional eq. $\left(
\ref{EnFunc}\right)  $ i.e.
\begin{equation}
\mathcal{E}\left(  \bm{c}_{\#},\bm{\tilde{\psi}}_{\#}\right)  =\min
_{\bm{c},\bm{\tilde{\psi}}}\mathcal{E}\left(  \bm{c},\bm{\tilde{\psi}}\right)
\end{equation}
one can express the total N-electron energy in terms of the occupation numbers
$\left\{  \left.  N_{j}\left(  \bm{c}_{\#}\right)  \right\vert 1\leq j\leq
r\right\}  $, Ionization Potentials (IP), external potentials and kinetic
energies $\left\{  \left.  I\left(  \tilde{\psi}_{\#j}\right)  ,h_{1}\left(
...
that the phase factor dependence has been absorbed into the GSO as shown
...
\begin{equation}
I\left(  \tilde{\psi}_{\#j}\right)  =\mathcal{E}\left(  \bm{c}_{\#}%
,\bm{\tilde{\psi}}_{\#}\right)  -\mathcal{E}_{N\,-1}\left(  \bm{c}_{\#}%
,\bm{\tilde{\psi}}_{\#}\bm{,}\tilde{\psi}_{\#j}\right)
\end{equation}
...
\begin{equation}
\mathcal{E}\left(  \bm{c}_{\#},\bm{\tilde{\psi}}_{\#}\right)  =\frac{1}{2}%
\sum_{1\leq j\leq2s}N_{j}\left(  \bm{c}_{\#}\right)  \kappa\left(  \tilde
{\psi}_{\#j}\right)  ,
\end{equation}
where
\begin{equation}
\kappa\left(  \tilde{\psi}_{\#j}\right)  =I\left(  \tilde{\psi}_{\#j}\right)
+h_{1}\left(  \tilde{\psi}_{\#j}\right)
\end{equation}
and $N_{j}\left(  \bm{c}_{\#}\right)  $ is the optimized occupation number of
the GSO\ $\left\vert \tilde{\psi}_{\#j}\right\rangle .$

\section{Density Matrix Functional\label{DenMatFun}}

In this section we return to the non redundant parameterization of eq.
$\left(  \ref{gem}\right)  ,$which explicitly involves the phase. As the FORDO
is left invariant by the change of phase, while the $N$-electron state and
energy are not, one can define an AGP DMF (a functional defined over the
domain of AGP states) by%
\begin{equation}
\mathcal{F}\left(  D^{1}\left(  \bm{c},\bm{\psi}\right)  \right)
=\min_{\bm{\theta}}\mathcal{E}\left(  \bm{\theta}\right)
\end{equation}
where for fixed $\bm{c}$ and $\bm{\psi}$ the energy is just a function of the
phases $\left\{  \left.  \theta_{j}\right\vert 1\leq j<k\leq s\right\}  $
given in eq. $\left(  \ref{Phases}\right)  $, by
\begin{align}
&  \mathcal{E}\left(  \bm{\theta}\right)  =\frac{\mathcal{H}\left(
...
simple examples that involve only two electrons. These demonstrate
...
spin orbitals or even orbitals and two electron systems. Thus we restrict
attention to geminals that are low spin eigen functions of $\ $the spin
operator $\mathcal{S}_{0}$ (see Appendix \ref{Spin}) i.e.
\begin{equation}
...
\right\}  $ and set $N=2$. The most general AGP of this type is in fact a
geminal of the form%
\begin{equation}
\left\vert g\left(  \bm{c},\bm{\varphi,\theta}\right)  \right\rangle
=\sum_{1\leq j\leq s}c_{j}e^{i\theta_{j}}\left\vert \varphi_{j}\alpha
\varphi_{j+s}\beta\right\rangle , \label{spinorbgem}%
\end{equation}
...
FORDO that are determined by spin orbitals and occupation numbers is given
when we set (i)
\begin{equation}
s=2;c_{1}=\frac{1}{\sqrt{2}};c_{2}=\frac{1}{\sqrt{2}};\theta=0
\end{equation}
producing%
\begin{equation}
...
and set (ii)
\begin{equation}
s=2;c_{1}=\frac{1}{\sqrt{2}};c_{2}=\frac{1}{\sqrt{2}};\theta=\pi
\end{equation}
producing
\begin{equation}
\left\vert g_{-}\right\rangle =\frac{1}{\sqrt{2}}\left\{  \left\vert
...
In the context of OET (section \ref{OET}) the states $\left\vert
...
\bm{\psi }\right]  $, but the phase dependence (in these cases the phases
$\theta=0$ and $\theta=\pi)$ \emph{have to be included} in the description of
the spin orbital.
...
2\alpha4\beta\right\rangle +\left\langle 1\alpha3\beta\right\vert \left\vert
2\alpha4\beta\right\rangle +\left\langle 1\alpha3\beta\right\vert \left\vert
2\alpha4\beta\right\rangle \overset{}{}\right\}  \label{E+}%
\end{align}
and
\begin{align}
&  \mathcal{E}\left(  g_{-}\right)  =\frac{1}{2}\left\{  \overset
...
\begin{equation}
\left\vert g\left(  \bm{c,\psi,}\theta\right)  \right\rangle =c_{1}\left\vert
1\alpha3\beta\right\rangle +c_{2}e^{i\theta}\left\vert 2\alpha4\beta
\right\rangle ,
\end{equation}
where
\begin{align}
...
\begin{equation}
\left\vert g\left(  \chi\bm{,\psi,}\theta\right)  \right\rangle =\cos
\chi\left\vert 1\alpha3\beta\right\rangle +e^{i\theta}\sin\chi\left\vert
2\alpha4\beta\right\rangle .
\end{equation}
All the states with a fixed $\left\{  \chi\bm{,\psi}\right\}  $, where
$\bm{\psi\in}\left[  \bm{\psi}\right]  $, have a fixed FORDO
\[
D^{1}\left(  \chi,\bm{\psi}\right)  =\cos^{2}\chi\left\{  \left\vert
...
\begin{align}
D^{2}\left(  \chi,\bm{\psi}\right)   &  =\cos^{2}\chi\left\vert 1\alpha
3\beta\right\rangle \left\langle 1\alpha3\beta\right\vert +\sin^{2}%
...
3\beta\right\rangle \right\}  \sin2\chi,
\end{align}
which after spin integration gives
\begin{align}
...
\nonumber\\
&  \equiv k\left(  \chi,\bm{\psi}\right)  +f\left(  \chi
,\bm{\psi ,\theta}\right)  ,
\end{align}
where
\begin{equation}
\left(  \left.  jk\right\vert lm\right)  =\int\varphi_{j}^{\ast}\left(
\bm{r}_{1}\right)  \varphi_{k}^{\ast}\left(  \bm{r}_{2}\right)  \frac
{1}{\left\Vert \bm{r}_{1}-\bm{r}_{2}\right\Vert }\varphi_{l}\left(
\bm{r}_{1}\right)  \varphi_{m}\left(  \bm{r}_{2}\right)  d\bm{r}_{1}%
d\bm{r}_{2},
\end{equation}
%
...
\bm{r}\right)  \left\{  V_{\text{ext}}\left(  \bm{r}\right)  -\nabla
_{\bm{r}}^{2}\right\}  \varphi_{j}\left(  \bm{r}\right)  d\bm{r,}
\end{equation}%
\begin{align}
k\left(  \chi,\bm{\psi}\right)   &  =\left\{  \left(  1\left\vert
h_{1}1\right.  \right)  +\left(  3\left\vert h_{1}3\right.  \right)  \right\}
\cos^{2}\chi+\left\{  \left(  2\left\vert h_{1}2\right.  \right)  +\left(
4\left\vert h_{1}4\right.  \right)  \right\}  \sin^{2}\chi\nonumber\\
...
and
\begin{equation}
f\left(  \chi,\bm{\psi,\theta}\right)  =\operatorname{Re}\left\{  e^{i\theta
}\left(  \left.  13\right\vert 24\right)  \sin2\chi\right\}  .
\end{equation}
Over the domain of such geminals one can define a DMF as
\begin{equation}
\mathcal{F}\left(  \chi,\bm{\psi}\right)  =\mathcal{F}\left(  D^{1}\left(
\chi,\bm{\psi}\right)  \right)  =\min_{\bm{\theta}}\mathcal{E}\left(
\bm{\theta}\right)  .
\end{equation}
The only part of the functional $\mathcal{E}$ that depends on $\theta$ is
$f\left(  \chi,\bm{\psi,}\theta\right)  ,$ which one can express as
\begin{equation}
f\left(  \chi,\bm{\psi,}\theta\right)  =\operatorname{Re}\left\{  e^{i\theta
}\left(  \left.  13\right\vert 24\right)  \right\}  =\operatorname{Re}\left\{
e^{i\theta}ae^{i\alpha}\right\}  =\operatorname{Re}\left\{  e^{i\left(
\theta+\alpha\right)  }a\right\}  =a\cos\left(  \theta+\alpha\right)  ,
\end{equation}
where
\begin{align}
...
\begin{align}
f\left(  \chi,\bm{\psi,}\theta_{\min}\right)   &  =\min_{\theta}\left\{
a\cos\left(  \theta+\alpha\right)  \right\}  =-a\\
\theta_{\min}  &  =\pi-\alpha.
\end{align}
Hence the value of the DMF, $\mathcal{F},$ at the point $D^{1}\left(
\chi,\bm{\psi}\right)  $ is%
\begin{equation}
\mathcal{F}\left(  \chi,\bm{\psi}\right)  =k\left(  \chi,\bm{\psi }\right)
-2\left\vert \left(  \left.  13\right\vert 24\right)  \right\vert \sin2\chi
\end{equation}
and the minimum of the AGP energy functional $\mathcal{E}\left(  \bm{c}\left(
\bm{\chi}\right)  ,\bm{\psi,\theta}\right)  $ determines the AGP\ ground state
is given by
\begin{equation}
\mathcal{E}\left(  \left\vert g_{GS}\right\rangle \right)  =\min
_{\bm{\chi,\psi}}\mathcal{F}\left(  \chi,\bm{\psi}\right)  .
\end{equation}
...
One can expand a geminal in a form that displays explictly degeneraces as
\begin{equation}
\left\vert g\left(  \bm{c},\bm{\psi,\theta}\right)  \right\rangle =\sum_{1\leq
k\leq d}c_{j_{k}}\sum_{s_{k}+1\leq j_{k}\leq s_{k}+d_{k}}e^{i\theta_{j_{k}}%
}\left\vert \psi_{j_{k}}\psi_{j_{k}+s}\right\rangle ,
\end{equation}
where $\left\{  d_{1},\cdots,d_{d}\right\}  $ are the degree of degeneracies
of the canonical coefficients $\left\{  c_{1},\cdots,c_{d}\right\}  ,$
$s_{k}=\sum\limits_{1\leq k\leq p\leq d}d_{k}$ and $\sum\limits_{1\leq k\leq
d}d_{k}=s_{d}=s.$ The simplest example of a degenerate geminal can be realized
by setting $s=3,d=2,\left\{  d_{1},d_{2}\right\}  =\left\{  2,1\right\}  ,$
i.e.
\begin{equation}
\left\vert g\left(  \bm{c},\bm{\psi,\theta}\right)  \right\rangle
=c_{1}\left\{  e^{i\theta_{1}}\left\vert \psi_{1}\psi_{4}\right\rangle
+e^{i\theta_{1}}\left\vert \psi_{2}\psi_{5}\right\rangle \right\}
+c_{2}e^{i\theta_{3}}\left\vert \psi_{3}\psi_{6}\right\rangle .
\end{equation}
Following \cite{AGPCoherent} and \cite{OrtizWeiner2006} one can define sets of
operators associated with the degenerate canonical coefficient $c_{1}$%
\begin{equation}
\left(
\begin{array}
[c]{c}%
...
\right)
\end{equation}
The set $\bm{u}=\left\{  u_{1},u_{2},u_{3},u_{4}\right\}  $ is a
representation of the lie algebra $u\left(  2\right)  $ that produces a
representation of the group $U\left(  2\right)  $ over the subspace generated
by $\left\{  \psi_{1},\psi_{2},\psi_{4},\psi_{5}\right\}  $ that changes
$\left\vert g\right\rangle ,$ while the set $\bm{v}=\left\{  v_{1},v_{2}%
,v_{3},v_{4}\right\}  $ is a representation of the lie algebra $su\left(
2\right)  $ that produce a representation of the group $SU\left(  2\right)  $
that leaves $\left\vert g\right\rangle $ invariant, as the operators $\left\{
\bm{u,v}\right\}  $ have the properties
\begin{equation}
v\left\vert g\right\rangle =0,\quad u\left\vert g\right\rangle \neq0;\quad
u\in\bm{u},v\in\bm{v.}
\end{equation}
Thus any GSO produced by the action of $\bm{v}$ produces the same geminal thus
all GSO's $\bm{v}\left\vert \bm{\psi}\right\rangle $ belong to the same
equivalence class $\left[  \bm{\psi}\right]  .$ This generalizes the
equivalence realtionship given eq. $\left(  \ref{Equival}\right)  .$
...
\end{align}
and substituting for $\bm{c,n}$ and $\bm{\theta}$ one obtains
\begin{align}
...
\right\}
\end{align}
and finally substituting for $\bm{\psi}$ one gets eq. $\left(  \ref{Dirgm}%
\right)  .$

\bibliographystyle{apsrev}
\bibliography{agpopt,bw,novel,San2008,coleman}

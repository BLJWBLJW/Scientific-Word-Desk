...
%TCIDATA{LastRevised=Wednesday, March 26, 2008 16:19:43}
...
Interaction (CI), Coupled Cluster, Perturbation Theory and many others have
been utilized that contain an explicit description of these interactions.
Although the resulting improved multi electron states still determine sets of
orbitals, now greater than the number of electrons and weighted with
occupation numbers, through the First Order Reduced Density Operator (FORDO),
these orbitals and their occupation numbers \emph{do not }, in general,
determine a unique multi electron state. Thus one looses the concrete
pictorial aspects of a one electron theory, as the multi electron state
\emph{cannot} be described solely in terms of single electron quantities and
geometric forms.
...
The analysis of AGP states further clearly shows that a model based on
orbitals and occupation numbers is not equivalent to one based on FORDO's, as
many sets of orbitals determine the same FORDO while determining different
N-electron states. This observation allows one to explicitly construct
examples of DMF's whose domain is restricted to AGP's.
...
in section 5 we describe simple examples of (i) how AGP states are determined
...
\right\rangle =\sum_{1\leq j\leq s}c_{j}e^{i\theta_{j}}\left\vert \psi_{j}%
\psi_{j+s}\right\rangle ,\label{gem}%
\end{equation}
...
$\boldsymbol{c}$ lie on the positive portion of the hypersphere
\begin{equation}
\sum\limits_{1\leq j\leq s}c_{j}^{2}=1;\,c_{j}\geq0;1\leq j\leq s,
\end{equation}
the angles $\boldsymbol{\theta}=\left(  \theta_{1},\theta_{2},\cdots
,\theta_{s}\right)  $ satisfy%
\begin{equation}
0\leq\theta_{2},\cdots,\theta_{s}<2\pi;\quad\theta_{1}=0\label{Phases}%
\end{equation}
...
}\right\}  ,$ where $\boldsymbol{\psi\in}\mathcal{P}$ are representatives of
the equivalence classes, $\left[  \boldsymbol{\psi}\right]  $, of paired
normalized GSO's, which for non degenerate AGP's are defined by
\begin{equation}
...
\begin{align}
&  D^{2}\left(  \boldsymbol{c},\boldsymbol{\psi,\theta}\right)  =\left(
S_{\frac{N}{2}}\right)  ^{-1}\left\{  \sum_{1\leq j\leq s}n_{j}S_{\frac{N}%
{2}-1}\left(  \hat{\jmath}\right)  \left\vert \psi_{j}\psi_{j+s}\right\rangle
...
\begin{equation}
\left\vert g\left(  \boldsymbol{c,\psi,\theta}\right)  ^{\frac{N}{2}%
}\right\rangle =\sum_{1\leq j_{1}<\cdots<j_{\frac{N}{2}}\leq s}c_{j_{1}}\cdots
...
\begin{align}
&  \mathcal{E}\left(  \boldsymbol{c},\boldsymbol{\psi,\theta}\right)
=\frac{\mathcal{H}\left(  \boldsymbol{c},\boldsymbol{\psi,\theta}\right)
}{S_{\frac{N}{2}}\left(  \boldsymbol{n}\right)  }=\nonumber\\
...
wave functions with coordinates $\left(  \boldsymbol{r,\xi}\right)  $ in
atomic units as
\begin{equation}
\left\langle \psi_{j}\left\vert h_{1}\psi_{j}\right.  \right\rangle =\int
\psi_{j}^{\ast}\left(  \boldsymbol{r,\xi}\right)  \left\{  V_{\text{ext}%
}\left(  \boldsymbol{r}\right)  -\frac{1}{2}\nabla_{\boldsymbol{r}}%
^{2}\right\}  \psi_{l}\left(  \boldsymbol{r,\xi}\right)  d\boldsymbol{r}d\xi
\end{equation}
and
\begin{align}
...
\right\}  d\boldsymbol{r}_{1}d\boldsymbol{r}_{2}d\xi_{1}d\xi_{2}%
\end{align}
respectively.
...
\left\vert g\left(  \boldsymbol{c},\boldsymbol{\psi,\theta}\right)
\right\rangle =\sum_{1\leq j\leq s}c_{j}e^{i\theta_{j}}\left\vert \psi_{j}%
\psi_{j+s}\right\rangle =\sum_{1\leq j\leq s}c_{j}\left\vert \tilde{\psi}%
_{j}\tilde{\psi}_{j+s}\right\rangle ,
\end{equation}
where
\begin{align}
...
$\left\{  \left.  N_{j}\left(  \boldsymbol{c}_{\#}\right)  \right\vert 1\leq
j\leq r\right\}  $, Ionization Potentials (IP), external potentials and
kinetic energies $\left\{  \left.  I\left(  \tilde{\psi}_{\#j}\right)
,h_{1}\left(  \tilde{\psi}_{\#j}\right)  \right\vert 1\leq j\leq r\right\}  $
associated with the optimized GSO's $\left\{  \left.  \left\vert \tilde{\psi
...
\boldsymbol{c}_{\#},\boldsymbol{\tilde{\psi}}_{\#}\boldsymbol{,}\tilde{\psi
}_{\#j}\right)
\end{equation}
...
s\right\}  $ given in eq. $\left(  \ref{Phases}\right)  $, by
\begin{align}
...
attention to geminals that are low spin eigen functions of the spin operator
$\mathcal{S}_{0}$ (see Appendix \ref{Spin}) i.e.
\begin{equation}
...
geminal of the form%
\begin{equation}
\left\vert g\left(  \boldsymbol{c},\boldsymbol{\varphi,\theta}\right)
\right\rangle =\sum_{1\leq j\leq s}c_{j}e^{i\theta_{j}}\left\vert \varphi
_{j}\alpha\varphi_{j+s}\beta\right\rangle ,\label{spinorbgem}%
\end{equation}
...
phases $\theta=0$ and $\theta=\pi)$ \emph{have to be included} in the
description of the spin orbital.
...
\begin{equation}
\left\vert g\left(  \boldsymbol{c,\psi,}\theta\right)  \right\rangle
=c_{1}\left\vert 1\alpha3\beta\right\rangle +c_{2}e^{i\theta}\left\vert
...
2\alpha4\beta\right\rangle .
\end{equation}
All the states $\left\vert g\left(  \chi\boldsymbol{,\psi,}\theta\right)
\right\rangle $ with a fixed $\left\{  \chi\boldsymbol{,\psi}\right\}  $,
where $\boldsymbol{\psi\in}\left[  \boldsymbol{\psi}\right]  $, have a fixed
FORDO
\[
...
\boldsymbol{r}\right)  \left\{  V_{\text{ext}}\left(  \boldsymbol{r}\right)
-\nabla_{\boldsymbol{r}}^{2}\right\}  \varphi_{j}\left(  \boldsymbol{r}%
...
}}\mathcal{E}\left(  \boldsymbol{\theta}\right)  .
\end{equation}
...
\boldsymbol{c}\left(  \boldsymbol{\chi}\right)  ,\boldsymbol{\psi,\theta
}\right)  ,$ which determines the AGP\ ground state, is given by
\begin{equation}
\mathcal{E}\left(  \left\vert g_{GS}\right\rangle \right)  =\min
_{\boldsymbol{\chi,\psi}}\mathcal{F}\left(  \chi,\boldsymbol{\psi}\right)  .
\end{equation}
...
of a multielectron system, although the electrons cannot now be identified
with individual orbitals. 
...
One can expand a geminal in a form that displays explicitly degeneracies as
\begin{equation}
\left\vert g\left(  \boldsymbol{c},\boldsymbol{\psi,\theta}\right)
\right\rangle =\sum_{1\leq k\leq d}c_{j_{k}}\sum_{s_{k}+1\leq j_{k}\leq
s_{k}+d_{k}}e^{i\theta_{j_{k}}}\left\vert \psi_{j_{k}}\psi_{j_{k}%
+s}\right\rangle ,
\end{equation}
...
\begin{equation}
\left\vert g\left(  \boldsymbol{c},\boldsymbol{\psi,\theta}\right)
\right\rangle =c_{1}\left\{  e^{i\theta_{1}}\left\vert \psi_{1}\psi
_{4}\right\rangle +e^{i\theta_{2}}\left\vert \psi_{2}\psi_{5}\right\rangle
...
\right)  .
\end{equation}
The set $\boldsymbol{u}=\left\{  u_{1},u_{2},u_{3},u_{4}\right\}  $ is a
...
$\left\vert g\right\rangle ,$ while the set $\boldsymbol{v}=\left\{
v_{1},v_{2},v_{3},v_{4}\right\}  $ is a representation of the lie algebra
$su\left(  2\right)  $ that produce a representation of the group $SU\left(
2\right)  $ that leaves $\left\vert g\right\rangle $ invariant, as the
operators $\left\{  \boldsymbol{u,v}\right\}  $ have the properties
\begin{equation}
v\left\vert g\right\rangle =0,\quad u\left\vert g\right\rangle \neq0;\quad
u\in\boldsymbol{u},v\in\boldsymbol{v.}%
\end{equation}
Thus any GSO produced by the action of $\boldsymbol{v}$ produces the same
geminal and thus all the GSO's $\boldsymbol{v}\left\vert \boldsymbol{\psi
}\right\rangle $ belong to the same equivalence class $\left[
\boldsymbol{\psi}\right]  .$ This generalizes the equivalence relationship
given eq. $\left(  \ref{Equival}\right)  .$
...
\end{align}
and substituting for $\boldsymbol{c,n}$ and $\boldsymbol{\theta}$ one obtains
\begin{align}

...
%TCIDATA{LastRevised=Friday, November 23, 2001 10:41:15}
...
\end{eqnarray}%
can be expressed in terms of the preceding {\em real }parameters and their
time derivative by using the chain rule on eq. $\left( \ref{La}\right) $ as 
\begin{equation}
...
\left( {\sf z}\right) }=0  \nonumber \\
&\equiv &\int \frac{\delta ^{2}{\cal L}\left( {\cal \varkappa },{\cal \dot{%
\varkappa}}\right) }{\delta {\cal \dot{\varkappa}}\left( {\sf z}\right)
\delta {\cal \dot{\varkappa}}\left( {\sf z}^{\prime }\right) }{\cal \ddot{%
\varkappa}}\left( {\sf z}^{\prime }\right) d{\sf z}^{\prime }+\int \frac{%
\delta ^{2}{\cal L}\left( {\cal \varkappa },{\cal \dot{\varkappa}}\right) }{%
\delta {\cal \dot{\varkappa}}\left( {\sf z}\right) \delta {\cal \varkappa }%
\left( {\sf z}^{\prime }\right) }{\cal \dot{\varkappa}}\left( {\sf z}%
^{\prime }\right) d{\sf z}^{\prime }-\frac{\delta {\cal L}\left( {\cal %
\varkappa },{\cal \dot{\varkappa}}\right) }{\delta {\cal \varkappa }\left( 
{\sf z}\right) }=0  \label{Lexpand}
\end{eqnarray}%
The Lagrangian eq. $\left( \ref{ladef}\right) $is first order in ${\cal \dot{%
\varkappa}}$ thus is degenerate (in fact it is suffcient that $\det \left\{ 
\frac{\delta ^{2}{\cal L}\left( {\cal \varkappa },{\cal \dot{\varkappa}}%
\right) }{\delta {\cal \dot{\varkappa}}\left( {\sf z}\right) \delta {\cal 
\dot{\varkappa}}\left( {\sf z}^{\prime }\right) }\right\} =0$ for degeneracy
thus our Lagrangian is {\em very }degenerate!). Evaluating all the terms eq. 
$\left( \ref{Lexpand}\right) $ becomes 
\begin{eqnarray}
&&\int \frac{\delta ^{2}{\cal L}\left( {\cal \varkappa },{\cal \dot{\varkappa%
}}\right) }{\delta {\cal \dot{\varkappa}}\left( {\sf z}\right) \delta {\cal %
\varkappa }\left( {\sf z}^{\prime }\right) }{\cal \dot{\varkappa}}\left( 
{\sf z}^{\prime }\right) d{\sf z}^{\prime }-\frac{\delta {\cal L}\left( 
{\cal \varkappa },{\cal \dot{\varkappa}}\right) }{\delta {\cal \varkappa }%
\left( {\sf z}\right) }=0  \nonumber \\
&\Longleftrightarrow &\int {\cal \dot{\varkappa}}\left( {\sf z}^{\prime
},t\right) \frac{\delta {\cal Z}\left( {\sf z}\right) }{\delta {\cal %
\varkappa }\left( {\sf z}^{\prime }\right) }d{\sf z}^{\prime }-\int {\cal 
\dot{\varkappa}}\left( {\sf z}^{\prime },t\right) \frac{\delta {\cal Z}%
\left( {\sf z}^{\prime }\right) }{\delta {\cal \varkappa }\left( {\sf z}%
\right) }d{\sf z}^{\prime }+\frac{\delta {\cal E}\left( {\cal \varkappa }%
\right) }{\delta {\cal \varkappa }\left( {\sf z}\right) }=0  \nonumber \\
&\Longleftrightarrow &\int \left\{ \frac{\delta {\cal Z}\left( {\sf z}%
,t\right) }{\delta {\cal \varkappa }\left( {\sf z}^{\prime }\right) }-\frac{%
\delta {\cal Z}\left( {\sf z}^{\prime },t\right) }{\delta {\cal \varkappa }%
\left( {\sf z}\right) }\right\} {\cal \dot{\varkappa}}\left( {\sf z}^{\prime
},t\right) d{\sf z}^{\prime }+\frac{\delta {\cal E}\left( {\cal \varkappa }%
\right) }{\delta {\cal \varkappa }\left( {\sf z}\right) }=0
\end{eqnarray}%
$\lceil $Degeneracy of the Lagrangian implies singularity of the Legendre
transformation (which transforms velocities to generalized momenta)\ and
thus makes it impossible to obtain a standard Hamiltonian form for the
equations of motion. If the Lagrangian were not degenerate one could produce
a Hamiltonian form of Lagranges' equation in terms of Hamiltons equations 
\begin{equation}
\left. 
\begin{array}{c}
...
\right) }  \label{conjmom}
\end{equation}%
The correspondence between Hamiltons and Lagranges EOM\ is then given by
considering 
\begin{equation}
...
d{\cal Z}\left( {\sf z}\right) }{dt}=\,-\stackrel{\cdot }{{\cal Z}}\left( 
{\sf z}\right) 
\end{equation}%
...
written is terms of the conjugate momenta as 
\begin{equation}
\frac{\delta {\cal L}\left( {\cal \varkappa },{\cal \dot{\varkappa}}\right) 
...
\QSubDoc{Include Part}{\input{Part.tex}}

\QSubDoc{Include VecBun}{\input{VecBun.tex}}

\QSubDoc{Include DualOp}{\input{DualOp.tex}}

\QSubDoc{Include DualPair}{\input{DualPair.tex}}

\QSubDoc{Include SolODE}{\input{SolODE.tex}}

\QSubDoc{Include TaySer}{\input{TaySer.tex}}

\QSubDoc{Include ComplexForm}{\input{ComplexForm.tex}}

\QSubDoc{Include Antisym}{\input{Antisym.tex}}

\QSubDoc{Include Group}{\input{Group.tex}}

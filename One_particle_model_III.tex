...
%TCIDATA{LastRevised=Monday, September 06, 2004 09:54:04}
...
which totally \emph{determine} the $2N$-electron state and the CGSO's and
their occupation numbers can be used as an effective and rigorous chemical
modelling tool.
...
\chi_{j}\right\rangle \right\}  $, but then the geminal $\left\vert g\left(
...
as one is assured that the pictures produced corresponds to multi electron states.
...
they are also totally determined by one particle properties.
...
functional with respect to NGSO's for fixed FORDO occupation numbers. In
section \ref{FD} we analyze the total energy in terms of this Fock operator
and show that the total state energy can be explicitly expressed in terms of
...
\right)  $, given by \cite{Coleman97}\cite{Yukalov00}
\begin{align}
...
expanded in terms of $\mathbf{\theta}$ dependent NGSO's $\left\{  \left.
...
\end{equation}
See Appendix \ref{LinfitEX} for an example. The accuracy of the fit can be
...
\cite{cioslowski:4156}\cite{staroverov:11107}\cite{cioslowski:67} and it
...
as
\begin{align}
\mathcal{M}  & \mathcal{=}\left\{  \left.  k\left(  \mathbf{\eta}\right)
p\left(  \mathbf{\theta}\right)  u\left(  \mathbf{\alpha}\right)  \left\vert
g_{0}^{N}\right\rangle \right\vert \quad0\leq\theta_{j}<2\pi;\,\eta_{j}%
\in\mathbb{R};\right.  \nonumber\\
...
=a_{j+s}^{\dagger}a_{j}\left\vert g_{0}\right\rangle =0;\,1\leq j\leq
s.\label{iso1}%
\end{equation}
...
\end{equation}
Taking into account the invariance group relatinship eq. $\left(
...
variations, hence even for fully optimized MCSCF states one has
\begin{equation}
...
for $1\leq j\neq k\leq s$ one can \cite{weiner_ad_04}\cite{Weiner83a}%
...
non singular transformation, $\mathbb{T},$ such that the blocks $\mathbb{T}%
...
\cite{Goscinski82a} with a sets of self adjoint operators $\left\{
u_{jk}\right\}  $ and $\left\{  v_{ij}\right\}  $ respectively, where
\begin{equation}
...
In contrast the general optimized MCSCF Brillouin condition is
\begin{equation}
\left\langle \left.  \Psi\right\vert Ha_{l}^{\dagger}a_{m}\Psi\right\rangle
\left\{
\begin{array}
[c]{c}%
=0\text{ for }m\text{ being an occupied GSO and }l\text{ being an unoccupied
GSO }\\
=0\text{ for }m\text{ being an unoccupied GSO and }l\text{ being an unoccupied
GSO}\\
...
$N$-representable SORDM $\mathbb{D}^{2}$'s and their associated FORDM,
$\mathbb{D}^{1}$'s giving
\begin{equation}
...
\end{equation}
Thus they define a generalized Fock operator for \emph{any }MCSCF state and
...
The generalized Fock operator associated with an optimized MCSCF\ state has
...
the positive operator $D^{2N}$. For a pure state $\left\vert \Psi\right\rangle
$ this becomes
\begin{equation}
F\left(  \mathbf{n},\mathbf{\varphi}\right)  _{qp}=\frac{1}{\left\langle
\left.  \Psi\right\vert \Psi\right\rangle }\left\langle \left.  \Psi
\right\vert \left\{  a_{p}^{\dagger}a_{q}H-a_{p}^{\dagger}Ha_{q}\right\}
\Psi\right\rangle .
\end{equation}
...
\begin{equation}
F\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  _{pp}=\frac{1}{\left\langle
\left.  \Psi\right\vert \Psi\right\rangle }\left\langle \left.  \Psi
\right\vert \left\{  a_{p}^{\dagger}a_{q}H-a_{p}^{\dagger}Ha_{q}\right\}
\Psi\right\rangle =\mathcal{E}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)
\Omega_{p}-\frac{1}{\left\langle \left.  \Psi\right\vert \Psi\right\rangle
...
from the state $\frac{1}{\left\langle \left.  \Psi\right\vert \Psi
\right\rangle }\Psi$ by removing an electron described by the GSO\ $\left\vert
\varphi_{p}\right\rangle $. If $\Omega_{p}$ $=0$ then $F\left(  \mathbb{D}%
^{2},\mathbf{\varphi}\right)  _{pp}=0$ as the GSO $\left\vert \varphi
_{p}\right\rangle $ is unoccupied. In Appendix \ref{Fock&Energy} we show that
the total energy can be expressed in terms of the Fock operator $F\left(
\mathbb{D}^{2},\mathbf{\varphi}\right)  \ $as
\begin{equation}
...
\end{equation}
where $\mathcal{E}_{1}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  $
expectation value of the one electron part of the Hamiltonian. The energy
$\mathcal{E}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  $ can be expressed
as
\begin{equation}
\mathcal{E}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  =\frac{1}{2}%
\sum_{1\leq p\leq2s}\Omega_{p}I\left(  \varphi_{p}\right)  +\mathcal{E}%
_{1}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  ,\label{TEexpec}%
\end{equation}
...
which is hermitian, in the CGSO basis is in general non diagonal. The matrix
...
$D^{1}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  \equiv D^{1}\left(
\mathbb{D}^{2}\left(  \mathbf{\varsigma}\right)  \right)  $ is still not in
...
One can, however, express the generalized Fock operator $F\left(
\mathbb{D}^{2},\mathbf{\varphi}\right)  $ in terms of the NGSO's $\left\{
...
%EndExpansion
N_{l}\kappa\left(  \upsilon_{p}\right)  ,
\end{equation}
...
\end{equation}
As $\left\{  \psi_{p}\right\}  $ are also NGSO's the energy $\mathcal{E}%
\left(  \mathbf{n},\mathbf{\psi}\right)  $ can be expressed as
\begin{equation}
...
implies that
\begin{equation}
\left[  \mathfrak{f}\left(  \mathbf{\varphi}\right)  ,D^{1}\right]
=0.\label{HF}%
\end{equation}
...
Together with eq. $\left(  \ref{HF}\right)  $ shows that the matrix
...
\begin{array}
[c]{cc}%
\mathbb{F}_{0}\left(  \mathbf{\varphi}\right)   & \mathbb{O}\\
...
\begin{equation}
\mathfrak{f}\left(  \mathbf{\varphi}\right)  \left\vert \varphi_{j}%
\right\rangle =\left\vert \varphi_{j}\right\rangle \varepsilon_{j};\quad1\leq
j\leq2s.
\end{equation}
...
\mathbf{\varphi}\right)  D^{1}$ of the IPS Fock operator $\mathfrak{f}\left(
\mathbf{\varphi}\right)  $ and the FORDO $D^{1}$. In the IPS\ case the
...
points of the GSO variation and is not diagonal.
...
the increment $\delta\mathbf{N}$ was small enough to produce consistency.
...
a full non redundant energy functional by varying the reference
...
where
\begin{equation}
\varphi_{j}\left(  \mathbf{r}_{1}\mathbf{,\xi}_{1}\right)  =\left\langle
...
\left\vert \varphi_{\mu}\right\rangle ;\quad1\leq\mu\leq2s.\label{FuncDerFock}%
\end{equation}
As
\begin{equation}
...
}\left(  \mathbf{\alpha}\right)  \right)  \right]  \right)  \right)
_{qp}.\label{derpq1}%
\end{equation}
...
2s}\left\vert \varphi_{k}\right\rangle \varepsilon_{kj}=0;\quad1\leq
...
g\left(  \mathbf{\eta},\mathbf{\lambda},\mathbf{\theta}\right)  ^{N}%
\right\vert \left[  H,a_{j}^{\dagger}a_{j+s}\right]  g\left(  \mathbf{\eta
},\mathbf{\lambda},\mathbf{\theta}\right)  ^{N}\right\rangle \nonumber\\
&  \qquad\qquad\qquad\qquad\qquad\left.  =\right.  \left\langle \left.
g\left(  \mathbf{\eta},\mathbf{\lambda},\mathbf{\theta}\right)  ^{N}%
\right\vert \left[  H,a_{j+s}^{\dagger}a_{j}\right]  g\left(  \mathbf{\eta
},\mathbf{\lambda},\mathbf{\theta}\right)  ^{N}\right\rangle \left.  =\right.
0
\end{align}
give
\begin{equation}
...
$c_{j}=c_{k}\Leftrightarrow n_{j}=n_{k}$ one can use the following non
...
\subsection{A Fock Identity\label{FockID}}
...
\end{equation}
leading to in the case of NGSO's to
\begin{equation}
\mathcal{E}\left(  \mathbb{D}^{2},\mathbf{\varphi}\right)  =\frac{1}{2\left(
...
\bibliography{agpopt,bw,Coleman,DMF1}

...
%TCIDATA{LastRevised=Monday, July 04, 2005 10:34:25}
...
eigevalues are equal to the derivatives of the exact total energy with respect
to the occupation numbers of the First Order Reduced Density Operator (FORDO)
of the model KS state. However many conceptual questions surround this
...
numbers. How are the eigenvalues related to the occupation numbers. How does
one define a KS\ functional that is differentiable with respect to occupation
numbers. 
...
occupation numbers and the special case of its value at integer occupation.  
...
domain $S_{N}$ and Appendix \ref{Operators} for the definition of
$\mathcal{B}_{1}\left(  \mathcal{H}^{N}\right)  ,$). The satisfisfaction of
the condition eq. $\left(  \ref{Number}\right)  $ for $D^{N}\in\mathcal{B}%
_{1}\left(  \mathcal{H}^{N}\right)  $ implies that the state is normalized
(see Appendix \ref{Normalization}) i.e.
\begin{equation}
\operatorname{Tr}D^{N}=1,
\end{equation}
and $D^{N}$ needs only to be constrained to lie in the cone (see Appendix
\ref{Operators}) of positive operators of $\mathcal{B}_{1}\left(
\mathcal{H}^{N}\right)  $. Thus $D^{N}$ can always be expressed in the form
\begin{equation}
D^{N}=QQ^{\dagger},
\end{equation}
...
chosen to be self adjoint and even positive.
...
b\right)  \mathbf{.}%
\end{equation}
The converse is also true. A set $\left\{  \left\vert y\right\}  \right\}  $
is an overcomplete basis if any element $g$ can be uniquely expanded as
\begin{equation}
...
However the optimization is carried out over states are not of these types,
i.e. thay have some density in the region of space where the potential is supported.
...
Q\left(  a,q\right)  \geq\mu_{Q}\left\Vert a\right\Vert ^{2}\right.
...
{}{a\left(  \lambda\right)  }\right)  \right\}  $ do not form a basis for the
orthogonal compliment of the tangent space. Is $D_{a}\Xi\left(  a_{0}\right)
$ surjective? $\lceil W$ is a proper subspace of $L_{1}\left(  \Delta\right)
$ if
\begin{equation}

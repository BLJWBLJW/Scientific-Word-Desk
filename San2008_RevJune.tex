...
%TCIDATA{LastRevised=Wednesday, September 10, 2008 14:01:50}
...
scientific endeavour. Perhaps one of the most ubiquitous examples of this is
the use of molecular orbitals in understanding structure, properties and
reactivity in chemistry and material science. This efficacy is due to the
...
\item orbitals provide a geometric representation of electrons and thus
...
their occupation numbers \emph{do not}, in general, determine a unique
multi-electron state. Thus one loses the concrete pictorial aspects of a
one-electron theory, as the multi-electron state \emph{cannot} be described
solely in terms of single electron quantities and geometric forms.
...
numbers. Originally the orbitals produced in KS-DFT\ were introduced as
mathematical artifacts, while recently \cite{Kummel2008} there has been some
discussion about their physical meaning as correlated orbitals but still their
connection to the $N$-electron state is not clear. However, there is a class
of correlated multi-electron states and hence their energies that can be
expressed exactly as known functionals of orbitals and occupation numbers,
they are the Antisymmetrized Geminal Power (AGP) States \cite{OrtizWeiner2006}.

The analysis of AGP states also clearly shows that a model based on orbitals
and occupation numbers is not equivalent to one based on FORDOs, as many sets
of orbitals determine the same FORDO while determining different $N$-electron
states. This observation allows one to explicitly construct examples of DMFs
whose domain is restricted to AGPs.

In this paper we review the definition of a correlated one-particle theory
(section \ref{OET}), the structure and parameterization of AGP\ states, their
$N$-electron energy functional, the expression of their energy in terms of
one-electron quantities and discuss how one can interpret them pictorially
(section \ref{AGP States}). In section \ref{DenMatFun} we define the AGP\ DMF
and in section \ref{Examples} we describe simple examples of (i) AGP states
that are determined by orbitals and occupation numbers and (ii) AGP DMFs and
we conclude with a discussion in section \ref{Discussion}.

As usual with AGPs, the presentation is restricted to systems with even number
of electrons. Extensions to odd numbers is sketched in section
\ref{Discussion}. A further caveat a reader should keep in mind is although an
exact quantum mechanical description of physical systems would require
infinite dimensional Hilbert spaces one generally considers finite dimensional
approximations. Thus the following material is expressed in terms of and
refers to finite dimensional Hilbert subspaces, with the understanding that
appropriate limits must be taken to obtain the exact infinite dimensional description.

\section{one-electron Theory\label{OET}}

In order to be unambiguous we introduce the following definition: a
one-electron Theory (OET) is one in which the

\begin{itemize}
\item $N$-electron ground state is \emph{totally determined} by a set of
orthonormal Generalized Spin Orbitals (GSOs), (see Appendix \ref{Spin}),
$\left\{  \left\vert \psi_{j}\right\rangle ;1\leq j\leq r;r\geq N\right\}  $
spanning one-particle space and a set of occupation numbers $\left\{
N_{j};1\leq j\leq r;r\geq N\right\}  $ of these GSOs
...
\end{align}
\emph{first} on the FORDO $D^{1}$
\begin{equation}
D^{1}=\sum_{1\leq j\leq r}N_{j}\left\vert \psi_{j}\right\rangle \left\langle
\psi_{j}\right\vert ,
\end{equation}
\emph{then} on the $N$-electron state density operator $D^{N}$ (assuming a
pure state)
\begin{equation}
...
\end{equation}
that contracts to $D^{1},$ where $\left\{  \left.  c_{j_{1}\cdots j_{N}%
}\right\vert 1\leq j_{1}<\,\cdots<j_{N}\leq r\right\}  $ are the CI
coefficients of the state vector forming $D^{N}$. One should note that $D^{N}$
is not in general diagonal in the basis $\left\{  \left.  \left\vert
\psi_{j_{1}}\cdots\psi_{j_{N}}\right\rangle \right\vert 1\leq j_{1}%
<\,\cdots<j_{N}\leq r\right\}  $ produced by the natural GSOs $\left\{
\left.  \psi_{j}\right\vert 1\leq j\leq r\right\}  $ of $D^{1}$. The subgroup
$U\left(  \boldsymbol{\theta}\right)  $ leaves $D^{1}$ invariant as
\begin{equation}
...
\psi_{j}\right\vert ,
\end{equation}
but it does not leave the state $D^{N}$ invariant (the off diagonal terms are
changed) as
\begin{align}
...
&  \neq D^{N}.
\end{align}
The non invariance of the $N$-electron state density operator will also be
shown in the following by simple examples that explicitly display the lack of
invariance of the Second Order Reduced Density Operator's (SORDO's) of
AGP\ states (thus any higher order density operators) to such transformations.
...
examples of such functionals will also be displayed in the following discussion.
...
is a complete orthonormal basis of GSO's, $\psi_{j+s}$ is the GSO
\emph{paired} with the GSO\ $\psi_{j}$, the canonical coefficients
...
equivalent pairs, $\mathcal{P},$ (see Appendix \ref{GSOpairs} and the next
subsection). This pairing \emph{can} have physical significance as in the BCS
theory of superconductivity \cite{Cooper1956,BCS1957,BCS2_1957} where
$\psi_{j}$ has the opposite spin and angular momenta to $\psi_{j+s},$ but can
also just reflect the basic antisymmetric properties of $\left\vert g\left(
\boldsymbol{c},\boldsymbol{\psi},\boldsymbol{\theta}\right)  \right\rangle $.
...
$, of paired normalized GSOs. For non degenerate AGPs theses are defined by
eq.$\left(  \ref{Equival}\right)  $
\begin{equation}
...
\end{equation}
For degenerate AGPs the equivalence is more involved and is discussed in
Appendix \ref{GSOpairs}. The occupation numbers $\left\{  N_{j}\left(
\boldsymbol{c}\right)  ;1\leq j\leq s\right\}  $ are given in terms of
$\left\{  n_{j}=c_{j}^{2};1\leq j\leq s\right\}  $ in a one-one fashion
\cite{Yukalov00} by
\begin{equation}
...
j}}n_{j_{1}}\cdots n_{j_{_{\frac{N}{2}-1}}},\label{sym2}\\
S_{0}\left(  \hat{\jmath}\right)   &  =1,\quad S_{m}\left(  \hat{\jmath
}\right)  =0,\,\quad m<0,\quad1\leq j\leq s, \label{sym3}%
\end{align}
and $\hat{\jmath}$ signifies that $j$ is missing from the sums in eqs.
$\left(  \ref{sym1})\text{ and }(\ref{sym2}\right)  $. For example in the case
$N=2$%
\end{subequations}
\begin{equation}
...
,\boldsymbol{\xi}\right)  $ in Hartree atomic units as
\begin{equation}
...
d\boldsymbol{r}d\boldsymbol{\xi} \label{H1}%
\end{equation}
and
\begin{align}
...
_{1}d\boldsymbol{\xi}_{2} \label{H2}%
\end{align}
respectively.
...
\end{align}
showing that the geminal is \emph{uniquely defined} by the GSOs $\left\{
...
In \cite{OrtizWeiner2006} it was shown that for AGPs states, $\left\vert
...
1\leq j\leq r\right\}  $, Ionization Potentials (IPs), $\left\{  \left.
...
with the optimized GSOs $\left\{  \left.  \left\vert \tilde{\psi}%
_{\#j}\right\rangle \right\vert 1\leq j\leq r\right\}  $. The tilde signifies
that the phase factor dependence has been absorbed into the GSOs as shown in
subsection \ref{OneElecPic}. The IP, $I\left(  \tilde{\psi}_{\#j}\right)  ,$
is the difference between the optimized $N$-electron state energy,
$\mathcal{E}\left(  \boldsymbol{c}_{\#},\boldsymbol{\tilde{\psi}}_{\#}\right)
$, and the $N$-$1$ electron state energy (not optimized), $\mathcal{E}%
_{N\,-1}\left(  \boldsymbol{c}_{\#},\boldsymbol{\tilde{\psi}}_{\#},\tilde
{\psi}_{\#j}\right)  $, formed by removing the the $j^{\text{th}}$ GSO from
the set of GSOs that determines the $N$-electron energy, and is given by
\begin{equation}
...
_{\#j}\right\rangle $. The energy $\mathcal{E}_{N\,-1}\left(  \boldsymbol{c}%
_{\#},\boldsymbol{\tilde{\psi}}_{\#},\tilde{\psi}_{\#j}\right)  $ is defined
in terms of second quantized operators $\left\{  \left.  a_{j}^{\dagger
}\right\vert 1\leq j\leq r=2s\right\}  $ that create the GSO's $\left\{
\left.  \tilde{\psi}_{\#j}\right\vert 1\leq j\leq r=2s\right\}  $ by
\begin{equation}
...
\end{equation}
where $H$ is the Hamiltonian defined by the operators whose matrix elements
are given by eqs. $\left(  \ref{H1}\right)  $ and $\left(  \ref{H2}\right)  $.
The one-electron expectation value $h_{1}\left(  \tilde{\psi}_{\#j}\right)  $
is the sum of the kinetic and external interaction energies and is given as
\begin{equation}
...
The one-electron independent particle model is based on GSOs obtained by
energy optimizing a single determinantal wave function, that have integer
occupation numbers. As the number of electrons and the number of orthonormal
GSOs $\left\{  \left.  \left\vert \psi_{j}\right\rangle \right\vert 1\leq
j\leq N\right\}  $ that define the IPS $N$-electron state are equal the
magnitudes squared of the GSOs can be identified with the Probability
Densities (PDs)
\begin{equation}
...
\end{equation}
which can then interpreted as charge clouds. These GSOs are not unique as
$N$-electron IPSs are invariant to the unitary group $U\left(  N\right)  $
that map the $N$-dimensional subspace generated by $\left\{  \left.
\left\vert \psi_{j}\right\rangle \right\vert 1\leq j\leq N\right\}  $ to
itself. Hence any set of orthonormal GSOs
\begin{equation}
\left\{  \left.  u\left\vert \psi_{j}\right\rangle \right\vert 1\leq j\leq
N\right\}  ;\quad u\in U\left(  N\right)  \label{UGSO}%
\end{equation}
and their associated PDs, \emph{which in general have different shapes}, give
...
operator, which is represented in the basis of optimized GSOs by a matrix
which is divided into two blocks one $N\times N$ (the occupied GSOs) the other
$\left(  r-N\right)  \times\left(  r-N\right)  $ (the virtual GSOs). Canonical
occupied GSOs can then be defined as the eigenvectors of the occupied block by
finding a $u\in U\left(  N\right)  $ that diagonalizes this block. The
associated eigenvalues are ionization potentials associated with these
canonical GSOs. As both the FORDO of the optimized IPS and the state itself
are invariant to the group $U\left(  N\right)  $ the PDs produced by the
canonical GSOs can then be used to form a standard pictorial description of
the electronic distribution. However, from basic quantum mechanical
considerations the canonical GSOs and the resulting PDs are not any more
...
1\leq j\leq N\right\}  $ are the eigenvalues of the occupied GSOs of the Fock
...
electrons described by AGP states, which contain IPSs as a special case. Now,
however, the $N$-electron state is determined by more than $N$ GSOs with non
integer occupation numbers and thus one cannot associate an individual
electron with a specific PD. It is still possible to visualize the structure
in terms of electrons distributed according to the PDs produced by the
representative canonical orbitals \emph{of the optimized geminal}, but now
...
_{\#j}\right)  -1=0
\end{equation}
as there may be no solution. The GSOs $\left\vert \tilde{\psi}_{\#j}%
...
next most energetic electron can be obtained in an analogous fashion.
...
\item the difference between an OET and a FORDO theory, which consequently
leads to the construction of an explicit form for a DMF.
\end{itemize}
...
restrict attention to geminals that are low spin eigenfunctions of the spin
operator $\mathcal{S}_{0}$ (see Appendix \ref{SpinA}) i.e.
\begin{equation}
...
\begin{equation}
s=2;\quad c_{1}=\frac{1}{\sqrt{2}},c_{2}=\frac{1}{\sqrt{2}};\quad\theta
_{1}=0,\theta_{2}=0
\end{equation}
producing%
\begin{equation}
...
\begin{equation}
j\equiv\varphi_{j};\quad1\leq j\leq2s
\end{equation}
...
\label{D2-}%
\end{equation}
The relationship between the SORDOs eqs. $\left(  \ref{D2+},\ref{D2-}\right)
...
\boldsymbol{\psi}\right]  $ (recall the discussion at eq. $\left(
\ref{Equival}\right)  )$, but the phase dependence (in these cases the phases
$\theta_{1}=0$ in eqs. $\left(  \ref{Tilde1},\ref{Tilde2}\right)  $,
...
\end{align}
The FORDO, SORDOs and energies can be obtained from eqs. $\left(
...
\left\{  e^{i\theta}\left(  \left.  13\right\vert 24\right)  \sin2\chi\right\}
\nonumber\\
...
We have shown how the universally used orbital picture of electrons that was
rigorously derived for IPSs i.e. for uncorrelated systems, but often used for
correlated systems, can in fact be rigorously extended to a description of
correlated ones. This allows one to maintain the single electron description
of a multi-electron system and construct a pictorial model as outline in
subsection \ref{OrbModel}. The manifold of $N$-electron states that are
determined by GSOs and their occupation numbers are formed by AGP\ states,
...
ground states that have a lower energy than IPSs.

OETs are more general than FORDO theories as GSOs with the same occupation
...
\subsection{General Spin Orbitals\label{SpinA}}

GSOs, $\left\vert \psi\right\rangle $, are not eigenstates of the spin
operator, $\mathcal{S}_{0},$ which is defined in second quantization as
\begin{equation}
\mathcal{S}_{0}=\sum_{1\leq j\leq s}a_{j\alpha}^{\dagger}a_{j\alpha}%
-\sum_{1\leq j\leq s}a_{j\beta}^{\dagger}a_{j\beta},
\end{equation}
where the fermion second quantized operators $\left\{  \left.  a_{j\sigma
}^{\dagger}\right\vert 1\leq j\leq s;\sigma=\alpha,\beta\right\}  $ are
defined by the action on the vacuum state $\left\vert \phi\right\rangle $ by
\begin{equation}
a_{j\sigma}^{\dagger}\left\vert \phi\right\rangle =\left\vert \varphi
_{j}\sigma\right\rangle ,\nonumber
\end{equation}
where $\left\vert \varphi_{j}\sigma\right\rangle $ is a product of position
and spin states and the anti commutation relationships
\begin{equation}
\left[  a_{j\sigma}^{\dagger},a_{k\nu}\right]  _{+}=a_{j\sigma}^{\dagger
}a_{k\nu}+a_{k\nu}a_{j\sigma}^{\dagger}=\delta_{jk}\delta_{\sigma\nu}.
\end{equation}
GSOs contain non zero components of both alpha and beta spin $\left\{  \left.
\left\vert \varphi_{j}\alpha\right\rangle ,\left\vert \varphi_{j}%
\beta\right\rangle \right\vert 1\leq j\leq s\right\}  $ i.e.%
\begin{equation}
...
\right\}  ,
\end{equation}
which individually are eigenstates of $\mathcal{S}_{0}$ that satisfy
\begin{equation}
...
\subsection{Pure Spin States \label{SpinB}}

Pure spin states are eigenstates of the spin squared operator, $\mathcal{S}%
^{2}$, which is given by
\begin{equation}
...
=S_{+}^{\dagger}.
\end{align}
The action of $\mathcal{S}^{2}$ on the geminals $\left\vert g_{\pm
}\right\rangle $ is given by
\begin{align}
...
eigenstates of $\mathcal{S}^{2}$. However if one sets $\varphi_{3}=\varphi
_{1}$ and $\varphi_{4}=\varphi_{2}$ to form restricted geminals one obtains
\begin{equation}
\mathcal{S}^{2}\left\vert g_{\text{res},\pm}\right\rangle =0,
\end{equation}
showing that $\left\vert g_{\pm}\right\rangle $ are low spin singlet
eigenstates of $\mathcal{S}^{2}$ that factor into a product of space and spin
parts i.e.
\begin{equation}
...
If the canonical coefficients are nondegenerate, i.e. unequal, the elements of
the equivalence classes $\left[  \boldsymbol{\psi}\right]  $ are related to
each other by transformations, $u$ with matrix representations $\mathbb{U},$
that belong to the special unitary group $SU\left(  2\right)  $ that transform
$\mathbb{C}^{2}$ to itself, i.e.%
\begin{equation}
\left\{  \psi_{a},\psi_{b}\right\}  \sim\left\{  \psi_{c},\psi_{d}\right\}
\Leftrightarrow\left(  \psi_{a},\psi_{b}\right)  =u\left(  \psi_{c},\psi
_{d}\right)  =\left(  u\psi_{c},u\psi_{d}\right)  =\left(  \psi_{c},\psi
_{d}\right)  \mathbb{U}. \label{NonDegEq}%
\end{equation}
This reflects the invariance of geminals $\left\vert g\right\rangle $ to
transformations $\tilde{u}$ of one-electron space
\begin{equation}
\tilde{u}\left(  \left\vert \psi_{1}\right\rangle ,\left\vert \psi
_{1+s}\right\rangle ,\cdots,\left\vert \psi_{s}\right\rangle ,\left\vert
\psi_{s+s}\right\rangle \right)  =\left(  \left\vert \psi_{1}\right\rangle
,\left\vert \psi_{1+s}\right\rangle ,\cdots,\left\vert \psi_{s}\right\rangle
,\left\vert \psi_{s+s}\right\rangle \right)  \left(  \left(
\begin{array}
[c]{cccc}%
\mathbb{U}_{1} & \mathbb{O} & \cdots & \mathbb{O}\\
\mathbb{O} & \ddots & \vdots & \vdots\\
\vdots & \vdots & \ddots & \mathbb{O}\\
\mathbb{O} & \cdots & \mathbb{O} & \mathbb{U}_{s}%
\end{array}
\right)  \right)  ,
\end{equation}
where $\left\{  \mathbb{U}_{j}\in2\times2\text{ matrix representation of
}SU\left(  2\right)  ;1\leq j\leq s\right\}  .$ The invariance is displayed
by
\begin{align}
...
\end{align}
as $\left\{  \psi_{j},\psi_{j+s}\right\}  \sim\left\{  \psi_{j}^{\prime}%
,\psi_{j+s}^{\prime}\right\}  $ for $1\leq j\leq s$, where $\Lambda\left(
\tilde{u}\right)  $ is the induced representation of $\tilde{u}$ on
two-electron space. In the nondegenerate case the set of all such
transformations forms the maximal invariance group of $\left\vert
g\right\rangle .$

If degeneracies exist the maximal invariance group and thus the equivalence
classes $\left[  \boldsymbol{\psi}\right]  $ are larger, as can be seen from
the following discussion. One can expand a geminal in a form that explicitly
displays degeneracies as
\begin{equation}
\left\vert g\left(  \boldsymbol{c},\boldsymbol{\psi},\boldsymbol{\theta
...
$s_{j}=\sum\limits_{1\leq l\leq j}d_{l},$ and $s_{0}=0.$ The simplest example
of a degenerate geminal can be realized by setting $s=3,d=2,\left\{
d_{1},d_{2}\right\}  =\left\{  2,1\right\}  ,$ i.e.
\begin{equation}
\left\vert g\left(  \boldsymbol{c},\boldsymbol{\psi},\boldsymbol{\theta
...
\label{DegGem}%
\end{equation}
The equivalence class of pairs for the geminal in eq. $\left(  \ref{DegGem}%
\right)  $ can be divided into two sets
\begin{equation}
\left[  \boldsymbol{\psi}\right]  =\left[  \left\{  \psi_{1},\psi_{4}\right\}
,\left\{  \psi_{2},\psi_{5}\right\}  ,\left\{  \psi_{3},\psi_{6}\right\}
\right]  =\left[  \left\{  \left\{  \psi_{1},\psi_{4}\right\}  ,\left\{
\psi_{2},\psi_{5}\right\}  \right\}  ,\left\{  \psi_{3},\psi_{6}\right\}
\right]  .
\end{equation}
The\ portion of $\left[  \boldsymbol{\psi}\right]  $ associated with $\left\{
\psi_{3},\psi_{6}\right\}  $ can be generated according to eq.$\left(
\ref{NonDegEq}\right)  ,$ however the portion associated with the degenerate
pairs $\left\{  \psi_{2},\psi_{5}\right\}  $ and $\left\{  \psi_{3},\psi
_{6}\right\}  $ is more difficult to describe and one must proceed in the
fashion outlined in \cite{OrtizWeiner2006,Weiner83a,AGPCoherent}. To this end
one defines a set of operators $\left\{  \boldsymbol{u},\boldsymbol{v}%
\right\}  $ in terms of second quantized operators $\left\{  \left.
a_{j}^{\dagger}\right\vert j=1,2,4,5\right\}  $ corresponding to the
orthonormal GSOs $\left\{  \left.  \psi_{j}\right\vert j=1,2,4,5\right\}  $
associated with the degenerate canonical coefficient $c_{1}$ as%
\begin{equation}
\left(
\begin{array}
[c]{c}%
...
geminal, hence all the GSOs $\left\{  USp\left(  4\right)  \left\vert
\boldsymbol{\psi}^{\prime}\right\rangle \equiv USp\left(  4\right)  \left(
\left\vert \psi_{1}\right\rangle ,\left\vert \psi_{2}\right\rangle ,\left\vert
\psi_{4}\right\rangle ,\left\vert \psi_{5}\right\rangle \right)  \right\}  $
belong to the same equivalence class $\left[  \boldsymbol{\psi}\right]  ,$
which generalizes the equivalence relationship given in eq. $\left(
\ref{Equival}\right)  .$ The set $\boldsymbol{u}^{\prime}$, formed in an
analogous manner to the set $\boldsymbol{v}^{\prime}$, is contained in a
representation of the Lie algebra $u\left(  4\right)  $ that produces a
representation of the group $U\left(  4\right)  $ that changes $\left\vert
g\right\rangle $

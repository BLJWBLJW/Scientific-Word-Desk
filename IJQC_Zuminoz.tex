\documentclass[preprint,aps,showkeys]{revtex4}%
\usepackage{amsfonts}
\usepackage{amsmath}
\usepackage{amssymb}
...
%TCIDATA{LastRevised=Thursday, June 26, 2003 17:21:49}
...
\keywords{Antisymmtrized geminal power, general spin orbitals, canonical form,
correlated one particle theory}
...
leads to expansions of antisymmetrized geminal power states solely in terms of
...
propagator \cite{weiner87,Weiner85,Weiner84a,Kurtz83,Jorgen83} in the
simultaneous production of dissociation curves for several low lying excited
states of light diatomic molecules which were in good agreement with
experimental results. Although AGP states are rarely the exact solutions of
the Schr\"{o}dinger equation they can serve as an unbiased, approximate ansatz
generalizing the Independent Particle State (IPS) approach by including a
better description of electron - electron interaction, while at the same time
maintaining a balanced and physically interpretable model of both molecular
and extended systems. In this paper we show that a $2N$-electron AGP\ state is
completely determined by geminal occupation numbers and Natural General Spin
Orbitals (NGSOs), two quantities that have a clear physical significance. A
$2N$-electron AGP\ state is the antisymmetrized product of $N$ identical
$2$-electron geminals. Thus the properties and form of the geminal determine
the characteristics of the product\ state, e.g. the expectation value
expressions and the various reduced density operators, and will form the focus
of this paper.

The methods used to optimize the energy of an AGP state in the previously
cited references were not numerically robust nor easily extendable to large
systems. Recently we have \cite{Weiner&Ortiz02} described a method that
overcomes these numerical difficulties and can be applied to molecules of
various sizes. A salient feature of this procedure is that the geminal
parameters always refer to a \emph{fixed} localized atomic orbital basis,
which allows one to visualize the AGP state in a chemically intuitive manner
and to introduce approximations to the integral matrix in a natural way. At
the same time, the various expressions for the energy and its gradient are
more conveniently formulated in terms of the canonical form (canonical
orbitals and coefficients) of the geminal, which was first discussed in a
quantum chemical context in references \cite{Coleman63} and \cite{Lowdin56}.
...
$\left\{  \chi_{k}\right\}  $ are a set of canonical NGSOs and $r=2s$ is the
size of the basis set. One can view the expansion eq. $\left(  \ref{gstate}%
\right)  $ in the coordinate representation in terms of normalized $2\times2$
...
canonical NGSOs, a nonuniqueness that is common in quantum chemistry amongst
eigenfunctions that are degenerate. It has not been broadly recognized that
the canonical coefficients $\left\{  \zeta_{k}\right\}  $ are defined only up
to a phase factor for
\begin{equation}
...
AGP\ state and the coefficients of its configurational interaction expansion.
As a consequence the energy of the $2N$-electron AGP\ state, and its various
reduced density operators, can be expressed solely in terms of the one
electron canonical generalized spin orbitals and the occupation numbers of the
geminal. Hence the AGP\ ansatz produces a one electron theory of correlated particles.
...
expansion coefficients. In section \ref{zumino} , Zumino's theorem and two
corollaries are developed in detail. In section \ref{numerical}, we describe a
...
procedure. In section \ref{discussion}, the ramifications of our analysis and
results are examined. We present a conclusion in section \ref{conclusion}.

\section{Geminals and Antisymmetric Complex Matrices\label{geminal}}

In this section we describe how one forms the matrix of NGSOs that bring the
complex antisymmetric matrix of geminal coefficients $\mathbf{c}$\textbf{ }to
block diagonal form (see section \ref{zumino}). It is important to note that
NGSO refers to spin orbitals having both alpha and beta spin components. This
is necessary on two fronts, one mathematical in order to obtain the correct
pairing associated with antisymmetry, and one practical in order to form the
most flexible approximation ansatz. To produce the NGSOs, one needs to
construct the FORDO of the $2$ -particle geminal state $\left\vert
g\right\rangle $ and then sum the indices relating to one of the
...
The contraction map \textrm{L}$_{2}^{1}$ \cite{Weiner84,kummar70,Lowdin55},
acting on $\left\vert g\right\rangle \left\langle g\right\vert $ produces
\begin{align}
...
\varphi_{j_{1}}\right\rangle \left\langle \varphi_{k_{1}}\right\vert .
\end{align}
The FORDO, $D^{1}\left(  g\right)  ,$ normalized to the number of electrons in
the system, (in this case $2)$, is then given by
\begin{equation}
D^{1}\left(  g\right)  =2\mathrm{L}_{2}^{1}\left(  \left\vert g\right\rangle
\left\langle g\right\vert \right)  .
\end{equation}
The First Order Reduced Density Matrix (FORDM), $\mathbb{D}_{\varphi}%
^{1}\left(  g\right)  ,$ which is the \emph{representation} of $D^{1}\left(
g\right)  $ in the $\left\{  \mathbf{\varphi}\right\}  $ basis is given by
\begin{equation}
\mathbb{D}_{\varphi}^{1}\left(  g\right)  =\frac{1}{2}\mathbf{cc}^{\dagger
}=\mathbf{\tilde{c}\tilde{c}}^{\dagger}.
\end{equation}
Both have a trace equal to $2,$ i.e.
\begin{equation}
...
\end{align}
The matrix $\mathbf{c}_{U}$ therefore has the block diagonal form
\begin{equation}
\mathbf{c}_{U}=\left(
\begin{array}
[c]{cccc}%
...
If $\eta_{\kappa}\neq0,$ then $\mathbb{B}_{\kappa}=\frac{1}{\sqrt{\eta
_{\kappa}}}\mathbf{c}_{\kappa}$ is a $d_{\kappa}\times d_{\kappa}$
antisymmetric unitary matrix
\begin{align}
...
\right)  ,
\end{equation}
which is valid even when $\eta_{\kappa}=0$, (note that $\boldsymbol{\theta
}_{\kappa}\equiv\left(  \theta_{\kappa1},\ldots,\theta_{\kappa d_{\kappa}%
}\right)  $ and $\theta_{\kappa i}\neq\theta_{\kappa j}$ in general), where
\begin{equation}
...
\end{align}
One can obtain a nonunique canonical expansion by setting
\begin{align}
...
\end{align}
thus recovering the form obtained by the main Zumino theorem. The
nonuniqueness of the complex canonical coefficients is then easily seen by
noting that
\begin{equation}
\left\vert g\right\rangle =\sum_{1\leq k\leq s}n_{k}^{\frac{1}{2}}%
...
\begin{proof}
Eq. $\left(  \ref{dub1}\right)  $ shows that if the eigenvalue $n_{j}$ is
nondegenerate then $c_{Ujk}=0;1\leq k\leq r$. Eq. $\left(  \ref{dub2}\right)
$ shows in particular that for such nondegenerate eigenvalues $\left\vert
c_{Ujj}\right\vert ^{2}=n_{j}=0$, thus all nondegenerate eigenvalues must be
equal to $0.$
...
antisymmetric, requirements that can easily be satisfied by nonzero
...
eigenvalues must be +/- pairs or zero). This is a contradiction as the matrix
$\mathbb{B}$ is also unitary and so cannot have zero eigenvalues. Thus the
block cannot be $3\times3,$ and the same argument holds for any odd
dimensioned block. Hence only even dimensional blocks can exist.
\end{proof}
\end{corollary}
...
g\right)  =\frac{1}{2}\mathbf{cc}^{\dagger}$ and then diagonalize it by a
unitary transformation $\mathbb{Y}$. One then transforms $\mathbf{c}$ by
...
produced from each block $\mathbf{c}_{\kappa}$ associated with a nonzero
eigenvalue by
\begin{equation}
...
diagonalize the other in the eigenbasis of the first. However, if the
dimensions of the nonzero blocks $\left\{  \mathbb{B}_{R},\mathbb{B}%
_{I}\right\}  $ are greater than $2$ e.g. $4,6,$ $\cdots,$ etc., there are
more than one positive and one negative eigenvalues (there are always equal
...
manner. Therefore one would need to formulate some principle of correspondence
and to apply a computational procedure to accomplish this.
...
orthogonal. The way to overcome this nonorthogonality and pairing problem is
...
\right)  . \label{reex1}%
\end{equation}
This result allows the construction of a unitary matrix $\widetilde
{\mathbb{O}}$ that brings $\mathbb{B}$ to a unique canonical form by defining
\begin{equation}
...
\right)  .
\end{equation}
The matrix $\mathbf{c}$ can then be expressed in terms of its block diagonal
components as
\begin{align}
...
In previous discussions of AGP states
\cite{Lindeberg77,Lindeberg79,Weiner85b,Coleman63,Lindeberg80} various subtle
properties of the canonical form of a geminal have been overlooked or not
emphasized, in particular it has not been widely advertised that the canonical
coefficients of the canonical forms usually displayed are not uniquely
defined. As we have shown in this paper it is, however, possible to construct
a unique set of canonical coefficients, by using a corollary in the Zumino
paper \cite{Zumino62} and an associated set of canonical NGSOs. This allows
the energy the of $2N$-electron AGP\ state, and its various reduced density
operators, to be expressed solely in terms of the canonical orbitals and the
occupation numbers of the geminal. Hence the AGP\ ansatz produces a one
electron theory of correlated particles and as such allows one to build
conceptual models in the vein of the Independent Particle ansatz, but now
including explicit interactions between the particles.
...
We would like to acknowledge the support of the Petroleum Research Foundation
of the American Chemical Society (grant PRF 34859-AC6).
...
\end{equation}
This product has no exchange symmetry but facilitates contractions as one
unrestricted summation over a single subscript. If $\varphi_{1}$ and
$\varphi_{2}$ are normalized then so is $\varphi_{1}\otimes\varphi_{2}$. The
two particle state wavefunction that is the antisymmetric tensor product
$\varphi_{1}\wedge\varphi_{2}\left(  \mathbf{r}_{1},\mathbf{r}_{2}\right)  $
of two one-particle state wavefunctions $\left\{  \varphi_{1}\left(
\mathbf{r}\right)  ,\varphi_{2}\left(  \mathbf{r}\right)  \right\}  $ is given
by
\begin{equation}
\varphi_{1}\wedge\varphi_{2}\left(  \mathbf{r}_{1},\mathbf{r}_{2}\right)
...
unitary transformation. Hence we show in the following that $\mathbb{N}$ and
$\mathbb{M}$ can simultaneously be brought to a canonical antisymmetric form
by using the same real orthogonal transformation that is constructed according
...
}^{\ast}\right\rangle \right)  $, can be viewed as a unitary matrix whose
...
\label{adag}%
\end{align}
Therefore, there exist simultaneous eigenvectors $\left\{  \mathbf{v}%
_{j}\right\}  $ to $\mathbb{A}$ and $\mathbb{A}^{\dagger}$ corresponding to
complex conjugate eigenvalues $\left\{  \alpha,\alpha^{\ast}\right\}  $ i.e.%
\end{subequations}
\begin{subequations}
\begin{align}

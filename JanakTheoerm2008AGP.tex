...
%TCIDATA{LastRevised=Tuesday, April 29, 2008 13:50:38}
...
\begin{document}
\preprint{ }
\title[Janaks Theorem]{Janaks Theorem and Orbital Energy Functionals }
\author{B. Weiner}
...
the formalism of super operators (section 2). This will allow us to express
the Levy-Lieb construction of energy functionals as an eigenvalue problem. In
section 3 we derive Janaks Theorem in the context of a constrained variational
approach and sensitivity theory. The KS\ density functional, density matrix
functional and the AGP functional determine the total energy in terms of a set
of occupations numbers and one particle orbitals. The first two have only been
defined implicitly while the AGP functional has been explicitly constructed
\cite{OrtizWeiner2006} in terms of General Spin Orbitals (GSO's), (see section
4). In section 5 we investigate the form of the derivatives of this functional
with respect to occupation numbers and the special case of its value at
integer occupation. In section 6 we conclude with a discussion and summary.

\section{Density Functionals \label{Standard}}

In this section we review some fundamental background needed to define HK and
KS density functionals. We will mention important technical mathematical
aspects that need to be examined, but defer a more detailed analysis and will
focus for simplicity on the mathematically most well behaved cases.

\subsection{Hohenberg-Kohn DFT\label{HKDFT}}
...
\subsubsection{Densities and States\label{DenStat}}

Density Functional Theory is based on the existence of a universal functional,
$\mathcal{F}\left(  \rho\right)  $ \cite{HK1964} of the charge density
...
\begin{equation}
\int\rho\left(  \boldsymbol{r}\right)  d\boldsymbol{r}=N, \label{Number}%
\end{equation}
...
describing different number of electrons. However in this article we focus on
$N$-electron states and define convex sets, $\left\{  \left[  \rho\right]
...
\end{equation}
As $D$ is a positive operator it can always be expressed as the square
\begin{equation}
D=A^{2},
\end{equation}
where
...
Hilbert-Schmidt operators acting on $N$-electron space $\mathcal{H}^{N}$. The
operator $A$ is the square root of $D$, which is not unique, as can be
observed from the spectral expansion
\begin{equation}
...
\end{equation}
$r_{N}=$ Dimension of $\mathcal{H}^{N}$ and $\left\{  \left.  \left\vert
\Upsilon_{j}\right\rangle \right\vert 1\leq j\leq r_{N}\right\}  $ is a
complete orthonormal set of eigenvectors of $D$. One could restrict $A$ to be
positive i.e. use the positive square roots in eq. $\left(  \ref{specA}%
\right)  ,$ but that would be too restrictive for our purposes, as the set of
positive Hilbert Schmidt operators forms a cone \cite{Encyclo} but do not form
a linear space. However, the multi valuedness of the square root function does
not create a problem as the different roots are separated from each other and
could only form \emph{isolated} local minimizers of the total energy
functional. The domain of variation of the $N$-electron energy functional can
thus be taken to be the \emph{real} Hilbert space $\mathcal{B}_{2sa}\left(
\mathcal{H}^{N}\right)  ,$ of dimension $r_{N}^{2},$ which has an orthonormal
bases
\begin{align}
...
\subsubsection{The Density Constraint\label{DenCon}}

The Levy-Leib construction \cite{MelLevy12151979,Lieb1983} is based on
defining the value of the density functional as the minimum of a energy
variation over states constrained to have a fixed density. The density
constraint can be formulated in terms of a quadratic functional $\Xi_{2},$
acting on the subspace of self adjoint Hilbert-Schmidt operators
$\mathcal{B}_{2sa}\left(  \mathcal{H}^{N}\right)  $, based on the linear
functional $\Xi_{1},$ where
\begin{align}
\Xi_{2}  &  :\mathcal{B}_{2sa}\left(  \mathcal{H}^{N}\right)  \rightarrow
L_{1+}\left(  \mathbb{R}^{3}\right)  \subset L_{1}\left(  \mathbb{R}%
^{3}\right) \nonumber\\
\Xi_{2}\left(  A\right)   &  =\operatorname{Tr}\left\{  A\Phi^{\dagger}\Phi
A\right\}  =\left(  A\left\vert \Phi^{\dagger}\Phi A\right.  \right)  =\rho,
\label{QuadCon}%
\end{align}
$L_{1+}\left(  \mathbb{R}^{3}\right)  $ is the positive cone of $L_{1}\left(
\mathbb{R}^{3}\right)  ,$ i.e. the set of positive absolute integrable
...
terms of the \emph{generalized} \emph{vector} super operator $\widehat
{\Phi^{\dagger}\Phi}$%
\begin{equation}
\Xi_{2}\left(  A\right)  =\left(  A\left\vert \widehat{\Phi^{\dagger}\Phi
}A\right.  \right)  =\rho,
\end{equation}
which has components, $\widehat{\Phi\left(  \boldsymbol{r}\right)  ^{\dagger
}\Phi\left(  \boldsymbol{r}\right)  },$ that are multiplication super operators.

As the operators $\left\{  \widehat{\Phi\left(  \boldsymbol{r}\right)
^{\dagger}\Phi\left(  \boldsymbol{r}\right)  }\right\}  $ are bounded one can
form the differential \cite{Encyclo}
\begin{equation}
...
A\right)  =\rho$ is regular everywhere.
...
\cite{MelLevy12151979,Lieb1983} as
\begin{equation}
...
}\left(  \rho\right)  \left\vert \widehat{\left(  K+V_{2}\right)  }A_{\ast
}\left(  \rho\right)  \right.  \right)  =E_{U}\left(  A_{\ast}\left(
\rho\right)  \right) \label{HKQa}\\
\Xi_{2}\left(  A\right)   &  =\left(  A\left\vert \widehat{\Phi^{\dagger}\Phi
}A\right.  \right)  =\rho, \label{HKQb}%
\end{align}
when the control densities $\rho\in\mathcal{D},$where
\end{subequations}
\begin{equation}
\mathcal{D}=\left\{  f\left\vert f\in L_{1}\left(  \mathbb{R}^{3}\right)
;\,f\left(  \boldsymbol{r}\right)  \geq0;\int f\left(  \boldsymbol{r}\right)
d\boldsymbol{r}=N\right.  \right\}  .
\end{equation}
This choice of the density $\rho$ coupled with insisting that $A\in
...
\end{equation}
the Lagrange multiplier function $\lambda$ has the effect
\begin{equation}
\left\langle \left.  \lambda\right\vert \left\{  \Xi_{2}\left(  A\right)
-\rho\right\}  \right\rangle =\int\lambda\left(  \boldsymbol{r}\right)
\left\{  \Xi_{2}\left(  A\right)  \left(  \boldsymbol{r}\right)  -\rho\left(
\boldsymbol{r}\right)  \right\}  d\boldsymbol{r,\,}\lambda\in L_{\infty
}\left(  \mathbb{R}^{3}\right)  ,\rho\in L_{1}\left(  \mathbb{R}^{3}\right)  ,
\end{equation}
$\left\langle \cdot|\cdot\right\rangle $ is the scalar product between the
dual spaces $\left\langle L_{\infty}\left(  \mathbb{R}^{3}\right)
,L_{1}\left(  \mathbb{R}^{3}\right)  \right\rangle $ i.e.
\begin{equation}
\left\langle \cdot|\cdot\right\rangle :L_{\infty}\left(  \mathbb{R}%
^{3}\right)  \times L_{1}\left(  \mathbb{R}^{3}\right)  \rightarrow\mathbb{R}%
\end{equation}
and the control density $\rho$ is chosen to belong to the convex set
$\mathcal{D}.$ The constraint term in the Lagrangian eq. $\left(
\ref{Lar}\right)  $ can be developed to give
\begin{equation}
\left\langle \lambda\left\vert \rho\right.  \right\rangle -\left(  A\left\vert
\widehat{\Omega\left(  \lambda\right)  }\right.  A\right)  =\left(
A\left\vert \left\{  \left\langle \lambda\left\vert \rho\right.  \right\rangle
\hat{I}-\widehat{\Omega\left(  \lambda\right)  }\right\}  \right.  A\right)  ,
\end{equation}
...
\end{equation}
and the associated super operator is given by
\begin{equation}
...
\begin{equation}
\mathcal{L}\left(  A,\lambda,\rho\right)  =\left(  A\left\vert \widehat
{\left(  K+V_{2}-\Omega\left(  \lambda\right)  \right)  }A\right.  \right)
+\left\langle \lambda\left\vert \rho\right.  \right\rangle .
\end{equation}


\subsubsection{Optimization Conditions\label{OpCon}}
...
\mathcal{H}^{N}\right)  \times L_{\infty}\left(  \mathbb{R}^{3}\right)
\rightarrow\mathbb{R}%
\end{equation}
is given by%
\begin{equation}
...
=2\left(  \cdot\left\vert \widehat{\left\{  K+V_{2}-\Omega\left(
...
&  =0\,\forall\varphi\in L_{\infty}\left(  \mathbb{R}^{3}\right)
.\label{statb}%
\end{align}
...
}\left(  \mathbb{R}^{3}\right)  $ these conditions are equivalent to
\end{subequations}
\begin{equation}
...
}\left(  \rho\right)  \left\vert \left\{  \widehat{\left\{  K+V_{2}%
-\Omega\left(  \lambda_{\ast}\right)  \right\}  }+\left\langle \left.
\lambda_{\ast}\left(  \rho\right)  \right\vert \rho\right\rangle \hat
{I}\right\}  \right.  A_{\ast}\left(  \rho\right)  \right)  =\left\langle
\left.  \lambda_{\ast}\left(  \rho\right)  \right\vert \rho\right\rangle
\nonumber\\
&  =\left(  A_{\ast}\left(  \rho\right)  \left\vert \widehat{K+V_{2}}\right.
A_{\ast}\left(  \rho\right)  \right)  =E_{U}\left(  A_{\ast}\left(
\rho\right)  \right)  .
\end{align}
...
\subsubsection{Sensitivity Analysis\label{SenAnal}}
...
\subsubsection{Decomposition of the Hohenburg-Kohn Functional\label{DecompHK}}
...
\subsubsection{Molecular System Energy\label{MolSysEn}}
...
\end{equation}
which one can make unconstrained in terms of real square integrable functions
$f$ by setting
\begin{equation}
...
L_{2}\left(  \mathbb{R}^{3}\right)  . \label{rho_f}%
\end{equation}
...
\subsection{Orbital Variables \label{GSO}}
...
\subsubsection{Correspondence between Orbitals and Densities\label{Orb_Den}}
...
\right]  \left(  \boldsymbol{r,r}\right)  =\sum_{1\leq j\leq r_{1}}%
N_{j}\left\vert \psi_{j}\left(  \boldsymbol{r}\right)  \right\vert
^{2},\label{NGSOexpan}%
\end{equation}
...
\chi_{j}\right\vert 1\leq j\leq N\right\}  $. In general one can express the
density as
\begin{equation}
\rho\left(  \boldsymbol{r}\right)  =\sum_{1\leq j\leq\nu}\left\vert
\varphi_{j}\left(  \boldsymbol{r}\right)  \right\vert ^{2},
\end{equation}
where
\[
...
the expansion. If one is explicitly considering spin effects then one can
replace orbitals by Spin Orbitals (SO's) and General Spin Orbitals (GSO's),
(see Appendix \ref{Spin}).

In general a density $\rho$ can be expressed in two ways, one in terms of an
orthogonal set of orbitals $\boldsymbol{\varphi}=\left(  \varphi_{1}%
,\cdots,\varphi_{r_{1}}\right)  \boldsymbol{,}$ normalized to the occupation
numbers $\boldsymbol{N}$, the other in terms of an orthonormal set
$\boldsymbol{\tilde{\varphi}}$ and occupation numbers $\boldsymbol{N}$ that
are considered to be independent variables. This can be made explicit through
the introduction of two functionals $\Gamma_{1}$ and $\Gamma_{2}$ that produce
a density that is normalized to the number of electrons $N.$ The first
functional $\Gamma_{1}$ is defined by
\begin{equation}
\Gamma_{1}:\mathfrak{S}_{1}\subset\prod\limits_{1\leq j\leq r_{1}}%
\mathcal{H}^{1}\rightarrow L_{1+}\left(  \mathbb{R}^{3}\right)  \subset
...
\boldsymbol{r}\right)   &  =\sum_{1\leq j\leq r_{1}}\left\vert \varphi
_{j}\left(  \boldsymbol{r}\right)  \right\vert ^{2}=\rho\left(  \boldsymbol{r}%
\right)  \label{Gamma}%
\end{align}
and%
\end{subequations}
...
N_{j}\leq N,1\leq j,k\leq r_{1};\sum_{1\leq j\leq r_{1}}N_{j}=N\right.
\right\}  .
\end{equation}
As many as $r_{1}-1$ of the component functions, $\varphi_{j},$ may be
...
\begin{equation}
\Gamma_{2}:\mathfrak{S}_{2}\subset\prod\limits_{1\leq j\leq r_{1}}%
\mathcal{H}^{1}\times\mathbb{R}^{r_{1}}\rightarrow L_{1+}\left(
\mathbb{R}^{3}\right)  \subset L_{1}\left(  \mathbb{R}^{3}\right)  ,
\end{equation}
where
\begin{equation}
\mathfrak{S}_{2}=\left\{  \left(  \boldsymbol{\varphi,N}\right)  \left\vert
\varphi_{j},\varphi_{k}\in\mathcal{H}^{1},\left\langle \left.  \varphi
_{j}\right\vert \varphi_{k}\right\rangle =\delta_{jk},\,\boldsymbol{N}%
\in\mathfrak{N}\left(  N\right)  \right.  \right\}  .\label{set2}%
\end{equation}
...
theory to relate the variation of the energy with respect the occupation
numbers to the eigenvalues of the Kohn-Sham operator we focus on $\Gamma_{1}.$

The functional $\Gamma_{1}$ is not one to one but is constant on the
equivalence class $\mathfrak{S}\left(  \rho\right)  $ of orbitals%
\begin{equation}
...
\end{equation}
Each preimage set $\Gamma_{1}^{-1}\left(  \rho\right)  =\mathfrak{S}\left(
\rho\right)  \subset\mathfrak{S}_{1}$ contains, in general, both single sets
of orthogonal orbitals normalized to different $\boldsymbol{N}\in
\mathfrak{N}\left(  N\right)  $ and different sets of orthogonal orbitals
normalized to the same $\boldsymbol{N}\in\mathfrak{N}\left(  N\right)  ,$
which corresponds to the fact that a given density can be expanded in many
ways in terms of occupation numbers and orbitals. This relationship can be
made more specific by considering the kinetic functional (introduced in the
next subsection) that essentially provides a one to one relationship between
densities and orbitals and thus leads to an inverse $\Gamma_{1}^{-1}$.

\subsubsection{Orbital functionals\label{GSOFunc}}

One can express the total energy via a universal orbital functional
$\mathcal{F}_{O}$, factored through the HK energy functional, as
\begin{equation}
...
\mathrm{v}\right)  =\min_{\boldsymbol{\varphi\in}\mathfrak{S}_{1}}%
\mathcal{E}_{O}\left(  \boldsymbol{\varphi,}\mathrm{v}\right)  , \label{KSmin}%
\end{equation}
where
\begin{equation}
...
r_{1}}\varepsilon_{jk}\left\{  \overset{\,}{\left\langle \left.  \varphi
_{j}\right\vert \varphi_{k}\right\rangle -N_{j}\delta_{jk}}\right\}
+\mu\left\{  \sum_{\,1\leq j\leq r_{1}}\left\langle \left.  \varphi
_{j}\right\vert \varphi_{j}\right\rangle -N\right\}
\end{equation}
i.e.
\begin{equation}
...
particle number constraint $\sum_{\,1\leq j\leq r_{1}}\left\langle \left.
\varphi_{j}\right\vert \varphi_{j}\right\rangle =N$ is actually redundant as
the argument $\boldsymbol{\varphi}$ of $\mathcal{E}_{O}$ satisfies $\Gamma
_{1}\left(  \boldsymbol{\varphi}\right)  =\rho\in\mathcal{D}$ which by
definition includes this constraint, however the explicit inclusion of this
constraint is useful as its relaxation introduces some physically valuable information.
...
{\delta\varphi_{j}^{\ast}} &  =0;\,1\leq j\leq r_{1},\label{KSC1}\\
\frac{\partial\mathcal{L}_{\boldsymbol{\varphi}}\left(  \boldsymbol{\varphi
},\boldsymbol{\varepsilon,}\mu,\boldsymbol{N,}\mathrm{v}\right)  }%
{\partial\varepsilon_{jk}} &  =0;\,1\leq j,k\leq r_{1}\label{KSC2}\\
\frac{\partial\mathcal{L}_{\boldsymbol{\varphi}}\left(  \boldsymbol{\varphi
...
}\left(  \varepsilon_{jk}\left(  \boldsymbol{N}\right)  -\mu\left(  N\right)
\delta_{jk}\right)  \left\vert \varphi_{k}\left(  \boldsymbol{N}\right)
\right\rangle =0;\,1\leq j\leq r_{1}.\label{LagPar}%
\end{equation}
...
=\sum_{1\leq k\leq r_{1}}\left(  \varepsilon_{jk}\left(  \boldsymbol{N}%
\right)  -\mu\left(  N\right)  \delta_{jk}\right)  \left\vert \varphi
_{k}\left(  \boldsymbol{N}\right)  \right\rangle ,\,\varepsilon_{jk}\left(
\boldsymbol{N}\right)  =\varepsilon_{kj}^{\ast}\left(  \boldsymbol{N}\right)
,\label{FockEq}%
\end{equation}
thus obtaining orbital equations corresponding to the condition eq. $\left(
\ref{KSC1}\right)  .$ One should note that the norm of the vectors $\left\{
\left\vert \varphi_{j}\left(  \boldsymbol{N}\right)  \right\rangle \right\}  $
are the occupation numbers $\boldsymbol{N}$ and eq. $\left(  \ref{FockEq}%
\right)  $ is \emph{not }an eigenvalue equation unless $\mathcal{E}_{O}\left(
...
\right)  $ that produce the same density.
...
\boldsymbol{N}\right)  ;1\leq j\leq r_{1}, \label{Janak}%
\end{equation}
which is Janak's Theorem for \emph{any }fixed set $\boldsymbol{\boldsymbol{N}%
...
$\left(  \ref{FockEq}\right)  $ into account we can obtain these relationship
...
^{\frac{1}{2}}}.
\end{equation}
By considering sensitivity of the energy functional to the average number of
electrons in the system one obtains
\begin{equation}
...
(b) In the second optimization one finds minima of $\mathcal{E}_{O}\left(
\boldsymbol{\varphi}\left(  \boldsymbol{N}\right)  \boldsymbol{,}%
\mathrm{v}\right)  $ with respect to a constrained variation of the
normalization control parameters $\boldsymbol{N}$ for a fixed $N$. The
constraints on the control parameter $\boldsymbol{N}$ can be held by setting
\begin{equation}
N_{j}\left(  \boldsymbol{\theta}\right)  =\frac{N\cos^{2}\theta_{j}}%
{\sum\limits_{1\leq j\leq r_{1}}\cos^{2}\theta_{j}}\equiv\frac{N\cos^{2}%
\theta_{j}}{\mathcal{N}\left(  \boldsymbol{\theta}\right)  };0\leq\theta
_{j}<\frac{\pi}{2}%
\end{equation}
leading to an unconstrained optimization over $\boldsymbol{\theta}$ producing
stationary points $\boldsymbol{\theta}_{\#}\left(  N\right)  $ determined by
\begin{equation}
\sum\limits_{1\leq j\leq r_{1}}\frac{\partial\mathcal{E}_{O}\left(
\boldsymbol{\varphi}\left(  \boldsymbol{N}\right)  \boldsymbol{,}%
\mathrm{v}\right)  }{\partial N_{j}}\frac{\partial N_{j}}{\partial\theta_{k}%
}=0;1\leq k\leq r_{1}. \label{ThetaDer}%
\end{equation}
Using eq. $\left(  \ref{ThetaDer}\right)  $ and observing that
\begin{equation}
\frac{\partial N_{j}}{\partial\theta_{k}}=\frac{N\sin2\theta_{k}}%
{\mathcal{N}\left(  \boldsymbol{\theta}\right)  ^{2}}-\frac{\sin2\theta
_{k}\delta_{jk}N}{\mathcal{N}\left(  \boldsymbol{\theta}\right)  }=\frac
{N\sin2\theta_{k}}{\mathcal{N}\left(  \boldsymbol{\theta}\right)  }\left\{
\frac{1}{\mathcal{N}\left(  \boldsymbol{\theta}\right)  }-\delta_{jk}\right\}
\end{equation}
and
\begin{equation}
\frac{\partial\mathcal{E}_{O}\left(  \boldsymbol{\varphi}\left(
\boldsymbol{N}\left(  \boldsymbol{\theta}\right)  \right)  \boldsymbol{,}%
\mathrm{v}\right)  }{\partial N_{j}}=\varepsilon_{jj}\left(  \boldsymbol{N}%
\left(  \boldsymbol{\theta}\right)  \right)
\end{equation}
this can be developed to give
\begin{align}
&  \frac{\partial\mathcal{E}_{O}\left(  \boldsymbol{\varphi}\left(
\boldsymbol{\theta}\right)  \boldsymbol{,}\mathrm{v}\right)  }{\partial
\theta_{k}}=\sum\limits_{1\leq j\leq r_{1}}\varepsilon_{jj}\left(
\boldsymbol{\theta}\right)  \frac{N\sin2\theta_{j}}{\mathcal{N}\left(
\boldsymbol{\theta}\right)  }\left\{  \frac{1}{\mathcal{N}\left(
\boldsymbol{\theta}\right)  }-\delta_{jk}\right\}  =0\nonumber\\
&  \Rightarrow\sum\limits_{1\leq j\neq k\leq r_{1}}\varepsilon_{jj}\left(
\boldsymbol{\theta}\right)  \sin2\theta_{j}=\left(  \mathcal{N}\left(
\boldsymbol{\theta}\right)  -1\right)  \varepsilon_{kk}\left(
\boldsymbol{\theta}\right)  \sin2\theta_{k} \label{Theta}%
\end{align}
which determines the ground state energy $E\left(  \rho_{GS}\boldsymbol{,}%
\mathrm{v}\right)  =\mathcal{E}_{O}\left(  \boldsymbol{\varphi}\left(
\boldsymbol{\theta}_{\#}\right)  ,\mathrm{v}\right)  .$ The ground state
density can then be expanded as
\begin{equation}
\rho_{GS}\left(  \boldsymbol{r}\right)  =\sum_{1\leq j\leq r_{1}}\left\vert
\overset{}{\left[  \varphi_{j}\left(  \boldsymbol{\theta}_{\#}\right)
\right]  \left(  \boldsymbol{r}\right)  }\right\vert ^{2}. \label{GSen}%
\end{equation}
The expansion eq. $\left(  \ref{GSen}\right)  $ is very non unique in that any
set $\boldsymbol{\varphi}\left(  \boldsymbol{\theta}_{\#}^{\prime}\right)
\in\mathfrak{S}\left(  \rho_{GS}\right)  $, where $\boldsymbol{\theta}%
_{\#}^{\prime}$ satisfies eq. $\left(  \ref{Theta}\right)  $ produces
$\rho_{GS}$ and the same value of the energy functional i.e. $\mathcal{E}%
_{O}\left(  \boldsymbol{\varphi}\left(  \boldsymbol{\theta}_{\#}^{\prime
}\right)  ,\mathrm{v}\right)  =\mathcal{E}_{O}\left(  \boldsymbol{\varphi
}\left(  \boldsymbol{\theta}_{\#}\right)  ,\mathrm{v}\right)  $.

\subsubsection{The Kohn-Sham Density Functional\label{KSDF}}
...
obtain such a representative is to consider the orbitals formed by the NO's
and occupation numbers of the FORDO of the $N$-electron ground state
$D_{0}\left(  \rho\right)  =A_{0}\left(  \rho\right)  ^{2}$ of a model
independent particle $N$-electron system$,$ that has a fixed density $\rho.$
This corresponds to finding the minimizers of the universal kinetic energy
functional $K_{U}$
\begin{equation}
...
\right)  .
\end{equation}
As $K-\Omega\left(  \lambda_{0}\left(  \rho\right)  \right)  $ is an one
electron operator the state $D_{0}\left(  \rho\right)  $ must be a mixture of
IPS's or a pure IPS and thus determined by the eigenvalues $\left\{
...
\right\}  $ are equal to either $1$ or $0$ (i.e. $N$ ones and $r_{1}-N$
zeroes), while if it is a mixed state than there will be some fractional
values and the number of zero values will be less than $r_{1}-N$. Expanding
$D_{0}^{1}\left(  \rho\right)  $ in terms of normalized NO's $\left\{  \left.
\left\vert \widetilde{\varphi_{0j}}\right\rangle \right\vert 1\leq j\leq
r_{1}\right\}  $ gives
\begin{equation}
...
\end{equation}
(where in contrast to eq. $\left(  \ref{Gamma}\right)  $ $r_{1}\geq r\geq N$
), which after renormalizing the NO's to the occupation numbers i.e.
\begin{equation}
\left\vert \varphi_{0j}\left(  \rho\right)  \right\rangle =N_{0j}\left(
\rho\right)  ^{\frac{1}{2}}\left\vert \widetilde{\varphi_{0j}\left(
\rho\right)  }\right\rangle ,1\leq j\leq r_{1}%
\end{equation}
results in
\begin{equation}
\rho\left(  \boldsymbol{r}\right)  =\sum_{1\leq j\leq r_{1}}\left\vert \left[
...
FORDO's $\left\{  \left.  D_{0\kappa}^{1}\left(  \rho\right)  \right\vert
...
the $\left\{  \left.  N_{0j}\right\vert 1\leq j\leq r_{1}\right\}  $ can have
non integer values even for degenerate pure states$\boldsymbol{.}$
...
_{0}\right)  $ formed from the NO's $\boldsymbol{\varphi}_{0}$ has two
energies associated with it. One, $E_{0}\left(  \boldsymbol{\varphi}%
_{0}\left(  \boldsymbol{N}_{0}\right)  \right)  $, is the ground state energy
of the Hamiltonian%
\begin{equation}
H_{0}=K+\mathrm{v},
\end{equation}
...
\right)  =\sum_{1\leq j\leq r_{1}}\left\langle \tilde{\varphi}_{0j}\left\vert
...
been expressed as an explicit functional of GSO's and parameters that
determine occupation numbers \cite{OrtizWeiner2006} The minimization of this
explicit functional can then be compared to the minimization of the implicit
...
density functionals) it is invariant to the phases of the GSO's i.e. the group
$U\left(  1\right)  .$
...
geminal power state are at the least invariant to the group $SU\left(
2\right)  $ that is formed by $2\times2$ \emph{special} unitary
transformations of the paired CGSO's $\left\{  \left.  \left\vert \psi
_{j}\right\rangle ,\left\vert \psi_{j+s}\right\rangle \right\vert 1\leq j\leq
...
\cite{AGPCoherent}. The canonical CGSO's are unique up to transformations
...
B\right)  =0\forall\boldsymbol{r}\right\}  \subseteq\ker D\Xi_{2}\left(
A\right)
\end{equation}
...
\Phi\left(  \boldsymbol{r}\right)  ^{\dagger}\Phi\left(  \boldsymbol{r}%
\right)  A\right)  \equiv\operatorname{Tr}\left\{  A\Phi\left(  \boldsymbol{r}%
\right)  ^{\dagger}\Phi\left(  \boldsymbol{r}\right)  \cdot\right\}
\right\vert \boldsymbol{r\in}\mathbb{R}^{3}\right\}  $ is a generalized basis,
...
,N}\right)  \right)  =\mathcal{F}_{HK}\left(  \rho\right)  ,
\end{equation}
...
\end{equation}
determines the ground state energy $E\left(  \rho_{GS}\boldsymbol{,}%
...
\begin{equation}
\rho_{GS}\left(  \boldsymbol{r}\right)  =\sum_{1\leq j\leq r_{1}}%
N_{j}\left\vert \left[  \tilde{\varphi}_{j}\left(  \boldsymbol{\theta}%
_{\#}\left(  N\right)  \right)  \right]  \left(  \boldsymbol{r}\right)
\right\vert ^{2}.
\end{equation}
...
\mathrm{v}\right)  }{\delta\rho\left(  \boldsymbol{r}^{\prime}\right)  }%
\frac{\delta\rho\left(  \boldsymbol{r}^{\prime}\right)  }{\delta\tilde
{\varphi}_{j}^{\ast}}d\boldsymbol{r}^{\prime}\nonumber\\
&  =\int\frac{\delta\mathcal{E}_{O}\left(  \boldsymbol{\tilde{\varphi}}\left(
\boldsymbol{N}\right)  ,\boldsymbol{N,}\mathrm{v}\right)  }{\delta\rho\left(
\boldsymbol{r}\right)  }\tilde{\varphi}_{j}\left(  \boldsymbol{r}^{\prime
}\right)  N_{j}\delta\left(  \boldsymbol{r-r}^{\prime}\right)  d\boldsymbol{r}%
^{\prime}=\frac{\delta\mathcal{E}_{O}\left(  \boldsymbol{\tilde{\varphi}%
}\left(  \boldsymbol{N}\right)  ,\boldsymbol{N,}\mathrm{v}\right)  }%
{\delta\rho\left(  \boldsymbol{r}\right)  }N_{j}\tilde{\varphi}_{j}\left(
\boldsymbol{r}\right)
\end{align}
gives%
\begin{equation}
\frac{\delta\mathcal{E}_{O}\left(  \boldsymbol{\tilde{\varphi}}\left(
\boldsymbol{N}\right)  ,\boldsymbol{N,}\mathrm{v}\right)  }{\delta\rho}%
...
{\delta\rho}$ is a multiplication operator given by
\begin{equation}
\frac{\delta\mathcal{E}_{O}\left(  \boldsymbol{\tilde{\varphi}}\left(
\boldsymbol{N}\right)  ,\boldsymbol{N,}\mathrm{v}\right)  }{\delta\rho
}\left\vert \tilde{\varphi}_{j}\left(  \boldsymbol{N}\right)  \right\rangle
...
{\delta\rho}\tilde{\varphi}_{j}\left(  \boldsymbol{N}\right)  \right\rangle ,
\end{equation}
where
\begin{equation}
...
\boldsymbol{r}\right)  =\frac{\delta\mathcal{E}_{O}\left(  \boldsymbol{\tilde
{\varphi}}\left(  \boldsymbol{N}\right)  ,\boldsymbol{N,}\mathrm{v}\right)
}{\delta\rho\left(  \boldsymbol{r}\right)  }\left[  \tilde{\varphi}_{j}\left(
\boldsymbol{N}\right)  \right]  \left(  \boldsymbol{r}\right)  .
\end{equation}
...
\end{equation}%
\[
\frac{\delta\rho\left(  \boldsymbol{r}^{\prime}\right)  }{\delta f\left(
\boldsymbol{r}\right)  }=\frac{1}{\left\langle f|f\right\rangle }\left\{
2Nf\left(  \boldsymbol{r}^{\prime}\right)  \delta\left(  \boldsymbol{r-r}%
^{\prime}\right)  -2\rho\left(  \boldsymbol{r}^{\prime}\right)  f\left(
\boldsymbol{r}\right)  \right\}
\]%
\begin{align*}
&  2\int\frac{\delta E\left(  f\boldsymbol{,}\mathrm{v}\right)  }{\delta
\rho\left(  \boldsymbol{r}^{\prime}\right)  }\left\{  \frac{1}{\left\langle
f|f\right\rangle }Nf\left(  \boldsymbol{r}^{\prime}\right)  \delta\left(
\boldsymbol{r-r}^{\prime}\right)  -\rho\left(  \boldsymbol{r}^{\prime}\right)
f\left(  \boldsymbol{r}\right)  \right\}  d\boldsymbol{r}^{\prime}=\\
&  2\int\frac{\delta E\left(  f\boldsymbol{,}\mathrm{v}\right)  }{\delta
\rho\left(  \boldsymbol{r}^{\prime}\right)  }\frac{1}{\left\langle
f|f\right\rangle }Nf\left(  \boldsymbol{r}^{\prime}\right)  \delta\left(
\boldsymbol{r-r}^{\prime}\right)  d\boldsymbol{r}^{\prime}-2\int\frac{\delta
E\left(  f\boldsymbol{,}\mathrm{v}\right)  }{\delta\rho\left(  \boldsymbol{r}%
^{\prime}\right)  }\rho\left(  \boldsymbol{r}^{\prime}\right)  f\left(
\boldsymbol{r}\right)  d\boldsymbol{r}^{\prime}=\\
\frac{\delta E\left(  f\boldsymbol{,}\mathrm{v}\right)  }{\delta f\left(
\boldsymbol{r}\right)  }=  &  2\frac{Nf\left(  \boldsymbol{r}\right)
}{\left\langle f|f\right\rangle }\frac{\delta E\left(  f\boldsymbol{,}%
\mathrm{v}\right)  }{\delta\rho\left(  \boldsymbol{r}\right)  }-2f\left(
\boldsymbol{r}\right)  \int\frac{\delta E\left(  f\boldsymbol{,}%
\mathrm{v}\right)  }{\delta\rho\left(  \boldsymbol{r}^{\prime}\right)  }%
\rho\left(  \boldsymbol{r}^{\prime}\right)  d\boldsymbol{r}^{\prime}=2f\left(
\boldsymbol{r}\right)  \left\{  \frac{N}{\left\langle f|f\right\rangle }%
\frac{\delta E\left(  f\boldsymbol{,}\mathrm{v}\right)  }{\delta\rho\left(
\boldsymbol{r}\right)  }-\int\frac{\delta E\left(  f\boldsymbol{,}%
\mathrm{v}\right)  }{\delta\rho\left(  \boldsymbol{r}^{\prime}\right)  }%
\rho\left(  \boldsymbol{r}^{\prime}\right)  d\boldsymbol{r}^{\prime}\right\}
\end{align*}

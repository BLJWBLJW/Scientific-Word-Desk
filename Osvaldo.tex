...
%TCIDATA{LastRevised=Tuesday, March 09, 2010 14:15:16}
...
their associated propagators. A major focus of the illusterious career of
professor Osvaldo Goscinski has been the analysis and construction of
approximate propagators. In particular the one-electron and polarization
propagators, where he effectively used the properties of super operators
acting on operator manifolds associated with a given reference state to
generate various orders of approximation \cite{Osvaldo}. In this article I
will concentrate on the polarization propagator, which describes the response
of the first order reduced density operator to electromagnetic radiation. The
poles of the polarization propagator give the excitation energies between
stationary states of the molecular system and the corresponding residues the
transition probabilities of these excitations caused by the electromagnetic
perturbation. If the initial molecular reference state is a stationary state
then the poles are the excitation energies and the residues the transition
probabilities from that state. In order for these approximations to this
propagator to be $N$-representable i.e. associated with some approximate
$N$-particle state, the adjoints of the excitation operators that diagonalize
this propagator must annihilate the reference state. However this in general
does not occur and one says that the approximation is inconsistent. It turns
out that the annihilation condition can be naturally expressed in terms of
group coherent states, where the reference state corresponds to a lowest or
heighest weight state associated with a representation of the group. In this
article we will consider propagators based on one particle operators.

A powerful method that is often used to evaluate the full quantum propagator
...
manifolds, that are also coset spaces of groups. The family of coherent states
chosen determines the specific geometric structure of the space of the complex
valued paths, $z(t)$, via a metric function $C\left(  z,z^{\prime}\right)  $.
The time development of the paths is described by a classical Hamiltonian
function $E\left(  z,z^{\prime}\right)  $. The form and degree of these
functions is determined by the family of coherent states, and thus the
group/coset pair. If the Lagrangian associated with the Hamiltonian function
is quadratic then the path integral can be evaluated exactly, if this is not
the case approximations must be invoked
...
integral by using the Stationary Phase Approximation (SPA) based on a specific
family of coherent states. This is equivalent to using the Time Dependent
Variation Principle (TDVP) based on the same family of states. Approximate
stationary states of the molecular system then correspond to the special
stationary phase "paths" that are critical points of the corresponding
classical Hamiltonian. The linear response of these approximate stationary
states to external perturbing fields then involves the full classical
Hamiltonian, which again, however, usually makes the problem numerically intractable.
...
approximation of the classical Hamiltonian around a critical point. This
construction is analogous to including first order corrections to the SPA of
the path integral by expanding the classical Hamiltonian in the Lagrangian to
second order around the classical path.

The second order approximation to the classical Hamiltonian clearly depends on
the coordinate system used, and hence on the family of coherent states being
...
terms of the Lie algebra generators of the group associated with the coherent
states. For systems in which the number of particles is a good quantum number
and is conserved, i.e. a molecular system, the family of coherent states
should thus be based on the group $SU(r)$, where $r=2s$ is the size of the
finite dimensional subspace one is approximating the full Hilbert space by.

The Lie algebra of the group $SU(r)$ has three direct sum Cartan
decompositions in terms of Lie algebras of maximal compact subgroups into%
\begin{subequations}
\begin{align}
su\left(  2s\right)   &  =so\left(  2s,r\right)  \oplus o\left(  2s,r\right)
^{\bot}\\
su\left(  2s\right)   &  =su\left(  N\right)  \oplus su\left(  2s-N\right)
^{\bot}\\
su\left(  2s\right)   &  =usp\left(  2s\right)  \oplus usp\left(  2s\right)
^{\bot}%
\end{align}
where the orthogonality is with respect to the killing form of $su(2s)$. These
decomposition define three distinct coset decompositions of $SU(r)$%
\end{subequations}
\begin{align}
&  SU\left(  r\right)
%TCIMACRO{\QATOPD{/}{.}{{}}{SU\left(  r\right)  \otimes SU\left(  r-N\right)
%}}%
%BeginExpansion
\genfrac{/}{.}{0pt}{}{{}}{SU\left(  r\right)  \otimes SU\left(  r-N\right)  }%
%EndExpansion
\\
&  SU\left(  r\right)
...
\\
&  SU\left(  r\right)
%TCIMACRO{\QATOPD{/}{.}{{}}{SO\left(  r\right)  }}%
%BeginExpansion
\genfrac{/}{.}{0pt}{}{{}}{SO\left(  r\right)  }%
%EndExpansion
\end{align}
...
%EndExpansion
\end{equation}
coherent states give the well known Thouless parameterization of Independent
Particle (IP) States and lead to the standard Random Phase Approximation (RPA)
for the linear response function. The
\begin{equation}
SU\left(  r\right)
%TCIMACRO{\QATOPD{/}{.}{{}}{USp\left(  2s\right)  }}%
%BeginExpansion
...
response function. At the present time the
\begin{equation}
SU\left(  r\right)
%TCIMACRO{\QATOPD{/}{.}{{}}{SO\left(  r\right)  }}%
%BeginExpansion
...
$\lceil$Coset decompositions of Lie groups when used in conjunction with
Parabolic Induction is a powerful method in the construction and
characterization of irreducible group representations. Representations are
induced from the action of the group on the coset space to the action of the
group on functions defined on the coset space. Points of the coset space
corresponds to coherent states, thus the induced representations act on
functions of coherent states. Hilbert spaces of these functions produce a
holomorphic representation of quantum states and thus of quantum mechanics.
Due to the coset structure and Lie group properties of the coherent state
manifold, quantum mechanical observables can be represented as differential
operators acting on functions defined on this manifold. By using such
differential operator representations one can obtain expressions for matrix
elements of observables in a coherent state overcomplete basis and thus for
expectation values for observables with respect to any state. These
expressions are of great importance in practical applications.$\rfloor$
...
used in the study of Noncommutative Harmonic Analysis on Symmetric spaces and
in the classification and properties of various classes of differential
...
$\left\{  Q_{n}^{\dagger}\right\}  $ from an energy optimized AGP state i.e.%
\begin{equation}
\left\vert \Psi_{n}\right\rangle =Q_{n}^{\dagger}\left\vert g^{\frac{N}{2}%
}\right\rangle =\sum_{jk}Q_{njk}a_{j}^{\dagger}a_{k}\left\vert g^{\frac{N}{2}%
}\right\rangle
\end{equation}
...
\begin{equation}
Q_{n}\left\vert g^{\frac{N}{2}}\right\rangle =0
\end{equation}

\end{description}

off diagonal blocks of the energy Hessian at stationary points being small in
norm i.e.%
\begin{equation}
...
}\right\vert g\left(  \xi\right)  ^{\frac{N}{2}}\right\rangle }%
\end{equation}%
\begin{equation}
...
number operators $\left\{  a_{j}^{\dagger}a_{j}\right\}  $, however, the
justification of including such operators was not well understood. In the case
of the exact propagator, for which $\left\{  Q_{n}^{\dagger}\right\}  $ are in
general multiparticle operators, all of the properties (a), (b) and (c) are
satisfied with the off diagonal Hessian block being identically zero.
...
basis of the approximation remained. Recently, however, this approximation has
been placed within the context of coherent states and the group structure of
many particle fermion state space. Within this context the original
approximation (including the modifications to improve it) can be rigorously
derived, and its relation to many other mathematical and physical properties
fully displayed. This new formulation also produces more efficient and
flexible numerical implementations.
...
\subsection{Exact Integrals\label{exact}}

Solutions of the TDSE are of the form
\begin{equation}
\left\vert \Psi\left(  t\right)  \right\rangle =e^{-\frac{i}{h}Ht}\left\vert
\Psi\left(  0\right)  \right\rangle
\end{equation}
where $U\left(  t\right)  =e^{-\frac{i}{h}Ht}$ is called the evolution
operator or propagator and $\left\vert \Psi\left(  0\right)  \right\rangle $
is an arbitrary initial state. Introducing a resolution of the identity
\begin{equation}
I=\int\left\vert z\right\rangle \left\langle z\right\vert d\mu\left(
z\right)
\end{equation}
the evolution operators matrix elements $\equiv$ propagator kernel in this
representation are
\begin{equation}
P\left(  z^{\prime},z;t\right)  =\left\langle z^{\prime}\left\vert
e^{-\frac{i}{h}Ht}\right.  z\right\rangle
\end{equation}
where%
\begin{equation}
\left\vert \Psi\left(  t\right)  \right\rangle =\int\left\vert z\right\rangle
P\left(  z^{\prime},z;t\right)  \left\langle z\right\vert d\mu\left(
z\right)  \left\vert \Psi\left(  0\right)  \right\rangle
\end{equation}
Using the coherent state construction the propagator kernel becomes an
integral over paths with fixed end points, $z\left(  0\right)  =z_{i};\quad
z\left(  T\right)  =z_{f}$
\begin{equation}
P\left(  z_{f},z_{i};T\right)  =\int_{z_{i}}^{z_{f}}e^{\left\{  \frac
{i}{\hslash}\int_{0}^{T}\left\{  \frac{i\hslash}{2}\left.  \ln\left\langle
\left.  z^{\prime}\left(  t^{\prime}\right)  \right\vert z\left(  t\right)
\right\rangle \right\vert _{t=t^{\prime}}-E\left(  z\left(  t\right)  \right)
\right\}  dt\right\}  }d\mu\left(  z\right)
\end{equation}
where
\begin{equation}
E\left(  z\left(  t\right)  \right)  =\frac{\left\langle \left.  z\left(
t\right)  \right\vert Hz\left(  t\right)  \right\rangle }{\left\langle \left.
z\left(  t\right)  \right\vert z\left(  t\right)  \right\rangle }%
\end{equation}


The path integral Action and Lagrangian are defined as
\begin{subequations}
\begin{align}
P\left(  z_{f},z_{i};T\right)   &  =\int_{z_{i}}^{z_{f}}e^{\frac{i}{\hslash
}A\left(  z,T\right)  }d\mu\left(  z\right) \\
A\left(  z,T\right)   &  =\int_{0}^{T}L\left(  z,\dot{z},t\right)  dt\\
L\left(  z,\dot{z},t\right)   &  =\frac{i\hslash}{2}\left.  \ln\left\langle
\left.  z^{\prime}\left(  t^{\prime}\right)  \right\vert z\left(  t\right)
\right\rangle \right\vert _{t=t^{\prime}}-E\left(  z\left(  t\right)  \right)
\end{align}
...
The stationary phase approximation $\delta A\left(  z_{c},T\right)  =0$, with
fixed end points $z_{c}\left(  T\right)  =z_{f},z_{c}\left(  0\right)  =z_{i}$
produces the generalized classical equations
\end{subequations}
\begin{align}
i\hslash\sum C_{jk}\frac{dz_{k}}{dt}  &  =\frac{\partial E}{\partial\bar
{z}_{j}}\\
-i\hslash\sum\bar{C}_{jk}\frac{d\bar{z}_{k}}{dt}  &  =\frac{\partial
E}{\partial z_{j}}%
\end{align}
over the phase space formed by the manifold of states $\left\{  \left\vert
z\right\rangle \right\}  .$ Applying the SPA\ one obtains
\begin{equation}
\int_{z_{i}}^{z_{f}}e^{\frac{i}{\hslash}A\left(  z,T\right)  }d\mu\left(
z\right)  \approx\sum_{c}K_{c}\left\vert \det\left(  \frac{i}{2\pi\hslash
}\frac{\partial^{2}A\left(  z_{c},T\right)  }{\partial x\partial x}\right)
\right\vert ^{-\frac{1}{2}}e^{\frac{i}{\hslash}A\left(  z_{c},T\right)  }%
\end{equation}
where $x=z$ or $\bar{z}$ , with the conditions
\begin{equation}
\delta A\left(  z_{c},T\right)  =0\Leftrightarrow\delta L\left(  z_{c},\dot
{z}_{c},t\right)  =0;\quad z_{c}\left(  T\right)  =z_{f},z_{c}\left(
0\right)  =z_{i}%
\end{equation}
and the paths $\left\{  z_{c}\right\}  $ are solutions to the classical
equations of motion.

If the Lagrangian is quadratic in the paths the classical paths $\left\{
z_{c}\right\}  $ produce an exact propagator
\begin{equation}
P\left(  z_{f},z_{i};T\right)  =\sum_{c}\sqrt{\det\left(  \frac{i}{\hslash
}\frac{\partial^{2}A\left(  z_{c},T\right)  }{\partial z_{f}\partial z_{i}%
}\right)  }e^{\frac{i}{\hslash}A\left(  z_{c},T\right)  }%
\end{equation}
so for SPA\ or quadratic Lagrangians%
\begin{align}
\left\langle \left.  z\right\vert \Psi\left(  t\right)  \right\rangle  &
\approx\sum_{c}\Omega_{c}\left(  z_{c}\left(  t\right)  \right)  e^{\frac
{i}{\hslash}A\left(  z_{c},t\right)  }\left\langle \left.  z\right\vert
\Psi\left(  0\right)  \right\rangle \nonumber\\
&  \Updownarrow\nonumber\\
\left\langle \left.  z\left(  t\right)  \right\vert \Psi\right\rangle  &
\approx\sum_{c}\Omega_{c}\left(  z_{c}\left(  t\right)  \right)  e^{\frac
{i}{\hslash}A\left(  z_{c},t\right)  }\left\langle \left.  z\left(  0\right)
\right\vert \Psi\right\rangle \nonumber\\
&  \Updownarrow\nonumber\\
\Psi\left(  z\left(  t\right)  \right)   &  \approx\sum_{c}\Omega_{c}\left(
z_{c}\left(  t\right)  \right)  e^{\frac{i}{\hslash}A\left(  z_{c},t\right)
}\Psi\left(  z\left(  0\right)  \right)
\end{align}


The SPA approximation can be improved by using a quadratic approximation to
the classical Hamiltonian around stationary paths, produced by truncating the
Taylor series expansion about such points at second order.
...
_{\mathcal{K}}\left(  G\right)  $ of a compact group $G$ on subspace
$\mathcal{K}$ of $\mathcal{H}$. The groups we will consider are $SU\left(
\mathcal{H}^{N}\right)  $ and $SU\left(  \mathcal{H}^{1}\right)  .$ In the
finite dimensional case $SU\left(  \mathcal{H}^{N}\right)  \equiv SU\left(
\left(
%TCIMACRO{\QATOP{r}{N}}%
%BeginExpansion
\genfrac{}{}{0pt}{}{r}{N}%
%EndExpansion
\right)  \right)  $ and $SU\left(  \mathcal{H}^{1}\right)  \equiv SU\left(
r\right)  $ the groups associated with $N$ -electron state space and $1$
-electron state space respectively. The irreducible representations of
$\mathcal{R}_{\mathcal{H}^{N}}\left(  SU\left(  \mathcal{H}^{N}\right)
\right)  $ and $\mathcal{R}_{\mathcal{H}^{N}}\left(  SU\left(  \mathcal{H}%
^{1}\right)  \right)  $ are those carried by $\mathcal{H}.$

A reference state $\left\vert \Phi\right\rangle \in\mathcal{H}^{N}$ determines
a maximally compact isotropy subgroup $H_{\Phi}$ that is contained in
$G=SU\left(  \mathcal{H}^{N}\right)  $ where
\begin{equation}
H_{\Phi}=\left\{  \left.  g\right\vert \mathcal{R}_{\mathcal{H}^{N}}\left(
g\right)  \left\vert \Phi\right\rangle =e^{i\theta\left(  g\right)
}\left\vert \Phi\right\rangle \right\}
\end{equation}
$G$ can be then be factored in the following manner%
\begin{equation}
G=\left(  G/H_{\Phi}\right)  H_{\Phi}%
\end{equation}
The reference state we will be focusing on is an AGP\ state given by
\begin{equation}
\left\vert g^{\frac{N}{2}}\right\rangle =N!\sum_{1\leq j_{1}<j_{2}%
<\cdots<j_{\frac{N}{2}}\leq s}\zeta_{j_{1}}\cdots\zeta_{j_{\frac{N}{2}}%
}\left\vert \eta_{j_{1}}\eta_{j_{1}+s}\cdots\eta_{j_{N}\,}\eta_{j_{N}%
+s}\right\rangle \label{canexp}%
\end{equation}
...
\begin{equation}
\left\vert g\right\rangle =\sum_{1\leq j\leq s}\zeta_{j}\left\vert \eta
_{j}\eta_{j+s}\right\rangle =\sum_{1\leq j\leq s}n_{j}^{\frac{1}{2}}%
e^{i\theta_{j}}\left\vert \eta_{j}\eta_{j+s}\right\rangle ,
\end{equation}
...
$\left\vert \phi\right\rangle $ is the vacuum state of $\mathcal{H}_{F}$.
...
k\right\vert }}z_{+jk}\xi_{j}a_{k}^{\dagger}a_{j}+\sigma\left(  j\right)
\sigma\left(  k\right)  \xi_{k}a_{-j}^{\dagger}a_{-k}\right\}  =\exp\left\{
...
j<k\leq s\\\left\vert j\right\vert \neq\left\vert k\right\vert }}}\lambda
...
s\\\left\vert j\right\vert \neq\left\vert k\right\vert }}}\eta_{jk}%
...
\subsection{Differential Structure of the Coset Space\label{diffstruct}}

The phase space associated with the coset space $G_{H}/H_{H}$ has a symplectic
structure determined by
\begin{equation}
\omega=-\frac{i\hslash}{2}\sum_{jk}\left\{  \left.  \frac{\partial^{2}%
F}{\partial\bar{z}_{j}\partial z_{k}}\right\vert _{\mathbf{z=z\prime}%
}\right\}  d\mathbf{z}d\mathbf{z}^{\prime}%
\end{equation}
which acts on tangent vectors $\left\{  X_{f},X_{g}\right\}  $ of this
manifold by
\begin{equation}
\omega\left(  X_{f},X_{g}\right)  =-\omega\left(  X_{g},X_{f}\right)
\end{equation}
where
\begin{equation}
X_{f}=\left(
\begin{array}
[c]{c}%
\frac{\partial f}{\partial\mathbf{\bar{z}}}\\
...
\section{Functions defined on the Coset Space}

\subsection{Space of Holomorphic Quantum Wavefunctions}

Due to the irreducibility of the group G on the space $\mathcal{H}$ this set
of states forms an overcomplete basis in $\mathcal{H}$, thus one has the
resolution of the identity
\begin{equation}
I=\int\left\vert \mathbf{z}\right\rangle \left\langle \mathbf{z}\right\vert
d\mu\left(  \mathbf{z}\right)
\end{equation}
for a suitably chosen measure $\mu\left(  \mathbf{z}\right)  $ Using this
resolution we obtain
\begin{equation}
\left\vert \Psi\right\rangle =\int\left\vert \mathbf{z}\right\rangle
\left\langle \mathbf{z}\right\vert \left.  \Psi\right\rangle d\mu\left(
\mathbf{z}\right)  =\int\left\vert \mathbf{z}\right\rangle \Psi\left(
\mathbf{z}\right)  d\mu\left(  \mathbf{z}\right)
\end{equation}
where the amplitudes $\Psi\left(  \mathbf{z}\right)  $ are holomorphic and are
in 1-1 correspondence with the states $\left\vert \Psi\right\rangle $ The
space $\mathcal{H}\left(  \mathcal{M}\right)  $ of all such amplitudes is a
Hilbert space that is isomorphic to $\mathcal{H}$ Thus all quantum mechanical
operators that act in $\mathcal{H}$ have integral kernel representations in
the space of holomorphic amplitudes given by
\begin{equation}
\left\langle \left.  \Psi_{1}\right\vert H\Psi_{2}\right\rangle =\int\bar
{\Psi}_{1}\left(  \mathbf{z}^{\prime}\right)  H\left(  \mathbf{z}^{\prime
}\mathbf{,z}\right)  \Psi_{2}\left(  \mathbf{z}\right)  d\mu\left(
\mathbf{z}\right)
\end{equation}
$H\left(  \mathbf{z}^{\prime}\mathbf{,z}\right)  $ can be constructed as the
analytic extension of $H\left(  \mathbf{z,z}\right)  $ which is the
unnormalized energy functional used in the classical equations of motion.

\subsection{Representation of Observables as Differential Operators}

Elements of the universal enveloping algebra of $sl\left(  \mathcal{H}\right)
$ act as differential operators on functions defined on $\mathcal{H}\left(
\mathcal{M}\right)  $. As quantum mechanical observables formally belong to
the enveloping algebra, they too can be expressed as differential operators,
and their expectation values as the value of the action of these operators at
points in . In particular
\begin{equation}
\left\langle \left.  g^{\frac{N}{2}}\left(  \mathbf{z}\right)  \right\vert
a_{i}^{\dagger}a_{j}g^{\frac{N}{2}}\left(  \mathbf{z}\right)  \right\rangle
=P_{ij}\left(  \frac{\partial}{\partial\bar{z}},\frac{\partial}{\partial
z}\right)  \left\langle \left.  g^{\frac{N}{2}}\left(  \mathbf{z}\right)
\right\vert g^{\frac{N}{2}}\left(  \mathbf{z}\right)  \right\rangle
\end{equation}
where $P_{ij}\left(  \frac{\partial}{\partial\bar{z}},\frac{\partial}{\partial
z}\right)  $ is a polynomial in $\left\{  \frac{\partial}{\partial\bar{z}%
},\frac{\partial}{\partial z}\right\}  .$ Such relationships are important in
constructing efficient expressions for expectation values, as well as for the
various terms appearing the in generalized classical equations of motion on
the coherent state manifold and the associated linear response functions.

Thus on the space they can be represented both by integral kernels as well as
differential operators. There is and has been an active research effort in
classifying and analyzing the properties of differential operators on coset
manifolds in an abstract setting. We propose to apply this work to the study
of quantum mechanical observables involved in response functions obtained from
2 particle propagators.

\subsection{Induced Representations}

Elements of $G$ act on the coset space mapping it to itself, thus give rise to
a, in general nonlinear, representation of $G$. Thus
\begin{align}
G  &  :G/H\rightarrow G/H\Rightarrow\sigma\left(  g\right)
:\mathcal{M\rightarrow M}\\
\sigma\left(  g\right)  \mathbf{z}  &  \mathbf{\rightarrow z}^{\prime}\\
\sigma\left(  h\right)  \mathbf{z}  &  \mathbf{\rightarrow z\quad}h\in H
\end{align}
Thus a new representation of $G$ is induced on $\mathcal{H}\left(
\mathcal{M}\right)  $ by
\begin{equation}
\Pi\left(  g\right)  \Psi\left(  \mathbf{z}\right)  =\Psi\left(  \sigma\left(
g\right)  \mathbf{z}\right)
\end{equation}
The induced representation depends on the original representation and the
isotropy subgroup , which itself corresponds to the form of the coherent
state. All but two of the classical Riemannian symmetric coset spaces can be
expressed as exponential of off diagonal matrices, for these cases the action
of the group on the coset space is represented as fractional linear
transformations
\begin{equation}
\sigma\left(  g\right)  \mathbf{z}=\left(  \mathbb{A}\mathbf{z}+\mathbb{B}%
\right)  \left(  \mathbb{C}\mathbf{z}+\mathbb{D}\right)  =\mathbf{z}^{\prime}%
\end{equation}
where
\begin{equation}
\left(
...
\right)
\end{equation}
is the defining representation of $g\in G$. However for the other two cosets
$SU\left(  r\right)  /USp\left(  2s\right)  $ and $SU\left(  r\right)
/SO\left(  r\right)  $ and this is not the case. The coset $SU\left(
r\right)  /USp\left(  2s\right)  $ is the primary focus of this study, which
will investigate the details of the action of $SU\left(  r\right)  $ on
$SU\left(  r\right)  /USp\left(  2s\right)  $ and $SL\left(  r\right)  $ on
its complexification $SL\left(  r\right)  /Sp\left(  2s,\mathbb{C}\right)  $
The action of the group $SU\left(  r\right)  $ on the coset $SU\left(
r\right)  /USp\left(  2s\right)  $ is important in the study of physical
symmetries, such as spin and angular momentum. We have shown how one can
produce pure symmetry coherent states in the case of $SU\left(  r\right)
/\left(  U\left(  N\right)  \otimes U\left(  r-N\right)  \right)  $ coherent
states and in this work will extend such a construction to $SU\left(
r\right)  /USp\left(  2s\right)  $ ones.
